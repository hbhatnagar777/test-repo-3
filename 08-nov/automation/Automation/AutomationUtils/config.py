...
class ConfigReader:
    ...
    def __init__(self, json_path=constants.CONFIG_FILE_PATH):
        """
...
def get_config(reset=False, json_path=constants.CONFIG_FILE_PATH):
    ...
    """
    with CONFIG_LOCK:
        if (not CONFIG_CACHE.get(json_path)) or reset:
            CONFIG_CACHE[json_path] = ConfigReader(json_path).get_config()
        return CONFIG_CACHE[json_path]
