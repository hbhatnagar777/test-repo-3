...
class VirtualMachines:
    ...
    def configure_vm_restore_options(self, vm):
        ...
        if self.__restore_options.type == HypervisorDisplayName.VIRTUAL_CENTER:
            ...
            self.log.info(f"Selecting Network {vm.get('network')} and subnet : {vm.get('subnet')}, and createPublicIp ==> {vm.get("create_public_ip")}")
            self.gcp_network_interface(restore_options_dialog, vm, vm.get('network'), vm.get('subnet'), vm.get("create_public_ip"))
    ...
    def gcp_network_interface(self, restore_options_dialog, vm, network, subnet, create_public_ip):
        """
        ...
        Args:
           ...
           create_public_ip (bool) : True if creating public IP, otherwise False
        ...
        if create_public_ip:
            ...
            self.log.info("Public IP is enabled")

            self.log.info(f"Setting the public IP type as {vm.get('public_ip_type')}")
            restore_options_dialog.select_dropdown_values(drop_down_id = "public-ip-type",
                                                        values = [vm.get("public_ip_type")],
                                                        partial_selection = True)
            
            if vm.get("public_ip_type").lower() == "reserved static ip":
                self.log.info("Trying to select the reserve static ip {vm.get('reserved_static_ip')}")
                restore_options_dialog.select_dropdown_values(drop_down_id = "static-public-ip-dropdown",
                                                            values = [vm.get("public_reserved_static_ip")],
                                                            partial_selection = True)
        ...
        self.log.info(f"Setting the internal IP type as {vm.get('private_ip_type')}")
        restore_options_dialog.select_dropdown_values(drop_down_id = "private-ip-type",
                                                      values = [vm.get("private_ip_type")],
                                                      partial_selection = True)
        
        self.log.info("If ip type is custom or reserved static ip, we go into if block")
        
        if vm.get("private_ip_type").lower() == "custom":
            self.private_ip_address = vm.get("private_ip_address")
            self.log.info(f"the input IP is: {self.private_ip_address}")
            self.private_ip_address_split = self.private_ip_address.split(".")
            for octet,ip_octet in enumerate(self.private_ip_address_split):
                custom_ip_field = f"staticPrivateIP{octet}-:r2a:"
                self.log.info(f"Attempting to fill {ip_octet} in {octet+1} box")
                network_settings_modal.fill_text_in_field(element_id = custom_ip_field, text = ip_octet)
        
        elif vm.get("private_ip_type").lower() == "reserved static ip":
            self.log.info("Trying to select the reserve static ip {vm.get('reserved_static_ip')}")
            restore_options_dialog.select_dropdown_values(drop_down_id = "reserved-static-ip",
                                                          values = [vm.get("private_reserved_static_ip")],
                                                          partial_selection = True)
        
        self.log.info("Successfully set the IP config") #add name of reserved static ip
...
#-------------------------------------------------------------------------#
    ...
    def project_and_zone(self, restore_options_dialog, project_id, zone):
        ...
        self.__admin_console.wait_for_completion()
        self.log.info(f"proj id = {project_id}, zone = {zone}")
        self.log.info(f"zone[0:-2] = {zone[0:-2]}")
