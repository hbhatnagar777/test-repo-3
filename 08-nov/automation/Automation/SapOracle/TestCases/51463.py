...
class TestCase(CVTestCase):
    ...
    def _run_backup(self, subclient, backup_type):
        """Starts backup job"""
        log = logger.get_log()
        log.info("*" * 10 + " Starting Subclient {0} Backup "\
                 .format(backup_type) + "*" * 10)
        job = subclient.backup(backup_type)
        log.info("Started {0} backup with Job ID: {1}".\
    ...
    def run(self):
        ...
        try:
            ...
            self._saporacle_db_connectpassword = SAPOraclehelper.get_saporacle_db_connectpassword(self, \
                                             str(self._saporacle_instanceid))
            ...
            self.DBFile = SAPOraclehelper.getdatafile(self, self._saporacle_db_user,\
                                                      self._saporacle_db_connectpassword,\
                                                      self._saporacle_db_connectstring, tblSpaceG1)
            log.info("Datafile location is "+str(self.DBFile))
            log.info("adding one tablespace and table before incremental backup")
            retcode = SAPOraclehelper.create_test_tables(self, self._saporacle_db_user,\
                                                         self._saporacle_db_connectpassword,\
                                                         self._saporacle_db_connectstring,\
                                                         self.DBFile, tblSpaceG1, tblNameG1, True)
            if retcode == 0:
                log.info("test data creation is sucessful")
            ...
            if not job.wait_for_completion():
                raise Exception("Failed to run INCREMENTAL backup job with error: "\
                                + str(job.delay_reason))
            log.info("Successfully ran INCREMENTAL backup")
            ...
            if status == 0:
                ...
                log.info("there is some issue with tablespace cleanup\
