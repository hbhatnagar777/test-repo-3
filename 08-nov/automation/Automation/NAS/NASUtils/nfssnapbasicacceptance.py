...
class NFSSnapBasicAcceptance(SnapBasicAcceptance):
    ...
    def proxy_mount(self, mountpoint, server, share):
        if not self.proxy.check_directory_exists(mountpoint):
            self.proxy.create_directory(mountpoint)
        if self.proxy.is_path_mounted(mountpoint):
            self._log.info(f"Unmounting NFS path: {mountpoint}")
            self.proxy.unmount_path(mountpoint)
        self.proxy.mount_nfs_share(mountpoint, server, share, cleanup=False, version=None)
    ...
    def add_content(self):
        if not self.automount:
            ...
            for content in self.sccontent:
                ...
                server = temp[0]
                share = temp[1]
                self.proxy_mount(mountpoint, server, share)
    ...
    def run(self):
        ...
        self.mount_validation(job.job_id, snap_copy_name, self.proxy)
        self.unmount_snap(job.job_id, snap_copy_name)
        self.unmount_validation(job.job_id, snap_copy_name, self.proxy)
        ...
        if self._inputs.get('RestoreClient'):
            linux_restore_client, linux_restore_location = \
                self.get_restore_client(self._inputs.get('RestoreClient'))
        else:
            linux_restore_client, linux_restore_location = \
        ...
        job = self._subclient.restore_in_place(self.restorecontent,
        ...
        for content in self.sccontent:
            self._nas_helper.copy_test_data_to_proxy(self.proxy, content)
        if(self._nas_helper.nas_vendor(self._client).upper())not in ['ISILON', 'HUAWEI', 'HNAS']:
            self.revert_snap(job.job_id, snap_copy_name)
            self.revert_validation(job.job_id, snap_copy_name, self.proxy)
