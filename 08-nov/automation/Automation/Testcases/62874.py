...
TestCase:
    ...
    create_pool()  --  Creates secondary pool
    ...
    run_backups()  --  Runs backup by creating new content
    
    get_key_id_of_first_job() --  Fetch the AF ID list of first backup job
    
    validate_from_log()  --  Performs validation from log
    
    get_key_id_list_of_copy()  --  Fetches the list of key ID for primary copy
    ...
    get_key_id_count()  --  Fetch keyID count of jobs other than the first job
    
    get_key_id_count_for_copy()  --  Fetching key ID count for the copy
    
    invalidate_key_ids()  --  Marks the existing key ID as invalid for the primary pool
    
    create_key_id()  --  Running jobs to create key ID
    ...
    Note : No other aux copy running on the source MA. This case may be failed on that case.
...
class TestCase(CVTestCase):
    """Verify registry key KeyMapCriticalSize for encryption cleanup"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Verify registry key KeyMapCriticalSize for encryption cleanup"
    ...
    def setup(self):
        ...
        (self.commcell_machine, self.commcell_path) = self.ma_helper.generate_automation_path(self.commcell.commserv_client.name, 5000)
    ...
    def tear_down(self):
        ...
        self.log.info(f"Removing reg key KeyMapMaxSize from CVD of MA [{self.source_ma}]")
        self.commcell.clients.get(self.source_ma).delete_additional_setting("Cvd", "KeyMapMaxSize")

        self.log.info(f"Removing reg key KeyMapMaxTimeout from CVD of MA [{self.source_ma}]")
        self.commcell.clients.get(self.source_ma).delete_additional_setting("Cvd", "KeyMapMaxTimeout")

        self.log.info(f"Removing reg key bEncKeyPrintMaps from CVD of MA [{self.source_ma}]")
        self.commcell.clients.get(self.source_ma).delete_additional_setting("Cvd", "bEncKeyPrintMaps")

        self.log.info(f"Removing reg key KeyMapCriticalSize from CVD of MA [{self.source_ma}]")
        self.commcell.clients.get(self.source_ma).delete_additional_setting("Cvd", "KeyMapCriticalSize")
        ...
        self.log.info("Re-Setting client side deduplication configuration to \"User storage policy settings\"")
    ...
    def run_backups(self,client,client_content,subclient_o):
        ...
        self.ma_helper.create_uncompressable_data(client, client_content, 0.2)
    ...
    def validate_from_log(self):
        ...
        """
        self.log.info("Log verification started")
        matched_line = None
        key_of_1st_job = self.get_key_id_of_first_job()

        ptrn = f"Removing archive file encryption key for keyId \[{key_of_1st_job}\] from the map. cipherRef \[[0-9]*\] refCount\[[0-9]*\] lastAccessTime \[[0-9]*\]"
        ...
        if matched_line is not None:
            ...
            if len(matched_line) == 1:
                self.log.info(f"Verified that the keyID[{key_of_1st_job}] removed, verification passed")
            else:
                self.log.info(f"There should be exactly 1 log line as keyID[{key_of_1st_job}] should be removed only once but found multiple occurrence.")
                raise Exception(f"There should be exactly 1 log line as keyID[{key_of_1st_job}] should be removed only once but found multiple occurrence.")

        else:
            self.log.error(f"Log line not found. Pattern: {ptrn}")
            raise Exception(f"Log line not found. Pattern: {ptrn}")

        self.log.info(f"No other KeyID should be removed from the KeyID map as total KeyID count is {self.get_key_id_count_for_copy()} and Critical reg key threshold set to {self.get_key_id_count()}. Verifying that.")
        key_id_list = self.get_key_id_list_of_copy()
        self.log.info(f"KeyID list: {key_id_list}")

        for key in key_id_list:
            key = key[0] # DB query returns a list for each row.
            if key != key_of_1st_job:
                self.log.info(f"Checking log if KeyID[{key}] removed from KeyID map. It should not be removed")
                ptrn = f"Removing archive file encryption key for keyId \[{key}\] from the map. cipherRef \[[0-9]*\] refCount\[[0-9]*\] lastAccessTime \[[0-9]*\]"
                self.log.info(f"Finding the pattern [{ptrn}] on source MediaAgent[{self.source_ma}]")
                matched_line = None
                (matched_line, matched_string) = self.dedupe_helper.parse_log(self.source_ma, "CipherCache.log", ptrn, escape_regex=False, single_file=False)
                if matched_line is not None:
                    ...
                    for l in matched_line:
                        self.log.info(l)
                    self.log.info(f"As per log, keyID[{key}] removed from the KeyID map. It's not expected. Failing the case")
                    raise Exception(f"As per log, keyID[{key}] removed from the KeyID map. It's not expected. Failing the case")
                self.log.info(f"KeyID[{key}] not removed from KeyID map. Validation passed.")

        self.log.info("Log verification passed.")

    def get_key_id_list_of_copy(self):
        """
        Fetchs the list of key ID for primary copy
        return:
            List of key ID
        """
        self.log.info("Getting the list of keyID")
        q = f"select encKeyId from ArchEncKeys where archCopyId={self.primary_pool_copy_o.copy_id}"
        self.log.info("Executing the following query")
        self.log.info(q)
        self.csdb.execute(q)
    ...
    def configure_sub_client(self, bs_name, sc_name, client_content, agent):
        ...
        Args:
            bs_name (str) - Name of the backup set
            sc_name (str) - Name of the sub client
            client_content (str) - Path of client content
            agent (Object) - Object of Agent class
        Return:
            subcluent (Object) - Object of Subclient class
        """
    ...
    def get_key_id_count(self):
        """
        Fetch keyID count of jobs other than the first job
        Return:
            KeyID count (int) -Key ID count other than the first job
        """
        ...
        sql_query = f"select count(distinct afc.encKeyId) " \
            ...
            f" map.jobId not in ({self.job_obj1.job_id})" \
            f" and afc.encKeyId!=0" \
            f" and afc.archCopyId={self.sp_primary_copy_o.copy_id}"
    ...
    def get_key_id_count_for_copy(self):
        """
        Fetching key ID count for the copy
        Return:
            key ID count(int) - Total number of key ID for the primary pool
        """
        self.log.info("Checking if KeyID count is more than 16 or not")
        q = f"select count(*) from ArchEncKeys where archCopyId={self.primary_pool_copy_o.copy_id}"
        self.log.info("Executing the following query")
        self.log.info(q)
        self.csdb.execute(q)
        return self.csdb.fetch_one_row()[0]

    def invalidate_key_ids(self):
        """
        Marks the existing key ID as invalid for the primary pool
        """
        q = f"update ArchEncKeys set isActive = 0 where archCopyId={self.primary_pool_copy_o.copy_id}"
    ...
    def create_key_id(self):
        ...
        while True:
            ...
            key_id_count = self.get_key_id_count_for_copy()
            self.log.info(f"KeyID Count: {key_id_count}")

            if int(key_id_count) > 16:
                self.log.info(f"Minimum required keyID count: 17. Found: {key_id_count}. No need to run more jobs")
                return

            self.log.info("KeyID count target is minimum 17, will run more jobs")
            self.log.info("Marking the current KeyIDs as inactive to crate new key ID on next jobs")
            self.invalidate_key_ids()
    ...
    def run(self):
        try:
            ...
            self.sp_primary_copy_o = self.sp_obj.get_copy("Primary")
            ...
            self.commcell.clients.get(self.source_ma).add_additional_setting("Cvd", "KeyMapMaxSize", "INTEGER", "2")
            ...
            self.log.info(f"KeyID count: {key_id_count}")
            self.log.info(f"Setting reg key KeyMapCriticalSize on CVD of MA [{self.source_ma}]")
            self.commcell.clients.get(self.source_ma).add_additional_setting("Cvd", "KeyMapCriticalSize", "INTEGER", str(key_id_count))
