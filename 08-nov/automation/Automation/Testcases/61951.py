...
class TestCase(CVTestCase):
    """ Class for executing PostgreSQL Block level out of place restore from command center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "PostgreSQL Block level out of place restore from command center"
        self.browser = None
    ...
    def check_prerequisite(self):
        ...
        if "unix" not in machine_object.os_info.lower():
            raise CVTestStepFailure("Block level feature is supported only on unix platform")
    ...
    def populate_data_and_backup(self, subclient_page, dbhelper_object, backup_type="INCR"):
        ...
        job_id = subclient_page.backup(backup_type=backup_level, enable_data_for_incremental="INCR" in backup_type)
        self.wait_for_job_completion(job_id)
        self.log.info("FSBASED backup is completed")
    ...
    def run(self):
        ...
        try:
            ...
            incr_bkp = self.populate_data_and_backup(subclient_page, dbhelper_object)
            ...
            self.check_and_run_backup_copy(incr_bkp)
            self.run_restore_validate(
