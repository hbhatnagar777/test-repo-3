...
Example JSON input for running this test case:
 "59173": {
          ...
          "access_key": "XXXX",
          "secret_key": "XXXX",
          "dest_secret_key": "XXXX",
          "dest_access_key": "XXXX",
          "rds_instance": "XXXX",
...
class TestCase(CVTestCase):
    """Admin Console: Cross account replication support  for RDS instances on command Center"""
    ...
    def __init__(self):
        ...
        self.name = "Cross account replication support  for RDS instances on command Center"
        self.browser = None
    ...
    def setup(self):
        ...
        try:
            ...
            self.dest_rds_helper = AmazonRDSCLIHelper(
                self.tcinputs['dest_access_key'], self.tcinputs['dest_secret_key'])
    ...
    def add_instance(self, content):
        ...
        if any("N. Virginia" in region for region in content.keys()):
            content["US East (Virginia) (us-east-1)"] = \
                content.pop("US East (N. Virginia) (us-east-1)")
    ...
    def create_secondary_snap_copy(self):
        ...
        source = " ".join(self.tcinputs['source_region'].split(" ")[:-1])
        destination = " ".join(self.tcinputs['destination_region'].split(" ")[:-1])
    ...
    def validate_rds_aux_snapshot(self, snapshot_name, region):
        """Waits for 2 minutes after aux and validates if RDS snapshot is replicated
        ...
        snapshot_replicated = self.dest_rds_helper.is_snapshot_present(snapshot_name, region)
    ...
    def cleanup(self):
        ...
        if self.created_snapshot:
            ...
            self.dest_rds_helper.delete_snapshot(f"copyof-{self.created_snapshot}",
    ...
    def run(self):
        ...
        try:
            ...
            content = {self.tcinputs['source_region']: [instance_name]}
            ...
            self.rds_subclient.enable_cross_account_operations(
                self.tcinputs['destination_account'])
            self.submit_backup()
    ...
    def tear_down(self):
        ...
        self.rds_helper.delete_cluster(f"{self.tcinputs['rds_instance']}restored",
                                       self.destination_region)
