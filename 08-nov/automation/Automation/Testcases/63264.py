...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Metallic - File server interactive install & backup via gateway using new tenant admin"
        self.browser = None
        self.fs_obj = None
    ...
    def configure_server(self):
        ...
        self.admin_console.select_radio("INTERACTIVE")
        self.admin_console.select_radio(self.os_name.lower())
        self.set_installinputs(isClient=True)
    ...
    def configure_local_storage(self):
        ...
        if ":" not in self.tcinputs.get("Disklibpath"):
            isnetworkpath = True
            self.srcdrive = self.gatewaymachine.mount_network_path(self.tcinputs.get("Disklibpath"),
                                                                   self.tcinputs.get(
                                                                       "UNCusername"),
        ...
        if not self.metallic_fs.check_if_localstorage_exist(
            ...
            self.gatewaymachine.create_directory(
                self.mountedpath, force_create=True)
    ...
    def verify_plan(self, planname):
        ...
        if planname in myplans.list_plans():
            ...
            if "Run incremental every 2 hour" in rpo.get_schedules()[0]:
                self.log.info("plan %s have correct rpo settings", planname)
            else:
                raise Exception(
                    "plans %s does not have correct rpo settings", planname)
        else:
    ...
    def cleanup(self):
        ...
        self.gatewaymachine.modify_ace('Everyone', self.diskpath,
        ...
        self.navigator.navigate_to_servers()
        if myservers.is_client_exists(
                self.gatewayname, select_from_all_server=True):
            myservers.retire_server(
                self.gatewayname,
                select_from_all_server=True)
        time.sleep(30)
        ...
        testmachine = Machine(machine_name=self.tcinputs.get("GatewayFQDN"),
            username=self.tcinputs.get("GatewayUserName"),
            password=self.tcinputs.get("GatewayPassword"))
