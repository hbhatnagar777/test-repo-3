...
Testcase Params:
    - job_ids   (str)   -   comma seperated list of job ids. Example: "1,2,3,4,5"
                            will be used to populate active jobs. alternatively, app_ids can be given
    - app_ids   (str)   -   comma seperated list of app ids of subclients to start backup job.
                            will be used to populate active jobs
    Note: if both are not given, testcase will assume jobs table is already populated
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.browser = None
        self.name = "Searching, Sorting , filtering and view creation validation from active jobs page"
        self.tcinputs = {}
        self.error_tracking = {}
    ...
    def run_method_thread(self, method):
        ...
        try:
            ...
            jobs_helper = JobsHelper(admin_console=admin_console, commcell=self.commcell)
            jobs_helper.setup_active_jobs(sort=False, view='All')
            rt_helper = jobs_helper.active_jobs_table_helper
    ...
    def run(self):
        ...
        try:
            ...
            if len(self.commcell.job_controller.active_jobs(lookup_time=0)) < 5:
                self.jobs_helper.setup_multiple_jobs()
            else:
                self.log.info("avoiding jobs setup as there are enough active jobs present")
