...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Index Server k8s - Performance metrics for cied content collection load"
        self.tcinputs = {
            "ClusterName": None,
    ...
    def validate_push(self):
        ...
        _push_stats = self.kube_master.analyze_metrics(
            folder=_push_stats_folder,
            push_to_datasource=True,
    ...
    def validate_search(self):
        ...
        while _success_hits <= self._search_count:
            ...
            # content search
            _resp = self.cluster_api_obj.search_collection(name=self.collection, select_dict={
                dynamic_constants.FIELD_KEYWORD_SEARCH: f'"{_doc_gen.gen_word()}"'})
            if _resp[ctrl_const.FIELD_RESPONSE][ctrl_const.FIELD_NUMFOUND] > 0:
                _success_hits = _success_hits + 1
                self.log.info(f"Total Success hits count : {_success_hits}")
                if _success_hits > self._search_count:
            ...
            _resp = self.cluster_api_obj.search_collection(name=self.collection, select_dict={
                dynamic_constants.FIELD_KEYWORD_SEARCH: f'"{_doc_gen.gen_sentence(2,6)}"'})
            if _resp[ctrl_const.FIELD_RESPONSE][ctrl_const.FIELD_NUMFOUND] > 0:
                _success_hits = _success_hits + 1
                self.log.info(f"Total Success hits count : {_success_hits}")
                if _success_hits > self._search_count:
        ...
        time.sleep(self._wait_interval)
        self.kube_master.stop_monitor_threads = True
        _search_stats = self.kube_master.analyze_metrics(folder=_search_stats_folder)
        self.result_string = f"{self.result_string}{self._br_tag}" \
    ...
    def validate_load_collection(self):
        ...
        time.sleep(self._wait_interval)
        self.kube_master.stop_monitor_threads = True
        _after_stats = self.kube_master.analyze_metrics(folder=_after_stats_folder)
        ...
        self.result_string = f"{self.result_string}{'*' * 50} Collection Load Performance {'*' * 50}{self._br_tag}{self._br_tag}" \
                             f"Collection Type : Metadata + Content {self._br_tag} " \
                             ...
                             f"Loaded DCube Server ID : {_collection_stats[ctrl_const.FIELD_INGRESS_SERVER_ID]}{self._br_tag}{self._br_tag}" \
    ...
    def validate_unload(self):
        """unloads the collection and validates whether jvm is released"""
        _collection_stats = self.cluster_api_obj.get_loaded_collection_stats(dump_in_log=True)[self.collection]
        _running_pods = self.kube_master.get_pods(namespace=ctrl_const.IS_NAME_SPACE, ready_only=True)
        _servers = _collection_stats[ctrl_const.FIELD_ALL_CORE_SERVER_ID]
        _pods = [item for item in _running_pods if item.startswith(tuple(_servers))]
        # validate jvm release for all pods where collection cores was loaded
        _before_pod_load = self.cluster_api_obj.get_servers(dump_in_log=True)
        # unload the core
        self.log.info(f"Going to unload collection - {self.collection}")
        self.cluster_api_obj.unload_collection(collection_name=self.collection)
        time.sleep(self._wait_interval * 1.5)

        # collect current stats of pod after unload
        _after_pod_load = self.cluster_api_obj.get_servers(dump_in_log=True)

        self.result_string = f"{self.result_string}{self._br_tag}{'*' * 50} Collection Un-Load JVM Usage Metrics {'*' * 50}{self._br_tag}{self._br_tag}"
        self.log.info(f"Before Load - {_before_pod_load} After load - {_after_pod_load}")
        # analyze jvm release for all pods
        for old_pod in _before_pod_load:
            for new_pod in _after_pod_load:
                if old_pod[ctrl_const.FIELD_SERVER_ID] == new_pod[ctrl_const.FIELD_SERVER_ID]:
                    if ctrl_const.FIELD_USAGES in old_pod and ctrl_const.FIELD_USAGES in new_pod:
                        _before_memory = int(old_pod[ctrl_const.FIELD_USAGES][-1][ctrl_const.FIELD_JVM_PERCENT])
                        _after_memory = int(new_pod[ctrl_const.FIELD_USAGES][-1][ctrl_const.FIELD_JVM_PERCENT])
                        self.log.info(
                            f"POD : {new_pod[ctrl_const.FIELD_SERVER_ID]} | SPEC : {new_pod[ctrl_const.FIELD_RESOURCE_REQ]} |  JVM Memory usage Percent before unload [{_before_memory}] and after unload [{_after_memory}]")
                        self.result_string = f"{self.result_string}{self._br_tag}POD : {new_pod[ctrl_const.FIELD_SERVER_ID]} | SPEC : {new_pod[ctrl_const.FIELD_RESOURCE_REQ]} | JVM Memory usage Percent before unload [{_before_memory}] and after unload [{_after_memory}]{self._br_tag}{self._br_tag}"
    ...
    def run(self):
        ...
        try:
            ...
            report_folder = os.path.join(GeneralConstants.CONTROLLER_FOLDER_PATH, str(
                self.build_id), GeneralConstants.CONTROLLER_REPORTS_FOLDER_NAME,
                GeneralConstants.CONTROLLER_EXPORT_HTML_FOLDER_NAME)
            report_share_folder = os.path.join(GeneralConstants.CONTROLLER_SHARE_FOLDER_PATH, str(
                self.build_id), GeneralConstants.CONTROLLER_REPORTS_FOLDER_NAME,
                GeneralConstants.CONTROLLER_EXPORT_HTML_FOLDER_NAME)
            report_files = []
