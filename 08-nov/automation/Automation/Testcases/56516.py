...
class TestCase(CVTestCase):
    """ Class for executing Basic acceptance Test for MySQL IDA on Command Center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "ACCT1-Acceptance Test for MySQL IDA Command Center"
        self.browser = None
    ...
    def run(self):
        ...
        try:
            ...
            self.restore_panel = self.database_group.restore_folders(
                DBInstances.Types.MYSQL, content_to_restore)
            self.restore_validate(data_restore=True, log_restore=True,
                                  db_info=db_info_before_restore)
