...
class TestCase(CVTestCase):
    ...
    def run(self):
        ...
        try:
            ...
            modifysub = subclient._set_subclient_properties(
            "_sapForOracleSubclientProp['sapBackupDevice']", str(1))
            ...
            if retCode == True:
                self.log.info("hook File failSAPDataBackupB4Intimate\
                is creatinged sucessfully on client")
            ...
            if self.readpatteren.find(" set restartmode = true") >= 0:
                self.log.info("found correct brbackup restart string in ClsapAgent.log")
            if self.readpatteren.find("rmanConnectAddNull=[brbackup -d util_file -t online -m ") >= 0:
                self.log.info("found correct brbackup restart string for configphase in ClsapAgent.log")
            self.log.info("checking cs db for data archivefiles validation")
            self.log.info("##getting application size from CS db for the restarted backup job##")
            ...
            retCode = self.machine.remove_registry('OracleSapAgent',\
                                                   'nSAPEnableGUIConfigPhaseResume')
            if retCode == True:
            ...
            job = self.saporacle_helper.run_random_kill_process(subclient, 'FULL', self.machine)
            self.log.info(job)
            ...
            self.log.info("running 6th inplace restore job")
            job = self.instance.restore_in_place()
            self.log.info("Started Current time restore to same client job with \
            Job ID: {0} job_id ".format(str(job.job_id)))
