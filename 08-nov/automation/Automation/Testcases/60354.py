...
TestCase:
    ...
    review_move_action_filesystem()              -- Moves files from a FS datasource and verifies the operation
...
class TestCase(CVTestCase):
    """Class for executing move action in review page for all files with UNC path present on a page"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Move action for all files with UNC path in a review page in SDG"
        self.tcinputs = {
    ...
    def create_sdg_project(self):
        ...
        self.log.info(f"Sleeping for {cs.ONE_MINUTE} seconds")
        time.sleep(cs.ONE_MINUTE)
        self.count_before_review = int(
            self.gdpr_base.data_source_discover_obj.get_total_number_after_crawl())

    @test_step
    def review_move_action_filesystem(self):
        """
        Verifies that the files are moved to the destination path after move operation
        """
        count_after_move = 0
        ...
        files_before_review = self.source_machine.get_files_in_path(
            self.tcinputs["FileServerDirectoryPath"])
        self.count_before_review = len(files_before_review)
        ...
        try:
            ...
            self.log.info(
                f"Files present in the destination path {destination_path_filenames}")
            ...
            if count_after_move == self.count_reviewed_files and (not source_file_present):
                ...
                raise CVTestStepFailure(
                    "Couldn't move the files to the destination path")
    ...
    def review_move_action_full_crawl_filesystem(self):
        ...
        self.log.info(
            ...
            f"Number of files after the move operation {self.count_after_review}")
        if self.count_reviewed_files != self.count_before_review - self.count_after_review:
