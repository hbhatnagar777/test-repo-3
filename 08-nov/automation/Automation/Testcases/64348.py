...
from Web.AdminConsole.Salesforce.constants import SeedRecordType, GroupOperation, ColumnOperation
from Web.AdminConsole.Salesforce.restore import SimplifiedFilterModel, RuleModel, RuleGroupModel
...
APP_NAME = f"TC_64348_{datetime.now().strftime('%d_%B_%H_%M')}"
OUT_OF_PLACE_APP_NAME = f"{APP_NAME}_out_of_place"


class TestCase(CVTestCase):
    """Class for executing sandbox seeding with field filters test for Salesforce on Command Center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super().__init__()
        self.name = "Salesforce: Sandbox seeding using advance filter"
        self.browser = None
    ...
    def create_seed_template(self):
        """Method to create new sandbox seeding template with field filters"""
        try:
            ...
            template_config = TemplateConfig(self.sf_helper.sf_object, SeedRecordType.SQL, param=simplified_model)
