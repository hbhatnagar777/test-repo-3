...
"""Live Sync IO: VM Recovery ( DR Operations): Clone VM Operations for continuous pair
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Live Sync IO: VM Recovery ( DR Operations): Clone VM Operation"
        self.vm = None
        ...
        self.view_name = "56740_TC_view"
        self.ccrp_2 = "5 minutes"
        self.acrp_2 = "1 hours"
        self.clone_vm_options = {
            "clone_vm_name": "Automation56738_TestVm",
    ...
    def verify_pair_status(self, source_vm, destination_vm, pair_status):
        ...
        self.utils.assert_comparison(table_content['Synchronization status'], pair_status)
    ...
    @test_step
    def perform_clone_vm(self, clone_vm_options):
        """Perform test boot on the destination VM"""

        self.admin_console.navigator.navigate_to_continuous_replication()
        self.continuous_monitor.select_view(self.view_name)
        test_boot_job_id = self.continuous_monitor.continuous_clone_vm(clone_vm_options)
    ...
    @test_step
    def attach_network(self, clone_vm_name):
        """Attaches NIC card to test boot VM"""

        self.dr_helper.source_auto_instance.hvobj.VMs = clone_vm_name
        self.testboot_vm = self.dr_helper.source_auto_instance.hvobj.VMs[clone_vm_name]
    ...
    def test_diff(self):
        ...
        self.test_failover.update_testfailovervm_details(self.clone_vm_options['clone_vm_name'])
        self.test_failover.post_validation()

    @test_step
    def delete_clone_vm(self, source_vm_name, testboot_vm_name):
        """Delete the current test boot VM"""
        self.admin_console.navigator.navigate_to_continuous_replication()
        self.pair_details = self.continuous_monitor.access_pair_details(source_vm_name)
        self.pair_details.continuous_delete_testboot(self.clone_vm_options['clone_vm_name'])
    ...
    def run(self):
        ...
        try:
            ...
            sleep(200)
            self.clone_vm_options['recovery_type'] = "Oldest point in time"
            self.perform_clone_vm(self.clone_vm_options)
            sleep(150)
            self.attach_network(self.clone_vm_options['clone_vm_name'])
            self.test_diff()

            self.delete_clone_vm(self.source_vm, self.clone_vm_options['clone_vm_name'])
            ...
            sleep(600)
            self.get_rp_stats()
            self.clone_vm_options['recovery_point'] = self.rpstat_details1[-1]
            self.perform_clone_vm(self.clone_vm_options)
            sleep(50)
            self.attach_network(self.clone_vm_options['clone_vm_name'])
            self.test_diff()
            ...
            self.clone_vm_options['recovery_point'] = self.rpstat_details1[-1]
            self.clone_vm_options['recovery_type'] = "Application consistent recovery point time"
            self.perform_clone_vm(self.clone_vm_options)
            sleep(50)
            self.attach_network(self.clone_vm_options['clone_vm_name'])
            self.test_diff()
            ...
            self.admin_console.refresh_page()
            sleep(400)
            self.clone_vm_options['recovery_type'] = "Latest recovery point"
            self.perform_clone_vm(self.clone_vm_options)
            sleep(50)
            self.attach_network(self.clone_vm_options['clone_vm_name'])
            self.test_diff()
