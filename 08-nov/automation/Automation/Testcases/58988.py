...
APIs being validated inside collection file:
    GET /client
    POST /QCommand/qoperation execute using Modified DisplayName
    POST /Client/{{clientId}} using Modified DisplayName
    GET /client/{{clientId}}
    GET /Client/byName(clientName='{{ModifiedDisplayName}}')
    GET /Client/byName(clientName='{{clientName}}')
    DELETE /Client/{{clientId2}}?forceDelete=1
...
class TestCase(CVTestCase):
    """Class for executing set of REST APIs using display name of a client"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = ("[REST API]: Validation of APIs when display name of a client "
                     "is used as client name")
    ...
    def run(self):
        ...
        try:
            collection_json = 'APICallsWithDisplayNames.collection.json'
