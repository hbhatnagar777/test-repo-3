...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Exchange Hybrid (with on-premises Exchange Server) Mailbox GUID verification"
        ...
        self.local_exchange_users: list = list()
        self.migrated_users: list = list()
        self.online_users: list = list()
        self.mailboxes_list: list = list()
        self.smtp_list: list = list()
        self.powershell: ExchangePowerShell = None
        self.on_prem_powershell: ExchangePowerShell = None
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info(
                "-----------------------VERIFYING GUID for LOCAL USERS"
                "-------------------------------")
            local_users_guid = self.exmbclient_object.csdb_helper.get_mailbox_guid(
                mailbox_list=self.local_exchange_users)
            self.log.info("Local Exchange Users GUID from CSDB: {}".format(local_users_guid))

            local_users_ad_guid = self.on_prem_powershell.get_mailbox_guid(mailbox_list=self.local_exchange_users,
                                                                           environment="OnPrem")
            self.log.info("Local Exchange Users GUID from AD: {}".format(local_users_ad_guid))

            _discv_status = exmb_utils.verify_mailbox_guids_association(local_users_ad_guid.values(),
                                                                        local_users_guid.values())
            self.log.info("GUID verification for Local Exchange users: {}".format(_discv_status))

            if not _discv_status:
                self.log.error("Local Exchange Users GUID did not match")
                raise Exception("Local Excahnge Users GUIDs did not match")
