...
class TestCase(CVTestCase):
    """Class for executing
        Hadoop SnapDiff Data Protection - Full,Incremental,Differential,Synthfull
        ...
        Step15, Run a differential backup for the subclient and verify if job fails with exception as expected
        Step16, Add new data for the incremental
        Step17, Run a synthfull job
        Step18, Run an incremental backup after synthfull for the subclient and verify it completes without failures.
        Step19, Run a restore of the incremental backup data and verify correct data is restored.
        Step20, Run a find operation for the incremental job and verify the returned results.
        Step21, Perform all modifications on the existing data.
        Step22, Run a synthfull job
        Step23, Run an incremental backup after synthfull for the subclient and verify it completes without failures.
        Step24, Run a restore of the incremental backup data and verify correct data is restored.
        Step25, Run a find operation for the incremental job and verify the returned results.
        Step26, Run a restore of the complete subclient data and verify correct data is restored.
        Step27, Run a find operation for the entire subclient and verify the returned results.
        """

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Hadoop SnapDiff Data Protection - Full,Incremental,Differential,Synthfull"
    ...
    def setup(self):
        ...
        self.log.info("""Hadoop SnapDiff Data Protection - Full,Incremental,Differential,Synthfull
        This test case does the following
        Step1,  Create instance for this testcase if it doesn't exist.
        Step2,  Create subclient for this testcase if it doesn't exist.
        Step3,  Add full data for the current run.
        ...
        self.instance_name = f"Instance_{self.id}_optimized"
        ...
        self.client_machine.remove_directory(subclient_content[0])
    ...
    def run_backup_and_validate(self, backup_level):
        ...
        if backup_level != "Synthetic_full":
            old_snap_list = self.client_machine.fetch_snapshots(locations=self.helper.testcase.subclient.content)
        ...
        if backup_level != "Synthetic_full":
            self.helper.verify_scan_type_used(self.backup_job.job_id, validate_scan=self.scan_type)
            self.snaps_to_be_deleted = self.helper.verify_postops_functionality(self.backup_job.job_id,
                                                                                old_snap_list=old_snap_list,
                                                                                deleted_snap_list=self.snaps_to_be_deleted)
    ...
    def run_incremental_and_differential(self):
        ...
        self.log.info("Run a differential backup for the subclient and verify if job fails with exception as expected")
        try:
            self.helper.run_backup_verify(scan_type=self.scan_type, backup_level="Differential")[0]
        except Exception as excp:
            if "Differential Job not supported for FileServer Changelist Backup" in excp.args[0]:
                self.log.info(f"Differential job failed as expected")
            else:
                raise Exception(f"Differential job should be failed, exception:{excp}")
