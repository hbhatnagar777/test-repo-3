...
class TestCase(CVTestCase):
    """Command Center- Network topology- OneWay CC->CS(server)"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Command Center- Network topology- OneWay CC->CS(server)"
        self.browser = None
        self.topology_name = "CG_60331_CC_CS"
        self.admin_console = None
        self.navigator = None
        self._client_group_name1 = "CG_60331_Client"
        self._client_group_name2 = "CG_60331_Server"
        self.tcinputs = {
            "SourceClient": ""
        }
                ...
                if route_type == "passive":
                    ...
                    if int(streams) > 1:
                        if ("type=" + route_type in i and "proto=" + protocol + " " in i) and "streams=" + streams in i:
                            ...
                            #                         f' {client_group}')
                            pass # comment out until the validation is added
                    else:
    ...
    def download_network_summary(self):
        ...
        try:
            if self._client_group_name1 in self.smart_groups or self._client_group_name2 in self.smart_groups:
                ...
                # Validate if the files have the routes configured in topology
