...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "AMAZON : V2: Snap Synthetic backup and Full VM restore from Snap and Backup Copy Automated"
        self.browser = None
        self.vsa_obj = None
    ...
    def run(self):
        ...
        try:
            ...
            decorative_log("Running an Synthetic backup")
            ...
            try:
                decorative_log("Full VM Restore from VM Level - (Backup Copy)")
                self.vsa_obj.snap_restore = True
            ...
            try:
                decorative_log("Full VM Restore from VM Level - (Snap Copy)")
                self.vsa_obj.snap_restore = True
                self.vsa_obj.full_vm_restore(vm_level=True)
            except Exception as exp:
                self.utils.handle_testcase_exception(exp)
