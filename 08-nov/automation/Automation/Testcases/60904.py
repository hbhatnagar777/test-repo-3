...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Data Classifier - Validate classification model sync & Deletion of classifier using DC plan with Multiple CA"
        self.tcinputs = {
    ...
    def create_classifier(self):
        ...
        accuracy = self.gdpr_obj.classifier_obj.get_training_accuracy()
        accuracy = accuracy[:-1]
        if int(accuracy) >= int(self.tcinputs['ClassifierAccuracy']):
            self.log.info("Classifier accuracy matched - %s", accuracy)
        else:
            self.log.info("classifier accuracy not matched")
            raise Exception("Classifier accuracy not matched. Please check")
    ...
    def validate_delete_classifier(self):
        """Delete the classifier and validates ML data / Solr dataset data got deleted in backend for trained/Sync CA"""
        result, info = self.ca_helper.is_classifier_model_data_exists(name=self.classifier_name, trained_only=False)
        if not result:
