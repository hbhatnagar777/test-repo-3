...
is working correctly or not for v1 indexing client
...
input json file arguments required:
                        ...
                        "testcaseFactor": "factor to change the data content ->
                         can control the overlap of DDBbackup job
                         with sidb2.exe process to be online"
                        ...
                        "dedup_path": "enter the unix path to DDB
                         folder for creating dedup store path"

Design steps:
for v1 indexing client
...
3.	Run backup with new content (we need to run backups on multiple subclients,
    all pointing to same store with generated content so that
    SIDB2 process is up for some good enough time)
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = """UNIX Suspend / Resume case for DDBBackup Job for v1 indexing client"""
        self.tcinputs = {
            "MediaAgentName": None,
            "testcaseFactor": None,
            "mount_path": None,
    ...
    def validations_checks(self):
        ...
        # since the DDBBackup job was suspended thus snapshot creation will
        # take place again
    ...
    def tear_down(self):
        ...
        try:
            ...
            if self.commcell.storage_policies.has_policy(
                    self.storage_policy_name):
                self.commcell.storage_policies.delete(self.storage_policy_name)
                self.log.info("Testcase storage policy deleted")
            else:
                self.log.info("Testcase storage policy does not exist.")

            if self.commcell.disk_libraries.has_library(self.library_name):
                self.commcell.disk_libraries.delete(self.library_name)
                self.log.info("Testcase Library deleted")
            else:
                self.log.info("Testcase Library does not exist.")
