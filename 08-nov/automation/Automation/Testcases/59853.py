...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Basic acceptance test to verify all the permissions in " \
    ...
    @test_step
    def verify_folder_file_permissions(self):
        """
            Method to verify all the folder and file permissions
        """
        num_mismatched = 0
        self.entitlement_manager.browse.expand_tree()
        self.entitlement_manager.browse.read_nodes_permissions()
        all_permissions = self.entitlement_manager.browse.all_permissions
        self.log.info("Total number of files [{}]".format(len(all_permissions)))
        if len(all_permissions) > 0:
            for current in all_permissions:
                try:
                    ...
                    if not self.fso_helper.verify_permission(permissions_ui,
                                                             json.loads(permissions_source)):
                        num_mismatched = num_mismatched + 1
                        self.log.error("Total number of mismatch [{}]".format(num_mismatched))
                except Exception as e:
                    self.log.error(e)

            if num_mismatched > 0:
                self.test_case_error = "There are [{}] files/folders for which permissions " \
                                       "did not match".format(num_mismatched)
    ...
    def get_file_name(self, current):
        ...
        Args:
            current (str) - current file information
        Returns:
            str -  file_name
        """
    ...
    def run(self):
        try:
            ...
            self.verify_folder_file_permissions()
            self.perform_cleanup()
