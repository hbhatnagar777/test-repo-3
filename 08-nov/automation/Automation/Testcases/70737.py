...
class TestCase(CVTestCase):
    test_step = TestStep()
    """Class for executing
        IBMi - Backup and restore validation of pre-defined subclient *ALLUSR from command center.
    """

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "IBMi - Backup and restore validation of pre-defined subclient *ALLUSR from command center."
    ...
    def generate_client_data(self, run_count=0):
        ...
        if run_count == 0:
            ...
            self.client_machine.create_sourcepf(library=self.usr_lib[0], object_name='INC{0}'.format(self.id))
        self.client_machine.populate_lib_with_data(library_name=self.usr_lib[run_count], tc_id=self.id, count=2)
    ...
    def verify_client_logs(self, run_count=0):
        ...
        if run_count == 0:
            ...
            self.helper.verify_from_log('cvbkp*.log',
                                        ...
                                        jobid=self.job_id,
                                        expectedvalue='[SAVLIB]:[LIB({0}'.format(self.usr_lib[run_count])
                                        )
            self.helper.verify_from_log('cvbkp*.log',
        ...
        if run_count == 1:
            ...
            self.helper.verify_from_log('cvbkp*.log',
                                        'Processing JOBLOG for',
                                        jobid=self.job_id,
                                        expectedvalue='[SAVCHGOBJ]:[OBJ(*ALL) LIB({0})'.format(self.usr_lib[0])
                                        )
            self.log.info("Verify if New library is backedup as full using SAVLIB command.")
            self.helper.verify_from_log('cvbkp*.log',
                                        'Processing JOBLOG for',
                                        jobid=self.job_id,
                                        expectedvalue='[SAVLIB]:[LIB({0}'.format(self.usr_lib[1])
                                        )
        ...
        if run_count == 2:
            ...
            self.helper.verify_from_log('cvrest*.log',
                                        'QaneRsta',
                                        jobid=self.job_id,
                                        expectedvalue="SAVLIB({0}".format(self.usr_lib[0])
                                        )
            self.helper.verify_from_log('cvrest*.log',
                                        'QaneRsta',
                                        jobid=self.job_id,
        ...
        if run_count == 3:
            ...
            self.helper.verify_from_log('cvrest*.log',
                                        'QaneRsta',
                                        jobid=self.job_id,
                                        expectedvalue="SAVLIB({0}".format(self.usr_lib[1])
                                        )
            self.helper.verify_from_log('cvrest*.log',
                                        'QaneRsta',
                                        jobid=self.job_id,
    ...
    def run(self):
        ...
        try:
            ...
            self.cleanup()
            self.log.info("*** *ALLUSR subclient backup & Restore validation is completed successfully **")
