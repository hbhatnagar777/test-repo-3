...
class TestCase(CVTestCase):
    """Class for executing Basic Acceptance for NDMP - HNAS Advanced Backup Option - Use snapshot rule"""

    def __init__(self):
        """"Initializes TestCase object"""
        super(TestCase, self).__init__()
        self.name = "NDMP - HNAS Advanced Backup Option - Use snapshot rule"
        self.tcinputs = {
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Run full backup job for NDMP iDA with option use snapshot rule")
            job = self.subclient.backup("FULL", snap_name=self.tcinputs['UseSnapshotRule'])
            ...
            if mamachine.get_logs_for_job_from_file(
                ...
                self.log.info(
                    'NDMP backup job is run with Use snapshot Rule set to ' + str(self.tcinputs['UseSnapshotRule']))
            else:
                self.log.info(
                    'NDMP backup job is NOT run with Use snapshot Rule set to '+ str(self.tcinputs['UseSnapshotRule']))
                raise Exception(
                    'NDMP backup job is NOT run with Use snapshot Rule set to '+ str(self.tcinputs['UseSnapshotRule']))
