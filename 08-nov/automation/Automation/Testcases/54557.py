...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Test case for Simultaneously replace two bricks on a " \
                    "single sub-volume while a few backups are running"
        self.result_string = ""
        self.policy = ""
        self.policy_name = ""
        self.subclient_obj = ""
    ...
    def run(self):
        ...
        """
        try:
            self.log.info("*************Simultaneously replace two bricks on a single "
            ...
            self.sub_vol_replace = self.sub_vol[0][0]
            ...
            self.log.info("Bricks of sub volume %s for replacement are %s ", self.sub_vol_replace,
                          replace_bricks)
            ...
            self.node1_machine = Machine(self.node1, self.commcell)
            self.node2_machine = Machine(self.node2, self.commcell)
            ...
            self.log.info("Nodes %s and %s part of Pool %s", self.node1, self.node2, self.storage_pool_name)
            ...
            self.hyperscale_helper.check_gluster_brick_online(self.node1,
                                                              self.storage_pool_name,
                                                              self.device_os_path1)
            self.hyperscale_helper.gluster_healthy_brick_status(self.node_id1,
                                                                self.device_os_path1)

            self.hyperscale_helper.check_gluster_brick_online(self.node2,
                                                              self.storage_pool_name,
                                                              self.device_os_path2)
            self.hyperscale_helper.gluster_healthy_brick_status(self.node_id2,
                                                                self.device_os_path2)
            ...
            if not flag1 & 4 and not flag2 & 4:
                ...
                while True:
                    if self.job_obj.phase == "Backup":
                        ...
                        lines_cvma = self.hyperscale_helper.get_lines_in_log_file(self.node1_machine, self.node_cvma_log_path)
                        ...
                        replace_flag = self.hyperscale_helper.brick_flag_status(self.node_id1, self.device_os_path1)
                        ...
                        status, response_log = self.hyperscale_helper.replace_log_status(self.node1, from_line=lines_cvma)
                        self.log.info("Status of Replace %s", status)
                        for log in response_log:
                            self.log.info(log)
                        # Repalce failure re mounting the disk again
                        if not status:
                            self.hyperscale_helper.mount_brick(self.node1,
                                                               self.device_os_path1,
                                                               self.storage_pool_name)
                        ...
                        flag2 = self.hyperscale_helper.brick_flag_status(hostid=self.node_id2,
                                                                         device_os=self.device_os_path2)
                        count = 20
                        while not flag2 & 16 and count >= 20:
                        ...
                        lines_cvma = self.hyperscale_helper.get_lines_in_log_file(self.node2_machine, self.node_cvma_log_path)
                        ...
                        # Replace success failure logs
                        status, response_log = self.hyperscale_helper.replace_log_status(self.node2, from_line=lines_cvma)
                        self.log.info("Status of Replace %s", status)
                        for log in response_log:
                            self.log.info(log)
                        # Repalce failure re mounting the disk again
                        if not status:
                            self.hyperscale_helper.mount_brick(self.node2,
                                                               self.device_os_path2,
                                                               self.storage_pool_name)
                            self.log.error('Replacement failed')
                            raise Exception(response_log)
