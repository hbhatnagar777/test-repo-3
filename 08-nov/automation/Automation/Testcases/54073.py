...
class TestCase(CVTestCase):
    '''
        Scenario 1: verify that prescan with all modifying/removing  operations will be'
                    'picked as modified files and trigger a backup DropBox Path from dropbox moniker.
        Pre requirement : DropBox should be configured and dropbox path should be given in the inputs.

            1: Adding subclient with dropbox moniker along with plan monikers.
    ...
    def __init__(self):
        ...
        self.name = "DropBox moniker prescan validation"
    ...
    def run(self):
        ...
        DropBoxPath = self.tcinputs['DropBoxPath']
        ...
        try:
            ...
            DropBoxPath = DropBoxPath + self.slash_format + "Inc1"
            ...
            self._utility.create_directory(machine, DropBoxPath)
            #machine.create_directory(DropBoxPath)
            if self.slash_format in '/':
                machine.generate_test_data(DropBoxPath, hlinks=False, slinks=False, sparse=False,
            ...
            machine.modify_test_data(DropBoxPath, modify=True)
            ...
            machine.modify_test_data(DropBoxPath, rename=True)
            ...
            machine.remove_directory(DropBoxPath)
