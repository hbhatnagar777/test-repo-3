...
class TestCase(CVTestCase):
    """Class for executing
    Verification of search filter with single values,
    ...
    def __init__(self):
        ...
        self.name = "Verification of search filter with single values, " \
                    "for Compliance Search from admin console"
    ...
    def init_tc(self):
        ...
        try:
            ...
            self.suggestion_folder_name = self.tcinputs['Folder_suggestion']
    ...
    def query_indexserver_get_results(self):
        ...
        if self.fq_string == "":
            self.fq_string = self.fq_string + "DocumentType:2"
        else:
            self.fq_string = self.fq_string + " AND DocumentType:2"
    ...
    def apply_search_filters_from_clientname_attachment_custodian(self):
        ...
        """
        from_user = random.choice(self.sender_users_list)
        client_name = random.choice(self.clients_filter_list)
        client_id = self.solr_search_obj.get_client_id(client_name)
        has_attach_value_single = random.choice(self.has_attach_list)
        custodian_user = random.choice(self.custodian_users_list)

        filters_list = {
            "From": [from_user],
            "Client name": [client_name],
            "Attachment": [has_attach_value_single],
        ...
        if parameters_search["From"]:
            from_fq_search_string = "FromDisplay_idx:\"" + from_user + "\""
            self.fq_string = self.fq_string + from_fq_search_string + " AND "

        if parameters_search["Custodian"]:
            custodian_fq_search_string = "OwnerName_idx:\"" + custodian_user + "\""
            self.fq_string = self.fq_string + custodian_fq_search_string + " AND "

        if parameters_search["Client name"]:
            client_fq_search_string = "ClientId:" + str(client_id)
            self.fq_string = self.fq_string + client_fq_search_string + " AND "

        if parameters_search["Attachment"]:
            hasattach_fq_search_string = "HasAttachment:" + has_attach_value_single
            self.fq_string = self.fq_string + hasattach_fq_search_string
        else:
            self.fq_string = self.fq_string[:-5]

    @test_step
    def apply_search_filters_textbox(self):
        """
        Apply filter by providing facet value in the textbox
            fire compliancesearch and perform Indexserver query
        """
        try:
            self.custom_filter.enter_custom_filters("Email folder", self.suggestion_folder_name)
            folder_fq_search_string = "Folder_sort:\"" + self.suggestion_folder_name + "\""
        except Exception as e:
            self.log.info(f"Couldnt find the facet")
            folder_fq_search_string = ""
        self.count_compliance_search = self.app.get_total_rows_count()


        self.fq_string = folder_fq_search_string
    ...
    def apply_search_filters_custodian_folder_modified_date_size(self):
        ...
        custodian_user = random.choice(self.custodian_users_list)
        test_folder = random.choice(self.test_folders_list)
        daterange = random.choice(self.modified_date_range_list)
        sizerange = random.choice(self.size_range_list)

        filters_list = {"Email folder": [test_folder],
                        "Custodian": [custodian_user]}

        parameters_search = self.custom_filter.apply_custom_filters_with_search(
            filters_list)
        if self.custom_filter.apply_date_filters([daterange]):
            parameters_search["Modified date"] = True
        else:
            parameters_search["Modified date"] = False

        if self.custom_filter.apply_size_filters([sizerange]):
        ...
        solr_date_range = self.solr_search_obj.get_date_range([daterange])
        solr_size_range = self.solr_search_obj.get_size_range([sizerange])
        ...
        if parameters_search["Email folder"]:
            folder_fq_search_string = "Folder_sort:\"" + test_folder + "\""
    ...
    def run(self):
        try:
            ...
            self.gov_app.select_compliance_search()
            self.compliancesearch_keyword_search()
            self.apply_search_filters_textbox()
            self.query_indexserver_get_results()
            self.validate_search_results()
