...
TestCase:
    ...
    verify_stub_and_recall()           --  Verify file meet archiving rules were stubbed and recall works

    verify_out_of_place_restore()      --  Run out of place restore and verify the restore result

    verify_in_place_restore()          --  Run in place restore and verify the restore result
...
class TestCase(CVTestCase):
    """Class for NFS AutoMount Multiple Paths Archive and Create Stub verification"""
    ...
    def __init__(self):
        ...
        self.name = "NFS AutoMount archiving with multiple paths - archive and create stub"
    ...
    def generate_test_data(self):
        ...
        for i in range(self.num_path):
            ...
            self.log.info(
                "Changing time stamps of all files to older than 90 days")
    ...
    def verify_stub_and_recall(self):
        """verify stubs are created correctly and recall stubs"""
        for i in range(self.num_path):
            self.OPHelper.test_file_list = self.test_file_list[i]
            self.OPHelper.client_machine.mount_nfs_share(
                nfs_client_mount_dir=self.mount_path,
            ...
            time.sleep(180)
            self.OPHelper.verify_stub(
                path=self.origin_mount_path,
                is_nas_turbo_type=True)

            # verify recall for some files
            if i == 0:
                self.OPHelper.recall(
                    org_hashcode=self.orig_hashcode[i],
                    path=self.origin_mount_path)
                self.verify_modified_time(path=self.origin_mount_path)
    ...
    def verify_number_of_backup_files(self):
        ...
        self.log.info("backed up items in dir: " + str(len(backedup_list)))
    ...
    def verify_out_of_place_restore(self, restoredata=True):
        ...
        self.OPHelper.restore_out_of_place(
            ...
            paths=file_paths,
            fs_options={
                'restoreDataInsteadOfStub': restoredata},
        ...
        if _matched is False:
            raise Exception(
                "At least one restored file does not match with the original")
    ...
    def verify_in_place_restore(self, restoredata=True):
        ...
        if not restoredata:
            self.verify_stub_and_recall()
            self.log.info(
                "Successfully verified onepass in place stub restore")
        if restoredata:
            for i in range(self.num_path):
                self.OPHelper.client_machine.mount_nfs_share(
                    nfs_client_mount_dir=self.mount_path,
                ...
                self.OPHelper.client_machine.unmount_path(
                    mount_path=self.mount_path, force_unmount=True)
                self.log.info(
                    "Successfully verified onepass in place data restore")
    ...
    def run(self):
        ...
        4. Verify the files are stubbed correctly, atime/mtime not changed and stubs can be recalled
        5. Run couple of more archive jobs to backup stubs.
        6. Restore out of place as data.
        7. Verify files are restored as data.
        8. Restore in place restore as data.
        9. Verify data are restored.
        10. In place restore as stub
        11. verify stubs are restored, atime/mtime not changed and could be recalled
        """
        try:
            self.log.info(_desc)
            ...
            self.log.info("Re-stub")
            self.OPHelper.run_archive(repeats=2)
            self.verify_out_of_place_restore()
            self.verify_in_place_restore(restoredata=True)

            self.log.info(
                'NFS AutoMount Multiple Path Archive and stub test case run successfully')
            ...
            try:
                ...
                self.log.info(Exception)
            self.log.error(
                'NFS AutoMount Multiple Path Archive and stub test is not running correctly with error: %s',
                exp)
