            ...
            "64266":
                {
                    ...
                    "RedirectPDBPath": redirect path for pdb clone,
                    "PDBName": "name of pdb to clone"
...
class TestCase(CVTestCase):
    """ Class for executing wallet restore for in place pdb clone Test for oracle rac """
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Oracle RAC on Command Center - In place pdb clone with wallet"
        self.browser = None
        self.admin_console = None
        self.navigator = None
        self.tablespace_name = 'CV_64266'
    ...
    def run_restore(self):
        ...
        restore_panel = self.db_instance_details.restore_folders(
            database_type=self.database_type, items_to_restore=[self.tcinputs["PDBName"]])
        job_id = restore_panel.out_of_place_restore(
            self.tcinputs["DestRacClusterName"], self.tcinputs["DestRacInstanceName"], staging_path="/tmp",
            redirect_all_pdb_path=self.tcinputs.get("RedirectPDBPath", "+DATA"),
            redirect_all_pdb_name=[f"{self.tcinputs['PDBName']}_clone"],
            source_wallet_password = self.tcinputs.get("SrcWalletPassword", "test"),
            dest_wallet_password = self.tcinputs.get("SrcWalletPassword", "test"))
    ...
    def validate_restore(self, job_id, tablespaces):
        ...
        dest_tablespaces = self.oracle_helper_object.get_encrypted_tablespaces()
