...
Checking for a given block deduplication size 32kb,
...
9. si block size is 32kb or not
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "MA side dedup with block size 32kb"
        self.tcinputs = {
            "ClientName": None,
            "AgentName": None,
            "MediaAgentName": None
    ...
    def run(self):
        ...
        try:
            ...
            for iterator in range(1, 3):
                ...
                self.log.info(
                    "CASE 9: check if block deduplication size is 32kb  ")
                (matched_line, matched_string) = self.dedup_helper.parse_log(
                ...
                self.log.info(
                    ...
                    result)
                if matched_line or (result == 32):
                    self.log.info("Result :Pass")
                else:
                    self.log.error("Result: Failed")
                    raise ValueError("the block size is not 32kb")
