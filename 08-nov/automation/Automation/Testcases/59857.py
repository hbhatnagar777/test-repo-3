...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - Verify all IBMi advanced subclient options with VTL multiple stream backup
        Step1, configure BackupSet and Subclient with one set of advanced options for TC with Parallel option.
        Step2: On client, create a library T57827.
    ...
    def __init__(self):
        ...
        self.name = "IBMi - Verify all IBMi advanced subclient options with VTL multiple stream backup"
    ...
    def run(self):
        ...
        try:
            ...
            elif self.IBMiMode == "VTL":
                self.log.info("Not Enabling multiple drives option for VTL Backup")
            ...
            if self.IBMiMode == "VTLParallel":
                self.log.info("Step4: verify the full backup logs and validate Media definition is used to "
                              "multiple streams.")
                self.helper.verify_vtl_multistream(job=self.job,
                                                   no_of_streams=2)
            self.log.info("Step5: verify the full backup logs and validate all advanced options.")
            self.helper.verify_ibmi_vtl_sc_options(jobid=self.job.job_id,
                                                   **sc_options)
            self.log.info("Step6: Run a Incremental backup for the subclient.")
            ...
            self.helper.verify_ibmi_vtl_sc_options(jobid=self.job.job_id,
                                                   ...
                                                   **sc_options)
            self.log.info("Step8: OOP Restore and validate the restored content. Validate if parallel backup "
                          "tapes are used for restore.")
            self.job = self.restore_verify(source=srclib, destination=destlib)
            if self.IBMiMode == "VTLParallel":
                self.helper.verify_vtl_multistream_restore(job=self.job,
                                                           no_of_streams=2)

            self.log.info("Step9: Run a Differential backup for the subclient.")
            ...
            self.log.info("Step10: verify the Differential backup logs and validate all advanced options.")
            if self.IBMiMode == "VTLParallel":
                self.helper.verify_vtl_multistream(job=self.job,
                                                   no_of_streams=2)
            self.helper.verify_ibmi_vtl_sc_options(jobid=self.job.job_id,
                                                   ...
                                                   **sc_options)
            self.log.info("Step11: OOP Restore and validate the restored content. Validate if parallel backup "
                          "tapes are used for restore.")
            self.job = self.restore_verify(source=srclib, destination=destlib)
            if self.IBMiMode == "VTLParallel":
                self.helper.verify_vtl_multistream_restore(job=self.job,
            ...
            self.log.info("Step12: Update the SC with new values for additional options.")
            ...
            self.log.info("Step13: Run a full backup for the subclient.")
            self.job = self.helper.run_backup(backup_level="Full")[0]
            self.log.info("Step14: verify the full backup logs and validate all advanced options.")
            if self.IBMiMode == "VTLParallel":
                self.helper.verify_vtl_multistream(job=self.job,
                                                   no_of_streams=2)
            self.helper.verify_ibmi_vtl_sc_options(jobid=self.job.job_id,
                                                   **sc_options1)
            self.log.info("Step15: Run a Inc backup for the subclient.")
            self.client_machine.create_sourcepf(library=srclib, object_name='INC2')
            self.job = self.helper.run_backup(backup_level="Incremental")[0]
            self.log.info("Step16: verify the Inc backup logs and validate all advanced options.")
            if self.IBMiMode == "VTLParallel":
                self.helper.verify_vtl_multistream(job=self.job,
                                                   no_of_streams=2)
            self.helper.verify_ibmi_vtl_sc_options(jobid=self.job.job_id,
                                                   backup_level="Incremental",
                                                   **sc_options1)
            self.log.info("Step17: OOP Restore and validate the restored content. Validate if parallel backup "
                          "tapes are used for restore.")
            self.job = self.restore_verify(source=srclib, destination=destlib)
            if self.IBMiMode == "VTLParallel":
                self.helper.verify_vtl_multistream_restore(job=self.job,
                                                           no_of_streams=2)
            self.log.info("Step18: Run a Differential backup for the subclient.")
            self.client_machine.create_sourcepf(library=srclib, object_name='DIFF2')
            self.job = self.helper.run_backup(backup_level="Differential")[0]
            self.log.info("Step19: verify the Differential backup logs and validate all advanced options.")
            if self.IBMiMode == "VTLParallel":
                self.helper.verify_vtl_multistream(job=self.job,
                                                   no_of_streams=2)
            self.helper.verify_ibmi_vtl_sc_options(jobid=self.job.job_id,
                                                   backup_level="Differential",
                                                   **sc_options1)
            self.log.info("Step20: OOP Restore and validate the restored content. Validate if parallel backup "
                          "tapes are used for restore.")
            self.job = self.restore_verify(source=srclib, destination=destlib)
            if self.IBMiMode == "VTLParallel":
                self.helper.verify_vtl_multistream_restore(job=self.job,
                                                           no_of_streams=2)
            self.log.info("Step21: Cleanup libraries from client disk")
            self.client_machine.manage_library(operation='delete', object_name=srclib)

            self.log.info("**PARALLEL VTL BACKUP RUN VALIDATION OF SC OPTIONS COMPLETED SUCCESSFULLY**")
