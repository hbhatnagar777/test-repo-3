...
"""Test case for performing a pdb clone from Command Center
...
Input Example:
    Providing all the inputs mentioned below is required for the testcase.
    ...
    "testCases": {
        "62401": {
          ...
          "RedirectAllPdbPath": "datafilepath",
          "RedirectAllPdbName": ["newpdbname"],
        }
    }
"""
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        super().__init__()
        self.name = "Testcase to verify oracle pdbclone (Redirect All Pdb Path) (62401)"
        self.browser = None
        ...
        self.tcinputs = {"SysPassword": None,
                         ...
                         "StagingPath": None,
                         "RedirectAllPdbPath": None,
                         "RedirectAllPdbName": None,
                         }
    ...
    def tear_down(self):
        ...
        for pdb in range(len(self.tcinputs["RedirectAllPdbName"])):
            self.oracle_helper.db_drop_pdb(pdb_name=self.tcinputs["RedirectAllPdbName"][pdb])
    ...
    def populate_table(self):
        ...
        for pdb in range(len(self.tcinputs["FileFolders"])):
            ...
            self.oracle_helper.db_create_table(tablespace_name="vedtbspace", table_prefix=f"newpdbtb{pdb}",
                                                   user="SYS", number=1)
    ...
    def restore(self):
        ...
        try:
            job_id = self.oracle_restore_panel.out_of_place_restore(
                ...
                redirect_all_pdb_path=self.tcinputs["RedirectAllPdbPath"],
                redirect_all_pdb_name=self.tcinputs["RedirectAllPdbName"])
    ...
    def validate(self):
        ...
        for pdb in range(len(self.tcinputs["FileFolders"])):
            self.oracle_helper.db_connect_to_pdb(pdb_name=self.tcinputs["RedirectAllPdbName"][pdb])
            no_of_rec = self.oracle_helper.db_table_validate(user="SYS", tablename=f"newpdbtb{pdb}01")
            if no_of_rec != 10:
