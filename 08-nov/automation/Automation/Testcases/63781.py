...
TestCase to perform Data Server IP Backup and Restore with client as not FQDN.
...
TestCase Design:
    Configure a client with nethostname set as IP address.
    Use this client as 3dnfs server and run backups using DSIP controller and also test restore.

Sample Input:
    {
        "MediaAgentRegular": "MediaAgent1", ***(Please provide a MA whose NetHostName is IP)
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "DSIP -- NetHostName is not FQDN"
    ...
    def setup(self):
        ...
        self.storage_policy_name = f'storage_policy_{self.id}_{self.media_agent_regular}'
    ...
    def tear_down(self):
        ...
        try:
            if self.status != constants.FAILED:
                ...
                self.log.warning("Testcase shows failure in execution")
            self._cleanup()
