...
DATABASE = 'tc_62175'
APP_NAME = f"TC_62175_{datetime.now().strftime('%d_%B_%H_%M')}"
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = "Salesforce: File Backup and Restore"
        self.browser = None
    ...
    def run(self):
        ...
        try:
            ...
            content_version, self.content_document = self.sf_helper.create_files()
            self.sf_helper.validate_files_in_salesforce(content_version, self.content_document)
            # Run full backup
            self.backup_full()
            # Modify records in Salesforce object
            content_version, self.content_document = self.sf_helper.create_incremental_files(
            ...
            self.sf_helper.delete_object_data(CONTENT_DOCUMENT, self.content_document)
