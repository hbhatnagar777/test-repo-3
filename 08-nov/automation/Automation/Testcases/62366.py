    ...
    "testCases":
            {
                "62366": {
                    ...
                    "accountKey": "access_key_for_storage_account",
                    "container-name": "name_of_the_container",
                    "cloudStorageLoc": "Metallic - Microsoft Azure storage - East US 2 (Virginia)"
                }
            }
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Azure Blob Acceptance case for Metallic Hub"
        self.browser = None
    ...
    def setup(self):
        ...
        self.hub_management.delete_companies_with_prefix(prefix='AzureBlob-keys')
        self.create_tenant()
    ...
    def create_tenant(self):
        ...
        """
        self.company_name = datetime.datetime.now().strftime("AzureBlob-keys-%d-%B-%H-%M")
    ...
    def configure_new_client(self):
        ...
        self.oss_metallic_helper = RAzureObjectStorageInstance(self.admin_console, 'Azure Blob')
        self.oss_metallic_helper.configure(self.byos_bucket_name, self.plan, self.client_name, content=content,
        ...
        self.object_storage = ObjectStorage(self.admin_console)
    ...
    def backup(self, backup_type):
        """ Performs backup for object storage client
                ...
                """
        if backup_type is Backup.BackupType.FULL:
        ...
        if backup_type is not Backup.BackupType.SYNTH:
    ...
    def run(self):
        try:
            ...
            self.backup(Backup.BackupType.FULL)

            self.object_storage.edit_content_group(self.content_group_name, self.tcinputs['incr_bkp_content'])

            self.backup(Backup.BackupType.INCR)

            self.backup(Backup.BackupType.SYNTH)

            self.object_storage.validate_backup_jobs(self.backup_job_list)
