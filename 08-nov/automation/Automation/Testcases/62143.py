...
TestCase:
    ...
    wait_for_job()  -- monitors the job status
...
class TestCase(CVTestCase):
    """KMIP KMS with access Node"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "KMIP KMS with access Node"
    ...
    def setup(self):
        ...
        self.storage_pool = "TestCase_"+str(self.id) +"_Pool"
        ...
        self.kmip_kms_name = "TestCase_" + str(self.id) + "_KMIP_KMS"
        ...
        self.kmip_access_node = self.tcinputs["KMIPAccessNode"]
        self.result_string = ""

        if self.MediaAgent.lower() == self.kmip_access_node.lower():
            raise Exception("MediaAgent and KMIP KMS access node can't be same")
    ...
    def wait_for_job(self, job_obj):
        """ will check the job status

        Args:
                job_obj     (object) -- object of Job class
        """
        ...
        if job_obj.wait_for_completion():
           self.log.info("Job completed successfully")
            ...
            raise("Job not completed successfully")
    ...
    def run(self):
        ...
        try:
            ...
            kms_details = {
                ...
                "ACCESS_NODE_NAME": self.kmip_access_node,
            ...
            self.log.info("Verifying the master key creation request sent to KMS access node by parsing log on KMS access node [%s]", self.kmip_access_node)
            ...
            self.log_verification(log_line_to_match, self.kmip_access_node, "cvd.log")
            ...
            matched_for_backup = self.log_verification(log_line_to_match, self.kmip_access_node, "cvd.log")
            ...
            matched_for_restore = self.log_verification(log_line_to_match, self.kmip_access_node, "cvd.log")
            ...
            matched_for_aux = self.log_verification(log_line_to_match, self.kmip_access_node, "cvd.log")
