...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Create an Index Server, backup some mailboxes, add a Node, run Load Balancing operation"
        self.tcinputs = {
    ...
    def setup(self):
        ...
        if isinstance(self.tcinputs["MachineUsernames"], str):
            self.tcinputs["MachineUsernames"] = self.tcinputs["MachineUsernames"].split(",")
        if isinstance(self.tcinputs["MachinePasswords"], str):
            self.tcinputs["MachinePasswords"] = self.tcinputs["MachinePasswords"].split(",")
        option_selector_obj = OptionsSelector(self.commcell)
        self.configure_machine_inputs(self.tcinputs['IndexServerNodeNames'])
        restore_directory_drive_letter = option_selector_obj.get_drive(media_agent_machine_obj)
        ...
        self.log.info('Creating an Index Server with one Node')
    ...
    def run(self):
        ...
        try:
            ...
            self.navigator.navigate_to_index_servers()
            self.index_server_obj.add_node(
                self.tcinputs['IndexServer'], self.tcinputs['IndexServerNodeNames'][1],
                self.index_directory[1])
            self.log.info('Sleeping for 120 seconds')
            time.sleep(120)
            index_server_helper = IndexServerHelper(self.commcell, self.tcinputs['IndexServer'])
            index_server_helper.invoke_index_server_stats_sync(
