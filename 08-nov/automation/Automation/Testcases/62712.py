...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Kubernetes - Command Center Automation for Manifest Restore"
    ...
    def init_inputs(self):
        ...
        self.destination_path = self.tcinputs.get("ManifestRestoreDestination", "/tmp") + "/" + self.testbed_name
    ...
    def delete_testbed(self):
        ...
        self.proxy_obj.remove_directory(self.destination_path)
        self.log.info(f"Removed directory [{self.destination_path}] from access node [{self.access_node}]")
    ...
    @test_step
    def run_manifest_restore(self):
        """Step 4 -- Run Manifest Files Restore"""

        self.proxy_obj.create_directory(self.destination_path, force_create=True)
        self.k8s_helper.run_manifest_restore_job(
            cluster_name=self.server_name,
            source_app=self.pod_name,
            destination_client=self.destination_access_node,
            destination_path=self.destination_path,
            app_group_name=self.app_grp_name,
            unconditional_overwrite=True
        )
        self.log.info("Manifest Files Restore complete.. Proceeding with next steps..")
