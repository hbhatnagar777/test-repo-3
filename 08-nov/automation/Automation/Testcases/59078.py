...
"""Test case for creating and validation of a point-in-time BLR pair from Command center
Sample JSON: {
...
class TestCase(CVTestCase):
    """Class for validating FSBLR point-in-time pair operations from command center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super().__init__()
        self.name = "Command center - FSBLR point-in-time replication validation"
        self.tcinputs = {
            "rp_store": None,
            "source_name": None,
            "source_volume": None,
    ...
    def create_blr_pair(self):
        ...
        blr.add_block_level_replication(self.source_name, self.destination_name,
                                        [self.source_volume], [self.destination_volume], 1,
                                        rpstore_args={"name": self.tcinputs['rp_store']},
                                        interval_args={"ccrp": "30 seconds"})

    @test_step
    def verify_pair_creation(self):
        """Verifies the creating of a point-in-time BLR pair in the replication monitor's continuous tab"""
        self.admin_console.navigator.navigate_to_continuous_replication()
    ...
    def create_replica_copy(self, idx):
        ...
        if idx == 1:
            # Full(oldest point in time) replica job on first cycle
            job_id = replica_copy.submit_replica_job(self.copy_volumes[idx-1], 0)
        else:
            # Perform all changes replica
            job_id = replica_copy.submit_replica_job(self.copy_volumes[idx-1], 1, rptime=None)
        self.logout()
