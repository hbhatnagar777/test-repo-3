...
TestCase:
    ...
    edit_backup_plan()                           --  Add or modify backup plan on index server

    edit_index_server_roles()                    --  Edit index server Roles
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Edit index server configurations (Port number, JVM memory, Roles & backup plan) and" \
                    " verify if it is updating"
        self.tcinputs = {
    ...
    def configure_machine_inputs(self, node_names):
        ...
        self.machine_obj = Machine(self.tcinputs['NodeToEdit'], self.commcell)
        total_memory = int(self.machine_obj.get_hardware_info()['RAM'][: -2]) * 1024
        self.memory = randint(4097, min(total_memory, 32768))
        self.port = randint(20001, 20010)
    ...
    def edit_index_server_node(self):
        ...
        """
        self.navigator.navigate_to_index_servers()
        self.index_servers_obj.edit_index_server_node(self.index_server_name, self.tcinputs['NodeToEdit'],
                                                      port=self.port, memory=self.memory)

    @test_step
    def edit_backup_plan(self):
        """
        Add or modify backup plan on index server
        """
        self.navigator.navigate_to_index_servers()
        self.index_servers_obj.edit_backup_plan(self.index_server_name, self.tcinputs['BackupPlan'])

    @test_step
    def edit_index_server_roles(self):
        """
        Edit index server Roles
        """
        self.navigator.navigate_to_index_servers()
        self.index_servers_obj.edit_index_server_roles(self.index_server_name, self.tcinputs['Roles'])
    ...
    def run(self):
        try:
            ...
            self.run_crawl_job()
            self.edit_backup_plan()
            self.edit_index_server_roles()
