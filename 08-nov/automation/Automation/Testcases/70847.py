...
"""ACC test case for postgresql cluster patroni
...
TestCase:
    ...
    cluster_mgr_validate() -- Validates whether the patroni has started by checking the log backup on the master node
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.name = "ACC test case for postgresql cluster Patroni"
        self.browser = None
    ...
    def cluster_mgr_validate(self):
        """
        Validates whether patroni has started by checking the restore logs the master node.
        ...
        try:
            ...
            self.log.info("Checking Patroni startup for job id %s", job_id)
            ...
            for node in node_priority.values():
                ...
                command = f"cat {log_path} | grep {job_id} | grep 'Successfully started Patroni on the Node'"
                ...
                if str(job_id) in output.formatted_output:
                    self.log.info("patroni startup confirmed for job id %s", job_id)
                else:
                    self.log.info("patroni startup not found for job id %s", job_id)

        except Exception as e:
            self.log.error("Error checking patroni startup: %s", str(e))
            raise
