...
class TestCase(CVTestCase):
    """
    Testcase to create Kubernetes Cluster, perform backup and out of place restore of applications.
    ...
    def __init__(self):
        ...
        self.name = "Kubernetes - Application Content : Backup and Out of Place Restore"
    ...
    def run(self):
        ...
        try:
            ...
            VirtualServerUtils.decorative_log("Step 1 -- Verify FULL backup")
            self.kubehelper.backup('FULL')

            VirtualServerUtils.decorative_log("Step 2 -- Running Full Application OOP Restore")
            ...
            VirtualServerUtils.decorative_log("Step 4 -- Verify Full Application Out of place Restore after Incremental"
                                              )
            self.kubehelper.restore_out_of_place(
                self.clientName,
                self.restore_namespace,
            ...
            self.kubehelper.k8s_verify_restore_files(self.namespace, self.restore_namespace)
            VirtualServerUtils.decorative_log("Step 5 -- Running Synthetic full job ")
            self.kubehelper.backup('SYNTHETIC_FULL')
            VirtualServerUtils.decorative_log("Step 6 -- Verify SYNTH Full Application Out of place Restore")
            self.kubehelper.restore_out_of_place(
                self.clientName,
                self.restore_namespace,
                self.storageclass
            ...
            VirtualServerUtils.decorative_log("Step 7 -- Delete testbed, delete client ")
            self.delete_testbed()
