...
class TestCase(CVTestCase):
    """Class for executing
        IBMi- LFS wildcard content behavior check with library level and object level backups
    ...
    def __init__(self):
        ...
        self.name = "IBMi- LFS wildcard content behavior check with library level and object level backups"
    ...
    def init_tc(self):
        ...
        self.bkp_libs = ["R{0}".format(self.id),
                         "D{0}".format(self.id),
                         "O1P",
                         "T{0}3".format(self.id),
                         "T{0}4".format(self.id),
                         "F{0}4".format(self.id)]
        self.content = ["/QSYS.LIB/[R,D]*.LIB",
                        "/QSYS.LIB/O?P.LIB",
                        "/QSYS.LIB/T{0}[3-4].LIB".format(self.id),
                        "/QSYS.LIB/F{0}[!2-3].LIB".format(self.id)]
        self.bkp_objs = ["/QSYS.LIB/FR{0}.LIB/DIR1.FILE".format(self.id),
                         "/QSYS.LIB/FR{0}.LIB/DIR2.FILE".format(self.id),
                         "/QSYS.LIB/FR{0}.LIB/FUL1.FILE".format(self.id),
                         "/QSYS.LIB/FR{0}.LIB/TESTAA.FILE".format(self.id),
                         "/QSYS.LIB/FR{0}.LIB/INC1.FILE".format(self.id),
                         "/QSYS.LIB/FR{0}.LIB/INC2.FILE".format(self.id),
                         "/QSYS.LIB/FT{0}.LIB/DIR1.FILE".format(self.id),
                         "/QSYS.LIB/FT{0}.LIB/DIR2.FILE".format(self.id),
                         "/QSYS.LIB/FT{0}.LIB/DIR3.FILE".format(self.id)]
        self.content1 = ["/QSYS.LIB/FR{0}.LIB/DIR[1-2].FILE".format(self.id),
                         "/QSYS.LIB/FR{0}.LIB/FUL[!2-3].FILE".format(self.id),
                         "/QSYS.LIB/FR{0}.LIB/INC*.FILE".format(self.id),
                         "/QSYS.LIB/FR{0}.LIB/TES?AA.FILE".format(self.id),
                         "/QSYS.LIB/FT{0}.LIB/***".format(self.id)]
    ...
    def configure_sc(self, opt_type):
        ...
        if opt_type == "LIB_LEVEL":
            ...
            self.helper.create_subclient(name=self.subclient_name,
                                         ...
                                         delete=True)
            self.helper.set_object_level_backup()
            self.client_machine.manage_library(object_name="FR{0}".format(self.id))
            self.client_machine.manage_library(object_name="FT{0}".format(self.id))
            obj_names = ["DIR1", "DIR2", "DIR3", "FUL1", "FUL2", "FUL3", "TESTAA", "TESTAB", "INC1", "INC2"]
            for each in obj_names:
                self.client_machine.create_sourcepf(library="FR{0}".format(self.id), object_name=each)
            obj_names = ["DIR1", "DIR2", "DIR3"]
            for each in obj_names:
                self.client_machine.create_sourcepf(library="FT{0}".format(self.id), object_name=each)
    ...
    def cleanup(self):
        ...
        for each in self.usr_lib:
            self.client_machine.manage_library(operation='delete', object_name=each)
        self.client_machine.manage_library(operation='delete', object_name=self.dest_lib)
        self.client_machine.manage_library(operation='delete', object_name="FR{0}".format(self.id))
        self.client_machine.manage_library(operation='delete', object_name="FT{0}".format(self.id))
