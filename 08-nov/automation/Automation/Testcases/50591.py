...
TestCase:
    ...
    restore_validate()  --  runs restore and validates data
...
class TestCase(CVTestCase):
    """Class for executing PostgreSQL SNAP backup and Restore with proxy test case"""
    ...
    def run(self):
        ...
        try:
            ...
            if "native" not in snap_engine:
                ...
                copy_precedence = dbhelper_object.run_backup_copy(
                    self.subclient.storage_policy)
            ...
            volume_id_list = dbhelper_object.get_volume_id(full_job.job_id)
            self.log.info("Trying to unmount the volume list: %s", volume_id_list)
            unmount_job = self.commcell.array_management.unmount(volume_id_list)
            self.log.info(
