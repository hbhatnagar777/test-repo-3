...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Verification of CvScale Manager for Office 365 single app type (OneDrive) with single instance"
        self.azure_rg = None
        ...
        self.O365_plan = None
        self.instances_during_backup = None
        self.instances_during_restore = None
    ...
    def setup(self):
        ...
        self.admin_console.login(self.tenant_user_name, self.inputJSONnode
    ...
    @test_step
    def verify_instances(self, exp_count, operation=None):
        """
        ...
        try:
            if operation=="backup":
                self.backup_instance = self.azure_compute.get_VM_scale_set_instances_list(self.tcinputs["VM_Scale_Set"])
                ins_count = len(self.backup_instance)
                ...
                self.log.info('verification of instance passed')
            elif operation=="restore":
                restore_instance = self.azure_compute.get_VM_scale_set_instances_list(self.tcinputs["VM_Scale_Set"])
                ins_count = len(restore_instance)
                self.log.info('Number of instances running are %s during job', ins_count)
                if ins_count != exp_count:
                    raise Exception("Instances count is not %s during job", exp_count)
                if restore_instance[0] != self.backup_instance[0]:
                    raise Exception("Same instance not used for restore that used for backup")
                self.log.info('verification of instance passed')
        except Exception:
    ...
    @test_step
    def verify_backup(self, bkp_job_details):
        """
    ...
    @test_step
    def verify_restore(self, rest_job_details):
        """
        ...
        """
        try:
            if int(rest_job_details["No of files restored"]) != self.input_doc_count:
    ...
    def run(self):
        ...
        try:
            ...
            bkp_job_id = self.office365_obj.start_backup_job()
            ...
            self.verify_streams(bkp_job_id, self.tcinputs["max_streams_before_backup"])
            self.verify_instances(exp_count=1, operation="backup")
            self.azure_compute.compare_registry()
            bkp_job_details = self.wait_till_job_completion(bkp_job_id)
            self.verify_backup(bkp_job_details)
            self.office365_obj.goto_office365_app(self.app_name)
            self.verify_browse()
            self.admin_console.driver.back()
            rest_job_id = self.office365_obj.start_restore_job()
            self.verify_streams(rest_job_id, self.tcinputs["max_streams_before_restore"])
            self.verify_instances(exp_count=1, operation="restore")
            rest_job_details = self.wait_till_job_completion(rest_job_id)
            self.verify_restore(rest_job_details)
