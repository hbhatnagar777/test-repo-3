...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.name = "Installation with the different client name, same hostname"
    ...
    def run(self):
        ...
        try:
            ...
            self._log.info(
                "Registering client {0} with hostname {1} to CS".format(self.client_1, self.client_hostname))
            ...
            self._log.info("Trying to install different client on the same host")

            # Validate client name
            self.validate_client_name(self.client_2)

            self._log.info(
                "Registering client {0} with same hostname {1} to CS".format(self.client_2, self.client_hostname))
            try:
                # Register a new client with main user
                self.sim_caller.install_new_client(
                    self.client_2,
                    ...
                    self.password_enc)
                self._log.info(
                    "Installed client {0} with same hostname:{1} to CS".format(self.client_2, self.client_hostname))
            ...
            # Register a new client with main user when overwrite flag is set
            self.sim_caller.install_new_client(
                self.client_2,
                self.client_hostname,
                self.username,
            ...
            self.user_helper.delete_user(self.username, self.default, 'master')
    ...
    def tear_down(self):
        ...
        try:
            if self.commcell.users.has_user(self.username):
                self.user_helper.delete_user(self.username, self.default, 'master')
