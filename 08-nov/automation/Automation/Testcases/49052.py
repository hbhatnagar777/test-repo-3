...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Virtual Server - VMW - Live sync - INCR VM backup," \
                    " after backup pause and resume replication and validation"
    ...
    def run(self):
        ...
        try:
            ...
            last_replication_job = job.job_id
            ...
            # Get latest replication job
            job = self.live_sync_utils.get_recent_replication_job()

            retry = 20
            while job.job_id <= last_replication_job and retry > 0:
                job = self.live_sync_utils.get_recent_replication_job()
                retry -= 1
                time.sleep(20)
            ...
            try:
                ...
                self.log.info(exp)
                self.status = False
                raise Exception("Failed to Pause Job")
            self.log.info(f"Job : {job.job_id} paused")
            time.sleep(10)
            try:
                ...
                self.log.info(exp)
                self.status = False
                raise Exception("Failed to Resume Job")

            self.log.info(f"Job : {job.job_id} resumed")
            if not job.wait_for_completion():
                self.log.info(f"Replication job with job id: {job.job_id} failed")
                raise Exception("Replication Failed")
