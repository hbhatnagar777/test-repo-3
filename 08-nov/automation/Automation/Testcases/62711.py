...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Kubernetes - Command Center Automation for Application Files restore and FS Destination restore"
        ...
        self.destination_access_node = None
        ...
        self.pvc_destination_path = None
        self.proxy_obj = None
    ...
    def init_inputs(self):
        ...
        self.destination_access_node = self.tcinputs.get("DestinationAccessNode", automation_config.ACCESS_NODE)
        self.k8s_config = self.tcinputs.get('ConfigFile', automation_config.KUBECONFIG_FILE)
        self.controller = self.commcell.clients.get(self.destination_access_node)
        ...
        self.destination_path = self.tcinputs.get("FsDestinationPath", "/bkp1")
        self.pvc_destination_path = "/mnt/data/" + self.testbed_name
    ...
    def create_testbed(self):
        ...
        self.kubehelper.create_cv_pvc(self.pvc_name, self.restore_namespace, storage_class=self.storageclass)
    ...
    def create_cluster(self):
        """Step 1 -- Create Cluster and app group from Command Center"""
    ...
    def run_full_backup(self):
        """Step 2 -- Run FULL Backup for application group"""
    ...
    def run_app_file_restore(self):
        """Step 3 -- Run Application Files Restore"""

        self.k8s_helper.run_app_file_restore_job(
            cluster_name=self.server_name,
            destination_namespace=self.restore_namespace,
            destination_pvc=self.pvc_name,
            destination_path=self.pvc_destination_path,
            file_path=f"{self.pod_name}/{self.pvc_name}",
            app_group_name=self.app_grp_name,
            access_node=self.access_node,
            unconditional_overwrite=True
        )
        self.log.info("Application Files Restore complete.. Proceeding with next steps..")

    @test_step
    def run_fs_dest_restore(self):
        """Step 4 -- Run File System Destination Restore"""

        self.proxy_obj.create_directory(self.destination_path, force_create=True)
        self.k8s_helper.run_fs_dest_restore_job(
            cluster_name=self.clientName,
            app_group_name=self.app_grp_name,
            destination_client=self.destination_access_node,
            destination_path=self.destination_path,
            file_path=f"{self.pod_name}/{self.pvc_name}"
        )
        self.log.info("FS Destination Restore complete.. Proceeding with next steps..")

    @test_step
    def delete_app_group(self):
        """Step 5 -- Delete Application Group"""
    ...
    def run(self):
        ...
        try:
            ...
            # Step 3 - Run Full Application Restore
            self.run_app_file_restore()

            # Step 4 - Run Full Application Restore In-Place
