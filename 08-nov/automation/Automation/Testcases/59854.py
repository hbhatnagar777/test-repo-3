...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Test case to cover permission change in " \
    ...
    def run(self):
        try:
            ...
            if self.fso_helper.detect_file_permission_change(old_permissions_list,
                                                             new_permissions_list, file_name
                                                             ):
                self.log.info(
                    "Total number of files [{}] to compare".format(len(new_permissions_list)))
                if len(new_permissions_list) > 0:
                    for current in new_permissions_list:
                        try:
                    ...
                    if num_mismatched > 0:
                        ...
                        self.log.info("All permissions matched")
            self.perform_cleanup()
