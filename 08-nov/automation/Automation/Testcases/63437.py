...
class TestCase(CVTestCase):
    """Class for executing Basic acceptance Test of Streaming Amazon Backup & In Place Full VM Restore test case"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "AMAZON : V2: Streaming Full backup and In Place Full VM restore Automated"
    ...
    def run(self):
        ...
        try:
            ...
            try:
                decorative_log("Performing In Place Full VM Restore")
                self.vsa_obj._inplace_overwrite = True
                self.vsa_obj.full_vm_restore()
            except Exception as exp:
                self.utils.handle_testcase_exception(exp)

            try:
                decorative_log("Performing In Place Full VM Restore from VM Level")
                self.vsa_obj._inplace_overwrite = True
