...
DATABASE = 'tc_62170'
APP_NAME = f"TC_62170_{datetime.now().strftime('%d_%B_%H_%M')}"
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = "Salesforce: Incremental backups with schema change (SQL Server)"
        self.browser = None
    ...
    def setup(self):
        ...
        if "ClientName" in self.tcinputs:
            ...
            self.sync_db_name = self.sf_helper.sync_db_name(self.app_name)
        else:
    ...
    def run_restore_to_database_and_validate(self):
        ...
        try:
            ...
            self.sf_helper.create_new_sqlserver_database(restore_db_name)
            ...
            self.sf_helper.validate_schema_in_sqlserver_database(self.sf_object, restore_db_name)
        except Exception as exp:
            raise CVTestStepFailure from exp
    ...
    def run(self):
        ...
        try:
            ...
            self.backup_full()
            self.sf_helper.validate_schema_in_sqlserver_database(self.sf_object, self.sync_db_name)
            ...
            self.backup_incremental()
            self.sf_helper.validate_schema_in_sqlserver_database(self.sf_object, self.sync_db_name, deleted_field)
    ...
    def tear_down(self):
        ...
        if self.status == PASSED:
            ...
            if "ClientName" not in self.tcinputs:
                ...
                self.sf_helper.delete_sqlserver_database(DATABASE + "_restore")
