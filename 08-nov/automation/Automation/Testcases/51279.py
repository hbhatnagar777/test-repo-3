...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        super().__init__()
        self.name = ("Install- Admin Console - Verify download software using the second "
                     "option 'Latest hotfixes for the installed Service Pack")
        self.factory = None
        self.browser = None
        self.driver = None
        self.login_obj = None
    ...
    def run(self):
        try:
            ...
            self.log.info("Starting download of latest hotfixes for windows OS")
            ...
            self.log.info("Download of latest hotfixes for windows OS Successful")

            self.log.info("Starting download of latest hotfixes for linux-x8664")
            self.deployment_helper.run_download_software(
                download_option=acc.DownloadOptions.LATEST_HF_FOR_INSTALLED_SP.value,
                os_to_download=[acc.DownloadOSID.UNIX_LINUX64.value])
            self.log.info("Download of latest hotfixes for linux-x8664 Successful")
