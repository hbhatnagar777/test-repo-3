...
"""
Testcase: This test case verifies vm provisioning setting is functional and enabling auto-scale works from an Admin Account
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Command Center : Auto scale configuration for Snap Backup from Admin Account"
    ...
    def run(self):
        ...
        try:
            ...
            # Backup Job
            try:
                if self.auto_vsa_subclient.subclient.is_intelli_snap_enabled:
                    ...
                    backup_option.advance_options = {'create_backup_copy_immediately': True}
                    ...
                    while not self.auto_scale_obj.backup_job.is_finished:
                        ...
                        if self.auto_scale_jobs:
                            ...
                            if self.auto_scale_obj.wait_for_vm_management_child_jobs_to_complete():
                                ...
                                raise Exception(
                                    "One or more VMManagement jobs failed. Please check logs for more info.")
                        else:
                            self.log.info(
                    ...
                    if auto_scale_utils_obj.validator_error_string:
                        raise Exception(
                            f"Error while performing post backup validation: {auto_scale_utils_obj.validator_error_string}")
                else:
                    raise Exception("The VM Group is not Snap Enabled")
            except Exception as exp:
