...
class TestCase(CVTestCase):
    """Class for Cross Platform stubs restore Local to Celerra, Netapp, DMNAS."""
    ...
    def __init__(self):
        ...
        self.name = "Cross Platform stubs restore Local to Celerra, Netapp, DMNAS."
    ...
    def prepare_subclient(self, path):
        ...
        self.OPHelper.create_archiveset(delete=True)
    ...
    def run(self):
        ...
        2. Run archive jobs to backup, stub and backup stubs on local.
        3. Verify file size, file size on disk, attributes on local.
        4. Recall file and verify modified time and attributes.
        5. Out of place Restore stub to Dmnas.
        6. Verify file size, file size on disk, attributes on Dmnas.
        7. Recall file and verify modified time and attributes on Dmnas.
        8. Out of place Restore stub to Celerra.
        9. Verify file size, file size on disk, attributes on Celerra.
        10. Recall file and verify modified time and attributes on Celerra.
        ...
        try:
            self.log.info(_desc)
            self.prepare_test_data(self.local_folder_path)
            self.prepare_subclient(self.local_folder_path)
            self.OPHelper.run_archive(repeats=3)

            self.log.info("For Local Client")
            time.sleep(240)
            self.verify_file_attributes(path=self.local_folder_path,
                                        valid_attributes=self.tcinputs.get('LocalStubAttributes',
                                                                           ['OFFLINE', 'SPARSEFILE',
                                                                            'REPARSEPOINT']))
            ...
            self.OPHelper.restore_out_of_place(destination_path=self.dmnas_folder_path,
                                               paths=[self.OPHelper.client_machine.join_path(
                                                   self.local_folder_path, file[0])
            ...
            self.OPHelper.restore_out_of_place(destination_path=self.celerra_folder_path,
                                               paths=[self.OPHelper.client_machine.join_path(
                                                   self.local_folder_path, file[0])
                                                   for file in self.OPHelper.test_file_list],
                                               impersonate_user=self.tcinputs.get("ImpersonateUser"),
            ...
            self.verify_file_attributes(path=self.celerra_folder_path,
                                        valid_attributes=self.tcinputs.get('CelerraRestoredStubAttributes',
            ...
            self.verify_file_size_on_disk(path=self.netapp_folder_path,
                                          valid_size=self.tcinputs.get("NetappStubSizeOnDisk", 8192))
            ...
            self.log.info('Cross Platform stubs restore Local to Celerra, Netapp, DMNAS passed')

        except Exception as exp:
            self.log.error('Cross Platform stubs restore Local to Celerra, Netapp, DMNAS failed with error: %s', exp)
