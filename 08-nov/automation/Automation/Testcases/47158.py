...
class TestCase(CVTestCase):
    """Class for executing basic network case to validate acceptance case for
        proxy connection CS-->proxy<--CC

        Setup requirements to run this test case:
        4 clients -- can be combination of windows, mac and unix

        One of the input clients will be used as proxy
    ...
    def __init__(self):
        ...
        self.name = ("[Firewall] : Firewall Connection via Proxy - Acceptance "
                     "validation (win, mac and unix)")
        ...
        self._client_group_name = networkconstants.CLIENT_GROUP_NAME[3]
        self._cs_client_group = networkconstants.CS_CLIENT_GROUP_NAME[3]
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Going to perform check readiness on the input clients")

            self._network.serverbase.check_client_readiness(self.client_list)

            self.log.info("Started executing {0} testcase".format(self.id))

            self._network.set_via_proxy({'entity': self.commserv,
                                         'isClient': True},
                                        {'entity': self.tcinputs['FirewallClient2'],
                                         'isClient': True},
                                        {'entity': self.tcinputs['proxy'],
                                         'isClient': True})

            self._network.push_config_client([self.tcinputs['FirewallClient2'],
                                              self.tcinputs['proxy'],
                                              self.commserv, ])

            self._network.serverbase.check_client_readiness(self.client_list)

            self.log.info("Adding client {0} to client group".
                          format(self.tcinputs['FirewallClient3']))

            self.client_group_obj.add_clients([self.tcinputs['FirewallClient3']])

            self.cs_client_group_obj.add_clients([self.commserv])

            # Setting firewall via proxy between Commserv and client group
            self._network.set_via_proxy({'entity': self.commserv,
                                         'isClient': True},
                                        {'entity': self._client_group_name,
                                         'isClient': False},
                                        {'entity': self.tcinputs['proxy'],
                                         'isClient': True})
            ...
            self._network.push_config_client(
                [self.tcinputs['proxy'], self.commserv])

            self._network.serverbase.check_client_readiness(
                [self.commserv, self.tcinputs['FirewallClient3']])

            self.log.info("Adding client {0} to client group for validating "
                          "inherited rules".format(self.tcinputs['FirewallClient4']))
            ...
            self._network.enable_firewall([self.tcinputs['proxy'],
                                           self.tcinputs['proxy'],
                                           self.commserv,
                                           self.tcinputs['FirewallClient4']],
                                          [8403, 8408, 8403, 8403])
            ...
            self._network.validate_tunnel_port([{'clientName': self.client_list[2]},
                                                {'clientGroupName': self._client_group_name}])

            self._network.validate_keep_alive([{'clientName': self.client_list[0]},
                                               {'clientGroupName': self._client_group_name}])

            self._network.validate_tunnel_init([{'clientName': self.client_list[2]},
                                                {'clientGroupName': self._client_group_name}])
            ...
            self.log.info("Removing client {0} from client group to validate "
                          "rules are removed".format(self.tcinputs['FirewallClient4']))
            ...
            self._network.serverbase.check_client_readiness([self.tcinputs['FirewallClient4']])

            self._network.serverbase.restart_services([self.tcinputs['proxy']])
            self._network.enable_firewall([self.tcinputs['proxy'],
                                           self.tcinputs['proxy'],
                                           self.commserv],
                                          [8403, 8408, 8403])
            ...
            self._network.serverbase.check_client_readiness([self.client_list[0],
                                                             self.client_list[1],
                                                             self.client_list[2]])
            ...
            self._network.set_via_proxy({'entity': self._cs_client_group,
                                         'isClient': False},
                                        {'entity': self._client_group_name,
                                         'isClient': False},
                                        {'entity': self.tcinputs['proxy'],
            ...
            self._network.push_config_clientgroup([self._client_group_name,
                                                   self._cs_client_group])
            self._network.push_config_client([self.tcinputs['proxy']])
            ...
            self._network.serverbase.check_client_readiness([self.tcinputs['proxy'],
                                                             self.tcinputs['FirewallClient3'],
                                                             self.commserv])
