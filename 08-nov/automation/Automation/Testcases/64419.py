...
TestCase:
    ...
    create_backupset()                  --  Adds the input backupset to input instance
    ...
    prune_history_before_timestamp()    --  Deletes the DB history file before the given timestamp

    validate_backup()                   --  Validates the backup job

    cmd_backup()                        --  Initiates the command line backup jobs
    ...
    validate_logs_loadcopy()            --  Verifies if generated logs and load copy images are present inside dump on MA

    run_and_validate_sweep_job()        --  Adds SweepStartTime registry key to MA, triggers sweep job and validates it

    log_loadcopy_generation()           --  Uses methods generate_logs, generate_loadcopy_images to generate both logs
                                            and load copy images

    prepare_db_for_restore()            --  Unquiesce's and quiesce's both instance and DB before restore

    cmd_recover()                       --  Runs the command line recover job of input backupset

    cmd_restore()                       --  Runs the command line restore job of input backupset
    ...
    gui_restore()                       --  Runs GUI recover and restore jobs
...
class TestCase(CVTestCase):
    """ GUI Backup's and restore check from both Dump location and MA post sweep job """
    ...
    def __init__(self):
        ...
        self.name = "ACCT1 Test for DB2 1 Min RPO from GUI to verify restores from Both Dump location and MA Post Sweep"
    ...
    def setup(self):
        ...
        self.table_name = "T64419"
        self.tablespace_name = "TS64419"
    ...
    def create_backupset(self):
        """ Adds the input backupset to input instance
        ...
        """
        try:
            self.log.info(f"Adding backupset {self.db2_dbname} to the instance {self.instance.instance_name}")
            self.log.info("If backupset already exists, it will be deleted and recreated ")
            if self.backupsets.has_backupset(self.db2_dbname):
                ...
                time.sleep(10)
            self.backupsets.add(self.db2_dbname, storage_policy=self.storagepolicy)
            self.log.info("Backupset Created successfully")
            self.backupset = self.backupsets.get(self.db2_dbname)
    ...
    @test_step
    def prune_history_before_timestamp(self,backup_time_stamp):
        """ Deletes the DB history file before the given timestamp

        Args:

            backup_time_stamp (Str) -- Time stamp of the backup job

        Raises:
            Exception:
                If Unable to Prune the DB History file
        ...
        try:
            if 'windows' in self.os_info.lower():
                ...
                self.db2_helper.third_party_command(
                    "db2 connect to {0}; db2 prune history {1} with force option and delete".format(
                        self.db2_dbname, backup_time_stamp))
            self.log.info("Successfully pruned the history file before given time stamp")
        except Exception as exp:
            self.log.info("Unable to prune the history file before given time stamp")
            raise CVTestStepFailure(exp) from exp
    ...
    @test_step
    def cmd_backup(self, backup_type):
        """ Initiates the command line backup jobs

        Args:

            backup_type (Str)   -- FULL/ INCREMENTAL/ DELTA type of backup job that needs to be triggered
        ...
        Raises:
            Exception:
                If Unable to initiate the command line backup job

        """
        try:
            self.log.info(f"#####Running  Command Line online {backup_type} Backup#######")
            backup_time_stamp = self.db2_helper.third_party_command_backup(self.backupset.backupset_name.upper(),
                                                                           backup_type)
            self.log.info(f"Command line {backup_type} backup is successfull Timestamp : {backup_time_stamp}")
            return backup_time_stamp
        except Exception as exp:
            self.log.info(f"Command line {backup_type} backup failed")
            raise CVTestStepFailure(exp) from exp
    ...
    @test_step
    def validate_logs_loadcopy(self, active_log_number, loadcopy_output):
        """ Verifies if generated logs and load copy images are present inside dump on MA

            Args:

                active_log_number (Int) -- Active log number of the database that needs to present on MA in dump

                loadcopy_output (Str)   -- Output of load copy images that are generated

            Raises:
                Exception:
                    If Failed to validate or logs/ load copy images are not present inside Dump location in MA
        ...
        try:
            ...
            raise Exception(f"Failed to validate logs and load copy images generated inside MA with exception {exp}")
    ...
    @test_step
    def log_loadcopy_generation(self):
        """ Uses methods generate_logs, generate_loadcopy_images to generate both logs and load copy images

            Returns:

                active_log_number (Int) -- Active log number of given backupset

                loadcopy_output (Str)   -- Output of load copy images generated

            Raises:
                Exception:
                    If Logs or Load copy images are not able to generate inside client
        ...
        try :
            ...
            self.generate_logs(200)
            self.log.info("Sleeping for 60 sec before generating load copy images")
            time.sleep(60)
            self.log.info("Generating 5 load copy images")
            loadcopy_output = self.generate_loadcopy_images(5)
            self.log.info("sleeping for 60 sec before verifying logs and load copy images inside MA")
            time.sleep(60)
            return active_log_number, loadcopy_output
        except Exception as exp:
            raise Exception(f"Failed to generate or validate logs or load copy images inside MA with exception {exp}")

    @test_step
    def prepare_db_for_restore(self):
        """ Unquiesce's and quiesce's both instance and DB before restore

        Raises:
            Exception:
                If Unquiesce or quiesce operation fails inside Client at instance level
        ...
        try:
            ...
            raise Exception(f"Unable to unquiesce or quiesce the instance with exception {exp}")

    @test_step
    def cmd_recover(self):
        """ Runs the command line recover job of input backupset

        Raises:
            Exception:
                If command line Recover job fails

        """
        try:
            self.db2_helper.disconnect_applications(self.backupset.backupset_name)
            self.log.info("#####Running Third Party Command Line Recover#######")
            self.db2_helper.third_party_command_recover(db_name=self.backupset.backupset_name.upper())
        except Exception as exp:
            raise Exception(f"Recover of db failed with the exception {exp}")

    @test_step
    def cmd_restore(self,backup_time_stamp):
        """ Runs the command line restore job of input backupset

        Args:

            backup_time_stamp (Str) -- Backup job time stamp to trigger command line restore

        Raises:
            Exception:
                If command line restore job fails

        """
        try:
            self.log.info("#####Running Third Party Command Line Restore#######")
            self.db2_helper.disconnect_applications(self.backupset.backupset_name)
            self.db2_helper.third_party_command_restore(self.backupset.backupset_name.upper(), backup_time_stamp,
                                                        self.version, True)
        except Exception as exp:
            raise Exception(f"Recover of db failed with the exception {exp}")
    ...
    def restore_validation(self):
        ...
        try:
            self.db2_helper.reconnect()
            self.db2_helper.restore_validation(self.tablespace_name, self.table_name, self.tblcont_full)
    ...
    @test_step
    def gui_restore(self, recover_db):
        """ Runs GUI recover and restore jobs

        Args:

            recover_db (Bool) -- True/False depends on if recover operation should be enabled or disabled

        Raises:
            Exception:
                If Recover or Restore GUI Jobs fails

        """
        try:
            self.db2_helper.disconnect_applications(self.backupset.backupset_name)
            if recover_db:
                self.log.info("###Submitting Restore with recover job starts here###")
                job = self.db2_helper.run_restore(self.backupset)
                self.log.info("Started Current time restore to same client job with Job ID: %s job_id ", job.job_id)
            else:
                self.log.info("###Submitting Restore without recover job starts here###")
                job = self.db2_helper.run_restore(self.backupset, recover_db=False)
                self.log.info("Started Current time restore to same client job with Job ID: %s job_id ", job.job_id)
                self.log.info("#### Reconnection to Database####")
                self.db2_helper.reconnect()
                if not job.wait_for_completion():
                    raise Exception("Failed to run current time restore job with error: " + str(job.delay_reason))
        except Exception as exp:
            raise Exception(f"Failed to restore the database with the exception : {exp}")
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info(" ##### CASE 1 : FULL/INCR/DELTA command line backups ####")

            backup_time_stamp = self.cmd_backup("FULL")
            self.prune_history_before_timestamp(backup_time_stamp)
            backup_time_stamp1 = self.cmd_backup("FULL")
            self.validate_backup(self.operation_type[0], self.tablespace_list, backup_time_stamp1)

            self.create_test_data("_INCR", False)
            backup_time_stamp2 = self.cmd_backup("INCREMENTAL")
            self.validate_backup(self.operation_type[1], self.tablespace_list, backup_time_stamp2)

            self.create_test_data("_DELTA", False)
            backup_time_stamp3 = self.cmd_backup("DELTA")
            self.validate_backup(self.operation_type[2], self.tablespace_list, backup_time_stamp3)

            self.validate_one_min_rpo()

            self.log.info(" #### CASE 2 : Archiving logs and checking if logs are moving to dump location #### ")

            # Making sure logs are both in MA and dump before restore
            (active_log_number, loadcopy_output) = self.log_loadcopy_generation()
            self.validate_logs_loadcopy(active_log_number, loadcopy_output)
            ...
            self.prepare_db_for_restore()
            self.cmd_recover()
            self.cmd_restore(backup_time_stamp2)
            ...
            backup_time_stamp1 = self.gui_backup("INCREMENTAL")
            self.log.info("Sleeping for 30 sec before running backup validation")
            time.sleep(30)
            self.validate_backup(self.operation_type[1], self.tablespace_list, backup_time_stamp1)

            (active_log_number, loadcopy_output) = self.log_loadcopy_generation()

            backup_time_stamp2 = self.gui_backup("DIFFERENTIAL")
            self.log.info("Sleeping for 30 sec before running backup validation")
            time.sleep(30)
            self.validate_backup(self.operation_type[2], self.tablespace_list, backup_time_stamp2)
            ...
            self.log.info("#### CASE 5 : Run GUI RECOVER and RESTORE jobs ####")

            self.cleanup_testdata()
            self.gui_restore("False")
            self.gui_restore("True")
            self.restore_validation()
