...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "N Level Reseller - Companies - Rented Storage and BYOS"
    ...
    def setup(self):
        ...
        # uninstalling clients if already present as a MSP persona
        OrganizationHelper(self.commcell).uninstall_client(client_name= self.company_client_name)
        OrganizationHelper(self.commcell).uninstall_client(client_name= self.msp_client_name)
        ...
        # creating company - 1
        self.action_company_name = f'TC_58973_Action_Company_{randint(1, 1000)}'
        self.company, self.tenant_admin_1, self.tenant_operator_1 = self.setup_company(company_name= self.action_company_name)
        self.log.info(f"Company Name : {self.company['company_name']}")

        # creating company - 2
        self.commcell.refresh()
        self.view_company_name = f'TC_58973_View_Company_{randint(1, 1000)}'
        self.view_company, self.tenant_admin_2, self.tenant_operator_2 = self.setup_company(company_name= self.view_company_name)
    ...
    def setup_company(self, company_name):
        """Creating Company"""
        tries = 3
        while tries:
            try:
                company = OrganizationHelper(self.commcell).setup_company(company_name=company_name, ta_password= self.common_password, to_password= self.common_password)
    ...
    def verify_entities_deletion(self):
        ...
        # known issue: shared entities are moved to commcell instead of parent company
        # so, for now, via MSP persona, lets verify whether shared entities are not deleted atleast.
        # once issue is fixed, we need to replace self.original_commcell with self.commcell
        self.original_commcell.refresh()
        self.log.info('Checking for plans..')
        if not self.original_commcell.plans.has_plan(plan_name= self.byos_plan.plan_name):
            raise Exception(f'Shared Plan: [{self.byos_plan.plan_name}] Got Deleted!')
        
        if self.original_commcell.plans.has_plan(plan_name= self.byos_plan_2.plan_name):
        ...
        if not self.original_commcell.storage_pools.has_storage_pool(name= self.company_storage_pool.storage_pool_name):
        ...
        if not self.original_commcell.clients.has_client(client_name= self.company_client_name):
            raise Exception(f'Shared Media Agent: [{self.company_client_name}] Also Got Deleted!')
        
        ma_client = self.original_commcell.clients.get(self.company_client_name)
        self.log.info(f'Current Company of MA Client : {ma_client.company_name}')
        if ma_client.company_name.lower() != 'commcell': raise Exception(f'MA Client not associated to Commcell! Current Company : {ma_client.company_name}')
        
        # TODO: once the issue is fixed, we need to uncomment below code and remove above code
        # self.company_client.refresh()
        # self.log.info(f'Current Company of MA Client : {self.company_client.company_name}')
        # if self.company_client.company_name != self.reseller_company_info['company_name']: raise Exception('MA Client Still Associated to Company!?')
        
        self.original_commcell.plans.refresh(mongodb=True, hard=True)
        plans = [i.lower() for i in list(self.original_commcell.plans.filter_plans(plan_type= 'Server', company_name= 'Commcell').keys())]
        
        if self.byos_plan.plan_name in plans:
            self.log.info(f'Plan: [{self.byos_plan.plan_name}] is moved to Commcell as expected..')
        else:
    ...
    @test_step
    def run_backup(self, company_obj: OrganizationHelper):
