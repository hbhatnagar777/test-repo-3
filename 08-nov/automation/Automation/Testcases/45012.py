...
or not with cache enabled on client with Variable Content Alignment, default size of cache
...
4. run two backup jobs
    ...
    7. Check Variable Content Alignment (query  / log check)
8. Run an incremental job
9. Run a SynthFull job
10. Synthfull job validation by checking for new
signatures encountered - count should be 0,
 primary objects = 0, secondary objects > 0
11. Remove the resources created for this testcase.
"""
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "client side dedupe cache with Variable content alignment"
        self.tcinputs = {
            "MediaAgentName": None
        }
    ...
    def run(self):
        ...
        try:
            ...
            # enabling client side dedupe cache with VCA
            self.log.info("enabling client side dedupe cache with Variable content Alignment")
            self.client.set_dedup_property(
                "clientSideDeduplication", "ON_CLIENT", True, 1024, False, True)
            self.log.info("enabling client side cache with VCA: Done")
            ...
            for iterator in range(1, 3):
                ...
                config_strings_clbackup = [
                    ...
                    'Block alignment is enabled [Block Size =']
                error_flag = []
                ...
                self.log.info(" CASE 8: Block alignment - Yes  [VCA : Variable contest alignment Enabled]")
                (matched_line, matched_string) = self.dedup_helper.parse_log(
            ...
            if self.mm_helper.create_uncompressable_data(
                    ...
                    self.client_machine.join_path(self.content_path, "Additional_data"), 0.5, 1):
                self.log.info(
    ...
    def tear_down(self):
        ...
        try:
            ...
            self.log.info("setting client deduplication to default: Use storage policy settings and disable VCA ")
            self.client.set_dedup_property("clientSideDeduplication", "ON_CLIENT", True, 8192, False, False)
