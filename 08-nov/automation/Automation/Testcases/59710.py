...
This testcase verifies that the number of items added to the index are correct when indexserver and logmanager
processes are terminated randomly.
...
TestCase:
    ...
    kill_indexing_processes()   --  Kills the indexing process as per the app name
...
class TestCase(CVTestCase):
    ...
    processes are terminated randomly.
        ...
        Steps:
            ...
            2) Kill indexserver/logmanager processes while the backup job is running
            3) Repeat step 2 multiple times.
            4) Wait for the job to complete.
            5) Verify if the items in the index is correct as expected.
            6) Perform restore for a folder and validate the number of items restored.
    ...
    def __init__(self):
        ...
        self.name = 'Indexing - Scale test - Process termination'
    ...
    def setup(self):
        ...
        if self._subclient is None:
            ...
            self.backupset = self.idx_tc.create_backupset('scale_test_pkill', for_validation=False)

            self.subclient = self.idx_tc.create_subclient(
                name='sc1_scale_pkill',
    ...
    def run(self):
        ...
        try:
            ...
            for attempt in range(total_interrupts):
                ...
                self.kill_indexing_processes('LogManager')
                self.kill_indexing_processes('IndexServer')
    ...
    def kill_indexing_processes(self, app):
        """Kills the indexing process as per the app name

            Args:
                app         (str)   --  The name of the indexing app (IndexServer/LogManager)

            Returns:
                The process ID

            Raises:
                Exception when process is invalid not running

        """

        process_id = None
        for i in range(5):
            self.log.info(f'Getting process ID attempt [{i+1}/5]')
            ids = self.idx_db.isc_machine.get_process_id('CVODS.exe', app + '%%' + self.idx_db.index_server.instance)
            if not ids:
                self.log.info(f'Cannot get [{app}] process ID. Looks like process has not started yet. Trying again')
                time.sleep(10)
                continue

            process_id = ids[0]
            self.log.info(f'Current [{app}] is [{process_id}]')
            break

        if not process_id:
            self.log.info('Process has not started yet. Skipping kill operation')
            return

        self.idx_db.isc_machine.kill_process(process_id=process_id)
        self.log.info(f'{app} is killed')
