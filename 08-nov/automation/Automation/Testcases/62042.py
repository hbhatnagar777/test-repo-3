...
TestCase:
    ...
    create_backupset()                  --  Creates a new backupset on the client, adds subclient. sets content
                                            and runs backup
...
class TestCase(CVTestCase):
    """Class For executing Activate Automation to support SDG File system datasource to read from backup
    on a client in the tenant environment for verifying create and edit of inventory,
    plan, project and add data source and validate the collected information in Project dashboard"""

    test_step = TestStep()

    def __init__(self):
        super(TestCase, self).__init__()
        self.name = "Activate Automation to support SDG File system datasource to read from backup" \
                    "on a client in the tenant environment for verifying create and " \
                    "edit of inventory, plan, project and add data source and validate the collected " \
                    "information in Project dashboard"
        # Test Case constants
    ...
    def run(self):
        ...
        try:
            self.activate_tenant_helper.cleanup_all(self.company_prod_name, self.prod_index_server,
                                                    ...
                                                    backupset=self.backupset_name, agent=self.agent,
    ...
    def create_backupset(self):
        """
        Creates a new backupset on the client, adds subclient, sets content and runs backup
        """
        ida_obj = idautils.CommonUtils(self.commcell)
        self.log.info(f"Creating backupset for client: {self.client_name} under agent: [{self.agent.agent_name}]")
        backupset_obj = self.agent.backupsets.add(self.backupset_name)
        self.log.info(f"Successfully created backupset: [{backupset_obj.backupset_name}]")
        self.log.info(f"Creating subclient: [{self.subclient_name}] under backupset: [{backupset_obj.backupset_name}]")
        subclient_obj = backupset_obj.subclients.add(self.subclient_name, self.server_plan_name)
        self.log.info(f"Successfully created subclient: [{subclient_obj.subclient_name}]")
        subclient_obj.content = [self.directory]
        self.log.info(f"Successfully set subclient content: [{subclient_obj.content}]")
        ida_obj.subclient_backup(subclient_obj, "FULL")
        self.log.info(f"Successfully completed full backup")
