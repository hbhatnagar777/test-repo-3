...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "ExportSet Share - CVPysdk - Permissions - Positive Case"
        self.tcinputs = {
            "IndexServer": None,
            "Search": None,
            "ExportCount": None,
    ...
    def share_export_set(self):
        """Testcase step to share the ExportSet to user2 with all permissions"""
        self.log.info(f"User 1 : Now sharing the ExportSet to user {self.user_2_name} with Add & Delete permissions")
        self.export_set.share(
            self.user_2_name,
            [ComplianceConstants.PERMISSION_ADD_NAME, ComplianceConstants.PERMISSION_DELETE_NAME])
        self.log.info(f"User 1 : ExportSet shared with {self.user_2_name} with Add & Delete permission")
    ...
    def search_and_export_items(self):
        ...
        self.log.info(f"User 2 : Search result items exported to export {self.export_name} successfully")
        if restore_job_id != 0:
            self.log.info(f"User 2 : Waiting for job {restore_job_id} to get complete")
            if not CrawlJobHelper.monitor_job(
                    commcell_object=self.user_2_commcell,
                    job=int(restore_job_id)):
                self.log.error("User 2 : Restore job failed to complete")
                raise Exception(f"User 2 : Restore job {restore_job_id} failed to complete")
            self.log.info(f"User 2 : Restore job {restore_job_id} completed successfully")
    ...
    def cleanup(self):
        ...
        self.log.info(f"User 2 : {self.export_name} deleted successfully")
