...
class TestCase(CVTestCase):
    """Class For executing basic acceptance test of Sensitive data governance for share point files \
         configured to use Sharepoint Classic (V1) Modern Authentication from Activate in AdminConsole"""
    test_step = TestStep()

    def __init__(self):
        super(TestCase, self).__init__()
        self.name = ("Basic acceptance of Sensitive data governance for share point files \
         configured to use Sharepoint Classic (V1) Modern Authentication from Activate in AdminConsole")
    ...
    def add_sharepoint_datasource(self):
        ...
        """
        list_of_sites = f"{_SHAREPOINT_CONFIG_DATA.HostName},{_SHAREPOINT_CONFIG_DATA.V1.ListOfSites}"
        ...
        self.gdpr_obj.file_server_lookup_obj.add_share_point_server(
            _SHAREPOINT_CONFIG_DATA.V1.ClientName,
            cs.CLIENT_NAME,
            self.sharepoint_server_display_name,
            cs.USA_COUNTRY_NAME,
            backupset=_SHAREPOINT_CONFIG_DATA.V1.Backupset.ModernAuthentication,
            sites=list_of_sites,
