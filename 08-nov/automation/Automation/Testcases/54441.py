...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Cloud Laptop]: Monikers [Office] Acceptance test"
    ...
    def setup(self):
        ...
        self.subclient_object.content = ['\%Office%']
    ...
    def run(self):
        ...
        # -------------------------------------------------------------------------------------
        #   # Adding text files to validate Office moniker
        #    SCENARIO-1:
        #       - Office moniker validation with new data
        #    SCENARIO-2:
        #       - Office moniker validation with Modified data
        #    SCENARIO-3:
        #       - Office moniker validation with renamed data
        #    SCENARIO-4:
        #       - Remove %Office moniker and validate backup
        # -------------------------------------------------------------------------------------
            local_time = int(time.time())
            doc_file = 'testDoc_'+str(local_time)+".docx"
            text_file = 'file_'+str(local_time)+".txt"
            excel_file = 'testExcel_'+str(local_time)+".xlsx"

            officefiles_path = self.utility.create_directory(self.machine_object)
            ...
            while each_scenario < len(scenarios_list):
                ...
                # -------------------------------------------------------------------------------------
                    ...
                    SCENARIO-1:
                        - Add new content under Office path
                ...
                # -------------------------------------------------------------------------------------
                    while True:
                        ...
                        # -- Status: 0 - means currently no backup job is running --#
                            ...
                            self._log.info("Adding data under Office path: %s", str(officefiles_path))
                            doc_file = officefiles_path + '\\' + doc_file
                            text_file = officefiles_path + '\\' + text_file
                            excel_file = officefiles_path + '\\' + excel_file
                            self.laptop_utils.create_file(self.machine_object, officefiles_path, doc_file)
                            self.laptop_utils.create_file(self.machine_object, officefiles_path, text_file)
                            self.laptop_utils.create_file(self.machine_object, officefiles_path, excel_file)
                            self.cloud_object.wait_for_incremenatl_backup(self.machine_object)
                            self.cloud_object.source_dir = officefiles_path
                            self.cloud_object.subclient_content_dir = officefiles_path
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        SCENARIO-2:
                            - Modify the files under Office path
                    ...
                    while True:
                        ...
                        # -- Status: 0 - means currently no backup job is running --#
                            ...
                            self._log.info("Modifying test data under Office path: %s", str(officefiles_path))
                            self.machine_object.modify_test_data(officefiles_path, modify=True)
                            self.cloud_object.wait_for_incremenatl_backup(self.machine_object)
                            self.cloud_object.source_dir = officefiles_path
                            self.cloud_object.subclient_content_dir = officefiles_path
                            self.cloud_object.out_of_place_restore(self.machine_object, self.subclient_object, cleanup=False)
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        SCENARIO-3:
                            - Rename all files under Office path
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        while True:
                            ...
                            # -- Status: 0 - means currently no backup job is running --#
                                ...
                                self._log.info("Rename the test data under Office path: %s", str(officefiles_path))
                                self.laptop_utils.create_file(self.machine_object, officefiles_path, files=5)
                                self.machine_object.modify_test_data(officefiles_path, rename=True)
                                self.cloud_object.wait_for_incremenatl_backup(self.machine_object)
                                self.cloud_object.source_dir = officefiles_path
                                self.cloud_object.subclient_content_dir = officefiles_path
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        SCENARIO-4:
                            - Remove %Office moniker from subclient content and add some test under Office path
                            - Wait for the backup job finished as per interval
                            - Perform browse and verify above content backed up or not
                                [added file under Office path should not be backed up]
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        while True:
                            ...
                            # -- Status: 0 - means currently no backup job is running --#
                                ...
                                dont_backup_path = officefiles_path + '\\' + 'Dont_backup'
                                self._log.info("Adding test data under Documents path: %s", str(dont_backup_path))

                                doc_file = dont_backup_path + '\\' + doc_file
                                text_file = dont_backup_path + '\\' + text_file
                                excel_file = dont_backup_path + '\\' + excel_file
                                self.laptop_utils.create_file(self.machine_object, dont_backup_path, doc_file)
                                self.laptop_utils.create_file(self.machine_object, dont_backup_path, text_file)
                                self.laptop_utils.create_file(self.machine_object, dont_backup_path, excel_file)
                                self.cloud_object.wait_for_incremenatl_backup(self.machine_object)
                                browse_result = self.subclient_object.browse(path=dont_backup_path)
                                if browse_result[0] or browse_result[1]: # browse returns tuple of list and dictioanry values
                                    raise Exception("Data backed up after [Office Moniker] removed [{0}]" .format(browse_result))
                                self._log.info("*** Validation of backup after [Office Moniker] removal completed successfully ****")
            ...
            try:
                ...
                self.machine_object.remove_directory(officefiles_path)
