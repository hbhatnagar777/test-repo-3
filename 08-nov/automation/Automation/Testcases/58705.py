...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Openstack volume snapshot size restore quota validation"
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info(
                '---Resetting volumesnap size quota limits for a project to original limits----')
            try:
                self.hvobj.OpenStackHandler.reset_volumesnapsize_limits(project_name)
                self.log.info(
                    'Reset volumesnap size quota limits for the project successfully'
                    +str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to reset volumesnap size quota limits for the project'+str(project_name))
                raise Exception
            self.log.info(
                '-----Getting volume limits for a project-----')
            try:
                quotalimit1 = self.hvobj.OpenStackHandler.get_volume_limits(project_name)
                self.log.info('Got volumesnapsize quota limits for the project'+str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to get volumesnapsize quota limits for the project'+str(project_name))
                raise Exception
            self.log.info('-----setting volume quota limits for a project-----')
            try:
                self.hvobj.OpenStackHandler.set_quota_volumesnapshot_size(project_name)
                self.log.info(
                    'set volumesnapsize quota limits for the project successfully'+str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to set volumesnapsize quota limits for the project'+str(project_name))
                raise Exception
            try:
                self.log.info(
            ...
            failurereason = 'Insufficient cinder quota to create disks: Failed to meet minimum 10 percent avaliable quota limit'
            if output != ():
                if failurereason not in output[0][0]:
                    self.log.error(
                        '---Failed reason not updated on Database for job--'
                        +str(vm_restore_job.job_id))
                    raise Exception
            self.log.info(
                '--Failed reason updated on the Database for job'+str(vm_restore_job.job_id))
            self.log.info(
                '---Reseting back volumesnapsize quota limits for a project to original limits----')
            try:
                self.hvobj.OpenStackHandler.reset_volumesnapsize_limits(project_name)
                self.log.info(
                    'Reseting volumesnapsize quota limits for the project successfully'
                    +str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to reset volumesnapsize quota limits for the project'+str(project_name))
                raise Exception
            ...
            try:
                ...
                raise Exception
            self.log.info(
                '---validating if volumesnapsize quota limits are set to original limits----')
            try:
                ...
                self.log.error(
                    '---volumesnapsize quota limits are not set to original limits---'
                    +str(project_name))
                raise Exception
        finally:
