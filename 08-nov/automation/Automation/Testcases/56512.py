...
class TestCase(CVTestCase):
    """Admin console: DynamoDB Restore to the same cloud account with
    same table name as source"""
    ...
    def __init__(self):
        ...
        self.name = "DynamoDB Restore to the same cloud account with same table name as source"
        self.browser = None
    ...
    @test_step
    def create_test_data(self, table_names, partition_key):
    ...
    @test_step
    def submit_restore_and_validate(self, table_names):
        """Submits in place restore and runs validation after it completes
        Args:
            table_names (list): The list of tables that needs to be restored
        """
        self.dynamodb_subclient.access_restore()
        mapping_dict = {
            'us-east-2': table_names
        }
        restore_panel_obj = self.dynamodb_subclient.restore_files_from_multiple_pages(
            self.database_type, mapping_dict, 'Sub_56512')
        restore_panel_obj.set_streams(4)
    ...
    def run(self):
        ...
        try:
            self.init_tc()
            table_names = ['TC_56512_1', 'TC_56512_2']
            self.create_test_data(table_names, 'id')
            ...
            add_subclient_obj.add_dynamodb_subclient('Sub_56512', self.tcinputs['plan'],
                                                     subclient_content)
            self.submit_backup()
            self.admin_console.refresh_page()
            self.submit_restore_and_validate(table_names)
            self.admin_console.refresh_page()
            self.cleanup()
    ...
    def tear_down(self):
        ...
        self.dynamodb_helper.delete_dynamodb_table('TC_56512_1')
        self.dynamodb_helper.delete_dynamodb_table('TC_56512_2')
