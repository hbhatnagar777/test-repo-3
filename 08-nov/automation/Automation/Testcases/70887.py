# -*- coding: utf-8 -*-

# --------------------------------------------------------------------------
# Copyright Commvault Systems, Inc.
# See LICENSE.txt in the project root for
# license information.
# --------------------------------------------------------------------------

"""
Main file for executing this test case

This test case verifies alert rule "Restore Job failed"

TestCase is the only class defined in this file.

TestCase: Class for executing this test case
"""

import socket
import threading

from AutomationUtils.cvtestcase import CVTestCase
from Web.AdminConsole.adminconsole import AdminConsole
from Web.Common.cvbrowser import BrowserFactory
from Web.Common.page_object import handle_testcase_exception, TestStep
from Web.AdminConsole.AdminConsolePages.AlertDefinitions import RAlertDefinitions
from Web.AdminConsole.AdminConsolePages.Alerts import Ralerts
from Web.AdminConsole.Helper.AlertHelper import RAlertMain

class TestCase(CVTestCase):
    """Class for executing this test case"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object
            Properties to be initialized:
                name            (str)       --  name of this test case
        """
        super(TestCase, self).__init__()
        self.name = "Web automation to test workflow as a notification type to an alert"
        self.browser = None
        self.admin_console = None
        self.exp = None
        self.subclients = None
        self.tcinputs = {
            "clientName": None,
            "instanceName": None,
            "storagePolicy": None,
            "restorePath": None
        }

    def setup(self):
        """Setup function of this test case"""
        self.cs_user = self.inputJSONnode['commcell']['commcellUsername']
        self.cs_password = self.inputJSONnode['commcell']['commcellPassword']
        self.client_name = self.tcinputs['clientName']
        self.instance_name = self.tcinputs['instanceName']
        self.storage_policy = self.tcinputs['storagePolicy']
        self.restore_path = self.tcinputs['restorePath']
        self.browser = BrowserFactory().create_browser_object()
        self.browser.open()
        self.admin_console = AdminConsole(
            self.browser, self.commcell.webconsole_hostname)
        self.admin_console.login(self.cs_user, self.cs_password)
        self.navigator = self.admin_console.navigator
        self.alert_helper = RAlertMain(self.admin_console)
        self.alerts = Ralerts(self.admin_console)
        self.alert_definitions = RAlertDefinitions(self.admin_console)
        self.client_obj = self.commcell.clients.get(self.client_name)
        self.agent_obj = self.client_obj.agents.get('file system')
        self.instance_obj = self.agent_obj.instances.get(self.instance_name)
        self.subclients = self.instance_obj.subclients
        self.subclient_obj = self.subclients.get("default")

    def run(self):
        """Run function of this test case"""

        try:
            # 1) Finding IP of current machine
            controller_port = 9000
            controller_ip = "http://"+str(socket.gethostbyname(socket.gethostname()))+":"+str(controller_port)
            # 2) Navigating to alert definitions page
            self.navigator.navigate_to_alerts()
            self.alerts.select_alert_definitions()

            # 3) adding the alert definition
            self.alert_definitions.select_add_alert_definitions()
            self.alert_definitions.create_alert_definition({
                "general":{
                    "input":
                    [
                        {"id":"name", "text_to_fill":"Test Custom Alert"}
                    ],
                    "dropdown":
                    [
                        {"id":"alertType", "options_to_select":["Backup for subclient failed consecutively for X attempts"]}
                    ]
                },
                "entities":{
                    "checkbox":
                    [
                        {"label":"Server groups"},
                        {"label":"Servers"}
                    ]
                },
                "criteria":{
                    "input":[
                        {"id":"Backup for subclient failed consecutively for X attempts_0", "text_to_fill":"3"}
                    ]
                },
                "notification":{
                    "locale":"English",
                    "console":{
                        "to":["testautomation3@devmgt.commvault.com", "Administrator"]
                    },
                    "email":{
                        "to":["testautomation3@devmgt.commvault.com", "Administrator"]
                    },
                    "workflow":{
                        "dropdown":[
                            {"id":"workflow", "options_to_select":["PostJSON"]},
                            {"id":"workflowEngines", "options_to_select":["Any"]}
                        ],
                        "combobox":[
                            {"id":"URI", "options_to_select":[controller_ip]},
                            {"id":"JSON", "options_to_select":["<ALERT NAME>", "<DETECTED CRITERIA>"]},
                        ]
                    }
                    }
                }
            )

            # 4) Triggering the alert & running the server to recieve posted value
            self.log.info("Creating a broken subclient")
            subclient_obj = self.create_broken_subclient(self.subclients, "testFailingSubclient", "C:\\thisDoesNotExist")
            
            post_body = []
            server_thread = threading.Thread(target=self.alert_helper.run_server_return_post_data, args=(post_body, controller_port, 600))
            other_task_thread = threading.Thread(target=self.alert_helper.trigger_backup_fail_three_times_alert, args=(subclient_obj,))

            server_thread.start()
            other_task_thread.start()
            other_task_thread.join()
            server_thread.join()
            
            # 5) Verify the tokens posted by the workflow
            self.log.info("Payload Recieved from workflow: {}".format(post_body[0]))

            alert_name = "Test Custom Alert"
            detected_criteria = "Backup for subclient failed consecutively for 3 attempts"
            if alert_name not in post_body[0] or detected_criteria not in post_body[0]:
                raise Exception("Token value does match expected value")
            self.log.info("Token value matches expected value")

            # 5) delete the custom alert
            self.log.info("Deleting the custom alert")
            self.alert_helper.navigate_and_delete_custom_alert("Test Custom Alert")

        except Exception as exp:
            handle_testcase_exception(self, exp)

    def tear_down(self):
        """Tear down function of this test case"""
        if self.subclients.get("testFailingSubclient"):
            self.subclients.delete("testFailingSubclient")
        AdminConsole.logout_silently(self.admin_console)
        self.browser.close_silently(self.browser)


    @test_step
    def create_broken_subclient(self, subclients, new_subclient_name, broken_path):
        """ 
            Create a broken subclient 

            Args:
                subclients (obj): Subclients object
                new_subclient_name (str): Name of the new subclient
                broken_path (str): Path that does not exist

            Returns:
                Subclient: Subclient object
        """
        subclient_obj = subclients.add(new_subclient_name, self.storage_policy)
        subclient_obj.content = [broken_path]
        subclient_obj.refresh()
        return subclient_obj

