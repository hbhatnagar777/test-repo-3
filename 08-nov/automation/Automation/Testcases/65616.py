...
TestCase:
    ...
    encrypt_storage()                           -- Enables encryption on storage disk

    decrypt_storage()                           -- Disables encryption on storage disk
    ...
    get_active_log_chain_id()                   --  Fetches DB2 Database active log file number and its chain ID

    generate_logs()                             -- Generates archive logs for given DB2 Database

    run_and_validate_sweep_job()                -- Adds SweepStartTime registry key to MA, triggers sweep job and
                                                   validates it

    restore_database()                          -- Does a FULL Restore of given DB with latest browse options

    generate_logs_validate_logs_ma()            -- Generates logs and validates if generated logs are present in MA

    prepare_db_for_restore()                    -- Unquiesce's and quiesce's both instance and database before restore

    verify_retrieve_logs()                      -- Verifies logs files under retrieve path dir inside client
...
class TestCase(CVTestCase):
    """ Command center: DB2 One Min RPO Encryption Key Rotation Check"""
    ...
    def __init__(self):
        ...
        self.name = "DB2 One Min RPO Encryption Key Rotation Acceptance TestCase"
        self.browser = None
    ...
    def setup(self):
        ...
        try:
            ...
            self.table_name = 'TBL65616'
            self.tablespace_name = 'TBS65616'
            self.active_log_numbers = []
    ...
    def encrypt_storage(self):
        ...
        try:
            ...
            self.log.info(f"Using {cipher} cipher and key length {key_length} to encrypt the {self.storage_name}")
            self.storage_helper.encrypt_disk_storage(self.storage_name, cipher, key_length)
        except Exception as exception:
            self.log.info("Failed to Encrypt the storage disk")
            raise CVTestStepFailure(exception) from exception

    @test_step
    def decrypt_storage(self):
        """ Decrypts the storage disk """
        try:
            self.log.info(f"Decrypting the storage disk {self.storage_name}")
            self.storage_helper.decrypt_disk_storage(self.storage_name)
        except Exception as exception:
            self.log.info("Failed to Decrypt the storage disk")
            raise CVTestStepFailure(exception) from Exception
    ...
    @test_step
    def get_active_log_chain_id(self):
        """ gets current active log file number and the current chain id"""
        try:
            ...
            chain_id = self.db2_helper.get_chain_number()
            self.log.info(f"Active log file number of given DB {self.tcinputs['database_name']} is {active_log_number}")
            self.log.info(f"Current chain id of the given DB is {chain_id}")
            return active_log_number, chain_id
        except Exception as exception:
    ...
    def run_and_validate_sweep_job(self):
        """ Runs sweep job on MA and validates it also checks if sweep job uses any encryption key"""
        try:
            self.log.info("Adding the registry key to run sweep job")
            sweep_job_id = self.db2_helper.run_sweep_job_using_regkey(self.tcinputs['media_agent'])
            self.log.info("Successfully ran the sweep job and validated it")
            self.log.info(f"verifying the encryption keys fro sweep job ran {sweep_job_id}")
            enc_key_id = self.db2_helper.verify_encryption_key(sweep_job_id, self.plan_name)
            self.log.info(f"Successfully verified the encryption keys and the encryption id is {enc_key_id}")
            return enc_key_id
        except Exception as exception:
    ...
    @test_step
    def generate_logs_validate_logs_ma(self):
        """ Generates Archive logs and validate if generated logs are present in dump location on MA"""
        try:
            active_log_number, chain_id = self.get_active_log_chain_id()
            self.generate_logs()
            self.db2_helper.verify_logs_on_ma(ma_obj=self.ma_obj, active_log_number=active_log_number)
            self.active_log_numbers.append((active_log_number, chain_id))
        except Exception as exception:
            self.log.info("Failed to generate logs or to verify the sweep job")
            raise CVTestStepFailure(exception) from exception
    ...
    @test_step
    def verify_retrieve_logs(self):
        """ Verifies if restored logs are present in DB2 RetrievePath on Client """
        try:
            for x in self.active_log_numbers:
                if not self.db2_helper.verify_retrieve_logs_on_client(active_log_number=x[0], chain_id=x[1]):
                    self.log.info(f"Failed to find the log number {x} in retrieve path on client")
                    raise Exception("Log file missing in retrieve path")
        except Exception as exception:
            self.log.info("Failed to verify the retrieve logs on client")
            raise CVTestStepFailure(exception) from exception
    ...
    def run(self):
        ...
        try:
            ...
            self.edit_instance()
            self.add_database()
            # uncoment below line only when restart of services are needed on client if one min rpo is not getting enabled
            ...
            self.generate_logs_validate_logs_ma()
            enc_key_id = self.run_and_validate_sweep_job()
            self.db2_helper.rotate_encryption_key(enc_key_id)
