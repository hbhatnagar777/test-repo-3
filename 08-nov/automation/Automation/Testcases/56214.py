...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Test case for Replacing Disk, Verifying Reset command over SP16+"
        self.result_string = ""
        self.policy = ""
        self.policy_name = ""
        self.subclient_obj = ""
    ...
    def run(self):
        ...
        """
        try:
            self.log.info("*************Replacing Disk, Verifying Reset command over SP16+**************")
            status_fstab = False
            ...
            self.log.info("**************RUNNING BACKUP JOB After REPLACEMENT******************")
            # Starting backup
            self.log.info("Starting Backup")
            self.job_obj = self.subclient_obj.backup("FULL")
            if self.job_obj.wait_for_completion():
