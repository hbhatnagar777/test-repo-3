...
TestCase:
    ...
    create_tc_env()                     --  create testcase environment
    ...
    change_sidb_creation_time()			--	change sidb creation time

    wait_for_resync_completion()		--	wait till resync flag is reset

    delete_resync_history()				--	delete entries from idxsidbresynchistory table
    ...
    get_afs_common_in_csdb_ddb()		--	get AFs which are common in DDB and CSDB for
                                            further processing
    Sample JSON File:
    "52703": {
	...
	Steps:
	    ...
	    12. Move SIDB creation time back by 30 days
	    13. Enable DDB store Level pruning
	    14. Delete existing IdxSIDBResyncHistory rows for this DDB
	    15. Sleep for 2 MM config thread invocations - 10 minutes
	    16. Wait until Resync bit is cleared from the DDB store
	    17. Verify IdxSIDBResyncHistory table has a new row with status=0,
	        NumResyncedAFIds = AFIDs corresponding to deleted jobs,
	        MaintenanceReason = 11 and ResyncFlags = 5
	    18. Run SIDB listaf command to make sure that resynced AFs are not present in CSDB
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "resync - periodic resync"
        self.tcinputs = {
            "MediaAgentName": None,
           }
    ...
    def change_sidb_creation_time(self):
        """
        Change SIDB Creation time to current date - 30 days
        """

        query = "select CreatedTime from IdxSIDBStore where sidbstoreid=%s"%self.engineid
        self.log.info("QUERY : %s", query)
        self.csdb.execute(query)
        creation_time = self.csdb.fetch_one_row()
        self.log.info("Original CSDB Creation Time is ==> %s", str(creation_time))


        self.log.info("Changing SIDB Cration time to Current Time  - 30 days")
        query = "update idxsidbstore set CreatedTime = CreatedTime - (30*24*60*60) where sidbstoreid=%s"%self.engineid
        self.log.info("QUERY : %s", query)
        self.mmhelper_obj.execute_update_query(query, self.sql_password, "sqladmin_cv")
        ...
        self.csdb.execute(query)
        creation_time = self.csdb.fetch_one_row()
        self.log.info("CSDB Creation Time Moved To ==> %s", str(creation_time))
    ...
    def verify_idxsidb_resync_history(self, expected_num_afids):
        ...
        if int(resync_row[3]) == 5:
            self.log.info("Successfully verified ResyncFlags = 5")
        else:
            self.log.error("ResyncFlags value is not correct : Expected = 5 Actual = %s", resync_row[3])
            failure = True

        if int(resync_row[5]) == 11:
            self.log.info("Successfully verified MaintenanceReason = 11")
        else:
            self.log.error("MaintenanceReason value is not correct : Expected = 11 Actual = %s", resync_row[5])
            failure = True
    ...
    def delete_resync_history(self):
        """
        Delete entries from IdxSIDBResyncHistory table for DDB
        """
        self.log.info("Deleting Entries from  IdxSIDBResyncHistory table for DDB %s", self.engineid)
        query = "Delete from IdxSIDBResyncHistory where sidbstoreid = %s" % (self.engineid)
        self.log.info("QUERY : %s", query)
        self.mmhelper_obj.execute_update_query(query, self.sql_password, "sqladmin_cv")
    ...
    def run(self):
        ...
        try:
            ...
            #Change SIDB cration time to past
            #We can not check for resync flags because MM thread will mark and pick up store for resync at same time
            self.change_sidb_creation_time()
            #Not enabling Pruning On Purpose to Test
            self.change_store_pruning_value(self.ddb_store_obj, True)

            #Delete entries for this DDB from IdxSIDBResyncHistory table - If DDB gets reused, these entries can
            #make MM to postpone running controlled AF pruning
            self.delete_resync_history()

            #Sleep for at least 8 minutes
            self.log.info("Sleeping for %s minutes - waiting for at least 2 Maintenance thread invocations.",
                          self.maintenance_interval)
            time.sleep(self.maintenance_interval * 60 * 2)
            self.wait_for_resync_completion(self.maintenance_interval)
