...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = 'Indexing - VSA - File Indexing - Index reconstruction, logs restore'
        self.tcinputs = {
            'RestoreClient': None,
            'RestorePath': None
            # 'ForceFull': None,
    ...
    def run(self):
        """Run function of this test case
            ...
            Steps:
                ...
                1) Run FULL --> INC --> SFULL
                2) Delete Index DB
                3) Delete Index logs (both data logs and catalog logs)
                4) Delete RFC folder
                5) Do browse and start index recon
                6) Verify if index recon completes
                7) Run INC and verify browse again
        ...
        try:
            ...
            self.log.info('********** Starting index reconstruction **********')
            self.log.info('***** Restore from previous cycle [%s] *****', inc1_job.job_id)
            self.idx_vsa.verify_gf_browse_restore({
                'job_id': inc1_job.job_id
            })

            self.log.info('Index reconstruction is complete and browse/restore is successful')
