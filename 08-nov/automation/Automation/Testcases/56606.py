...
class TestCase(CVTestCase):
    """class for Verifying the created recovery point is accessible"""
    ...
    def __init__(self):
        ...
        self.name = "SQL Block Level Recovery Point - Basic Acceptance "
    ...
    def setup(self):
        ...
        creates a subclient with a storage policy
        ...
        self.sqlhelper.sql_setup(storagepolicy=self.storage_policy,
                                 noof_dbs=1,
                                 noof_ffg_db=1,
                                 noof_tables_ffg=3)
    ...
    def run(self):
        """
            configures the subclient...
            backup up the subclient and verifies
            if the restore point is created successfully and verifies data

            returns
                    NONE
         """
        try:
            self.log.info("executing the run proceedure")
            # configuring block level subclient
            self.log.info("configuring block level subclient")
            self.sqlhelper.subclient.blocklevel_backup_option = True

            self.subclient = self.sqlhelper.subclient

            if not self.sqlhelper.subclient.blocklevel_backup_option:
                raise Exception("failed to configure block level subclient")
            ...
            if self.dbs in self.instance.browse()[0]:
                ...
                raise Exception("failed to add database to granular restore at instance level")

            # creating recovery point
            self.log.info("creating the recovery Point")
            job, recovery_point_id, restored_dbname = self.instance.create_recovery_point(self.dbs)
