...
"testCases": {
        "64385": {
            ...
            "PlanName": "",
            "TestPath": "",
            "os_type": "" -> unix/Windows (64-bit) for oci,
            "CredentialName": "",
            "RegionName": "",
            "GatewayFQDN": "",
            "CloudStorageName": "",
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Metallic GuardRails for File Servers - OCI Cloud VM interactive install, \
                        FS backups using existing OCI gateway & OCI BYOS storage"
        self.browser = None
        ...
        self.client_ocid = None
        self.gateway_ocid = None
    ...
    def set_installinputs(self):
        ...
        elif "unix" in inputs['os_type']:
            inputs["full_package_path"] = self.custompkg_directory + \
                "\\LinuxFileServer64.tar"
        elif "windows (64-bit)" in inputs['os_type']:
            inputs["full_package_path"] = self.custompkg_directory + \
    ...
    def configure_plan(self):
        ...
        """
        self.metallic_fs.select_plan(self.planname)
    ...
    def start_entities(self):
        """
        Starts the instances in OCI
        """

        self.log.info("Starting the entities...")
        self.oci_helper.set_instance_state(self.client_ocid, "start")
        self.oci_helper.set_instance_state(self.gateway_ocid, "start")

    def stop_entities(self):
        """
        Stops the instances in OCI
        """

        self.log.info("Stopping the entities...")
        self.oci_helper.set_instance_state(self.client_ocid, "stop")
        self.oci_helper.set_instance_state(self.gateway_ocid, "stop")
    ...
    def setup(self):
        ...
        self.client_ocid = self.tcinputs.get("ClientOCID")
        self.gateway_ocid = self.tcinputs.get("GatewayOCID")
        ...
        # Updating gateway before installing the client
    ...
    def configure_iam_role(self):
        """
        Method to select a saved credential
        """

        self.metallic_fs.configure_iam_role_for_oci(self.tcinputs["CredentialName"])
    
    def configure_region(self):
        """
        Method to selec the region
        """

        self.metallic_fs.configure_region([self.tcinputs["RegionName"]])
    ...
    def run(self):
        ...
        try:
            ...
            self.tcinputs["DeploymentClientName"] = self.gateway.display_name

            self.deployment_helper.action_update_software(self.tcinputs["DeploymentClientName"])
            ...
            self.metallic_fs.select_file_server_env(backuptocloud=False, 
                                                    on_premise=False, cloudvendor="Oracle Cloud Infrastructure")
