...
from Application.Exchange.ExchangeMailbox.exchangelib_helper import CVMailbox, CVFolder


class TestCase(CVTestCase):
    """PST Ingestion: Basic Test of pst archive and restore without mailbox data"""
    ...
    def __init__(self):
        ...
        self._client = None
        self.name = "PST Ingestion: Basic Test of pst archive and restore without mailbox data"
    ...
    def setup(self):
        ...
        self.smtp_list = []
        for _mbiterator in test_data.json_value.Mailbox:
            alias_name = f'{_mbiterator.ALIASNAME}_{test_data.testcase_id}'
            self.smtp_list.append((alias_name + "@" + test_data.ex_object.domain_name))
    ...
    def run(self):
        ...
        try:
            ...
            before_backup_object.mail_users = [self.smtp_list[0].lower(), self.smtp_list[1].lower(),
                                               self.smtp_list[2].lower(), self.smtp_list[3].lower()]
            for mbx in before_backup_object.mail_users:
                cvmbx = CVMailbox()
                cvfolder = CVFolder()
                cvfolder.folder_name = "Top of Information Store"
                cvmbx.folders.append(cvfolder)
                before_backup_object.mailbox_prop[mbx] = cvmbx
