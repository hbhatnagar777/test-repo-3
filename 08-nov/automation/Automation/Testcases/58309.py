...
class TestCase(CVTestCase):
    """
    Testcase to create Kubernetes Cluster, perform backup and restore of Pods, Deployments, Statefulset.
    ...
    def __init__(self):
        ...
        self.name = "Kubernetes Entities validation - Out of Place Restores"
    ...
    def run(self):
        ...
        try:
            ...
            VirtualServerUtils.decorative_log("Step 1 -- Verify FULL Backup Job")

            for pod in self.kubehelper.get_namespace_pods(self.namespace):
                self.kubehelper.create_random_cv_pod_data(
                    pod, self.namespace, foldername="FULL", slink=True, hlink=True
                )
                time.sleep(10)

            self.log.info("Run FULL Backup job...")
            before_backup = self.kubehelper.get_all_resources(self.namespace)
            self.kubehelper.backup('FULL')

            VirtualServerUtils.decorative_log("Step 2 -- Verify FULL APPLICATION Out of place Restore")
            ...
            for pod in self.kubehelper.get_namespace_pods(self.namespace):
                self.kubehelper.create_random_cv_pod_data(
                    pod, self.namespace, foldername="INC", slink=True, hlink=True
                )
                time.sleep(10)
            ...
            self.log.info("Step 4 -- Verify Full Application Out of place Restore")
            self.kubehelper.run_restore_validate(
                self.clientName,
                self.storageclass
            )
            ...
            VirtualServerUtils.decorative_log("Step 5 -- Move content and verify INCREMENTAL job")
            ...
            VirtualServerUtils.decorative_log("Step 6 -- Verify Full Application Out of place Restore")
            self.kubehelper.run_restore_validate(
                self.clientName,
                self.storageclass
            )
            ...
            VirtualServerUtils.decorative_log("Step 7 -- Verify SYNTHETIC FULL job ")

            before_backup = self.kubehelper.get_all_resources(self.namespace)
            self.kubehelper.backup('SYNTHETIC_FULL')

            VirtualServerUtils.decorative_log("Step 8 -- Run Full application out of place restore")
            self.kubehelper.run_restore_validate(
                self.clientName,
                self.storageclass
            )
            ...
            VirtualServerUtils.decorative_log("Step 9 -- Delete testbed, delete client ")
            self.delete_testbed()
