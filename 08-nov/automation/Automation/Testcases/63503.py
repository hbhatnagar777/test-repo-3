...
class TestCase(CVTestCase):
    """ [Laptop] [AdminConsole][EdgeMode] - Upload and download of Folder /Live Browse functionality for Laptop Clients"""
    ...
    def __init__(self):
        ...
        self.name = "[Laptop] [AdminConsole][EdgeMode] - Upload and download of Folder /Live Browse functionality for Laptop Clients"
    ...
    @test_step
    def validate_livebrowse_folder_upload(self):
        """
        verification of folder creation and upload file functionality in live browse
        """
        # - in angular when folder created and single file is uploaded to that folder in live browse
        #      created folder and uploaded file shows from browse without any hard refresh
        # - in automation also not refreshing the page 
        self.log.info("verification of upload data to folder functionality started for client: {0}" .format(self.tcinputs["Client_name"]))
        ...
        self.edgerestore.create_folder(self.folder_name)
    ...
    @test_step
    def validate_livebrowse_folder_download(self):
        """
        verification of folder download functionality in live browse
        """
        ...
        downloaded_folder_rs = self.local_machine.get_folder_hash(directory_path)
    ...
    def create_testpath(self):
        ...
        self.folder_name = 'Livebrowse_TC_'+str(self.id)
        self.file_name = 'Livebrowse_upload_'+str(local_time)+".txt"

        if self.tcinputs['os_type']=="Windows":
            self.upload_file_path = temp_path + '\\' + self.file_name
            self.folder_path = self.tcinputs["Test_data_path"] + '\\' + self.folder_name
            self.client_file_path = self.folder_path + '\\' + self.file_name
        else:
            self.upload_file_path = temp_path + '/' + self.file_name
            self.folder_path = self.tcinputs["Test_data_path"] + '/' + self.folder_name
            self.client_file_path = self.folder_path + '/' + self.file_name
    ...
    def run(self):
        ...
        try:
            ...
            self.validate_livebrowse_folder_upload()
            self.validate_livebrowse_folder_download()
