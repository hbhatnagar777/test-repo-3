...
class TestCase(CVTestCase):
    """
    Testcase to create Kubernetes Cluster, perform backup and restore of applications and verify FS operations.
    ...
    10. Move content in pod for Incremental  validation.
    11. Initiate Incremental Backup for App group created and verify job completed
    12. Initiate out-of-place Full Application Restore and verify job completed
    ...
    def __init__(self):
        ...
        self.name = "Kubernetes : FS Operations Add,Move,Delete"
    ...
    def run(self):
        ...
        try:
            ...
            self.kubehelper.create_random_cv_pod_data(self.pod_name, self.namespace, foldername="INCR")
            self.kubehelper.backup('INCREMENTAL')

            VirtualServerUtils.decorative_log(
                "Step 4 -- Verify Full Application Out of place Restore after Incremental"
            )
            self.kubehelper.restore_out_of_place(
                self.clientName,
                self.restore_namespace,
            ...
            VirtualServerUtils.decorative_log(
                "Step 5 -- Moving the content and run Incremental Backup, followed by OOP Restore"
            )
            self.kubehelper.move_cv_pod_data(self.pod_name, self.namespace,  foldername="FULL")
            self.kubehelper.backup('INCREMENTAL')
            self.kubehelper.restore_out_of_place(
                self.clientName,
                self.restore_namespace,
            ...
            self.kubehelper.k8s_verify_restore_files(self.namespace, self.restore_namespace)
            VirtualServerUtils.decorative_log("Step 6 - Running Synthetic full backup job ")
            self.kubehelper.backup('SYNTHETIC_FULL')

            VirtualServerUtils.decorative_log("Step 7 -- Verify SYNTH Full Application Out of place Restore")
            self.kubehelper.restore_out_of_place(
                self.clientName,
                self.restore_namespace,
                self.storageclass
