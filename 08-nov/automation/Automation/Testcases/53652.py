...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Basic Acceptance Test - Commcell level User creation, Validation in CC"
    ...
    def random_user_groups(self):
        local_user_groups = [
            ug for ug in self.commcell.user_groups.non_system_usergroups()
    ...
    def run(self):
        ...
        for attempt in range(retries):
            try:
                self.user_obj.validate_company_filter()
                break
            except Exception as exp:
                self.log.error(exp)
                self.log.error(traceback.format_exc())
                if attempt == retries - 1:
                    errors.append(f"Error during user listing validation after attempt {attempt}: {exp}")
        ...
        try:
            ...
            self.user_obj.invite_user = not self.browser.get_js_variable('cv.isMetallicOEM')
