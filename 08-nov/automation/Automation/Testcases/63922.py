...
TestCase:
    ...
    verify_mark_corrupt_action()                 -- Marks the file corrupt and verifies
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Testcase to validate the threat scan mark corrupt remediation action"
        self.tcinputs = {
    ...
    def run_data_curation(self):
        ...
        self.generate_sensitive_data(num_files=10, encrypt=True)
        ...
        self.run_threat_analysis(
            self.client_name, self.index_server_name)
    ...
    def run_threat_analysis(self, client_name, index_server_name, anomaly):
        """
        Runs file analysis job
    ...
    @test_step
    def verify_mark_corrupt_action(self):
        """Marks the file corrupt and verifies"""

        self.log.info("Marking a file corrupt")
        ...
        corrupt_files_count = self.threat_scan.get_corrupt_files_count()
        self.log.info(f"Number of corrupt files present {corrupt_files_count}")
        self.threat_scan.mark_corrupt(search_keyword, file)
        # Run an incremental backup job
        self.run_backup_job(level=cs.INCREMENTAL)
        # Navigate to unusual file activity
        self.navigator.navigate_to_unusual_file_activity()
        ...
        new_count = self.threat_scan.get_corrupt_files_count()
        self.log.info(f"Number of corrupt files present now: {new_count}")
        self.threat_scan.search_for_keyword(search_keyword)
        num = self.threat_scan.get_row_count()
        if num != 0 and new_count != corrupt_files_count+1:
            raise CVTestStepFailure(
                "Marked corrupt file is present in the report")
    ...
    def run(self):
        try:
            ...
            self.verify_mark_corrupt_action()
            self.cleanup()
