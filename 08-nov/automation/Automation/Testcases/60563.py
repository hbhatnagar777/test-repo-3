...
class TestCase(CVTestCase):
    """ Command center: Validate Archive and restore functionality for existing GPFS client from command center """
    test_step = TestStep()

    def __init__(self):
        """ Initializing the reference variables """
        super(TestCase, self).__init__()
        self.name = "Testcase to Validate Archive and restore functionality for existing GPFS client from command center"
        self.browser = None
    ...
    def add_server(self):
        ...
        self.archiving.add_distributed_app(pkg=DistributedClusterPkgName.GPFS,
                                           server_name=self.hostname,
                                           access_nodes=[self.access_node],
    ...
    def run(self):
        ...
        _desc = """
        1) Add a GPFS File Server at FileServer Page.
        2) Create Archive plan.
        3) Add an Existing GPFS server on Archiver Server Page
        4) Run Archive job.
