...
class TestCase(CVTestCase):
    """Class for executing Basic Acceptance for NDMP Advanced NAS Restore option Recursive ON/OFF - \
    Dell EMC Celerra/VNX, Dell EMC Unity"""

    def __init__(self):
        """"Initializes TestCase object"""
        super(TestCase, self).__init__()
        self.name = "NDMP Advanced NAS Restore option Recursive ON/OFF - Dell EMC Celerra/VNX, Dell EMC Unity"
        self.tcinputs = {
    ...
    def run(self):
        ...
        try:
            ...
            # Run restore out-of-place with Recursive Restore ON
            ...
            if mamachine.get_logs_for_job_from_file(job.job_id, "nasRestore.log", r"ENV \[RECURSIVE\]=\[Y\]"):
                self.log.info('NDMP restore job is run with Recursive Restore ON')
            else:
                self.log.info('NDMP restore job is NOT run with Recursive Restore ON')
                raise Exception('NDMP restore job is NOT run with Recursive Restore ON. Failing the testcase')
            ...
            # Run restore in-place with Recursive Restore ON
            self.log.info("Run Restore in place")
            job = self.subclient.restore_in_place([f'{self.subclient.content[0]}'], noRecursive=True)
            ...
            # Run restore out-of-place with Recursive Restore OFF
            job = self.subclient.restore_out_of_place(
                self.client.client_name,
                str(self.tcinputs['FilerRestoreLocation']),
                self.subclient.content,
            ...
            if mamachine.get_logs_for_job_from_file(job.job_id, "nasRestore.log", r"ENV \[RECURSIVE\]=\[N\]"):
                self.log.info('NDMP restore job is run with Recursive Restore OFF')
            else:
                self.log.info('NDMP restore job is NOT run with Recursive Restore OFF')
                raise Exception('NDMP restore job is NOT run with Recursive Restore OFF. Failing the testcase')
            ...
            # Run restore in-place with Recursive Restore OFF
            self.log.info("Run Restore in place with Recursive Restore OFF")
            job = self.subclient.restore_in_place([f'{self.subclient.content[0]}'], noRecursive=False)
            self.log.info("Started restore in place job with Job ID: %s", job.job_id)
