            ...
            "64268":
                {
                    ...
                    "DestInstanceName": "name of the destination stand-alone instance",
                    "DestClientName": "name of the destination client",
...
class TestCase(CVTestCase):
    """ Class for executing wallet RMAN duplicate restore for oracle rac """
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Oracle RAC on Command Center - RMAN duplicate to standalone db with wallet"
        self.browser = None
        self.admin_console = None
        self.navigator = None
        self.tablespace_name = 'CV_64268'
    ...
    def create_helper_object(self):
        ...
        self.dest_client = self.commcell.clients.get(self.tcinputs["DestClientName"])
        self.dest_instance = self.client.agents.get("oracle rac").instances.get(
            self.tcinputs["DestInstanceName"])
    ...
    def run_restore(self):
        ...
        job_id = restore_panel.out_of_place_restore(
            self.tcinputs["DestClientName"], self.tcinputs["DestInstanceName"],
            redirect_all_path=self.tcinputs.get("RedirectAllPath", None), rman_duplicate=True
        )
        self.wait_for_job_completion(job_id)
