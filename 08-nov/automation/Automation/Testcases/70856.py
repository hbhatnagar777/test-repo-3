...
TestCase:
    ...
    create_new_subclient()             --  creates new sap oracle subclient with util_file_online device

    delete_subclient_data_validation() -- Validates test data on destination client and deletes subclient
                                         if data restore validation is fine
...
class TestCase(CVTestCase):
    ...
    util_file_online device SAP for Oracle """
    test_step = TestStep()
    ...
    def __init__(self):
        ...
        self.name = "SAP Oracle db copy restore test using \
            util_file_online device from Command Center"
        self.browser = None
        self.admin_console = None
        self.navigator = None
        self.tablespace_name = 'CV_70856_TSP'
        self.table_name = "CV_70856_TABLE"
    ...
    def create_helper_object(self):
        ...
        self.saporacle_db_connectpassword = self.sap_oracle_helper_object. \
    ...
    def create_new_subclient(self):
        ...
        self.add_subclient.add_subclient(
            ...
            backup_device="UTIL_FILE_ONLINE")
        self.log.info("Waiting for subclient creation completion")
        self.automation_subclient = self.tablespace_name

    @test_step
    def delete_subclient_data_validation(self):
        """ validates restored tables and Deletes subclient if exists """
        status = self.dest_sap_oracle_helper_object.test_tables_validation(self.tablespace_name,\
                                                                           self.table_name)
        if status == 0:
            ...
            raise CVTestStepFailure(
                "some issue with restoring data to destination client")
    ...
    def db_shut_down(self):
        ...
        if self.machine.os_info == 'UNIX':
            ...
            self.db_cmd = '"sqlplus ' + self.saporacle_db_user + \
                          ...
                          self.saporacle_db_connectstring + ' as sysdba '
        self.sap_oracle_helper_object.db_shutdown(self.db_cmd, "dbshutdown.sql")
    ...
    def run(self):
        ...
        try:
            ...
            self.sap_oracle_helper_object.db_mount(self.db_cmd, "dbmount.sql")

            self.run_inplace_restore()

            self.run_outofplace_restore()

            self.delete_subclient_data_validation()
