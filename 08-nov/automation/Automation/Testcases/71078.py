...
class TestCase(CVTestCase):
    ...
    Basic Validation for Metallic Google Mail existing and new tenant
    """
    ...
    def __init__(self):
        ...
        self.name = "Metallic_Gmail_Acceptance for Service Catalogue"
        self.browser = None
    ...
    def create_tenant(self):
        ...
        self.tenant_user_name = self.hub_utils.create_tenant(
            ...
            email=f'cvautouser-{current_timestamp}@gmail{current_timestamp}.com')
    ...
    def setup(self):
        ...
        self.app_type = GoogleWorkspaceAppTypes.gmail
        ...
        self.mailboxes = self.tcinputs['Mailboxes'].split(",")
        self.inc_mailboxes = self.tcinputs['Incremental_Mailboxes'].split(",")
        self.full_msgs_count = self.tcinputs['FullMsgsCount']
        self.inc_msgs_count = self.tcinputs['IncrMsgsCount']
        self.rest_msgs_count = self.tcinputs['IncrMsgsCount']
    ...
    def run(self):
        ...
        try:
            ...
            self.google_obj.add_content(self.mailboxes, self.gw_plan)
            bkp_job_details = self.google_obj.run_backup()
            if int(bkp_job_details['No of objects backed up']) != self.full_msgs_count:
                raise Exception(f'Incremental backup is not backing up new items')
            self.navigator.navigate_to_googleworkspace()
            self.google_obj.access_googleworkspace_app(self.app_name)
            rows = self.google_obj.get_rows_from_browse()
            if rows==0 or rows!=len(self.mailboxes):
                raise Exception(f'No data shown in browse. Please check index playback')
            self.log.info("Browse is verified after full backup")
            self.admin_console.driver.back()
            self.google_obj.add_content(self.inc_mailboxes, self.gw_plan)
            bkp_job_details_incr = self.google_obj.run_backup()
            if int(bkp_job_details_incr['No of objects backed up']) != self.inc_msgs_count:
                raise Exception(f'Incremental backup is not backing up new items')
            self.log.info("Incremental is verified")
            self.navigator.navigate_to_googleworkspace()
            self.google_obj.access_googleworkspace_app(self.app_name)
            rows = self.google_obj.get_rows_from_browse()
            if rows==0 or rows!=len(self.mailboxes)+len(self.inc_mailboxes):
                raise Exception(f'No data shown in browse. Please check index playback')
            self.log.info("Browse is verified after incremental backup")
            self.admin_console.driver.back()
            restore_job_details = self.google_obj.run_restore(destination='Restore to original location',
                                                              restore_option='UNCONDITIONALLY_OVERWRITE',content=self.inc_mailboxes)
            if int(restore_job_details['Successful messages']) != self.rest_msgs_count or int(restore_job_details['Skipped messages']) != 0 or int(restore_job_details['Failed messages']) != 0:
