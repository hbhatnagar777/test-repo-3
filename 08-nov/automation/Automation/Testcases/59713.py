...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - Verify new data on disk is backed up by inc & diff job for LFS and IFS with Optimized scan
        Step1, configure BackupSet and Subclient with IFS data with Optimized scan.
        Step2, Run backup full + incremental and verify the data with a restore
        step3, Run backup differential and verify the data with a restore
        Step4, configure BackupSet and Subclient with LFS data with Optimized scan.
        Step5, Run backup full + incremental and verify the data with a restore
        step6, Run backup differential and verify the data with a restore
        Step7, Cleanup the data created on the disk.
    ...
    def __init__(self):
        ...
        self.name = "IBMi - Verify new data on disk is backed up by inc & diff job for LFS and IFS with Optimized scan"
    ...
    def run(self):
        ...
        try:
            ...
            self.restore_verify(source=self.srcdir, destination=self.tmp_path)
            self.log.info("Verified IFS backup and restore with new content on disk with %s", self.scan_type)
            self.log.info("Backup of IFS new content on disk verification is successful")
            ...
            self.log.info("Create subclient for LFS with %s",self.scan_type)
            self.helper.create_subclient(name=self.subclient_name,
                                         storage_policy=self.storage_policy,
            ...
            self.restore_verify(source=self.client_machine.lib_to_path(self.srclib),
                                destination=self.client_machine.lib_to_path(self.destlib))
            self.log.info("Verified LFS backup and restore with new content on disk with %s", self.scan_type)
            self.log.info("Backup of LFS new content on disk verification is successful")

            self.client_machine.manage_library(operation='delete', object_name=self.srclib)
            self.client_machine.manage_library(operation='delete', object_name=self.destlib)

            self.log.info("**LFS, IFS OPTIMIZED SCAN BACKUP HAS PICKED NEW CONTENT ON DISK SUCCESSFULLY**")
