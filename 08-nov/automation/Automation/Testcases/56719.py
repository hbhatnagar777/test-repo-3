...
"""Live Sync IO: Verify merging and pruning operation and perform test failover

Sample input: "56719": { "group_name":"Replication group name", "unc_path":"Enter the shared UNC path to which
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Live Sync IO: VM Recovery ( DR Operations): Prune and merge operation verification"
    ...
    def verify_pruning(self, prune_start_time):
        ...
        pattern = "\d+/\d+\s+\d+:\d+:\d+\s+\d+\s+\[testVM1\]\s+[BlrTail]+::[pruneUntil]+\(\)\s+-\s+[Streaming complete]+"
