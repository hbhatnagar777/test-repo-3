...
TestCase:
    ...
    run_restore_jobs()  --  initiates Restores for each of the subclients
...
5: Initiate Restore Jobs on all the 8 SubClients
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = 'Marking Bad Chunks by Restore Job for DiskLibraries'
        self.tcinputs = {
            "MediaAgentName": None
        }
        self.utility = None
    ...
    def run(self):
        ...
        try:
            ...
            self.run_restore_jobs()
            self.wait_for_jobs(self.restore_jobs)
            self.run_backups()
    ...
    def run_restore_jobs(self):
        """Runs the Verification Jobs"""
        self.log.info('Running the Verification Jobs')
        log_line = ''
        for index in range(len(self.subclients)):
            subclient = self.subclients[index]
            job = subclient.restore_out_of_place(self.tcinputs.get('ClientName'),
                                                 self.client_machine.join_path(self.restore_path, f'Data{index}'),
                                                 subclient.content, fs_options={'skipErrorsAndContinue': True})
    ...
    def produce_result(self):
        ...
        for sub_case in range(1, 9):
            ...
            # job dv
            if ((sub_case not in [5, 6, 7]) and
                expected_statuses[sub_case - 1] in self.verification_statuses[sub_case - 1]) \
