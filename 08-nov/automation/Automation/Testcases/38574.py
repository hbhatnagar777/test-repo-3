...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Backup Restore testcase for Informix iDA"
    ...
    def run(self):
        ...
        try:
            ...
            log.info("started the whole system restore Job with Id:%s",
                     job.job_id)
            if not job.wait_for_completion():
                raise Exception(
                    "Failed to run data only restore job with error: {1}".format(
            ...
            log.info("Whole System Restore job is now completed")
