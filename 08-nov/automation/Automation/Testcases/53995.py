...
TestCase:
    ...
    service_restart_ransomware_protect() -- toggle rswp and restart mm service

    run()           --  run function of this test case

    tear_down()     --  tear down function of this test case

DDB_backup_automation - This testcase verifies suspend and resume for DDB backup jobs.
...
Optional arguments:
    ...
    "num_subclients_TC": number of subclients you want to have in TC
                        default = 1
    "test_case_factor": to change the content size for backup jobs
                        default = 1
...
"""
# design steps:
# get mediaAgent machine name in the input, create resources.
# ransomware protection to enable it if it is not enabled
# load ddb backup subclient and retrieve the sidb id.
# run dedupe backup jobs (multiple) to populate the ddb. (total content size = 2GB)
...
# start a dedupe backup job.
# immediately start the DDB backup job.
# (immediately started because we want to have DDB backup when a dedupe backup job is running.)
        # case specific scenario
        # get the number of readers - from job object/ set the number of readers
        # wait for DDB backup job to go to backup phase.
        # suspend the DDB backup job.
        # resume the DDB backup job.

# verifications
#         check the snapshot creation
#         CFileBackupFSW2K::InitializeOFM(639) - Subclient info suggest immediate VSS shadow creation
#         CsSnapRequestor::StartSnapshotSet() - Created shadow set 034c5ab8-e55a-47d3-a654-a2c9951889b1
#         CsSnapRequestor::AddVolumesToSnapshotSet() - Successfully added volume [C:\] to shadow set.
#         COpenFileManager::CacheShadowSetInfo(549) - Volume [C:\]. ShadowId is [034c5ab8-e55a-47d3-a654-a2c9951889b1], ShadowPath is [\\?\GLOBALROOT\Device\HarddiskVolumeShadowCopy97].

# check if the backup was suspended and resumed
#         FsBackupCtlr::HandleDDBBackup(9973) - The restart map is present for the backup 'C:\Program Files\Commvault\ContentStore\iDataAgent\JobResults\CV_JobResults\iDataAgent\FileSystemAgent\2\48027\CollectMap_Tot.cvf', restarting the backup

# attach to volume
#         COpenFileManagerDDB::CreateShadow1(781) - Volume [C:\] not deleted yet, attach
#
# check the number of readers
#         self.log.info("number of readers was set to 10 initially during the testcase")
#         query = """select count(*) from RMReservations where JobId = {0}""".format(str(self.ddb_backup_job.job_id))
# quiescing
# check if unquiescing call sent
# lastsnapjobid
#
# run regular recon using the DDB backup job from earlier.
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "DDB backup Suspend and resume - Windows"
    ...
    def setup(self):
        ...
        suffix = self.tcinputs.get("MediaAgentName") + self.tcinputs.get("ClientName")
        ...
        if self.is_user_defined_lib:
            ...
            self.library_name = f"{str(self.id)}_lib{suffix}"
    ...
    def create_common_items(self):
        ...
        if not cleanup_backup_job.wait_for_completion():
            raise Exception(
                "Job {0} Failed with JPR: {1}".format(cleanup_backup_job.job_id, cleanup_backup_job.delay_reason))
    ...
    def create_content_sc(self, size, subclient):
        ...
        if self.mm_helper.create_uncompressable_data(
            ...
            self.log.info("generated %0.2f GB unique data for subclient: %s at location: %s",
                          size * self.test_case_factor,
                          subclient.name,
                          self.client_machine.join_path(self.content_path, subclient.name))
        else:
    ...
    def verifications(self, jobs):
        ...
        self.log.info("******************************************")
        self.log.info("find lines in clbackup log to confirm if snapshot creation has happened during DDB backup job")
        failed_to_find = []

        find_string = f"Subclient info suggest immediate VSS shadow creation"
        ...
        if matched_lines:
            self.log.info(f"found line {matched_string}")
        else:
            failed_to_find.append(find_string)

        find_string = f"Created shadow set"
        (matched_lines, matched_string) = self.dedup_helper.parse_log(
            self.tcinputs.get("MediaAgentName"),
            "clBackup.log",
            regex=find_string,
        ...
        find_string = f"Successfully added volume [{self.dedup_store_path[:3]}] to shadow set"
        (matched_lines, matched_string) = self.dedup_helper.parse_log(
            self.tcinputs.get("MediaAgentName"),
            "clBackup.log",
            regex=find_string,
        ...
        find_string = f"Volume [{self.dedup_store_path[:3]}]. ShadowId is"
        (matched_lines, matched_string) = self.dedup_helper.parse_log(
            self.tcinputs.get("MediaAgentName"),
            "clBackup.log",
            regex=find_string,
        ...
        if not failed_to_find:
            self.log.info("logs indicate that snapshot was created successfully before suspending the DDB backup job")
            self.log.info("Result :Pass")
        else:
            self.log.error("Result: Failed")
            error_flag += [f"failed to find: {failed_to_find} in clbackup"]

        self.log.info("******************************************")

        self.log.info("******************************************")
        self.log.info("check if the backup was suspended and resumed")
        find_string = f"The restart map is present for the backup"
        (matched_lines, matched_string) = self.dedup_helper.parse_log(
            self.tcinputs.get("MediaAgentName"),
            "clBackup.log",
            regex=find_string,
        ...
        self.log.info("******************************************")
        self.log.info("check if the existing snapshot was used when the DDB Backup job is resumed")
        found = False
        find_string = "Volume .* not deleted yet, attach"
        (matched_lines, matched_string) = self.dedup_helper.parse_log(
            self.tcinputs.get("MediaAgentName"),
            "clBackup.log",
            regex=find_string,
            escape_regex=False, single_file=False, jobid=self.ddb_backup_job.job_id)

        if matched_lines:
            for matched_line in matched_lines:
                if self.dedup_store_path[:3] in matched_line:
                    found = True
        ...
        self.log.info("******************************************")
        self.log.info("check the number of readers after resuming the job")
        self.log.info("number of readers was set to 10 initially during the testcase")
        query = """select count(*) from RMReservations where JobId = {0}""".format(str(self.ddb_backup_job.job_id))
        self.log.info("Executing Query: %s", query)
        self.csdb.execute(query)
        num_streams = self.csdb.fetch_one_row()[0]
        self.log.info("Result: {%s}", num_streams)

        if int(num_streams) == 10 * 2:
            self.log.info("The number of streams after resuming the DDB backup job is 10, same as earlier.")
            self.log.info("Result: Pass")
        else:
            self.log.error("The number of streams after resuming DDB backup job is not the same as earlier.")
            self.log.info(f"number of stream after resuming is {num_streams}, before suspending the jobs it was 10.")
            self.log.info(
                "Based on 10 streams for the DDB backup job, In RMReservations we expect the number of entries to be 10*2.")
            self.log.error("Result: Failed")
            error_flag += [f"The number of streams after resuming DDB backup job is not the same as earlier."]
        self.log.info("******************************************")
    ...
    def service_restart_ransomware_protect(self, enable=False):
        """
        This method will-
            enable or disable ransomware protection on MA
            restart MM service

        Args:
            enable (bool) - True, if ransomware protection is to be enabled.
                            False, if ransomware protection is to be disabled.
        Returns:
            None
        Raises:
            raise exception if ransomware protection could not be changed
                or MM service could not be restarted.
        ...
        if enable:
            ...
            self.log.info("Disable ransomware protection on the MA")
            self.commcell.media_agents.get(self.tcinputs.get('MediaAgentName')).set_ransomware_protection(False)
    ...
    def run(self):
        ...
        try:
            ...
            # load ddb subclient
            self.ddb_subclient_load()
            self.ddbbackup_subclient.data_readers = 10
            ...
            if not (protection_enabled and driver_loaded):
                self.service_restart_ransomware_protect(True)

            filler_sc = self.mm_helper.configure_subclient(
                backupset_name=self.backupset_name,
                subclient_name=f"{self.subclient_name}_filler_sc",
                storage_policy_name=self.storage_policy_name)
            self.create_content_sc(subclient=filler_sc, size=0.5)
            ...
            # add random data in the DDB
            self.service_restart_ransomware_protect(False)

            self.mm_helper.create_uncompressable_data(self.tcinputs.get('MediaAgentName'),
                                                      self.media_agent_machine.join_path(self.dedup_store_path,
                                                                                         ...
                                                                                         "Split00"),
                                                      0.5, 1)
            self.log.info("generated content in the DDB at location %s",
                          self.media_agent_machine.join_path(self.dedup_store_path, "CV_SIDB", "2", str(self.sidb_id),
                                                             "Split00"))
            ...
            for subclient in self.subclient:
                self.create_content_sc(size=0.5, subclient=subclient)
            ...
            if self.sidb_is_up():
                ...
                count = 0
                while not self.ddb_backup_job.is_finished:
                    if self.ddb_backup_job.phase.lower() == "backup":
                        self.log.info("DDB backup job is the backup phase.")
                        break
                    elif count == 240:
                        raise Exception("DDB backup job did not go to backup phase in 4 mins")
                    time.sleep(2)
                    count += 2
                ...
                count = 0
                while count < 600:
                    self.ddb_backup_job.refresh()
                    if self.ddb_backup_job.num_of_files_transferred > 0:
                        break
                    time.sleep(1)
                    if count % 5 == 0:
                        self.log.info("Wait for 5 seconds for file transfer through DDB backup job.")
                    count += 1
                else:
                    raise Exception(
                        "We did not transfer any file within 10 minutes of DDB backup job being started, exiting the TC.")

                self.ddb_backup_job.pause(wait_for_job_to_pause=True)
                self.log.info("Suspended the DDB backup job")
                self.log.info("wait for 30 seconds")
                time.sleep(30)

                self.ddb_backup_job.resume(wait_for_job_to_resume=True)
                self.log.info("Resumed the DDB backup job")
            ...
            self.log.info(">>>>>Validation 2: check if LastSnapJobId is still the same as previously"
                          " run DDB backup job id")
            self.log.info("This will confirm that recon was indeed a regular recon and not"
                          " a Full Recon because in case of Full Recon - LastSnapJobId is set to 0")
            query = "select LastSnapJobId from IdxSIDBSubStore where SIDBStoreId = {0}".format(self.sidb_id)
            self.log.info("Executing Query: %s", query)
            self.csdb.execute(query)
            job_id = self.csdb.fetch_one_row()[0]
