...
class TestCase(CVTestCase):
    ...
    def run(self):
        ...
        try:
            ...
            query = f"select name from APP_Client where name like '{self.client.client_name}%'"
            self.csdb.execute(query)
            self.log.info("Checking if client is **not deleted from the commcell")
            new_client_name = self.csdb.fetch_one_row()[0]
            if not self.commcell.clients.has_client(new_client_name):
            ...
            if new_client_name:
                if self.commcell.clients.has_client(new_client_name):
                    self.commcell.clients.delete(new_client_name)
