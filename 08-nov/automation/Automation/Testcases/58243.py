...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.name = "Installation with the same client name and same hostname already existing in the commcell " \
                    "environment"
        ...
        try:
            ...
            self._log.info(
                "Reinstalling client {0} with hostname {1} to CS by same user {2}".format(self.client,
                                                                                          self.client_hostname,
                                                                                          self.username_1))
            # Register same client with main user
            self.sim_caller.install_new_client(
                self.client,
                self.client_hostname,
                self.username_1,
            ...
            self._log.info(
                "Reinstalling client {0} with hostname {1} to CS by user {2}".format(self.client,
                                                                                     self.client_hostname,
                                                                                     self.username_2))
