...
TestCase:
    ...
    disable_first_classifier()          --  Disables the first classifier created
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Data Classifier - Validate crawl job not classifying documents when classifier is disabled"
        self.tcinputs = {
    ...
    def setup(self):
        ...
        # Have small name as classifier else chart legend will return as ...
        self.classifier_name = "Auto_%s" % self.id
        self.second_classifier_name = "Auto2_%s" % self.id
    ...
    def create_plan(self):
        ...
        self.gdpr_obj.plans_obj.create_data_classification_plan(
            ...
            classifier_list=[self.classifier_name, self.second_classifier_name])
    ...
    @test_step
    def create_second_classifier(self):
        """Creates second new classifier on the commcell"""
        self.admin_console.navigator.navigate_to_governance_apps()
        self.gdpr_obj.inventory_details_obj.select_entity_manager(sub_type=1)
    ...
    def verify_classifier_legend(self):
        ...
        for legend in legend_text:
            if self.classifier_name in legend:
                raise Exception("Disabled classifier entity got extracted. Please check")
            if self.second_classifier_name in legend:
                self.log.info("Classification happened for enabled entity. Analyze doc count further")
        ...
        self.log.info("Data classification not happened for disabled entity. Consider as success")

    @test_step
    def disable_first_classifier(self):
        """Disables the classifier"""
        self.admin_console.navigator.navigate_to_governance_apps()
        self.gdpr_obj.inventory_details_obj.select_entity_manager(sub_type=1)
        self.gdpr_obj.classifier_obj.enable_or_disable_classifier(name=self.classifier_name, enable=False)
    ...
    def run(self):
        ...
        try:
            ...
            self.create_second_classifier()
            self.move_model_data()
            self.create_plan()
            self.create_inventory()
            self.disable_first_classifier()
