...
Test cases to validate ondemand and schedule jobs when activity control is disabled at IDA level.
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Acceptance]:Validate backup,restore jobs when activity disabled at IDA level"
    ...
    def run(self):
        ...
        try:
            ...
            subclient1_name = OptionsSelector.get_custom_str('subclient', '52760_1')
            subclient2_name = OptionsSelector.get_custom_str('subclient', '52760_2')
            ...
            server_tc.log_step(
                        ...
                        2. Disable Backup and Restore Activity Control at iDA level,
                            2.1. Start OnDemand / Scheduled Backup and Restore for this iDA-1.
                            2.2  Start Backup and Restore for other iDA,
                                for which Activity Control is not Disabled
                            2.3 Enable Activity Control at iDA-1
                        """, 200
                        )
            ...
            server_tc.log_step("""Step 2)
                                        Disable Backup and Restore Activity at iDA level""")
            object_acivity.modify_activity('backup', self.agent, 'disable')
            object_acivity.modify_activity('restore', self.agent, 'disable')
            server_tc.log_step("""Step 2.1)
                                        Start OnDemand Backup on  iDA-1 and  iDA-2""")
            ...
            job_object.check_job_status(host_name,
                                        _object2,
                                        subclient2_props['subclient']['agent'],
                                        entity_name='ida',
                                        backup=True,
                                        client_1=False)
            object_acivity.modify_activity('backup', self.agent, 'enable')
            for _job in job_list:
                job_object.job = _job
                job_object.wait_for_state('completed')
            server_tc.log_step("""Step 2.2)
                                    Start OnDemand restore on iDA-1 and  iDA-2""")
            job_list.clear()
            ...
            job_object.check_job_status(host_name,
                                        ...
                                        backup=False,
                                        client_1=False)
            object_acivity.modify_activity('restore', self.agent, 'enable')
            for _job in job_list:
                job_object.job = _job
                job_object.wait_for_state('completed')
            server_tc.log_step("""Step 2.3)
                                start schedule backup , restore on iDA-1""")
            object_acivity.modify_activity('backup', self.agent, 'disable')
            object_acivity.modify_activity('restore', self.agent, 'disable')
            job_list.clear()
