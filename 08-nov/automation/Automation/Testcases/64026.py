...
class TestCase(CVTestCase):
    """Class for executing the Software only user flow test case"""
    ...
    def __init__(self):
        ...
        self.name = "[Mirage]: Software only user flow"
        self.tcinputs = {
            "CommcellName": "",
        }
    ...
    def run(self):
        ...
        try:
            ...
            MirageApiHelper(self.commcell).retry_function_execution(func=self.validate_software_only_flow)
    ...
    @test_step
    def validate_software_only_flow(self):
        """Validates the software only user flow"""
        self.__open_browser()
        MirageTrialHelper(self.admin_console, self.commcell, self.tcinputs['CommcellName']).perform_mirage_trial()