...
class TestCase(CVTestCase):
    """Testcase : Deleting cache and triggering install with RolesManager Unix"""

    def __init__(self):
        """Initializes test case class object"""
        super().__init__()
        self.name = "Deleting cache and triggering install with RolesManager Unix"
        ...
        try:
            ...
            for jid, data in self.job_controller.active_jobs().items():
                if data['operation'] == 'Download Software':
                    ...
                    self.log.info("Job %s started for Download packages", job_obj.job_id)
                    try:
                        job_obj.wait_for_completion()
                    except Exception:
                        self.log.error("Download job Failed")
                    if not job_obj.wait_for_completion():
                        raise Exception("Failed to run Download job with error: " + job_obj.delay_reason)

            # Checking the install job is being triggered
            for jid, data in self.job_controller.active_jobs().items():
