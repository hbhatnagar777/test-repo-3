...
class TestCase(CVTestCase):
    """Class for executing Basic acceptance test of MongoDB backup and Restore test case"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Command Center: Verify out of place restore for mongodb with " \
                    "autoshutdown option and complete shard list"
    ...
    def init_tc(self):
        ...
        try:
            ...
            self.master_port = self.tcinputs['master_port']
            ...
            self.shard_list = self.tcinputs['shard_list']
            self.replica_set = self.tcinputs['replica_set']
            self.des_cluster = self.tcinputs['des_cluster']
    ...
    def _restore(self):
        ...
        self.restore_obj.restore_out_of_place(self.des_cluster,
                                              des_shardlist=self.shard_list, shutdown=True)
    ...
    def discover_and_backup(self):
        ...
        self.mongodb_helper = MongoDBHelper(self.commcell, self.master_node, self.master_port,
                                            self.db_user, self.db_pwd,
                                            replset=self.tcinputs['replica_set'] or None,
                                            bin_path=self.tcinputs['bin_path'])
    ...
    def autoshutdown_servers_and_restore(self):
        """get start commands,autoshutdown servers and starts restore"""
        ...
        self.overview.access_restore(self.tcinputs['pseudo_client_name'])
        self._restore()
    ...
    def validation_post_restore(self):
        ...
        if self.replica_set_or_shard == 0:
            ...
            des_cl_machine = self.shard_list[self.replica_set]["P"][0]
            des_host = str(des_cl_machine)
            des_port = int(self.shard_list[self.replica_set]["P"][1])
            ...
            tempmongos_cmd = self.mongodb_helper.disable_authentication_mongos(self.mongos_start)
            self.log.info("Start mongos without authentication")
            self.mongodb_helper.start_mongos_server(tempmongos_cmd)
