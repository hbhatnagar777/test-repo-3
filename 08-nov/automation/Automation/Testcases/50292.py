...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - Verify backup cycle Full-Differential-Incremental for IFS and LFS data
    ...
    def __init__(self):
        ...
        self.name = "IBMi - Verify backup cycle Full-Differential-Incremental for IFS and LFS data"
    ...
    def run(self):
        ...
        try:
            ...
            self.restore_verify(source=self.srcdir, destination=self.tmp_path)
            self.log.info("Verified IFS backup and restore with diff + inc backup cycle with %s", self.scan_type.name)
            ...
            for each in scans:
                ...
                self.restore_verify(source=self.client_machine.lib_to_path(self.srclib),
                                    destination=self.client_machine.lib_to_path(self.destlib))
                self.log.info("Verified LFS backup and restore with diff + Inc backup cycle with %s",
                              self.scan_type.name)
