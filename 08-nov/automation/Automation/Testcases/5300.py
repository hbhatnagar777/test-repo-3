...
TestCase:
    ...
    validate_rollforward_status()       --  Validates the 'Rollforward status' of a database
...
class TestCase(CVTestCase):
    """ Command center: DB2 Restore with Roll Forward Pending State for tablespace only from Online Backup """
    ...
    def __init__(self):
        ...
        self.name = "DB2 Restore with Roll Forward Pending State for tablespace only from Online Backup"
        self.browser = None
    ...
    def setup(self):
        ...
        try:
            ...
            self.database_name = self.tcinputs["database_name"]
    ...
    def run(self):
        ...
        try:
            ...
            self.run_backup()
            self.db2_helper.rename_datafile(self.tablespace_name)
            self.run_restore()
            self.validate_rollforward_status(self.database_name)
    ...
    def run_restore(self):
        ...
        restore_job = self.db_backupset.restore_folders(database_type=self.dbtype, items_to_restore=[self.tablespace_name])
        restore_job_id = restore_job.in_place_restore(deselect_roll_forward=True)
    ...
    @test_step
    def validate_rollforward_status(self, db_name):
        """
        Validates the 'Rollforward status' of a database.
        If the status is 'Pending', it logs a success message. If not, it raises an error.

        Args:
            db_name (str): The name of the database to validate.

        Raises:
            Exception: If the 'Rollforward status' is not pending.
        """

        resp = self.db2_helper.get_rollforward_status(db_name)
        status = ''.join(resp[-1]).split('=')[-1]

        if status.lower() == "tbspending":
            self.log.info(f"Rollforward status for {db_name} is in 'Pending' state")
        else:
            raise CVTestStepFailure(f"Rollforward status for {db_name} is not 'Pending'. Current status: {status}")
