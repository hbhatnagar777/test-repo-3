...
class TestCase(CVTestCase):
    """FSO basic acceptance test for AWS Cloud App"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "FSO basic acceptance test for AWS Cloud App"
    ...
    def setup(self):
        ...
        try:
            ...
            self.fso_helper.data_source_name = self.datasource_name
    ...
    def fetch_metadata_create_db(self):
        ...
        """
        s3_session = self.s3helper.create_session_s3(CONFIG.AccountName, CONFIG.AccessKey,
                                                     CONFIG.BucketRegion)
        meta_data = self.s3helper.fetch_file_metadata(s3_session, self.container_name)
        if not meta_data:
