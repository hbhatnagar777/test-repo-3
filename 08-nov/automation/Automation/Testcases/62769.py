...
class TestCase(CVTestCase):
    """Class for executing Basic Acceptance for NDMP EMC Celerra / VNX - Volume Based Backup"""

    def __init__(self):
        """"Initializes TestCase object"""
        super(TestCase, self).__init__()
        self.name = "NDMP EMC Celerra / VNX - Volume Based Backup"
        self.tcinputs = {
    ...
    def setup(self, is_cluster=False):
        ...
        self.log.info(self.sp_name, self.sc_name)
    ...
    def run(self):
        ...
        try:
            ...
            job = self.subclient.backup("FULL", volume_based_backup=True)
            ...
            self.log.info("MA is: %s", maclient)
            mamachine = Machine(maclient)

            if mamachine.get_logs_for_job_from_file(job.job_id, "NasBackup.log", r"BackupType\[vbb\]"):
                self.log.info('NDMP backup job is run with Volume Based Backup option')
            else:
                self.log.info('NDMP backup job is NOT run with Volume Based Backup option')
                raise Exception('NDMP backup job is NOT run with Volume Based Backup option. Failing the testcase')
            ...
            if mamachine.get_logs_for_job_from_file(job.job_id, "nasRestore.log", r"restore from backupType \[vbb\]"):
                self.log.info('NDMP restore job is a restore from backupType [vbb]')
            else:
                self.log.info('NDMP restore job is NOT a restore from backupType [vbb]')
                raise Exception('NDMP restore job is NOT a restore from backupType [vbb]. Failing the testcase')
