...
class TestCase(CVTestCase):
    """Class for executing Basic acceptance test of Exchange UserMailbox Onpremise
    ...
    def __init__(self):
        ...
        self.name = "Exchange Onpremise Discovery and assocaition Validation"
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info(
                "--------------------------VALIDATE DB DISCOVERY"
                "-----------------------------------"
            )

            active_directory.validate_db_discovery()
            ...
            databases_list = []
            for database in self._subclient.discover_databases:
                databases_list.append(database['databaseName'])
            allusers_content = {
            ...
            self.log.info(
                "--------------------------CREATE DB ASSOCAITION AND VALIDATE"
                "-----------------------------------"
            )

            active_directory.set_database_associations(database_content)
            active_directory.validate_database_assocaition(database_content)
            ...
            self._subclient.delete_database_assocaition(database_content)
            active_directory.delete_user_assocaitions(subclient_content)
