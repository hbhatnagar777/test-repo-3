...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Validate SDG local crawl job for multiple windows CA in plan with one CA down"
        self.tcinputs = {
            "CAClouds": None,
            "SDGClient": None,
            "IndexServer": None
    ...
    def setup(self):
        ...
        self.sdg_project_name = f"MultipleCA_Project_{self.id}"
        self.plan_name = f"MultipleCA_TestPlan_{self.id}"
        self.ds_name = f"MultipleCA_DataSource_{self.id}"
        self.inv_name = f"MultipleCA_Inventory_{self.id}"
    ...
    def run(self):
        ...
        try:
            self.cleanup()
            self.log.info(f"Bringing down content extractor service on CA node : [{self.ca_list[0]}]")
            self.client_obj = self.commcell.clients.get(self.ca_list[0])
            self.client_obj.stop_service(service_name=dynamic_constants.CE_SERVICE_NAME)
            self.log.info("Successfully stopped the content extractor process on CA client")
            is_helper = IndexServerHelper(self.commcell, self.index_server)
            is_helper.restart_svc_all_index_nodes()
    ...
    def tear_down(self):
        ...
        if self.status == constants.PASSED:
            self.client_obj.start_service(service_name=dynamic_constants.CE_SERVICE_NAME)
            self.log.info(f"Successfully started the content extractor process on CA client - {self.client_obj.name}")
            self.cleanup()
