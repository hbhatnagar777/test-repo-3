...
"62203": {
            ...
            "src_db2_username": "dummy_user",
            "src_db2_user_password": "test_passwd",
            "plan": "plan name",
            "src_database": "Source Database",
            "destination_client": "CvClientName",
            "dest_db2_username": "dummy_user",
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Command Center - DB2 Out of place Restore: To Same Instance on Different Machine"
        self.browser = None
        ...
        self.tcinputs = {
            "instance_name": None,
            "src_db2_username": None,
            "src_db2_user_password": None,
            "plan": None,
            "src_database": None,
    ...
    def setup(self):
        ...
        try:
            self.destination_client_obj = Client(commcell_object=self.commcell,
                                                 client_name=self.tcinputs["destination_client"])

            self.source_client = Machine(machine_name=self.client.client_hostname,
                                         username=self.tcinputs['src_db2_username'],
                                         password=self.tcinputs['src_db2_user_password'])
            self.destination_client = Machine(machine_name=self.destination_client_obj.client_hostname,
            ...
            self.db2_base = ""
            if "windows" in self.destination_client.os_info.lower():
    ...
    def run(self):
        ...
        try:
            ...
            self.db_instance.select_instance(database_type=self.dbtype,
                                             instance_name=self.tcinputs["instance_name"],
                                             client_name=self.destination_client_obj.display_name)
    ...
    def prerequisite_setup_test_case(self):
        ...
        self.remove_existing_logs_dir(client_machine=self.destination_client)
        self.drop_database_on_client(client_machine=self.destination_client,
                                     database_name=self.dest_db)

        self.src_home_path, self.tcinputs["src_db2_username"] = self.get_user_details(
            client_machine=self.source_client,
            client_object=self.client,
            username=self.tcinputs["src_db2_username"])

        self.dest_home_path, self.tcinputs["dest_db2_username"] = self.get_user_details(
            client_machine=self.destination_client,
            client_object=self.destination_client_obj,
            username=self.tcinputs["dest_db2_username"])

        self.get_redirect_path()

        self.navigator.navigate_to_db_instances()
        self.delete_existing_instance(client_name=self.destination_client_obj.display_name)
        self.add_db2_instance(server_name=self.destination_client_obj.display_name,
    ...
    def verify_db2_version(self):
        ...
        Raises:
            CVTestStepFailure - If DB2 versions do not match on source and destination instance
        """
        version_cmd = f"{self.db2_base.format(self.tcinputs['instance_name'])} db2level"
        source_version = self.source_client.execute_command(command=version_cmd).output.strip()
        ...
        version_cmd = f"{self.db2_base.format(self.tcinputs['instance_name'])} db2level"
        destination_version = self.destination_client.execute_command(command=version_cmd).output.strip()
    ...
    @test_step
    def remove_existing_logs_dir(self, client_machine):
        """
        ...
        self.delete_database_directory(client_machine=client_machine,
                                       path=f"{archive_path}{path_sep}{self.tcinputs['instance_name']}{path_sep}",
                                       database_name=self.dest_db)
        self.delete_database_directory(client_machine=client_machine,
                                       path=f"{retrieve_path}{path_sep}{self.tcinputs['instance_name']}{path_sep}",
                                       database_name=self.dest_db)
    ...
    @test_step
    def drop_database_on_client(self, client_machine, database_name):
        """
        ...
        database_cmd = f"{self.db2_base.format(self.tcinputs['instance_name'])} db2 force application all"
        ...
        database_cmd = f"{self.db2_base.format(self.tcinputs['instance_name'])} db2 deactivate db {database_name}"
        self.log.info("Deactivating database %s on client using command: %s", database_name, database_cmd)
        output = client_machine.execute_command(command=database_cmd)
        self.log.info(output.output)

        if "SQL1031N" in output.output:
            database_cmd = f"{self.db2_base.format(self.tcinputs['instance_name'])} db2 uncatalog database {database_name}"
        ...
        database_cmd = f"{self.db2_base.format(self.tcinputs['instance_name'])} db2 drop db {database_name}"
    ...
    def get_redirect_path(self):
        ...
        create_database = f"{self.db2_base.format(self.tcinputs['instance_name'])} db2 create db tp{self.id}"
        output = self.destination_client.execute_command(command=create_database)
        self.log.info(output.output)

        get_redirect_path = f"{self.db2_base.format(self.tcinputs['instance_name'])} db2 connect to tp{self.id}; " \
                            f"db2 \"SELECT DBPARTITIONNUM, TYPE, PATH FROM TABLE(ADMIN_LIST_DB_PATHS()) AS FILES " \
                            f"where type='DB_STORAGE_PATH'\""
        output = self.destination_client.execute_command(command=get_redirect_path).formatted_output
        self.log.info("Getting Redirect Path using command: %s", get_redirect_path)
        self.redirect_path = output[index][2]
        self.log.info("Redirect path for the instance %s:%s", self.tcinputs['instance_name'], self.redirect_path)
        self.log.info("Dropping temporary database.")
        self.drop_database_on_client(client_machine=self.destination_client,
                                     database_name=f"tp{self.id}")

        self.log.info("Setting Restore Grant to Prevent Authorization Issue:")
        # https://www.ibm.com/support/pages/apar/IC76330
        restore_grant_cmd = f"{self.db2_base.format(self.tcinputs['instance_name'])} " \
                            f"db2set DB2_RESTORE_GRANT_ADMIN_AUTHORITIES=ON; db2set"
        output = self.destination_client.execute_command(command=restore_grant_cmd)
        self.log.info(output.output)
    ...
    @test_step
    def delete_existing_instance(self, client_name):
        """
    ...
    @test_step
    def add_db2_instance(self, server_name, home_path, username, password, credential_name):
        """
    ...
    def run_restore(self):
        ...
        restore_job_id = restore_job.out_of_place_restore(destination_client=self.destination_client_obj.display_name,
                                                          ...
                                                          redirect_sto_grp_path={
                                                              ...
                                                              f"STG{self.id}": self.redirect_path
                                                          })
    ...
    def validate_restore_job(self):
        ...
        """
        self.commcell.refresh()
        dest_agent = self.destination_client_obj.agents.get(self.tcinputs['AgentName'])
        instance = dest_agent.instances.get(self.tcinputs['instance_name'])
        backupset = instance.backupsets.get(self.dest_db)

        dest_db2_helper = DB2(commcell=self.commcell,
                              client=self.destination_client_obj,
                              instance=instance,
                              backupset=backupset)
    ...
    def cleanup(self):
        ...
        restore_grant_reset_cmd = f"{self.db2_base.format(self.tcinputs['instance_name'])} " \
                                  f"db2set DB2_RESTORE_GRANT_ADMIN_AUTHORITIES= -immediate; db2set"
        output = self.destination_client.execute_command(command=restore_grant_reset_cmd)
        self.log.info(output.output)
        self.source_client.disconnect()
        self.destination_client.disconnect()
