...
class TestCase(CVTestCase):
    ...
    def review_move_action_filesystem(self):
        ...
        try:
            ...
            self.log.info(
                f"List of files present in the destination path {destination_files}")
            self.log.info(
            ...
            if count_after_move == 1 and filename in destination_files[0] and (not source_file_present):
                self.log.info(
                    f"Successfully moved {filename} to {destination_path}")
            else:
