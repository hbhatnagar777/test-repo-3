...
TestCase:
    ...
    review_ignorerisks_action_filesystem()       -- Verifies that a file's risk gets ignored in the review page
...
class TestCase(CVTestCase):
    """Class for executing ignore risks action for a single file with UNC path in Review page in SDG"""
    test_step = TestStep()
    ...
    def __init__(self):
        ...
        self.name = "Ignore risks action for a single file with UNC path in Review page in SDG"
        self.tcinputs = {
    ...
    def init_tc(self):
        ...
        try:
            ...
            self.log.info("Performing the conditional cleanup.")
    ...
    def create_sdg_project(self):
        ...
        if self.is_test_step_complete(sc.CREATE_SDG_PROJECT):
            ...
            self.gdpr_base.file_server_lookup_obj.add_file_server(
                ...
                access_node=self.tcinputs['AccessNode'], identity_server=self.tcinputs["NameServerAsset"])
    ...
    def review_ignorerisks_action_filesystem(self):
        """
        Verifies that risk for a single file is getting ignored
        """
        try:
            ...
            raise CVTestStepFailure(
                f"Ignore risks action failed: {error_status}")
