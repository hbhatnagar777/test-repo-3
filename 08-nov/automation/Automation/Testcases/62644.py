...
TestCase:
    ...
    review_ignorefiles_action_filesystem()       -- Verifies that the files on a page are getting ignored in the review page
...
from dynamicindex.utils.constants import (COMPLETED, HOST_NAME, ID,
                                          IGNORE_RISKS, RETENTION_NOT_SET,
                                          STATUS, USA_COUNTRY_NAME)
...
class TestCase(CVTestCase):
    """Class for executing ignore risks action with review request for files with local path"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Ignore risks action with review request for files with local path in SDG"
        self.tcinputs = {
    ...
    def review_request(self):
        ...
        self.review_obj.review_approve_request(
            self.request_name, self.sensitive_files)

    @test_step
    def review_ignorefiles_action_filesystem(self):
        """
        Verifies that the risk in files gets ignored
        """
        try:
            ...
            status = self.gdpr_base.data_source_review_obj.review_ignore_risks_actions("", [RETENTION_NOT_SET], all_items_in_page=True, review_request=True, request_name=self.request_name, reviewer=reviewer,
                                                                                       approver=approver
                                                                                       )
    ...
    def validate_review_action(self):
        ...
        for file in self.sensitive_files:
            status = self.gdpr_base.data_source_review_obj.risks_present(
                file, [RETENTION_NOT_SET]
            )
            if status:
                raise CVWebAutomationException(
                    f"File's {file} risk failed to get ignored")
