...
class TestCase(CVTestCase):
    ...
    """Class for validating WORKFLOW - Validate Reassociation of workflow schedules - One time"""

    def __init__(self):
        super(TestCase, self).__init__()
        self.name = "WORKFLOW - Validate Reassociation of workflow schedules - One Time"
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Created schedule successfully for type one_time")
            ...
            if not JobManager(_job=job_obj[0], commcell=self.commcell).wait_for_state(time_limit=60):
                raise Exception("Job did not succeed for type one_type")
            if job_obj[0].job_type.lower() != self.new_workflow_name.lower():
                raise Exception("Reassociation did not succeed")
            self.log.info("Schedule Validated Successfully for type one_type")
