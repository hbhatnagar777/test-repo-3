...
class TestCase(CVTestCase):
    """Testcase : Download software using HTTPS with HTTP proxy"""

    def __init__(self):
        """Initializes test case class object"""
        super().__init__()
        self.name = "Download software using HTTPS with HTTP proxy"
    ...
    def setup(self):
        ...
        self.client_proxy_server = self.config_json.HttpProxy.MACHINE_NAME
        self.client_proxy_port = self.config_json.HttpProxy.PORT
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info(f"Setting the client proxy in CS to {self.client_proxy_server}")
            self._internet_options_obj.set_http_proxy(self.client_proxy_server, self.client_proxy_port)
            time.sleep(30)
            self._internet_options_obj.set_no_gateway()
            time.sleep(30)
            self.download_helper.update_http_proxy_details_in_cs(self.client_proxy_server, self.client_proxy_port)
    ...
    def tear_down(self):
        ...
        if self.status == "FAILED":
            self.log.info(f"Removing incorrect host file entry for {self.config_json.Install.download_server}")
            self.cs_machine_obj.remove_host_file_entry(hostname=self.config_json.Install.download_server.split(":")[0])
        self.log.info("Resetting the client proxy in CS to default value")
        self._internet_options_obj.set_no_gateway()
        time.sleep(30)
        self._internet_options_obj.disable_http_proxy()
        time.sleep(30)
        self.download_helper.update_http_proxy_details_in_cs(reset=True)
