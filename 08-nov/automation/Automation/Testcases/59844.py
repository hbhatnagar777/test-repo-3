...
TestCase:
    ...
    client_v1_indexing()    -- turns on/off v1 indexing for the client of this testcase

    run_backup_job()   -- runs backup job of given type

    wait_for_job_backup_phase() -- cause the program to wait for the
                            backup phase of the job
...
checks if the DDBBackup job is working correctly or not, whether
the ddb is being backed up (non dedupe job) properly while regular
dedupe backup jobs are in progress for v1 indexing clients
    # basic DDBBackup job case for v1 indexing clients
...
input json file arguments required:
                        ...
                        "testcaseFactor": "factor to change the data content ->
                         can control the overlap of DDBbackup job with
                         sidb2.exe process to be online"
                        ...
                        # make sure for test
                        "mount_path" : "enter the unix path to library
                         folder for creating mount path"
                        "dedup_path": "enter the unix path to DDB folder
                         for creating dedup store path"

Design steps:
pre run: create resources
for a v1 indexing clients
1.	Have dedupe SP
2.	Make sure DDBBackup subclient exists and is associated to valid SP
3.	Run backup with new content (we need to run backups
    on multiple subclients, all pointing to same store with
    generated content so that SIDB2 process is up for some
    good enough time)
...
5.	After all the executing jobs complete
    a.	Validate, this DDB Backup job, quiesced the store from #3 (sidbengine log)
    b.	Validate, snapshot was created (clbackupParent log)
    c.	Validate, snapshot was deleted (query for mounted  snaps on unix)
"""
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = """UNIX basic case for DDBBackup Job - v1 indexing client"""
        self.tcinputs = {
            "MediaAgentName": None,
            "testcaseFactor": None,
            "mount_path": None,
    ...
    def client_v1_indexing(self, set_v1):
        """
        to enable / disable client v1 indexing

        Args:
            set_v1       (bool)           True - set v1 indexing
                                          False - set v2 indexing
        Returns:
            None
        ...
        """
        if set_v1:
            query = """ update  APP_ClientProp
                        set     attrVal=0
                        where   attrName='IndexingV2'
                        and     componentNameId = {0}""".format(self.client.client_id)
            self.log.info("EXECUTING QUERY: %s", query)
            self.opt_selector.update_commserve_db(query)
            self.log.info("Client indexing set to V1")
        else:
            query = """ update  APP_ClientProp
                        set     attrVal=1
                        where   attrName='IndexingV2'
            ...
            self.log.info("EXECUTING QUERY: %s", query)
            self.opt_selector.update_commserve_db(query)
            self.log.info("Client indexing set to V2")
    ...
    def wait_for_job_backup_phase(self, job):
        ...
        while not job.is_finished and job.status.lower not in [
                "failed", "killed"]:
            if job.phase.lower() == "backup" and previous_phase.lower() == "scan":
                return True
            time.sleep(1)
    ...
    def validations_checks(self):
        ...
        self.log.info("verify if V1 indexing is still set for the client")
        self.log.info("******************************************")
        ...
        if int(self.csdb.fetch_one_row()[0]) == 0:
            self.log.info("Indexing has been set to V1 for this client")
            self.log.info("Result: Pass")
        else:
            self.log.info("Result:Fail")
            error_flag += ["Indexing is set to V2 for this client"]
        self.log.info("******************************************")
        ...
        for matched_line in matched_lines:  # taken all occurrences
            if common in matched_line:  # verify if happened for our ddb store
                found = True
                break
        if found:
            self.log.info(
        ...
        (matched_line, matched_string) = self.dedup_helper.parse_log(
            ...
            "clBackupParent.log",
            regex=statement,
            jobid=self.ddb_backup_job.job_id,
            escape_regex=True,
            single_file=True)
        # have checked for the ddb backup job of this testcase
        ...
        # there can be multiple snapshots created which may lead
        # to multiple files created belonging to
    ...
    def run(self):
        ...
        try:
            ...
            # when ddb backup job runs, it will backup all the
            # partitions of the ddb on the particular
            ...
            # now wait for both the jobs to be completed
    ...
    def tear_down(self):
        ...
        try:
            ...
            self.log.info("setting the indexing back to v2 for the client")
            self.client_v1_indexing(False)
