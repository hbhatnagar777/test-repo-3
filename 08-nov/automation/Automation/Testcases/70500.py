...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Testcase to validate entropy for files marked as suspicious."
        self.tcinputs = {
    ...
    def run_data_curation(self):
        ...
        entropy_file = self.activateutils.get_random_file_from_path(
        self.source_machine, self.local_path)
        search_keyword = f"*{entropy_file}*"
        entropy_file_path = os.path.join(self.local_path,entropy_file)

        threat_scan.open_file_preview(search_keyword,entropy_file_path)
        file_preview_details = threat_scan.get_preview_details()

        entropy_value = float(file_preview_details['Entropy'])
        self.log.info(f"File entropy score {entropy_value} is displayed in the preview pane.")

        if (entropy_value >= 7.0):
            self.log.info("File is marked as suspicious due to high entropy")
        else:
            raise CVTestStepFailure("File Entropy is not high and is incorrectly marked as suspicious.")      
        
        threat_scan.close_file_preview()
