                ...
                "59358":
                        {
                          ...
                          "RedirectPath": "redirect/path/"(Redirect path for individual
                                                            tablespaces including end separator)
                          "RedirectRedo":{"nGroups":Number of Groups, 
                                          "nFiles":Number of Files in each Group,
                                          "redoSize":Size of each file, 
                                          "LVol":["Log Volume paths"]},
					      "RedirectTemp": "path"
                        }
            }
...
class TestCase(CVTestCase):
    """ Class for executing Out-of-place restore with redirect Individual tablespaces
     Test from databases listing page for oracle """
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Oracle IDA Command Center - Out-of-place restore with redirect " \
                    "Individual tablespaces"
        self.browser = None
        self.admin_console = None
        self.navigator = None
        self.tablespace_name = 'CV_59358'
    ...
    def navigate_to_instance(self):
        ...
        self.database_instances.access_databases_tab()
        ...
        job_id = restore_panel.out_of_place_restore(
            ...
            redirect_datafiles={
                self.tablespace_name:
                    [f"{self.tcinputs['RedirectPath']}{self.tablespace_name}",
                     f"{self.tcinputs['RedirectPath']}{self.tablespace_name}1"]
            },
            redirect_redo=self.tcinputs["RedirectRedo"], redirect_temp=self.tcinputs["RedirectTemp"])
