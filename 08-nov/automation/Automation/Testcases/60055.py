...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Metallic baisc acceptance test for Hyper-V"
        ...
        self.company_prefix = "VSA-Automation-HyperV-"
    ...
    def setup(self):
        try:
            ...
            download_path = os.path.join(constants.TEMP_DIR, CCVMKubernetesTypes.hyper_v.split()[0])
            ...
            self.vsa_metallic_helper.metallic_options.vm_group_name = \
                self.vsa_metallic_helper.metallic_options.vm_group_name + self.unique_param
            ...
            hyp_client = self.tcinputs['hyp_client_name'] + self.unique_param

            self.tcinputs['ClientName'] = hyp_client
            self.tcinputs['SubclientName'] = self.tcinputs['SubclientName'] + self.unique_param
    ...
    def cleanup_access_nodes(self):
        """
        uninstall the instances on the machines
        """
        access_node_details = {
        ...
        hyperv_proxy_details = {
            'name': self.vsa_metallic_helper.metallic_options.hyp_host_name,
            'username': self.vsa_metallic_helper.metallic_options.proxy_remote_username,
            'password': self.vsa_metallic_helper.metallic_options.proxy_remote_userpassword
        }
        self.vsa_metallic_helper.cleanup_metallic_instance_on_client(machine_details=access_node_details)
        self.vsa_metallic_helper.cleanup_metallic_instance_on_client(machine_details=hyperv_proxy_details)
