...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Selective Copy Acceptance Case for Hourly & First Full Configuration With SubClient " \
    ...
    def _run_backup(self, subclient_obj, jobType, return_list):
        ...
        try:
            ...
            self.log.error('Failed run a backup job for subclient %s:  %s', str(subclient_obj.subclient_id), str(exp))
            raise Exception("Basic backup jobs: [{0}]".format(str(exp)))
        ...
        while i < numOfJobsToRun:
            ...
            while True:
                ...
                if jobs_completed is False:
                    ...
                    # sometimes though we get a completed status, a new job submission gets an error
                    # of another job running. So giving JM 10 secs to clear out.
                    # todo: add this to other cases as well.
                    time.sleep(10)
                    break
            i += 1
    ...
    def create_new_subclient(self):
        ...
        subclient_name = str(self.id) + "_SC_" + str(n + 1)
        self.log.info("Adding a new subclient: %s", subclient_name)
        sc_obj = self.subclients.add(subclient_name, self.sp_name)
    ...
    def hourly_first_full_sc_disassoc(self):
        ...
        cmd = r"-sn QS_modifySubclientAssociationsForCopy -si 'e={5}' -si 'sp={0}' " \
              r"-si 'c={1}' -si 'cl={2}' -si 'a=Windows File System' -si 'b={3}' -si 's={4}'".format(
                  self.sp_name, copy1_name, self.client_machine.machine_name, self.backupset_name,
        ...
        # for existing subclients there could be jobs already run for the current period.
        # If so then we should wait until the next period begins
        ...
        # self.result_string += "SubClient Dis-Association For An Existing Sub-Client Case Passed"
        ...
        # self.result_string = "SubClient Dis-Association For A New Sub-Client Case Passed"

    def hourly_first_full_sc_reassoc(self):
        """ START OF BASIC CASE 4
            Steps:
        ...
        if not bPostRes:
            self.log.info("POST Failed")
            raise Exception("POST Command to re-associate sub-client failed")

        self.log.info(
            "Successfully re-associated subclient %s to selective copy : %s",
            sc4_obj.subclient_name,
            copy1_name)
        ...
        self.log.info("Successfully re-associated subclient %s to selective copy : %s",
                      sc3_obj.subclient_name,
                      copy1_name)
        ...
        self.log.info("Running jobs on re-associated sub-clients")
        # run fulls on sub-clients 3 & 4
        self.run_jobs('full', 2, -2)
        ...
        if bverdict:
            ...
            self.result_string += "SubClient Re-Association For An Existing Sub-Client Case Passed."
    ...
    def run(self):
        ...
        try:
            ...
            self.hourly_first_full_sc_reassoc()
