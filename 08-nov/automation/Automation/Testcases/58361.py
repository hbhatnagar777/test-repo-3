...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "VSA Nutanix AHV Clean up using NAS mode with Unix Proxy"
    ...
    def run(self):
        ...
        try:
            ...
            auto_subclient.validate_inputs("unix", "windows")

            try:
                VirtualServerUtils.decorative_log("Getting the proxy name used for the backup")
                proxy_name = auto_subclient.subclient.subclient_proxy
                self.log.info("VSA proxy machine name used for the backup job is %s", proxy_name[0])
            ...
            try:
                ...
                self.windows_machine = Machine(proxy_name[0], self.commcell)
            ...
            try:
                ...
                self.client_obj = self.commcell.clients.get(proxy_name[0])
                self.nfs_path = self.windows_machine.join_path(self.client_obj._job_results_directory, "NFSMountPoint")
                self.path_exists = self.windows_machine.check_directory_exists(self.nfs_path)
                self.log.info("NFS mount path on the proxy machine %s exits = %s", proxy_name[0], self.path_exists)
                self.paths = self.windows_machine.get_folders_in_path(self.nfs_path, False)
                self.paths.remove(self.nfs_path)
            ...
            try:
                ...
                folderlist = self.folder_list(self.paths)
                before_folder_list = len(folderlist)
                self.log.info("Total number of folder's before the backup in the NFS path is %d", before_folder_list)
                self.log.info("Before the backup folders list are ={}".format(folderlist))
            ...
            try:
                ...
                self.log.error("Failed to search the Saving Cleanup string in vsbkp.log")
                raise Exception

            try:
                VirtualServerUtils.decorative_log("Getting the folders list in NFS path")
                self.paths = self.windows_machine.get_folders_in_path(self.nfs_path, False)
                self.log.info("Getting the list of the folders in NFSMP mount path after the containers are mounted %s", self.paths)
                container = self.folder_list(self.paths)
                self.log.info("After mounting the containers list of the folders are {}".format(container))      
                folderlist = (len(self.paths))-1
                self.log.info("Total number of folders in NFS path is %d", folderlist)
                if folderlist >= before_folder_list:
            ...
            try:
                ...
                self.paths.remove(self.nfs_path)
                self.log.info("Total number of folders in NFS mount path after killing the backup job {}" .format(len(self.paths)))

                if len(self.paths) == 0:
                    self.log.info("NFS mount path is cleaned up successfully after killing backup job")
                else:
                    self.log.info("Cleanup of the container's did not happen and container are still mounted")
            except Exception as exp:
