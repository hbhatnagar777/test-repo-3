...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - Verify all IBMi advanced subclient options for SYNCLIB backup
        Step1, configure BackupSet and Subclient with SYNCLIB and one set of advanced options for TC
        Step2: On client, create a library TC58690 & TC586901.
        Step3: Run a full backup for the subclient.
        Step4: verify the full backup logs and validate all advanced options.
        Step5: Add few objects to both libraries and run a Inc backup for the subclient.
        Step6: verify the Inc backup logs and validate all advanced options.
        Step7: Add few objects to both libraries and run a Differential backup for the subclient.
        Step8: verify the Differential backup logs and validate all advanced options.
        Step9: re-create the SC with new values for additional options.
        Step10: Run a full backup for the subclient.
        Step11: verify the full backup logs and validate all advanced options.
        Step12: Add few objects to both libraries and run a Inc backup for the subclient.
        Step13: verify the Inc backup logs and validate all advanced options.
        Step14: Add few objects to both libraries and run a Differential backup for the subclient.
    ...
    def __init__(self):
        ...
        self.name = "IBMi - Verify all IBMi advanced subclient options for SYNCLIB backup"
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Step1, configure BackupSet and Subclient with advanced options for TC")
            ...
            srclib1 = "TC{0}".format(self.id)
            srclib2 = "TC{0}1".format(self.id)
            content = [self.client_machine.lib_to_path(srclib1), self.client_machine.lib_to_path(srclib2)]
            ...
            self.log.info("Step2: On client, create a library T57827.")
            self.client_machine.populate_lib_with_data(library_name=srclib1, tc_id=self.id, count=5, prefix="A")
            self.client_machine.populate_lib_with_data(library_name=srclib2, tc_id=self.id, count=5, prefix="B")
            ...
            self.client_machine.create_sourcepf(library=srclib1, object_name='INC11')
            self.client_machine.create_sourcepf(library=srclib2, object_name='INC12')
            ...
            self.client_machine.create_sourcepf(library=srclib1, object_name='DIFF11')
            self.client_machine.create_sourcepf(library=srclib2, object_name='DIFF12')
            ...
            self.log.info("Step9: re-create the SC with new values for additional options.")
            self.helper.create_subclient(name=self.subclient_name,
                                         storage_policy=self.storage_policy,
                                         content=content,
                                         scan_type=self.scan_type,
            ...
            self.client_machine.create_sourcepf(library=srclib1, object_name='INC21')
            self.client_machine.create_sourcepf(library=srclib2, object_name='INC22')
            job = self.helper.run_backup(backup_level="Incremental")[0]
            ...
            self.client_machine.create_sourcepf(library=srclib1, object_name='DIFF21')
            self.client_machine.create_sourcepf(library=srclib2, object_name='DIFF22')
            job = self.helper.run_backup(backup_level="Differential")[0]
            ...
            self.client_machine.manage_library(operation='delete', object_name=srclib1)
            self.client_machine.manage_library(operation='delete', object_name=srclib2)
