...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = " Push SP Upgrade of  windows and unix RCs with Akamai sync enabled"
        ...
        self.client_group = "SP Upgrade for RC"
        self.tcinputs = {
            'ServicePack': None
        }
    ...
    def install_client(self, q, bootstrapper_install=False):
        ...
        try:
            ...
            media_path = '' if bootstrapper_install else (self.tcinputs.get('MediaPath') if
                                                          self.tcinputs.get('MediaPath') else '')
            ...
            if '_' not in _service_pack:
                _service_pack_to_install = \
                    installer_utils.get_latest_recut_from_xml(_service_pack_to_install)
            ...
            if self.commcell.clients.has_client(client_machine.machine_name):
                ...
                raise Exception(
                    f"Client: {client_machine.machine_name} failed registering to the CS, "
            ...
            install_validation = InstallValidator(client_obj.client_name, self,
                                                  machine_object=client_machine,
    ...
    def run(self):
        ...
        try:
            ...
            if is_enabled:
                self.log.info(f"Akamai sync is successfully enabled on the client group"
                              f"  {self.client_group}")
            else:
