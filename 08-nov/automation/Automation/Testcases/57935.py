...
class TestCase(CVTestCase):
    """Power Management Aux Copy :: Primary Copy-Cloud and Aux Copy-non-cloud and DDB1-Non-cloud and DDB2-Cloud"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Power Management Aux Copy :: Primary Copy-Cloud and Aux Copy-non-cloud and DDB1-Non-cloud and DDB2-Cloud"
    ...
    def power_off_all_cloud_mediaagents(self):
        ...
        """
        self.power_management.power_off_media_agents([self.source_ma_obj, self.ddb_ma2_obj])

    def power_on_all_cloud_mediaagents(self):
        """
        Power-on all cloud MediaAgents
        """
        self.power_management.power_on_media_agents([self.source_ma_obj, self.ddb_ma2_obj])
    ...
    def verify_non_cloud_media_agents(self):
        ...
        if self.destination_ma_obj._is_power_management_enabled or self.ddb_ma1_obj._is_power_management_enabled:
    ...
    def run(self):
        try:
            ...
            self.verify_mediaagents_are_on([self.source_ma_obj, self.ddb_ma2_obj])
            ...
            self.power_management.validate_powermgmtjobtovmmap_table(self.job_obj.job_id,
                                                                     self.ddb_ma2_obj._media_agent_id, self.csdb)
            self.verify_mediaagents_are_on([self.ddb_ma2_obj,])
