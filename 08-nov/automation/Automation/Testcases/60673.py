...
class TestCase(CVTestCase):
    """ Command center: Validate Archive and restore functionality for new Hadoop client from command center """
    test_step = TestStep()

    def __init__(self):
        """ Initializing the reference variables """
        super(TestCase, self).__init__()
        self.name = "Testcase to Validate Archive and restore functionality for new Hadoop client from command center"
        self.browser = None
    ...
    def run(self):
        ...
        2) Add new Hadoop server.
        3) Run Archive job.
        4) Verify eligible files got deleted.
        5) Run another Archive Job to mark deleted in index.
        6) Restore the data with option restore deleted items selected and verify qualified data files are restored.
        7) Verify restored data files.
        8) Delete Server.
        9) Delete Archive plan.
        """
        try:
