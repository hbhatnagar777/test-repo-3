...
class TestCase(CVTestCase):
    """Class for executing this test case
        Testcase for Subclient Directory Cleanup - Extent Level Backups
    """
    ...
    def __init__(self):
        ...
        self.name = "Subclient Directory Cleanup - Extent Level Backups"
        self.tcinputs = {
            "TestPath": None,
    ...
    def setup(self):
        """Setup for automation -
            ...
            3. Creating regkeys CopySubclientJobResultsFolderAfterScan, mszFileExtentSlabs
        """
        ...
        self.log.info("Executing test case {} with Extent Slabs reduced.".format(self.id))
        self.log.info("2 MB will be backed up in extents and the extent size is 1 MB.")

        self.log.info("Add additional setting to reduce extent slabs.")
        ...
        self.log.info("Extent Slabs Modified.")
        ...
        try:
            for scan_type in ScanType:
                ...
                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_FULL_1",
                                         self.slash_format, scan_type, kwargs={"file_size": 4000})
                ...
                if not job_full.wait_for_completion():
                    ...
                    helper.validate_subclient_dir_cleanup(job_full, extent_level=True)
                ...
                if not job_inc.wait_for_completion():
                    ...
                    self.log.info("Vaidating Subclient Directory Cleanup...")
                    helper.validate_subclient_dir_cleanup(job_inc, extent_level=True)
