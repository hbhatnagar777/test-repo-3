...
from Web.AdminConsole.FSPages.RFsPages.RFs_Subclient_details import (
    SubclientOverview,
    Configuration,
)
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Block level React cases from command center"
        self.utils = None
        self.browser = None
        ...
        self.enable_blocklevel = True
        self.SUBCLIENT_NAME = "Test_57801"
    ...
    def setup(self):
        ...
        try:
            ...
            self.blk_test_path = "{0}{1}".format(
                self.test_path.rstrip(self.slash_format), self.slash_format
            )
    ...
    def run(self):
        ...
        try:
            ...
            self.define_content_and_filters('FULL')
            self.login()
            self.add_subclient()
            self.agent_details_subclient.access_subclient(self.SUBCLIENT_NAME)
            self.enable_blocklevel_option()
            self.backup_job(Backup.BackupType.FULL)
            self.file_level_restore()
            self.verify_restore()
            self.define_content_and_filters('INCR')
    ...
    def enable_blocklevel_option(self):
        self.log.info("**************Enabling blocklevel Option ******************")
        self.subclient_details_overview.enable_blocklevel(self.enable_blocklevel)
        self.log.info("********************************")


    def define_content_and_filters(self, job_level='FULL'):
        ...
        if job_level == 'FULL':
            ...
            self.incr_test_path= "{0}{1}{2}".format(
                    self.test_path.rstrip(self.slash_format), self.slash_format, res
                )
            self.fs_helper.generate_testdata([".html", ".css"], self.incr_test_path, 4)
    ...
    def login(self):
        ...
        self.subclient_details_subclient = Configuration(self.admin_console)
    ...
    def add_subclient(self):
        ...
        backup_data = self.blk_test_path
