...
5: Pick jobs for DV and Initiate Non-Scalable DV for each of Storage Policies
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = 'Marking Bad Chunks by Non-Scalable DV for DiskLibraries'
        self.tcinputs = {
            "MediaAgentName": None
        }
        self.utility = None
    ...
    def identify_chunks(self):
        ...
        for index in range(8):
            query = f'''select
                    ...
                    where
                        archFile.jobId = {self.backup_jobs[index].job_id}
    ...
    def run_verification_jobs(self):
        ...
        log_line = ''
        for index in range(len(self.backup_jobs)):
        ...
        for policy in self.storage_policies:
            job = policy.run_data_verification(use_scalable=False)
