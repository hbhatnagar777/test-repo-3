...
class TestCase(CVTestCase):
    """Class for performing failover and failback jobs with dvdf"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Azure - DR - Failover and Failback operation with DVDF enabled and validation of deletion of " \
                    "Destination VM"
    ...
    def run(self):
        ...
        # failover and failback with DVDF enabled
        try:
            ...
            # getting the first replication job after the RG creation
            time.sleep(30)
            job_manager = JobManager(commcell=self.commcell)
            active_jobs = job_manager.get_filtered_jobs(
            ...
            # performing planned failover operation
            self.replication_helper.do_planned_failover(self.auto_subclient.subclient_name + '_del_rg')

            # performing failback operation
            self.replication_helper.do_failback(self.auto_subclient.subclient_name + '_del_rg')

            # validating if the destination vm is deleted or not
            ...
            for vm_name in self.replication_helper.destination_vms:
                if vm_name not in self.tc_hvobj.VMs:
                    ...
                    if self.tc_hvobj.VMs[vm_name].vm_info:
                        raise Exception("Destination vm = %s is still present", vm_name)
