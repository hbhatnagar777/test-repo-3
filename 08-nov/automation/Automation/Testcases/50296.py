...
class TestCase(CVTestCase):
    """Class for executing
        IBMi- LFS wildcard exception content behavior check with library level and object level backups
    ...
    def __init__(self):
        ...
        self.name = "IBMi- LFS wildcard exception behavior check with library level and object level backups"
    ...
    def init_tc(self):
        ...
        self.usr_lib = []
        self.usr_obj = []
        self.content = [["/QSYS.LIB/L?{0}.LIB".format(self.id),
                         "/QSYS.LIB/LF{0}[2-4].LIB".format(self.id),
                         "/QSYS.LIB/LO1?.LIB".format(self.id),
        ...
        self.usr_lib = [["LD{0}".format(self.id), "LF{0}3".format(self.id), "LF{0}4".format(self.id),
                         ...
                         "LO1P", "LR{0}".format(self.id), "LH{0}".format(self.id)],
                        ["OD{0}".format(self.id), "OF{0}4".format(self.id), "OR{0}".format(self.id),
                         ...
                         "OO1P"],
                        ["FT{0}".format(self.id), "FT3{0}".format(self.id)]]

        self.usr_obj = [["DIR1", "DIR2"], ["DIR1", "DIR2"],
                        [["DIR1", "DIR2", "DIR3", "FUL1", "FUL2", "FUL3", "INC1", "INC2", "TESTAA", "TESTAB"],
                         ["DIR1", "DIR2", "DIR3"]]]
        self.validate = [["/QSYS.LIB/LD50296.LIB",
                          "/QSYS.LIB/LF502964.LIB",
                          "/QSYS.LIB/LO1P.LIB",
                          "/QSYS.LIB/LR50296.LIB",
                          "/QSYS.LIB/LT502963.LIB",
                          "/QSYS.LIB/LT502964.LIB"],
                         ["/QSYS.LIB/OD50296.LIB",
                          "/QSYS.LIB/OF502964.LIB",
                          "/QSYS.LIB/OO1P.LIB/DIR1.FILE",
                          "/QSYS.LIB/OR50296.LIB/DIR1.FILE",
                          "/QSYS.LIB/OT502963.LIB/DIR1.FILE",
                          "/QSYS.LIB/OT502964.LIB/DIR1.FILE"],
    ...
    def run(self):
        ...
        try:
            ...
            for each in [0, 1, 2]:
                ...
                self.log.info("***********************************************************")
                self.log.info("Starting validation of Wildcard exception for LFS with {0}".format(process))
                self.log.info("***********************************************************")
                ...
                self.cleanup(each)
            self.log.info("**IBMi: VALIDATION OF USING WILDCARD EXCEPTION CONTENT FOR LFS HAS COMPLETED SUCCESSFULLY**")
