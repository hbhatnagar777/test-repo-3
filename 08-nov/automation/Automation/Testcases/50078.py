...
class TestCase(CVTestCase):
    """Class for executing Basic Acceptance for NDMP - HNAS Advanced Backup Option - Backup External Links"""

    def __init__(self):
        """"Initializes TestCase object"""
        super(TestCase, self).__init__()
        self.name = "NDMP - HNAS Advanced Backup Option - Backup External Links"
        self.tcinputs = {
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Run full backup job for NDMP iDA with Backup External Links options set to ignore")
            job = self.subclient.backup("FULL", backup_external_links=1)

            self.log.info("Started full backup with Job ID: %s", job.job_id)
            full_job_id_a = str(job.job_id)
            ...
            if mamachine.get_logs_for_job_from_file(
                ...
                self.log.info('NDMP backup job is run with option Backup External Links set to ignore')
            else:
                self.log.info('NDMP backup job is NOT run with option Backup External Links set to ignore')
                raise Exception(
                    'NDMP backup job is NOT run with Backup External Links set to ignore. Failing the testcase')

            self.log.info("Run full backup job for NDMP iDA with Backup External Links options set to recreate_link")
            job = self.subclient.backup("FULL", backup_external_links=2)

            self.log.info("Started full backup with Job ID: %s", job.job_id)
            full_job_id_b = str(job.job_id)
            ...
            if mamachine.get_logs_for_job_from_file(
                ...
                self.log.info('NDMP backup job is run with option Backup External Links set to recreate_link')
            else:
                self.log.info('NDMP backup job is NOT run with option Backup External Links set to recreate_link')
                raise Exception(
                    'NDMP backup job is NOT run with Backup External Links set to recreate_link. Failing the testcase')

            self.log.info("Run full backup job for NDMP iDA with Backup External Links options set to remigrate")
            job = self.subclient.backup("FULL", backup_external_links=3)

            self.log.info("Started full backup with Job ID: %s", job.job_id)
            full_job_id_c = str(job.job_id)
            ...
            if mamachine.get_logs_for_job_from_file(
                ...
                self.log.info('NDMP backup job is run with option Backup External Links set to remigrate')
            else:
                self.log.info('NDMP backup job is NOT run with option Backup External Links set to remigrate')
                raise Exception(
                    'NDMP backup job is NOT run with Backup External Links set to remigrate. Failing the testcase')
            ...
            self.log.info("Deleting full backup jobs: %s %s %s", full_job_id_a, full_job_id_b, full_job_id_c)
            ...
            self.copy.delete_job(full_job_id_a)
            self.copy.delete_job(full_job_id_b)
            self.copy.delete_job(full_job_id_c)
