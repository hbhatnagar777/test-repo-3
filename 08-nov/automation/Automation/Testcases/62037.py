...
CONFIG = get_config().DynamicIndex.Activate.GoogleCloud


class TestCase(CVTestCase):
    """FSO basic acceptance test for GCP Cloud App"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "FSO basic acceptance test for Google Cloud App"
    ...
    def setup(self):
        ...
        try:
            ...
            self.tcinputs["key_file_path"] = CONFIG.KeyFilePath
            self.object_storage = ObjectStorageClient(self.admin_console, self.cloud_app_type, self.container_name,
                                                      CONFIG )
            self.google_helper = GoogleObjectStorage(self)
    ...
    def fetch_metadata_create_db(self):
        ...
        """
        meta_data = self.google_helper.fetch_file_metadata(self.container_name)
        if not meta_data:
    ...
    def run(self):
        ...
        try:
            ...
            self.activate_utils.run_data_generator(self.tcinputs["DataGeneratorDir"], self.cloud_app_type)
            # Get Container name in config from file written into by datagen util
            if 'BucketNameFilePath' in self.tcinputs.keys():
                with open(self.tcinputs['BucketNameFilePath']) as file:
                    self.container_name = file.read().strip()
                    self.log.info(f"Loaded the container_name: {self.container_name}")
                    self.object_storage.container_name = self.container_name
            ...
            try:
                ...
                self.error_dict[cs.FILE_OWNERSHIP_DASHBOARD] = str(exp)
                self.status = constants.FAILED
