...
class TestCase(CVTestCase):
    ...
    _BACKUPSET_NAME = 'Backupset_70776'
    def __init__(self):
        super(TestCase, self).__init__()
        self.client_name = None
        self.name = "File system Add/Remove exclusions/exceptions Folders"
        self.tcinputs = {
            "ClientName": None,
            "AgentName": None,
    ...
    def define_content_and_filters(self):
        ...
        self.log.info("Creating test data at %s on machine %s", self.exclusion_path, self.client_machine.machine_name)
        self.fs_helper.generate_testdata(['.mp3', '.mp4'], self.exclusion_path, 4)
        self.log.info("Creating test data at %s on machine %s", self.exception_path, self.client_machine.machine_name)
        self.fs_helper.generate_testdata(['.json', '.sh'], self.exception_path, 4)

        self.files_list = self.client_machine.get_files_in_path(self.test_path)
        self.exclusion_files_list = self.client_machine.get_files_in_path(self.exclusion_path)
        self.exception_files_list = self.client_machine.get_files_in_path(self.exception_path)
        self.exception_files = self.fs_helper.extract_filename(self.exception_files_list)
        self.exclusion_files = self.fs_helper.extract_filename(self.exclusion_files_list)
        self.log.info("Exclusion file list: %s", self.exclusion_files_list)
        self.log.info("Exception file list: %s", self.exception_files_list)
        self.log.info("exception_files list: %s", self.exception_files)
        self.log.info("exclusion_files list: %s", self.exclusion_files)
    ...
    def edit_content_and_filters(self):
        """ Define the subclient content, exclusions and exceptions """
        self.log.info("Edit content and filters")
        self.log.info("Creating test data at %s on machine %s", self.test_path, self.client_machine.machine_name)
        self.fs_helper.generate_testdata(['.pdf', '.doc'], self.test_path, 4)
        self.log.info("Creating test data at %s on machine %s", self.exclusion_path, self.client_machine.machine_name)
        self.fs_helper.generate_testdata(['.jpg', '.jpeg'], self.exclusion_path, 4)
        self.log.info("Creating test data at %s on machine %s", self.exception_path, self.client_machine.machine_name)
        self.fs_helper.generate_testdata(['.gzip', '.bzip'], self.exception_path, 4)
    ...
    def add_subclient(self):
        ...
        self.exclusions.append(self.exclusion_path)
        self.exceptions.append(self.exception_path)
    ...
    def verify_restore(self):
        ...
        self.fs_helper.validate_backup(dest_client=self.tcinputs.get('DestinationClientName'),
                                       ...
                                       add_exclusions=self.exclusion_files_list,
                                       exceptions_list=self.exception_files_list)
    ...
    def run(self):
        ...
        try:
            self.exclusion_path = self.client_machine.join_path(self.test_path, 'exclusion_folder')
            self.exception_path = self.client_machine.join_path(self.exclusion_path, 'exceptional_folder')
