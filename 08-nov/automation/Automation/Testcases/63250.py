...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Azure DevOps IDA Command Center - ACCT1 In Place Restore all services"
        self.browser = None
    ...
    def setup(self):
        ...
        try:
            ...
            self.tcinputs['repository_group'] = self.tcinputs.get('repository_group', 'auto_inplace')
    ...
    def download_data_and_backup(self):
        ...
        if repository_group is not None:
    ...
    def restore_and_validate_data(self):
        ...
        for iteration in range(0, 2):
            if iteration == 1:
                for project in content:
                    self.azhelper.cleanup(project, del_only_prj=True)
                services = ['Boards', 'Pipelines', 'Repos', 'Test Plans']
            else:
                services = ['Artifacts']
            rest = self.overview.restore_all()
            rest.restore_in_place(services=services)
            ...
            self.azhelper.validate_services_data({project: project for project in content})
        if iteration == 1:
            for project in content:
                feedname = f"feed{self.start_time.replace(' ','_').replace('-','_').replace(':','_')}"
                self.azhelper.connection.clients_v6_0.get_feed_client().create_feed({"name": feedname}, project=project)
