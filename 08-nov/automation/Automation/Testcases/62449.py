...
class TestCase(CVTestCase):
    """Class for executing Azure On demand access node - Create a Linux Access node using Command Center"""""

    def __init__(self):
        """" Initializes test case class objects"""""
        super(TestCase, self).__init__()
        self.name = "Azure On demand access node - Create a Linux Access node using Command Center"
    ...
    def setup(self):
        ...
        try:
            ...
            self.auto_scale_node_config.node_os = self.tcinputs.get("ProxyOperatingSystem", 'Unix')
    ...
    def deploy_vm(self, image_version):
        ...
        try:
            ...
            try:
                self.log.info(f"Deploying VM {vm_props['vm_name']} from image "
                              f"{self.auto_scale_node_config.gallery_image}")
    ...
    def create_managed_image_from_gallery(self, vm_obj):
        try:
        ...
        try:
            ...
            for entry_point in self.entry_points:
                ...
                self.vsa_obj.create_access_node(prov_hypervisor=self.prov_hypervisor,
                                                proxy_name=proxy_name, proxy_os=self.auto_scale_node_config.node_os, entry_point=entry_point)
            ...
            try:
                ...
                self.log.warning("Testcase and/or Restored VM cleanup was not completed: %s", err)
                pass
