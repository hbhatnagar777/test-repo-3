...
# license information.
# --------------------------------------------------------------------------
""" Verify Email Now Feature with compressed attachment"""
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Verify Email Now Feature with compressed attachment"
    ...
    def verify_email_attachment(self, file_type):
        ...
        for each_email in self.email_details:
            ...
            files = self.utils.get_attachment_files(ends_with='zip')

            if len(files) != 1:
                raise CVTestStepFailure(f"Expected 1 attached zip file, found {len(files)}")
            ...
            if len(filelist) != 1:
                raise CVTestStepFailure(f"Expected 1 file in compressed attachment, found {len(files)}")
            
            if not filelist[0].endswith(file_type.lower()):
                raise CVTestStepFailure(f"Expected file in compressed attachment to have extension {file_type.lower()}, found {filelist[0]}")
    ...
    def email_now(self, email_format):
        ...
        for each_report in self.reports:
            ...
            email_window.email_now(email_format, self.recipient_id, compress=True)
