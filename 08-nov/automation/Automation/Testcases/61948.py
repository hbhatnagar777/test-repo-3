...
class TestCase(CVTestCase):
    """Class to execute clone restore for PostgreSQL block level from command center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Clone restore for PostgreSQL block level from command center"
        self.browser = None
    ...
    def run(self):
        ...
        try:
            ...
            new_expiry_set = self.instant_clone.select_clone_extend(
                clone_jobid, {"days": 1})
            self.instant_clone.verify_expiry_time(clone_jobid, new_expiry_set)
