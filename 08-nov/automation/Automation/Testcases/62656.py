...
TestCase:
    ...
    review_move_action_filesystem()              -- Moves files from a FS datasource
...
class TestCase(CVTestCase):
    """Class for executing move action with review request for all files with local path"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Move action with review request for all files with local path in SDG"
        self.tcinputs = {
    ...
    def validate_move_action(self):
        ...
        for file in source_files:
            if any(file in rfile for rfile in self.reviewed_files):
        ...
        self.log.info(
            f"Files present in the destination path {destination_filenames}")
        if destination_filenames.sort() != self.reviewed_files.sort():
            raise CVTestStepFailure(
                f"Files in the destination don't match with the files reviewed.")

        count_after_move = len(destination_path_files)
        # Check that the destination path's file count is increased
        if count_after_move == count_reviewed_files and (not source_file_present):
            self.log.info(
                f"Moved all the {count_reviewed_files} files to {destination_path}")
        else:
            raise CVTestStepFailure(
                f"Move action was taken on {count_reviewed_files} but destination has {count_after_move} files")
    ...
    def review_move_action_filesystem(self):
        ...
        try:
            ...
            status = self.gdpr_base.data_source_review_obj.review_move_action(
                '', destination_path, username, password, all_items_in_page=True, review_request=True, request_name=self.request_name, reviewer=reviewer,
                approver=approver)
        ...
        except (CVTestStepFailure, CVWebAutomationException) as error_status:
    ...
    def review_move_action_full_crawl_filesystem(self):
        ...
        self.log.info(
            f"Number of files chosen for review action {count_reviewed_files}"
        ...
        if count_reviewed_files != self.count_before_review - count_after_review:
