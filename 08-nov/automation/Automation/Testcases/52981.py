...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Point in time Restore testcase for Informix iDA"
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Incremental backup end time: %s", job.end_time)
            self.log.info("Stopping informix server to perform restore")
            informix_helper_object.stop_informix_server()

            ####################### Running restore ###########################
            self.log.info("***************Starting POINT IN TIME restore Job*****************")
            job = self.instance.restore_in_place(
                db_space_list,
                restore_option_type="POINT_IN_TIME",
                to_time=job.end_time)

            self.log.info(
                "started the point in time restore Job with Id:%s",
                job.job_id)
            ...
            self.log.info("Point in time Restore job is now completed")
            ...
            self.log.info(
                "started the whole system point in time restore Job with Id:%s",
                job.job_id)
            if not job.wait_for_completion():
                raise Exception(
                    "Failed to run data only restore job with error: {1}".format(
            ...
            self.log.info("Whole System point in time Restore job is now completed")
