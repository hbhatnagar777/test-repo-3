...
class TestCase(CVTestCase):
    """ Class for executing RMAN Duplicate Restore for Oracle RAC to Oracle Single Node"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Oracle RAC to Single Node Restore on Command Center - RMAN Duplicate Restore"
        self.browser = None
        self.admin_console = None
        self.navigator = None
        self.tablespace_name = 'CV_71025'
    ...
    def run_restore(self, job_id):
        ...
        job_id = restore_panel.out_of_place_restore(
            self.tcinputs["DestRacClusterName"], self.tcinputs["DestRacInstanceName"], redirect_all_path=self.tcinputs.get("RedirectAllPath", None), recover_to="most recent backup", rman_duplicate=True)
        self.dbhelper.wait_for_job_completion(job_id)
    ...
    def run(self):
        ...
        try:
            ...
            self.run_restore(job_id)

            self.validate_restore(tablespaces)
