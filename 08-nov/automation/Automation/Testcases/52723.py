...
class TestCase(CVTestCase):
    """ Class for executing ACCT1 - SAP HANA 2.0 MDC IntelliSnap support feature """
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "ACCT1 - SAP HANA 2.0 MDC IntelliSnap support feature"
        self.browser = None
    ...
    @test_step
    def backup_operations(self, tenant_connection=False, backupset_name="SYSTEMDB", data_only=False, snap=False):
        """
    ...
    @test_step
    def restore_operations(self, backup_map, tenant_connection=False, backupset_name="SYSTEMDB", type=None):
        """
        ...
        match type:
            case "snap":
                self.run_restore(advanced_options={"hardwareRevert": True})
            case "tape":
                self.run_restore(advanced_options={"copyPrecedence": "Primary", "hardwareRevert": True})
            case "data only":
                self.run_restore(backup_prefix=self.backup_prefix, advanced_options={"hardwareRevert": True})
            case "pit":
                self.run_restore(point_in_time=self.point_in_time, advanced_options={"InitializeLogArea": True, "hardwareRevert": True})
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("#### Streaming Backup for System DB ####")
            self.backup_operations()
            self.log.info("#### Snap Backup ####")
            snap_bkp_metadata_1 = self.backup_operations(tenant_connection=True, backupset_name=self.tcinputs['InstanceName'], data_only=True, snap=True)
            ...
            snap_bkp_metadata_2 = self.backup_operations(tenant_connection=True, backupset_name=self.tcinputs['InstanceName'], snap=True)
            self.log.info("#### Backup Copy ####")
            self.backup_copy()
            self.log.info("#### Snap Most Recent Restore using Revert ###")
            self.restore_operations(snap_bkp_metadata_2, True, self.tcinputs['InstanceName'], type="snap")
            self.log.info("#### Backup Copy Most Recent Restore using Revert ###")
            self.restore_operations(snap_bkp_metadata_2, True, self.tcinputs['InstanceName'], type="tape")
            self.log.info("#### Point in Time Restore using Revert ###")
            self.restore_operations(snap_bkp_metadata_1, True, self.tcinputs['InstanceName'], type="pit")
            self.log.info("#### Data Only Restore using Revert ###")
