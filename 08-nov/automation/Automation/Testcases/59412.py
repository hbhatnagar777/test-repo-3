...
class TestCase(CVTestCase):
    """Class to verify Subclient retention - Storage policy Copy with the longest retention retains the job."""
    ...
    def __init__(self):
        ...
        self.name = "Subclient retention - Storage policy Copy with the longest retention retains the job."
    ...
    def setup(self):
        ...
        self.OPHelper.testcase.subclient.archiver_retention_days = 30
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Changing Job time to prior to 20 days.")
            for job in job_list:
                orig_start = job.start_timestamp
                orig_end = job.end_timestamp
                mod_start = int(orig_start) - (20 * 24 * 60 * 60)
                mod_end = int(orig_end) - (20 * 24 * 60 * 60)
            ...
            for job in job_list:
                ...
                if not self.MAHelper.validate_job_prune(copy_id=self.copy1.copy_id, job_id=job.job_id):
                    raise Exception(
                        "First Secondary copy job is not pruned. It is expected to be pruned according to retention rules.")
            ...
            self.log.info('Subclient retention - Storage policy Copy with the longest retention retains the job passed.')

        except Exception as exp:
            self.log.error('Subclient retention - Storage policy Copy with the longest retention retains the job failed with error: %s',
                           exp)
