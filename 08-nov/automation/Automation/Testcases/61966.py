...
TestCase:
    ...
    create_destination_cluster()        --  Create second cluster to perform cross cluster restore

    add_data_verify_backup()            --  Add data and verify backup job

    verify_cross_cluster_restore_step() --  Verify cross cluster restore job
...
class TestCase(CVTestCase):
    """
    Testcase to create Kubernetes Cluster, perform backup and cross cluster restore of applications.
    ...
    9. Add content for Incremental  validation.
    10. Initiate Incremental Backup for App group created and verify job completed
    11. Initiate out-of-place cross cluster Full Application Restore and verify job completed
    12. Add content for Incremental  validation.
    13. Initiate Incremental Backup for App group created and verify job completed
    14. Initiate out-of-place cross cluster Full Application Restore and verify job completed
    15. Cleanup testbed
    ...
    def __init__(self):
        ...
        self.name = "Kubernetes - Cross Cluster Full Application Restore"
        ...
        self.k8s_config_2 = None
        self.driver = None
        self.plan = None
        self.storageclass = None
        self.storageclass_2 = None
        self.kubehelper = None
        self.kubehelper_2 = None
        self.content = []
    ...
    def init_inputs(self):
        ...
        self.k8s_config_2 = self.tcinputs['ConfigFile2']
        ...
        self.kubehelper_2 = KubernetesHelper(self)

        # Initializing objects using KubernetesHelper for source cluster
        ...
        # Initializing objects using KubernetesHelper for destination cluster
        self.kubehelper_2.load_kubeconfig_file(self.k8s_config_2)
        self.storageclass_2 = self.tcinputs.get(
            'StorageClass', self.kubehelper_2.get_default_storage_class_from_cluster()
        )
        self.api_server_endpoint_2 = self.kubehelper_2.get_api_server_endpoint()
    ...
    def delete_testbed(self):
        ...
        self.kubehelper_2.delete_cv_namespace(self.namespace)
        self.kubehelper_2.delete_cv_namespace(self.restore_namespace)

        crb_name = self.testbed_name + '-crb'
        # Delete cluster role binding
        self.kubehelper.delete_cv_clusterrolebinding(crb_name)
        self.kubehelper_2.delete_cv_clusterrolebinding(crb_name)

        # Delete service account
        sa_namespace = self.tcinputs.get("ServiceAccountNamespace", "default")
        self.kubehelper.delete_cv_serviceaccount(sa_name=self.serviceaccount, sa_namespace=sa_namespace)
        self.kubehelper_2.delete_cv_serviceaccount(sa_name=self.serviceaccount, sa_namespace=sa_namespace)

        KubernetesUtils.delete_cluster(self, self.clientName)
        KubernetesUtils.delete_cluster(self, self.destclientName)

    @TestStep()
    def create_destination_cluster(self):
        """
        Create second cluster for destination
        """

        sa_namespace = self.tcinputs.get("ServiceAccountNamespace", "default")
        self.kubehelper_2.create_cv_serviceaccount(self.serviceaccount, sa_namespace)

        # Create cluster role binding
        crb_name = self.testbed_name + '-crb'
        cluster_role = self.tcinputs.get("ClusterRole", "cluster-admin")
        self.kubehelper_2.create_cv_clusterrolebinding(crb_name, self.serviceaccount, sa_namespace, cluster_role)

        time.sleep(30)
        self.servicetoken_2 = self.kubehelper_2.get_serviceaccount_token(self.serviceaccount, sa_namespace)
    ...
    @TestStep()
    def verify_cross_cluster_restore_step(self):
        """Verify Cross Cluster Restore Step
        """
        VirtualServerUtils.decorative_log("Verify Full Application Out of place Cross cluster restore")

        source_checksum = self.kubehelper.get_files_checksum(self.namespace)
        self.kubehelper.restore_out_of_place(self.destclientName, self.restore_namespace, self.storageclass_2)
        dest_checksum = self.kubehelper_2.get_files_checksum(self.restore_namespace)
    ...
    def run(self):
        ...
        try:
            ...
            # Create instance for the destination cluster
            self.create_destination_cluster()

            # Validate cross cluster restore
            self.verify_cross_cluster_restore_step()
            ...
            after_restore = self.kubehelper_2.get_all_resources(self.restore_namespace)
            self.kubehelper.validate_data(before_backup, after_restore)

            before_backup = self.kubehelper.get_all_resources(self.namespace)
            self.add_data_verify_backup("INCREMENTAL")

            # Validate cross cluster restore a second time
            self.verify_cross_cluster_restore_step()
