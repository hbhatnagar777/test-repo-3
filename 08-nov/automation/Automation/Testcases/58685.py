...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Openstack Ram quota validation"
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.log.info('Got ram quota limits for the project'+str(project_name))
            except Exception as err:
                self.log.error('Failed to get ram quota limits for the project'+str(project_name))
                raise Exception
            self.log.info('-----setting ram quota limits for a project-----')
            try:
                for each_vm in self.subclient._vmContent['children']:
                    self.hvobj.OpenStackHandler.set_quota_ram(project_name, each_vm['displayName'])
                    self.log.info(
                        'set ram quota limits for the project successfully'+str(project_name))
            except Exception as err:
                self.log.error('Failed to set ram quota limits for the project'+str(project_name))
                raise Exception
            ...
            failurereason = 'Could not create VM, Quota exceeded for ram'
            if output != ():
                if failurereason not in output[0][0]:
                    self.log.error('---Failed reason not updated on Database--')
                    raise Exception
            self.log.info(
                '--Failed reason updated on the Database')
            self.log.info(
                '---Reseting back quota limits for a project to original limits----')
            try:
                self.hvobj.OpenStackHandler.reset_quota_limits(project_name)
                self.log.info(
                    'Reseting ram quota limits for the project successfully'+str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to reset ram quota limits for the project'+str(project_name))
                raise Exception
            self.log.info(
            ...
            try:
                ...
                self.log.error(
                    'source VMs not on Openstack server')
                raise Exception
            self.log.info(
