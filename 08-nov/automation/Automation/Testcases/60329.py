...
TestCase:
    ...
    verify_MMLibraryController_table()  --  Verifies the MMLibraryContoller table
...
Library and Drive controller, both should be same MA.
...
class TestCase(CVTestCase):
    ...
    def setup(self):
        ...
        for MA in self.MediaAgents:
            ...
            #print(self.MediaAgents_obj[MA].media_agent_name)
    ...
    def verify_MMLibraryController_table(self, controller_id, after_delete=True):
        ...
        query = "select count(*) from mmlibrarycontroller where LibraryId = {0} and clientid= {1}".format(self.tape_lib_obj._library_id, controller_id)
        self.log.info(query)
            ...
            if not library_delete_tried:
                if job_details["jobDetail"]["progressInfo"]["state"] =="Running" and job_details["jobDetail"]["progressInfo"]["currentPhase"] == "Backup":
                    ...
                    if not flag:
                        ...
                        # Increasing the sleep time to 30 sec as we don't need to check every 5 sec
                        sleep_time = 30
                    else:
    ...
    def configure_tape_library(self):
        ...
        ma_list_to_pass=[]
        for MA in self.MediaAgents:
            #ma_list_to_pass.insert(len(ma_list_to_pass),int(self.MediaAgents_obj[MA].media_agent_id))
    ...
    def run(self):
        ...
        try:
            ...
            self.drive_pools = self.ma_helper.get_drive_pool(self.csdb, self.tape_lib_obj.library_name)
            self.master_pools = self.ma_helper.get_master_pool(self.csdb, self.tape_lib_obj.library_name)
            ...
            for ma_obj in self.MediaAgents_obj.values():
                self.verify_MMLibraryController_table(ma_obj.media_agent_id, False)
            ...
            for ma_obj in self.MediaAgents_obj.values():
                self.verify_MMLibraryController_table(ma_obj.media_agent_id)
