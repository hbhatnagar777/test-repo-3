...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Openstack compute quota validation"
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.log.info('Got compute quota limits for the project'+str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to get compute quota limits for the project'+str(project_name))
                raise Exception
            self.log.info('-----setting compute quota limits for a project-----')
            try:
                for each_vm in self.subclient._vmContent['children']:
                    self.hvobj.OpenStackHandler.set_quota_instance(project_name, each_vm['displayName'])
                self.log.info(
                    'set instance quota limits for the project successfully'+str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to set instance quota limits for the project'+str(project_name))
                raise Exception
            ...
            failurereason = 'Could not create VM, Quota exceeded for instances'
            if output != ():
