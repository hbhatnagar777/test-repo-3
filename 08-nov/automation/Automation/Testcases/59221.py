...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Daily Last Full Wait Until Configuration"
        self.tcinputs = {
            "MediaAgentName": None
        }
    ...
    def get_completed_jobs_list(self, appid):
        """ get completed jobs list for given sub-client"""
        # storage_policy_id
        jobs_list = []
        get_jobs_query = """
                        select appId, jobId
    ...
    def run_bkp_jobs(self, copy_obj, rel_sched_st_time_min, repeat_every_min, copy_day_starts_min):
        """ Set up 15 min run schedule for 2 subclients and run 3rd sub-client job manually."""
        ...
        # Wait till we hit copy's day start time.
        ...
        self.log.info("rel_sched_st_time_min: %d repeat_every_min: %d td.seconds: %d "
                      "copy_day_starts_min %d ",
                      rel_sched_st_time_min, repeat_every_min, td.seconds, copy_day_starts_min)

        # assuming we will never cross date here.
        if copy_day_starts_min > (today_now.hour * 60 + today_now.minute):
            self.log.info("Will sleep for %d sec till the copy's day start time starts.",
                          (copy_day_starts_min - (today_now.hour * 60 + today_now.minute))*60)
            time.sleep((copy_day_starts_min - (today_now.hour * 60 + today_now.minute))*60)

        self.log.info("We just hit copy's day-starts-on time. Since this is a last full wait "
                      "until, we will wait for another 60 min and by that time jobs from this time"
                      "period should have been picked up.")
        time.sleep(60*60)
        ...
        #         self.log.info("Found all jobs to have completed. Time %d min from copy's day start.",
        #                       ((tn.hour * 60) + tn.minute) - copy_day_starts_min)
        #         break
        #     if nt_slept > 60:
        #         self.log.info("We waited for 60 min since copy's day start time started. "
        #                       "we should have picked the jobs on copy bu this time")
        #         break
        #     self.log.info("Will sleep for 120 seconds for jobs to get picked. "
        #                   "Total time slept: %d min", nt_slept)
        #     nt_slept += 2
        #     time.sleep(120)
    ...
    def run_jobs(self, jobType, numOfJobsToRun, numOfsubClientsToRunOn=0):
        """Run jobs on subclients from sc_list """
        running_job_list = []
        startSubclient = 0
    ...
    def check_job_status_on_copy(
            ...
            isupdatecopy=None,
            updated_time=None):
        # Check what jobs are picked on copy
        """ todo :
            1. factor-in picked status for all fileType/dataType (archiveFile). Add this portion.
            2. factor-in sub-client association
        """
        updateQuery = ""
        ...
        # Here we are checking for jobs that have run during the last period. i.e minute before
        # copy's day start on.
        ...
        sqlquery = str("""
                        ...
                        (case when convert(date,Q.jobSelTime) = '1970-01-01' and Q.jobPickedState = 0 and Q.QualifiesByTime = 1 and Q.QualifiesByFreq = 1 and Q.jobRankByHr = 1
                                then DATEDIFF(hour, Q.jobRunTime, GETDATE())
            ...
            for row in resultset:
                ...
                if int(row[7]) == 1:
                    ...
                    if int(row[5]) == 1 and int(row[6]) == 1:
                        ...
                        if int(row[8]) > 24:  # daily full freq, so more than 24 hours.
    ...
    def update_job_run_times(self, days):
        # update the job run times of each sub-client. cannot use self.csdb.execute() as this
        # only has read-only access. so need to cretae MSSQL object to update run times.
        ...
        if not mssql_obj:
            self.log.info("Failed to get connection to CSDB")
            return
        # spread_days = 3
    ...
    def daily_last_full_wait_unitl_basic(self):
        """START OF BASIC CASE
        Steps:
        ...
        sp_obj.create_selective_copy(copy_name_lf1, self.disk_library_dest,
                                     self.tcinputs['MediaAgentName'], 'daily', 'LastFullWait',
        ...
        if bverdict:
            self.log.info("Daily Last full wait until selection passed.")
            self.result_string = "Daily Last full wait until passed. <br>"
        else:
        ...
        self.log.info("Checking picked job status on copy %s",
                      copy_name_lf1)

        bverdict, verdictstr, query_results = self.check_job_status_on_copy(copy_obj_lf)
        if bverdict:
            self.log.info("Daily Last full wait until selection for newly run jobs passed.")
            self.result_string += "Daily Last full until selection for newly run jobs passed.<br>"
        else:
            raise Exception("Daily Last Full until selection for newly run jobs. Unexpected jobs:[{0}]"
                            .format(verdictstr))
    ...
    def run(self):
        ...
        try:
            ...
            self.daily_last_full_wait_unitl_basic()
        except Exception as exp:
            self.log.error('Failed to execute test case with error: %s', exp)
