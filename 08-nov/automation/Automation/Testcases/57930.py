...
class TestCase(CVTestCase):
    """Power Management Aux Copy :: Primary Copy-Non-Cloud and Aux Copy-Non-cloud and DDB1-cloud and DDB2-Cloud"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Power Management Aux Copy :: Primary Copy-Non-Cloud and Aux Copy-Non-cloud and DDB1-cloud and DDB2-Cloud"
    ...
    def setup(self):
        ...
        self.ddb_ma2 = self.tcinputs['CloudMA2']
        ...
        if len(set(list_of_ma)) != 5:
            raise Exception('Each MediaAgent and client have to be unique machine. Make sure you have provided 4 unique MediaAgents and one client.')
        ...
        if self._commcell.clients.has_client(self.tcinputs['PsudoClientName']):
            ...
            self.log.info(f"Pseudo client(cloud controller) is not present with name [{self.tcinputs['PsudoClientName']}]")
        ...
        self.power_management.configure_cloud_mediaagent(self.tcinputs['PsudoClientName'], self.ddb_ma2_obj)
    ...
    def power_off_all_cloud_mediaagents(self):
        ...
        """
        self.power_management.power_off_media_agents([self.ddb_ma1_obj, self.ddb_ma2_obj])

    def power_on_all_cloud_mediaagents(self):
        """
        Power-on all cloud MediaAgents
        """
        self.power_management.power_on_media_agents([self.ddb_ma1_obj, self.ddb_ma2_obj])
    ...
    def create_multi_partitioned_pool(self, pool_name):
        ...
        pool_obj = self.commcell.storage_pools.add(pool_name, self.destination_library_path, self.destination_ma_obj,self.ddb_ma1_obj, self.ddb1_path)
    ...
    def verify_non_cloud_media_agents(self):
        ...
        if self.source_ma_obj._is_power_management_enabled or self.destination_ma_obj._is_power_management_enabled:
    ...
    def run(self):
        try:
            ...
            self.pool_obj = self.commcell.storage_pools.add(self.storage_pool, self.source_library_path,self.source_ma_obj, self.source_ma_obj,self.source_ddb_path)
            self.log.info("Storage pool created successfully")

            # Creating SP
            self.log.info(f"Creating storage policy[{self.storage_policy}]")
            self.sp_obj = self.commcell.storage_policies.add(self.storage_policy, global_policy_name = self.storage_pool, global_dedup_policy  = False)
            ...
            self.power_management.validate_powermgmtjobtovmmap_table(self.job_obj.job_id,self.ddb_ma2_obj._media_agent_id, self.csdb)
            self.verify_mediaagents_are_on([self.ddb_ma1_obj,self.ddb_ma2_obj])
            ...
            self.power_management.validate_powermgmtjobtovmmap_table(self.job_obj.job_id,
                                                                     self.ddb_ma2_obj._media_agent_id, self.csdb)
            self.verify_mediaagents_are_on([self.ddb_ma1_obj, self.ddb_ma2_obj])
