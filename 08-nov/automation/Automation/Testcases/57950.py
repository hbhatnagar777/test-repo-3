...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Test Case for expansion of pool failure and reconfigure it, updating name"
    ...
    def run(self):
        try:
            ...
            all_ma_ids = {}
            for ma in self.control_nodes:
                all_ma_ids[self.control_nodes[ma]] = self.hyperscale_helper.get_host_id(self.control_nodes[ma])
            ...
            status, response = self.hyperscale_helper.add_nodes(self.storage_pool_name, self.ma4, self.ma5, self.ma6)
            ...
            if resolution_status is True:
                while status is False and reconfigure_try != 0:
                    ...
                    if status is False:
                        self.log.error("Storage pool %s not reconfigured, waiting to reconfigure",
                                       self.storage_pool_name)
                    reconfigure_try -= 1
