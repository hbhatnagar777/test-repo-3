...
class TestCase(CVTestCase):
    """Class for executing Basic Acceptance for NDMP Advanced NAS Restore option overwrite ON - \
    Dell EMC Celerra/VNX, Dell EMC Unity, Hitachi NAS"""

    def __init__(self):
        """"Initializes TestCase object"""
        super(TestCase, self).__init__()
        self.name = "NDMP Advanced NAS Restore option overwrite ON - Dell EMC Celerra/VNX, Dell EMC Unity, Hitachi NAS"
        self.tcinputs = {
    ...
    def run(self):
        ...
        try:
            ...
            # Run restore out-of-place with overwrite ON
            ...
            if mamachine.get_logs_for_job_from_file(job.job_id, "nasRestore.log", r"ENV \[OVERWRITE\]=\[Y\]"):
                self.log.info('NDMP restore job is run with overwrite ON')
            else:
                self.log.info('NDMP restore job is NOT run with overwrite ON')
                raise Exception('NDMP restore job is NOT run with overwrite ON. Failing the testcase')
            ...
            # Run restore in-place with overwrite ON
