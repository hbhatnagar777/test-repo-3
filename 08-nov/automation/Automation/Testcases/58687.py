...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Openstack compute boundary quota validation"
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.log.info('Got quota limits for the project'+str(project_name))
            except Exception as err:
                self.log.error('Failed to get quota limits for the project'+str(project_name))
                raise Exception
            self.log.info('-----setting compute quota limits for a project-----')
            try:
                for each_vm in self.subclient._vmContent['children']:
                    self.hvobj.OpenStackHandler.set_boundary_quota_compute(
                        projectname, each_vm['displayName'])
                    self.log.info(
                        'set compute quota limits for the project successfully'+str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to set compute quota limits for the project'+str(project_name))
                raise Exception
            self.log.info(
                '-----Getting compute quota limits for a project after setting quota before backup-----')
            try:
                quotalimit3 = self.hvobj.OpenStackHandler.get_quota_limits(project_name)
                self.log.info(
                    'Got volume limits for the project'+str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to get volume limits for the project'+str(project_name))
                raise Exception
            self.log.info("-" * 25 + " Backup " + "-" * 25)
            backup_options = OptionsHelper.BackupOptions(auto_subclient)
            backup_options.backup_type = "INCREMENTAL"
            auto_subclient.backup(backup_options)
            self.log.info('-----Getting compute quota limits for a project after backup-----')
            try:
                quotalimit4 = self.hvobj.OpenStackHandler.get_quota_limits(project_name)
                self.log.info(
                    'Got volume limits for the project'+str(project_name))
            except Exception as err:
                self.log.error(
                ...
                raise Exception
            self.log.info(
                'validating if set quota limits are intact before and after the backup job')
            try:
                if quotalimit3 == quotalimit4:
                    self.log.info(
                        'set quota limits are intact before and after the backup job'+str(project_name))
            except Exception as err:
                self.log.error(
                    '---set quota limits are not intact before and after the backup job---'+str(project_name))
                raise Exception
            try:
                self.log.info(
            ...
            self.log.info(
                '-----Getting compute quota limits for a project after setting quota before backup-----')
            try:
                quotalimit5 = self.hvobj.OpenStackHandler.get_quota_limits(project_name)
                self.log.info(
                    'Got volume limits for the project'+str(project_name))
            except Exception as err:
                self.log.error(
                ...
                raise Exception
            self.log.info(
                'validating if set quota limits are intact before and after the restore job')
            try:
                if quotalimit4 == quotalimit5:
                    self.log.info(
                        'set quota limits are intact before and after the restore job'+str(project_name))
            except Exception as err:
                self.log.error(
                    '---set quota limits are not intact before and after the restore job---'+str(project_name))
                raise Exception
            self.log.info(
            ...
            try:
                ...
                raise Exception
            self.log.info(
                '---Reseting back quota limits for a project to original limits----')
            try:
                self.hvobj.OpenStackHandler.reset_quota_limits(project_name)
                self.log.info(
                    'Reseting compute quota limits for the project successfully'+str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to reset compute quota limits for the project'+str(project_name))
                raise Exception
            ...
            try:
                ...
                raise Exception
            self.log.info('---validating if quota limits are set to original limits----')
            try:
