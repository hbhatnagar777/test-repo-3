...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = 'Testcase for Checking that Alert Disable functionality works as expected'
        tcinputs = {
          "AgentName":None,
          "BackupsetName":None,
          "BackupType":None,
    ...
    def run(self):
        ...
        try:
            ...
            alert_data = alert_helper.get_alert_details(name='Alert Disable functionality works as expected',
            ...
            # Get alert corresponding to the created alert
            alert_obj = alert_helper.alerts_commcell.get(alert_data.get("alert_name"))

            # Disable the alert
            self.log.info('Disabling Alert %s', alert_data.get("alert_name"))
            alert_obj.disable()
            ...
            try:
                # Check if alert mail was received this time or not
                alert_helper.check_if_alert_mail_received(short_interval=40,
                                                          patterns=[self.cache.get("job_id"),
                                                                    self.tcinputs["BackupsetName"],
                                                                    self.tcinputs["SubclientName"]])
            except Exception as mail_not_received:
                self.log.info(str(mail_not_received))
                self.log.info('Alert Disable Functionality working as expected')
