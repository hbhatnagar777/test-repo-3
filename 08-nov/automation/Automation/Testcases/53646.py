...
class TestCase(CVTestCase):
    """Class for executing basic network case to validate different types of tunnel connection
        protocols for outgoing routes with firewall configuration via proxy

        Setup requirements to run this test case:
        4 clients -- can be any client in the commcell
        1 unix client
    ...
    def __init__(self):
        ...
        self.name = ("[Network & Firewall] : Tunnel connection protocol "
                     ...
                     "with firewall configuration via proxy")
        ...
        self._client_group_name = 'CG_53646'
        self.commserv = None
        self._network = None
        ...
        try:
            ...
            self._network.set_via_proxy({'entity': self.commserv,
                                        ...
                                        {'entity': self.tcinputs['FirewallClient1'],
                                         'isClient': True},
                                        {'entity': self.tcinputs['ProxyClient'],
                                         'isClient': True})
            ...
            self._network.outgoing_route_settings({'clientName': self.commserv},
                                                  is_client=True,
                                                  remote_entity=self.tcinputs['ProxyClient'],
            ...
            self._network.set_via_proxy({'entity': self.commserv,
                                        ...
                                        {'entity': self.tcinputs['FirewallClient1'],
                                         'isClient': True},
                                        {'entity': self.tcinputs['UnixProxy'],
                                         'isClient': True})
            ...
            self._network.outgoing_route_settings({'clientName': self.commserv},
                                                  is_client=True,
                                                  remote_entity=self.tcinputs['UnixProxy'],
                                                  connection_protocol=0,
                                                  streams=4)
            ...
            proxy_port = self._network.client_tunnel_port(self.tcinputs['ProxyClient'])

            proxy_port2 = self._network.client_tunnel_port(self.tcinputs['UnixProxy'])
            ...
            self._network.enable_firewall([self.tcinputs['FirewallClient1'],
                                           ...
                                           self.tcinputs['UnixProxy'],
                                           self.commserv],
                                          [client1_port, client2_port, client3_port,
                                           proxy_port, proxy_port2, 8403])
