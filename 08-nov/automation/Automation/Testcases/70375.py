...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "OCI Cross AD Restore"
    ...
    def run(self):
        ...
        try:
            ...
            self.vsa_obj.backup()
            try:
                decorative_log('Case 1: Perforing a Cross AD Restore to AD {}'.format(self.vsa_obj.availability_domain))
                self.vsa_obj.restore_proxy = self.tcinputs['Restore_Proxy']
                self.vsa_obj.do_restore_validation_options = {'CrossAD_Restore': True}
                self.vsa_obj.availability_domain = self.tcinputs['AvailabilityDomain']
                self.vsa_obj.vm_restore_prefix = "DeleteMe_AC_AD" + self.vsa_obj.availability_domain[-1] + "_"
                self.vsa_obj.unconditional_overwrite = True
                self.vsa_obj.job_status = ["completed", "completed w/ one or more errors"]
                self.vsa_obj.expected_job_errors = [".*Failed to delete volume backup:.*Error: \"Cannot delete Volume Backup.*as it is actively being restored\""]
