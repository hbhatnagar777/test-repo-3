...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Basic Acceptance case for file actions"
        self.tcinputs = {
    ...
    def run(self):
        """Run function of this test case
            ...
            Select files and call the file actions API
            Validates whether the file actions were reflected in Solr or not
        """
        try:
            ...
            self.log.info('Selecting random clusters of file for actions API')
            files_data = self.activate_utils.get_random_files_for_actions(self.commcell, self.data_source_name)
            self.log.info("Random files selected, now calling the API")
            for operation_name in files_data:
                self.data_source_helper.datasource_file_actions(operation_name, files_data[operation_name])
            self.log.info("Validating selected files for the action chosen")
            for operation_name in files_data:
                self.log.info(f"Validation {operation_name} action")
                self.activate_utils.validate_files_marked_for_actions(
                    self.commcell, files_data[operation_name],
                    self.data_source_name, operation_name)
                self.log.info(f"Validation for action {operation_name} is completed")
