...
Test cases to validate Basic operation window Data Management features at agrnt level
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Functional] : Basic operation window Data Management features validation at agent(File System)" \
                    " level for monthly rule with do not submit job enabled"
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info(
                "Validating agent level operation window for the features:%s",
                operations_list)

            self.log.info(
                "Initialising the subclient that honors the operation window")
