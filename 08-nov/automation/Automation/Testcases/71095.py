...
class TestCase(CVTestCase):
    """ Class for executing Block level backups with proxy server for MySQL Instance on Command Center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = 'Block level backups with proxy server for MYSQL'
        self.browser = None
    ...
    def setup(self):
        ...
        connection_info = {
            ...
            'socket_file': self.tcinputs['SocketFile']
        }
    ...
    def check_prerequisite(self, use_source=False):
        """ To check the snap and block level properties
        Args:
    ...
    def restore_and_validate(self, db_list):
        ...
        restore_panel = self.database_group.restore_folders(
            DBInstances.Types.MYSQL, all_files=True)
        self.log.info("Data + Log Restore")
        job_id = restore_panel.in_place_restore(data_restore=True,
                                                log_restore=True)
