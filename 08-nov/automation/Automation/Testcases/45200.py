...
TestCase:
    ...
    set_registry_key()   -- set registry keys  to increase log debug level

    remove_registry_key()   -- remove registry keys added to increase log debug level

This testcase checks if client side dedupe is working
or not with cache enabled on client, default size of cache
1024 is used for the purpose of this case.
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "client side dedupe with cache case"
        self.tcinputs = {
            "MediaAgentName": None
        }
    ...
    def set_registry_key(self):
        """Set the log debug level to 3"""
        if self.client_machine.os_info == 'WINDOWS':
            self.client_machine.create_registry("EventManager", "CLBACKUP_DEBUGLEVEL", 3, 'DWord')

        else:
            self.client_machine.create_registry("EventManager", "clBackupChild_DEBUGLEVEL", 3)
            self.client_machine.create_registry("EventManager", "clBackupParent_DEBUGLEVEL", 3)
        self.log.info(f"Client {self.client_machine.machine_name} : clBackup Debug level set to 3 ")


    def remove_registry_key(self):
        """Reset the log debug level to 3"""
        if self.client_machine.os_info == 'WINDOWS':
            self.client_machine.remove_registry("EventManager", "CLBACKUP_DEBUGLEVEL")

        else:
            self.client_machine.remove_registry("EventManager", "clBackupChild_DEBUGLEVEL")
            self.client_machine.remove_registry("EventManager", "clBackupParent_DEBUGLEVEL")
        self.log.info(f"Client {self.client_machine.machine_name} : Reset ClBackup Debug level to default ")
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Increasing the debug level")
            ...
            # stop the testcase if high latency optimisation option is checked
            self.log.info("checking for high latency optimisation option")
            query = """ select attrVal
                        from APP_ClientProp
                        where componentNameId={0}
                        and attrName='Optimize for High Latency Networks'""".format(self.client._get_client_id())
            self.log.info("EXECUTING QUERY %s", query)
            self.csdb.execute(query)
            result = self.csdb.fetch_one_row()[0]
            self.log.info(f"QUERY OUTPUT : {result}")
            if len(result) == 0:
                self.log.info("high latency option has not been set")
            elif int(result) == 1:
                self.log.info("high latency option has not been unchecked")
                ...
                xml = """<App_SetClientPropertiesRequest>
                        ...
                        </App_SetClientPropertiesRequest>""".format(self.client.client_name)
                self.log.info(
                    "Running q command using xml for disabling high latency optimisation on client")
                self.commcell.qoperation_execute(xml)
            self.log.info("High latency option is unchecked")
            ...
            for iterator in range(1, 3):
                ...
                self.log.info(f"QURY OUTPUT : {result}")
    ...
    def tear_down(self):
        ...
        try:
            ...
            self.log.info("restoring default loglevel")
