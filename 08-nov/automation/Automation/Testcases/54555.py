...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Test case for Replacing Brick, with node down redundancy after replacement and restoring"
        self.result_string = ""
        self.policy = ""
        self.policy_name = ""
        self.subclient_obj = ""
    ...
    def run(self):
        ...
        """
        try:
            self.log.info("*************Replacing Brick, with node down redundancy after replacement"
                          " and restoring **************")
            status_fstab = False
            ...
            # Hitting Redundancy Factor, MA services down on node
            offline = ""
            for node in nodes:
                if int(node) != int(self.node_id):
                    offline = self.hyperscale_helper.get_hostname(node)
                    self.hyperscale_helper.ma_service_down(offline)
                    break
            ...
            difference = self.hyperscale_helper.check_restore_folders(self.media_agent, self.tcinputs["Content"],
                                                                      restore_veri)
            if not difference:
            ...
            # Rectifying node network again and bring MA services up again
            self.hyperscale_helper.ma_service_up(offline)

            self.log.info("**********RUNNING JOB TO VERIFY POOL ACCESSIBLE************")
            # Running backup
