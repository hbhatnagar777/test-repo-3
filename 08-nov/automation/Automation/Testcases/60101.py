...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - VTL backup and restore of pre-defined subclient *CFG and validate the subclient defaults
        Step1, configure BackupSet and pre-defined Subclients for TC with VTL SP
        Step2: On client, Re-create the DEVD object
        Step3: Run a full backup for the subclient *CFG
        Step4: Check backup logs to confirm stage file SD* has backedup.
        Step5: On client, Create another DEVD object
        Step6: Run an incremental job for the subclient and verify it completes without failures.
        Step7: Check backup logs to confirm if VTL tapes are used for backup.
        Step8: delete DEVD objects from the disk.
        Step9: In-place restore of DEVD objects
        Step10: Verify restore logs and confirm correct command is used for restoring CFG
        Step11: Check devd objects existence on disk after restore and cleanup
    """

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "IBMi - VTL backup and restore of pre-defined subclient *CFG and validate the subclient defaults"
    ...
    def run(self):
        ...
        try:
            ...
            self.sc_name = "*CFG"
            self.log.info("*** STARTING RUN FOR SC: {0} with VTL STORAGE POLICY ** ".format(self.sc_name))
            ...
            self.helper.configure_ibmi_default_sc(backupset_name="backupset_{0}".format(self.id),
                                                  ...
                                                  delete=False)
            cfg_obj = ["AUTV{0}".format(self.id), "AUTV{0}1".format(self.id)]
            ...
            self.helper.verify_from_log('cvbkpvtl*.log',
                                        ...
                                        expectedvalue='SAVCFG')
            self.helper.verify_from_log('cvbkpvtl*.log',
                                        'runEachCommand',
                                        jobid=self.job.job_id,
                                        expectedvalue='VOL(')
            self.log.info("Step5: On client, Create a DEVD object %s.", cfg_obj[1])
            ...
            self.helper.verify_from_log('cvbkpvtl*.log',
                                        ...
                                        jobid=self.job.job_id,
                                        expectedvalue='VOL(')
            self.log.info("Step8: delete DEVD objects {0} and {1} from disk.".format(cfg_obj[0], cfg_obj[1]))
            cfg_obj_path = []
            for each in cfg_obj:
                self.client_machine.manage_devopt(operation="delete", object_name="{0}".format(each))
                cfg_obj_path.append("/<System Configuration>/{0}.DEVD".format(each))
            self.log.info("Step9: in.place restore of DEVD objects {0} and {1}.".format(cfg_obj[0], cfg_obj[1]))
            self.job = self.helper.restore_in_place(cfg_obj_path)
            self.log.info("Step10: Verify restore logs and confirm correct command is used for restoring CFG")
            self.helper.verify_from_log('cvrest*.log',
                                        regex='Submitted job with command',
                                        jobid=self.job.job_id,
                                        expectedvalue='RSTCFG OBJ({0} {1}) OBJTYPE(*DEVD)'.format(cfg_obj[0],
                                                                                                  cfg_obj[1])
                                        )
            self.log.info("Step11: Check devd objects existence on disk after restore {0} and {1}."
            ...
            for each in cfg_obj:
                ...
                self.client_machine.manage_devopt(operation="delete", object_name="{0}".format(each))
            self.log.info("** VTL BACKUP OF *CFG COMPLETED SUCCESSFULLY**")
