                ...
                "58929":
                        {
                            ...
                            "ec2_secret_key": "SECRET KEY OF EC2 ACCOUNT",
                            "ec2_access_key": "ACCESS KEY OF EC2 ACCOUNT"
                        }
            }
"""
import time
...
from Application.CloudApps.amazon_helper import AmazonRedshiftCLIHelper, AmazonCLIHelper


class TestCase(CVTestCase):
    """Command Center: REDSHIFT ACC1 test case with STS ASSUME ROLE Authentication"""
    ...
    def __init__(self):
        ...
        self.name = "REDSHIFT ACC1 test case with STS ASSUME ROLE Authentication"
        self.browser = None
    ...
    def setup(self):
        ...
        try:
            ...
            self.ec2_helper = AmazonCLIHelper(
                self.tcinputs['ec2_access_key'], self.tcinputs['ec2_secret_key'])
            self.log.info("Starting ec2 instance")
            if self.ec2_helper.ec2_state(
                    self.tcinputs['EC2InstanceName'], self.tcinputs['EC2Region']).lower() != "running":
                self.ec2_helper.start_ec2_instance(
    ...
    def tear_down(self):
        ...
        if self.ec2_turned_on:
            self.log.info("Stopping ec2 instance")
            self.ec2_helper.stop_ec2_instance(
                self.tcin.tcinputs['EC2InstanceName'], self.tcinputs['EC2Region'])
    ...
    def run(self):
        ...
        try:
            ...
            self.database_instance.add_redshift_instance(
                ...
                auth_type="STS", access_node=self.tcinputs['access_node'],
                credential_name=self.tcinputs['credential_name'],content=content)
