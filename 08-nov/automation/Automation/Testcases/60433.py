...
TestCase:
    ...
    run_backup_ci                                --  Runs a file system subclient backup and CI
    ...
    review_delete_from_ci_action_fs()            --  Deletes from CI files for a FS datasource and verify
...
class TestCase(CVTestCase):
    """Class for executing delete from backup action for all CI-ed files with local path"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Delete from backup action for all CI-ed files with local path in SDG"
        self.tcinputs = {
    ...
    def create_commcell_entities(self):
        ...
        self.subclient_obj = self.activateutils.create_commcell_entities(
            ...
            id=self.id,  enable_ci=True,
            index_server_name=self.tcinputs['IndexServerName'], access_node=self.tcinputs['AccessNode'])

    @test_step
    def run_backup_ci(self):
        """Runs a backup and CI job on subclient"""
        ...
        self.log.info("Now running the CI job.")
        ci_job = self.commcell.policies.configuration_policies.run_content_indexing(
    ...
    def init_tc(self):
        ...
        try:
            ...
            self.file_policy_name = f"{self.id}_filepolicy"
    ...
    @test_step
    def review_delete_from_ci_action_fs(self):
        """
        Performs and verifies delete from backup action on the files present in review page
        """
    ...
    def run(self):
        try:
            ...
            self.review_delete_from_ci_action_fs()
            self.cleanup()
