...
class TestCase(CVTestCase):
    """Grant access to user by selecting the period from the date picker.
        Check that the access is removed once the granted period is over.
        User should get access for the days selected.
        Access to download of all SET folders(at Commcell level) should be removed after the granted period is over."""
    ...
    def __init__(self):
        ...
        self.name = "CV Cloud Request Access: Access Removal once granted period is over"
        self.helper = None
        self.tcinputs = {
    ...
    def run(self):
        ...
        try:
            ...
            self.helper_cloud_user.commcell_guid = random.choice(companyname_companyid_commcellguid)
            ...
            validity = validity + timedelta(minutes=2)
            ...
            _ = self.helper_cloud_admin.approve_deny_revoke_request( int(request_id),
                                                                    ...
                                                                    validity,
                                                                    self.CommentsForApproveDenyRevoke)
