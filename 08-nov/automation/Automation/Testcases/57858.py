...
from Application.Exchange.ExchangeMailbox.solr_helper import SolrHelper as SHelper
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Cloud index server: Solr jvm memory validation"
        self.tcinputs = {
        ...
        self.index_server_role = ['Exchange Index']
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Getting solr base url for node: %s" % self.tcinputs['IndexServerNodeNames'])
            self.solr_base_url = self.solr_helper.get_solr_baseurl(self.tcinputs['IndexServerNodeNames'][0], 5)
            self.log.info("Solr base url : %s" % self.solr_base_url)
            self.initial_registry_memory = int(self.node_machine_obj_list[0].get_registry_value(
            ...
            index_pool_obj = self.commcell.index_pools.get(self.index_pool_name)
            index_pool_obj.modify_node(self.tcinputs['IndexServerNodeNames'][0],
                                       node_params=[{
                                           "name": "JVMMAXMEMORY",
            ...
            self.log.info("Index pool modified : %s" % self.index_pool_name)
            ...
            self.updated_registry_memory = int(self.node_machine_obj_list[0].get_registry_value(
                win_key=self.jvm_memory_reg_key_path, value=self.jvm_memory_reg_value))
            ...
            self.solr_helper = SHelper(self.exchange_mailbox_client)
            select_dict = {
                "DocumentType": 2,
