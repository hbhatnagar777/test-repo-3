...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Upgrade Automation - Encryption Configurations with Client Using Custom Encryption"
        self.tcinputs = {
            "ServicePack": None,
            "CSHostName": None,
    ...
    def initial_setup(self):
        """
        Configures the Environment required for TC
        """
    ...
    def pre_upgrade_steps(self):
        ...
        self.log.info("Setting Client, Subclient encryption settings: Encrypt on Client: BlowFish 256."
        ...
        self.log.info(f"Client Enc. Settings: "
                      ...
                      f" CipherType - {self.client.properties.get('clientProps').get('CipherType')},"
                      f" KeyLen - {self.client.properties.get('clientProps').get('EncryptKeyLength')}")
    ...
    def post_upgrade_validations(self):
        ...
        if (self.client.properties.get('clientProps').get('encryptionSettings') == 1
                and self.client.properties.get('clientProps').get('CipherType') == 2
                and self.client.properties.get('clientProps').get('EncryptKeyLength') == 256):
        ...
        self.log.info("Changing Encryption on Pools. Serpent 128 on Pools 0(Non Dedup Pool), 2(Dedup Pool) "
                      " and AES 128 on Pools 1(Non Dedup Pool), 3(Dedup Pool)")
        for index in range(4):
            if index % 2 == 0:
                self.storage_pool_list[index].get_copy().set_encryption_properties(
                    re_encryption=True, encryption_type="AES", encryption_length=128)
            else:
                self.storage_pool_list[index].get_copy().set_encryption_properties(
