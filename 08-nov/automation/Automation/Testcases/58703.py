...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Openstack volume snapshot backup quota validation"
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.log.info('Got volume snapshot quota limits for the project'
                              +str(project_name))
            except Exception as err:
                self.log.error('Failed to get volume snapshot quota limits for the project'
                               +str(project_name))
                raise Exception
            ...
            try:
                ...
                self.log.info(
                    'set volume snapshot quota limits for the project successfully'
                    +str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to set volume snapshot quota limits for the project'
                    +str(project_name))
                raise Exception
            ...
            failurereason = 'Insufficient cinder quota to create VM Snapshot: Failed to meet snapshots quota limit'
            if output != ():
                if failurereason not in output[0][0]:
                    self.log.error('---Failed reason not updated on Database for job--'
                                   +str(job_obj.job_id))
                    raise Exception
            self.log.info('--Failed reason updated on the Database for job'
                          +str(job_obj.job_id))
            self.log.info(
            ...
            try:
                self.hvobj.OpenStackHandler.reset_volumesnapshot_limits(project_name)
                self.log.info(
                    'Reseting volume snapshot quota limits for the project successfully'
                    +str(project_name))
            except Exception as err:
                self.log.error(
                    'Failed to reset volume snapshot quota limits for the project'
                    +str(project_name))
                raise Exception
