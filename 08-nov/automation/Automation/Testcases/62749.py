...
TestCase:
    ...
    verify_archive_review_action()  --  Verifies that archived items are not picked up during re-crawl
...
class TestCase(CVTestCase):
    """Test case for FSO Archive review action along with request manager and filters in Command Center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "FSO: Advanced test case for Archive Review actions along with request manager and filters"
    ...
    @test_step
    def review_archive_fso_action(self):
        """
        ...
        try:
            ...
            archive_action_status = self.gdpr_obj.data_source_review_obj.review_archive_action(
                '', archive_plan=self.tcinputs['ArchivePlan'], is_fso=True, all_items_in_datasource=True,
                review_request=True, reviewer=RM_CONFIG_DATA.Reviewer, request_name=self.archive_request_name,
            ...
            self.error_dict[f'Review Archive Files Actions'] = str(error_status)
        ...
        if not archive_action_status and not exp_flag:
            self.error_dict[f'Review Archive Files Actions'] = 'Files still found in review page'
    ...
    @test_step
    def verify_archive_review_action(self):
        """
        Verifies that archived items are not picked up during re-crawl
        """
        self.navigator.navigate_to_governance_apps()
        self.gdpr_obj.inventory_details_obj.select_file_storage_optimization()
        self.fso_helper.fso_obj.select_details_action(self.tcinputs['FSOClient'])
    ...
    def perform_cleanup(self):
        ...
        self.fso_helper.fso_cleanup(
            ...
            review_request=[self.archive_request_name])
    ...
    def run(self):
        ...
        try:
            ...
            time.sleep(self.wait_time)
            self.review_archive_fso_action()
            self.verify_archive_review_action()
