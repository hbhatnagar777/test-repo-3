...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Cloud Laptop]: PST evaluation backup"
    ...
    def run(self):
        ...
        try:
            ...
            self.cloud_object.wait_for_incremental_backup(self.machine_object)

            self.cloud_object.source_dir = self.test_folder
            self.cloud_object.subclient_content_dir = self.test_folder
            # cloud_object.out_of_place_restore(machine_object, subclient_object, validate=False, cleanup=False)
            ...
            self._log.info("PST file backup is validated")
            ...
            self.cloud_object.wait_for_incremental_backup(self.machine_object)
            self.pstfile_validation(validate=False)
            if not set(source_contents).issubset(set(collect_contents)):
                raise Exception("Pst file is not backedup")
            ...
            self._log.info("***** Validation of PST file evaluation backup completed successfully *****")
