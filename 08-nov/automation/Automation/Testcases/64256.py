                ...
                "64256":
                        {
                            ...
                            "DestinationDatabaseUserName": None,
                            "DestinationDatabasePassword": None,
...
class TestCase(CVTestCase):
    """ Class for executing acceptance Test for SAP HANA on Metallic out of place restore """
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Metallic out of place restore for SAP HANA"
        self.browser = None
        ...
        self.src_client_name = None
        self.dst_client_name = None

    def setup(self):
        """ Method to setup test variables """
        if self.tcinputs.get("HyperVVMName"):
    ...
    def initial_setup(self):
        ...
        cloud_storage_inputs = {"StorageAccount": self.tcinputs.get("StorageAccount") or "Metallic HOT GRS Tier",
                                "CloudProvider": self.tcinputs.get("CloudProvider") or "Azure Blob Storage",
        ...
        self.plan_name = self.hana_metallic_configuration.configure_hana_database(
        ...
        if self.tcinputs['HostName'] == self.tcinputs['DestinationHostName']:
            self.hana_metallic_configuration.do_pkg_install(
                commcell=self.commcell, pkg_file_path=pkg_file_path, install_inputs=[install_inputs])
        else:
            self.hana_metallic_configuration.do_pkg_install(
                commcell=self.commcell, pkg_file_path=pkg_file_path,
                install_inputs=[install_inputs, install_inputs_dest])
    ...
    def upgrade_client_software(self):
        ...
        job = self.servers_obj.action_update_software(client_name=self.tcinputs['DestinationHostName'].split('.')[0])
        self.wait_for_job_completion(job)
    ...
    def update_display_names(self):
        ...
        for client_name in self.commcell.clients.all_clients:
            if client_name.endswith(self.tcinputs.get("SID").lower()):
                self.src_display_name = client_name
                self.src_client_name = client_name.rstrip(self.tcinputs.get("SID").lower()) + self.tcinputs.get("SID")
                break
        for client_name in self.commcell.clients.all_clients:
            if client_name.endswith(self.tcinputs.get("DestinationSID").lower()):
                self.dst_display_name = client_name
                self.dst_client_name = (client_name.rstrip(self.tcinputs.get("DestinationSID").lower())
                                        + self.tcinputs.get("DestinationSID"))
                break

    @test_step
    def navigate_to_instance(self, sid, client_name):
    ...
    def navigate_to_subclient(self):
        ...
        self.navigate_to_instance(self.tcinputs['DestinationSID'], self.dst_client_name)
        self.db_instance_details.update_credentials(
            self.tcinputs["DestinationDatabaseUserName"], self.tcinputs["DestinationDatabasePassword"])
        self.navigate_to_instance(self.tcinputs['SID'], self.src_client_name)
        self.db_instance_details.update_credentials(
            self.tcinputs["DatabaseUserName"], self.tcinputs["DatabasePassword"])
    ...
    def run_restore(self):
        ...
        restore_job = restore_panel.out_of_place_restore(destination_client=self.dst_client_name,
                                                         destination_instance=self.tcinputs['DestinationSID'],
                                                         destination_database=self.tcinputs['DestinationSID'])
    ...
    def kill_active_jobs(self, client_name):
        ...
        self.commcell.refresh()
        active_jobs = self.commcell.job_controller.active_jobs(client_name)
        ...
        if active_jobs:
            ...
            active_jobs = self.commcell.job_controller.active_jobs(client_name)
            if active_jobs:
                self.kill_active_jobs(client_name)
    ...
    def cleanup(self):
        ...
        if self.is_metallic_success:
            ...
            if self.database_instances.is_instance_exists(
                    ...
                    self.tcinputs['SID'], self.src_client_name):
                self.kill_active_jobs(self.src_display_name)
                self.database_instances.select_instance(
                    self.database_instances.Types.SAP_HANA,
                    self.tcinputs['SID'],
                    self.src_client_name)
                self.db_instance_details.delete_instance()
            self.navigator.navigate_to_db_instances()
            if self.database_instances.is_instance_exists(
                    self.database_instances.Types.SAP_HANA,
                    self.tcinputs['DestinationSID'], self.dst_client_name):
                self.kill_active_jobs(self.dst_display_name)
                self.database_instances.select_instance(
                    self.database_instances.Types.SAP_HANA,
                    self.tcinputs['DestinationSID'],
                    self.dst_client_name)
