...
class TestCase(CVTestCase):
    """Class for executing AWS On demand access node - Create an ARM Linux Access node using Command Center"""""

    def __init__(self):
        """" Initializes test case class objects"""""
        super(TestCase, self).__init__()
        self.name = "AWS On demand access node - Create an ARM Linux MA/FREL using Command Center"
    ...
    def run(self):
        ...
        try:
            ...
            for entry_point in self.entry_points:
                ...
                try:
                    self.log.info(
                        "Adding bAmazonDirectReadEnabled Registry Key for newly created Access Node: {}".format(
                            self.proxy_name))
                    self.vsa_obj.one_click_node_obj.machine.update_registry(key='VirtualServer',
                                                                            value='bAmazonDirectReadEnabled', data=0,
                                                                            reg_type='DWord')
                except Exception as exp:
                    self.log.error("Failed to update bAmazonDirectReadEnabled Registry Key for newly created Access "
                                   "Node: {}".format(self.proxy_name))
                    self.utils.handle_testcase_exception(exp)
