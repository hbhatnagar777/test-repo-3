...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Azure DevOps IDA - Verify VOI count is correctly populated in LSR, LSR-WW and SCPU reports"
        self.browser = None
    ...
    def validate_voi_count(self, gen_data=False, repo=None, temp_client=False, delete_case=False, case=None, voi_init=None):
        ...
        validates voi count
        """
        azure_app_name = f"{self.tcinputs['azure_app_name']}{'_temp' if temp_client else ''}"
        if voi_init is None:
        ...
        if not delete_case:
            ...
            if gen_data and repo:
                ...
                cl_id = self.commcell.clients.get(self.tcinputs['azure_app_name']).client_id
            ...
            client_id = self.commcell.clients.get(azure_app_name).client_id
            if not repo:
                self.commcell.clients.delete(azure_app_name)
                backup_count = -len(self.lsr_entry[client_id])
                self.lsr_entry.pop(client_id, None)
                expected_outcome = f"VOI count should be decreased by {backup_count}"
            else:
                self.access_or_create_client(azure_app_name)
    ...
    def run(self):
        ...
        try:
            ...
            for azure_app in [azure_app, f"{azure_app}_temp"]:
