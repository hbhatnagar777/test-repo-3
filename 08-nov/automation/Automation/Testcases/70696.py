...
from dynamicindex.utils.constants import USA_COUNTRY_NAME, CLIENT_NAME, FSO_SUBCLIENT_PROPS, TREE_SIZE_DASHBOARD
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Validation of Export to CSV for TreeSize dashboard -> Folders only (Backed up datasource)"
        self.tcinputs = {
            "IndexServer": None,
    ...
    def download_csv(self):
        ...
        try:
            self.fso_helper.navigate_to_datasource_details(self.tcinputs['HostNameToAnalyze'],
                                                           self.file_server_display_name)
            self.fso_helper.fso_data_source_discover.select_fso_dashboard(self.admin_console.props['reports.treeSize'])
            self.fso_helper.fso_data_source_discover.download_csv_treesize_dashboard(all_data=False)
    ...
    def run(self):
        try:
            ...
            attributes = {"AccessTime", "Url", "OwnerName", "FileCount_iv", "FolderCount_iv", "FolderSize_lv",
                          "DocumentType"}
