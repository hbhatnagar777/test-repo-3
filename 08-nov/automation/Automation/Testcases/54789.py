...
"""Test Case for validating CVEntity Cache for server page
...
class TestCase(CVTestCase):
    """Test case class validating CVEntity Cache for server page"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = ("page entitycache testing")
    ...
    def setup(self):
        ...
        self.helper = entitycache.EntityCacheHelper(
            self.commcell, self.client, webconsole, 'vmgroup', self.log)
        self.helper.stop_service()

    def run(self):
        """ Main function for test case execution.
        This Method validates VMGroup entity cache.
        Raises:
            SDKException:
        ...
        try:
            ...
            for i in range(iterations):
                ...
                if 'subClientProperties' in response:
                    for value in response['subClientProperties']:
                        if 'subClientEntity' in value:
                            if 'subclientName' in value['subClientEntity']:
                                if i == 0:
                                    validate_list_start.append(
                                        value['subClientEntity']['subclientName'])
                                else:
                                    validate_list.append(value['subClientEntity']['subclientName'])
            ...
            for self.helper.iter in range(iterations):
                ...
                if validate and 'subClientProperties' in response:
                    if 'subClientEntity' in response['subClientProperties'][0]:
                        self.log.info("subclientProperties are available")
                if validate and 'subClientProperties' in response:
                    if 'planEntity' in response['subClientProperties'][0]:
                        self.log.error("planEntity are available")
                        raise Exception("Validation failed")

                if validate and 'subClientProperties' in response:
                    if 'vsaSubclientProp' in response['subClientProperties'][0]:
                        self.log.error("vsaSubclientProp are available")
                        raise Exception("Validation failed")

                self.helper.constraint_dict['subclient'] = "default"
                ...
                self.helper.constraint_dict['client'] = "vm8vcenter"
                fq_query = self.helper.construct_query(self.helper.constraint_dict)
                url = (self.helper.browser_urls +
                       fq_query +
                       self.helper.start_0)
                ...
                self.helper.constraint_dict['search'] = "VMware"
                fq_query = self.helper.construct_query(self.helper.constraint_dict)
                url = (self.helper.browser_urls +
                       fq_query +
                       self.helper.start_0)
                ...
                self.helper.constraint_dict['instance'] = "VMware"
                fq_query = self.helper.construct_query(self.helper.constraint_dict)
                url = (self.helper.browser_urls +
                       fq_query +
                       self.helper.start_0)
                ...
                for newthread in range(number_of_threads):
                    lib_thread = threading.Thread(
                        target=self.helper.calculate, name="vmgroup", args={url, })
