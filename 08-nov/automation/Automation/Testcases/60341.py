...
TestCase:
    ...
    Verify the cascading network topology in command center (laptops)

Inputs to the testcase provided

    LaptopClient    --  A client computer which is a laptop client

    InfrastructureClient : A client computer

    ProxyClient : A client computer

    LaptopGateway   --  A client computer which acts as gateways for laptop

Inputs for smart topology (Optional)

    is_smart : True or False for smart topology

    MediaAgent : A media agent client name other than clients
...
class TestCase(CVTestCase):
    """Command Center- Network topology-  via Cascading Topology( Laptop)"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Command Center- Network topology-  via Cascading Topology( Laptop)"
        self.browser = None
        self.topology_name = "CG_60341_CASCADING_GATEWAY"
        self.admin_console = None
        self.navigator = None
        self._client_group_name1 = "CG_60341_Laptop"
        self._client_group_name2 = "CG_60341_INFRASTRUCTURE"
        self._client_group_name3 = "CG_60341_Laptop_GATEWAY"
        self._client_group_name4 = "CG_60341_PROXY"
        self.tcinputs = {
            "LaptopClient": "",
    ...
    def init_tc(self):
        ...
        try:
            ...
            self.client_group_obj1.add_clients([self.tcinputs["LaptopClient"]])
            ...
            self.client_group_obj3.add_clients([self.tcinputs["LaptopGateway"]])
            ...
            self.laptop_client = self.tcinputs['LaptopClient']
            self.Infrastructure = self.tcinputs["InfrastructureClient"]
            self.proxy = self.tcinputs['ProxyClient']
            self.laptopgateway = self.tcinputs['LaptopGateway']
            self.client_name_list.extend([self.laptop_client, self.Infrastructure, self.laptopgateway, self.proxy])
            self.laptop_obj = self.commcell.clients.get(self.tcinputs['LaptopClient'])
            ...
            self.laptopgateway_obj = self.commcell.clients.get(self.tcinputs['LaptopGateway'])

            self.client_obj_list.extend([self.laptop_obj, self.infrastructure_obj,
                                         self.laptopgateway_obj, self.proxy_obj])
            self.tunnel_port_list.extend([self.laptop_obj.network.tunnel_connection_port,
                                          self.infrastructure_obj.network.tunnel_connection_port,
                                          self.laptopgateway_obj.network.tunnel_connection_port,
                                          self.proxy_obj.network.tunnel_connection_port])
            self.summary_list.extend([self.laptop_summary, self.infrastructure_summary,
                                      self.laptopgateway_summary, self.proxy_summary])
    ...
    def pre_check_summary(self):
        ...
        if self.validate_no_routes(self.client_name_list, self.summary_list):
            ...
            raise CVTestStepFailure("Already a route has been configured between the laptop client and Infrastructure")
    ...
    def validate_no_routes(self, client_list, summary_list):
        ...
        laptopgateway_summary = summary_list[2].split("\n")
        laptop_check = 0
        infrastructure_check = 0
        proxy_check = 0
        laptopgateway_check = 0
        ...
        if laptop_check > 0 and infrastructure_check > 0 and proxy_check > 0 and laptopgateway_check > 0:
            return False
    ...
    def validate_summary(self, summary, source, destination, group_type, streams,
        ...
        """Validates the network summary of a client
            ...
            Args --
                ...
                source : client name of laptops or infrastructure

                destination : client name of laptops or infrastructure

                group_type (int) : Group type in topology 1 for laptops 2 for insfrastructure 3 for proxy
                ...
                **kwargs :
                    Supported key word arguments are
                        Proxy : Proxy Client/ DMZ client name

                        Gateway : LaptopGateway client name (Alter the gateway name and proxy for the infrastructure)
        ...
        # Count the routes in laptops as well as infrastructure
        check_laptop = 0
        ...
        for i in summary_split:
            ...
            # Validation of Outgoing network routes for laptops/ infrastructure
            if group_type <= 2:
                if source + " " + gateway + " " in i and ("type=persistent" in i and " proto=" + protocol + " " in i):
                    if int(streams) > 1 and "streams=" + streams in i:
                        ...
                        check_laptop += 1
                if group_type == 1:
                    if source + " " + "*" in i and "proxy=" + gateway in i:
                        check_laptop += 2
                else:
            ...
            # Validation of outgoing routes in the laptop gateway
            if group_type == 3:
        ...
        if (check_proxy == 3 or check_laptop == 3 or check_gateway == 3) and check_tunnel_port and check_keep_alive:
            self.log.info(f"Validated the network routes of {client_group}")
            return True
        else:
            self.log.info(summary)
            # Commented until validation of network summary is fixed
    ...
    def create_cascading_gateway(self, client_group_list, client_list):
        ...
        try:
            # Create a cascading gateways from client to Infrastructure client group through the laptop gateways and
            # Dmz Gateways
            self.networkpage.add_topology(self.topology_name, "Cascading gateways", client_group_list, "clienttype",
                                          wildcard_proxy=True)

            self.get_summaries()

            # Validation of network summary of laptops
    ...
    def validate_additional_settings(self, client_group_list: list, client_list: list):
        ...
        if client_list[3] in self.smart_groups:
    ...
    def validate_encrypt_network(self, client_group_list, client_list):
        ...
        try:
            ...
            # Validate after setting protocol back to the authenticated
            self.networkpage.edit_topology(self.topology_name, None, EncryptTraffic=False,
                                           TunnelProtocol="Authenticated")
            # Validation of network summary of servers
            self.validate_summary(self.summary_list[0], client_list[0], client_list[1],
    ...
    def download_network_summary(self, client_group_list, client_list, client_obj_list):
        ...
        try:
            if client_group_list[0] in self.smart_groups or client_group_list[1] in self.smart_groups \
                ...
                # Download the summary files of all clients
    ...
    def validate_smart_topology(self):
        ...
        cg_type = 2  # Either servers or infrastructure
        ...
        try:
            ...
            self.networkpage.add_topology(self.topology_name, "Cascading gateways", self.client_group_list,
                                          "clienttype")
            ...
            cl_list = [self.laptop_client, self.Infrastructure, self.proxy]
    ...
    def run(self):
        try:
            ...
            # Validate the network routes for smart topology
