...
"""
Ring Routing: Remote Operations on Child Company [UI]

This test verifies the remote operations from reseller idp to remote child company for:
    Company creation
...
class TestCase(CVTestCase):
    """Class for executing Ring Routing: Remote Operations on Child Company [UI]"""
    ...
    def __init__(self):
        ...
        self.rr_helper = None
        self.reseller = None
        self.name = "Ring Routing: Remote Operations on Child Company [UI]"
        self.tcinputs = {}
    ...
    def run(self):
        ...
        personas = {
            # todo: add child operator and non admin msp operator
            'Tenant Admin': self.rr_helper.use_tenant_admin,
            'MSP as Operator': lambda: self.rr_helper.use_msp_admin(as_operator_of='reseller')
        }
        ...
        try:
            for persona in personas:
                ...
                # SETUP BEFORE TESTING FOR PERSONA
                if not self.child:
                ...
                # THE TESTS
                try:
                    if persona == 'Tenant Admin':
                        pass  # persona_rrhelper.validate_manage_operators()
