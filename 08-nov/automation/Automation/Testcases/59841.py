...
dedupe backup jobs are in progress.
    # basic DDBBackup job case
...
input json file arguments required:
                        ...
                        "testcaseFactor": "factor to change the data content -> can
                         control the overlap of DDBbackup job with sidb2.exe
                        ...
                        # make sure for test
                        "mount_path" : "enter the unix path to
                         library folder for creating mount path"
...
1.	Have dedupe SP
2.	Make sure DDBBackup subclient exists and
    is associated to valid SP
3.	Run backup with new content (we need to run
    backups on multiple subclients,all pointing to same
    store with generated content so that SIDB2
    process is up for some good enough time)

Run:
4.	After creating resources and running one "FULL" backup,
    run another "FULL" backup with new content,
    while the job is in progress and reaches 'backup' phase,
    start a DDB backup job
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = """UNIX basic case for DDBBackup Job -
                    checks if DDBBackup job is working correctly or not"""
        self.tcinputs = {
            "MediaAgentName": None,
            "testcaseFactor": None,
            "mount_path": None,
    ...
    def create_resources(self):
        ...
        size = 5 * int(self.tcinputs.get("testcaseFactor"))
        if self.mm_helper.create_uncompressable_data(
