...
class TestCase(CVTestCase):
    """Class for verifying SynthFull Backup and Restore for SOLR Index Server"""
    test_step = TestStep()
    ...
    def __init__(self):
        ...
        self.name = "Verifying SynthFull Backup and Restore for SOLR Index Server"
    ...
    def config_and_run_backup(self):
        ...
        job_instance = Job(commcell_object=self.commcell, job_id=int(backup_job_id))
        self.full_backup_size = job_instance.size_of_application


    @test_step
    def run_incremental_backup(self, backup_type=constants.INCREMENTAL):
        ...
        inc_job_id = self.index_servers_obj.backup_index_server(self.index_server_name, backup_type)

        if backup_type == constants.INCREMENTAL:
            job_instance = Job(commcell_object=self.commcell, job_id=int(inc_job_id))
            inc_backup_size = job_instance.size_of_application

            if not (inc_backup_size < self.full_backup_size):
                raise Exception("Size of Incremental Backup should not be greater than Full Backup ")
    ...
    def run(self):
        ...
        try:
            ...
            self.run_incremental_backup()
            self.run_incremental_backup(constants.SYNTHETIC_FULL)
