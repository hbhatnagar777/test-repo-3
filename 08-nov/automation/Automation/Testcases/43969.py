...
This test case verifies the basic behaviour of start restore which is part of the indexing service in ODS.
...
class TestCase(CVTestCase):
    """This test case verifies the basic behaviour of start restore which is part of the indexing service in ODS.
        ...
        Steps:
            ...
            2) Assign testdata to subclient where there are at least 10k 1 kb items
            3) Run FULL, INC, INC
            4) Start 10 single stream restore jobs in parallel.
            5) While the jobs are running randomly pick couple of jobs, kill some, suspend and resume some and leave
            some jobs untouched.
            6) Wait for the restore jobs to complete.
            7) Validate the restore results for the completed jobs.
    ...
    def __init__(self):
        ...
        self.name = 'Indexing - Single stream restores'
    ...
    def setup(self):
        ...
        self.backupset = self.idx_tc.create_backupset('restore_single_stream', for_validation=False)

        self.subclient = self.idx_tc.create_subclient(
            name='sc1_single_stream',
    ...
    def do_restore(self, restore_id, action):
        ...
        try:
            job = self.backupset.restore_out_of_place(
                ...
                paths=['/'],
                fs_options={
                    'no_of_streams': self.tcinputs.get('Streams', 1)
                }
            )
