...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Selective Copy Cases for Hourly & First Full Configuration with user defined DayStartsOn "
        self.tcinputs = {
            "MediaAgentName": None
        }
    ...
    def hourly_first_full_daystartson(self):
        """Before Jobs Accrued:
            Create a copy with DaystartsOn
        ...
        copy1_name = str(self.id) + '_selcopy_daystartson'
        self.log.info("Adding a new selective copy: %s", copy1_name)

        # backup on or after date
        today = date.today()
        # dayStartsOn value hardcoded for now
        daystartson = {"hours": "5", "minutes": "30", "seconds": "0", "ampm": "AM"}
        sp_obj.create_selective_copy(copy1_name, self.disk_library_dest, self.tcinputs['MediaAgentName'], 'hourly',
                                     'first', str(today), daystartson)
        ...
        if bverdict:
            self.log.info("Basic DayStartsOn Test Case passed")
            self.result_string = "Basic DayStartsOn Test Case passed"
        else:
    ...
    def run(self):
        ...
        try:
            ...
            self.hourly_first_full_daystartson()
