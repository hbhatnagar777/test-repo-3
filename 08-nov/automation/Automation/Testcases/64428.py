...
class TestCase(CVTestCase):
    """ GUI Backup and inplace restore from multiple MA's post sweep job """
    ...
    def __init__(self):
        ...
        self.name = "Test for DB2 1 Min RPO from GUI to verify Inplace restore using multiple ma's"
    ...
    def setup(self):
        ...
        self.table_name = "T64428"
        self.tablespace_name = "TS64428"
        ...
        self.media_agents = self.dbhelper_object.get_ma_names(self.storagepolicy)
    ...
    @test_step
    def run_and_validate_sweep_job(self, ma_name):
        ...
        """
        try:
            self.log.info(f"Adding the registry key to {ma_name} to run sweep job")
            self.db2_helper.run_sweep_job_using_regkey(ma_name)
    ...
    def stop_cvd_on_ma(self,ma_obj):
        ...
        self.log.info("Sleeping for 3 minutes after stopping cvd on media agent..")
        time.sleep(180)

    def start_cvd_on_ma(self, ma_obj):
        """ Restart services on MA"""
        self.log.info("Restarting media agent..")
        try:
            ma_obj.start_all_cv_services()
        except Exception :
            self.log.info("Entered exception block as Expected. CVD service Started.")
        self.log.info("Sleeping for 3 minutes after restarting cvd on media agent..")
        time.sleep(180)
    ...
    def run(self):
        ...
        try:
            ...
            for x in range(len(self.media_agents)):
                self.db2_helper.reconnect()
                ...
                active_ma = self.db2_helper.get_active_ma()
                active_ma_obj = Machine(active_ma,self.commcell)
                self.db2_helper.verify_logs_on_ma(active_log_number, active_ma_obj)
                used_ma.append((active_ma, active_ma_obj))
                self.stop_cvd_on_ma(active_ma_obj)

            for x in used_ma:
                self.start_cvd_on_ma(x[1])

            self.log.info(f"Triggering sweep job using sweep start time reg key on all media agent {self.media_agents}")
            for x in used_ma:
                self.run_and_validate_sweep_job(x[0])
