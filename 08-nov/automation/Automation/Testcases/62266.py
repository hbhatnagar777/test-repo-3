...
TestCase:
    ...
    delete_index_v2_backupset()             -- Deletes V2 index for backupset and restart services
...
class TestCase(CVTestCase):
    """Class for executing Index reconstruction case for DB2 V2 clients"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()

        self.name = "DB2 Indexing V2: Reconstruction Case for DB2 Table Level."
    ...
    def setup(self):
        ...
        self.src_database_name = "IDXV2TB"
        ...
        if not self.db2_helper_object.is_index_v2_db2():
    ...
    def run(self):
        ...
        try:
            ...
            # Log backup
            self.db2_helper_object.db2_archive_log(self.backupset.backupset_name.upper(), 15)
            ...
            self.delete_index_v2_backupset()
            self.log.info("Sleeping for 30 seconds")
            time.sleep(30)
    ...
    def create_database(self, database_name):
        ...
        database_cmd = f"{self.db2_base} db2 create db {database_name}"
    ...
    def add_subclient(self):
        ...
        self.backupset.subclients.add(subclient_name=self.id,
                                      storage_policy=self.tcinputs['storage_policy'],
                                      description="Index V2 Reconstruction DB2 Table Level")
        self.backupset.refresh()
        ...
        self.subclient.enable_table_level()
    ...
    def run_backup(self, backup_type):
        ...
        if backup_type == "log":
            ...
            self.validate_backup_job(backup_job_object=backup_job)
    ...
    def validate_backup_job(self, backup_job_object=None):
        """
        ...
        Args:
            backup_job_object (Job) -- Backup Job Object
                default: None
        """
        backup_time_stamp, _ = self.db2_helper_object.get_backup_time_stamp_and_streams(jobid=backup_job_object.job_id)
        self.log.info("### Running Backup Validation ###")
        time.sleep(30)
        self.db2_helper_object.backup_validation(operation_type="N",
                                                 tablespaces_count=self.table_data[1],
                                                 backup_time_stamp=backup_time_stamp)

        commonutils = CommonUtils(self.commcell)
        commonutils.backup_validation(backup_job_object.job_id, "Full")

        self.log.info("Successfully ran %s backup.", backup_job_object.backup_level)

    def delete_index_v2_backupset(self):
        """
        Deletes V2 index for each subclient and restart services
        """
        self.log.info("Performing index delete operation after backup for backupset: %s", self.backupset.name)
        self.db_helper.delete_v2_index_restart_service(backupset=self.backupset)

    def get_redirect_path(self):
        """
        Get Redirect Path
        """
        self.log.info("### Getting Instance Redirect Path ###")
        index = 9
        if "windows" in self.client_machine.os_info.lower():
            index = 1

        get_redirect_path = f"{self.db2_base} db2 connect to {self.src_database_name}; db2 \"SELECT DBPARTITIONNUM," \
    ...
    def run_restore(self):
        ...
        Runs restore
        """
        self.log.info("### Table Level restore ###")
        self.commcell.refresh()

        self.create_database(database_name=self.dest_db)
        path_sep = "\\" if "windows" in self.client.os_info.lower() else "/"
        staging_path = f"{self.redirect_path}{path_sep}dump{self.id}"
        self.client_machine.create_directory(directory_name=staging_path,
                                             force_create=True)
        ...
        tables_list = [
            f"{path_sep}+tblview+{path_sep}{self.tcinputs['instance_name']}{path_sep}{self.src_database_name}"
            f"{path_sep}SCHEMAFULL{path_sep}{self.table_name}_FULL{path_sep}**"
        ]
        self.log.info("Starting Restore")
        restore_job = self.backupset.restore_table_level(aux_client_name=self.client.client_name,
                                                         aux_instance_name=self.instance.instance_name,
                                                         aux_backupset_name=self.src_database_name,
                                                         dest_client_name=self.client.client_name,
                                                         dest_instance_name=self.tcinputs['instance_name'],
                                                         dest_backupset_name=self.dest_db,
                                                         target_path=self.redirect_path,
                                                         staging_path=staging_path,
                                                         tables_path=tables_list,
                                                         user_name=self.tcinputs["db2_username"],
                                                         password=self.tcinputs["db2_user_password"],
                                                         server_port=int(self.db2_helper_object.instance_port),
                                                         clearAuxiliary=True,
                                                         dropTable=False)
    ...
    def validate_restore_job(self):
        ...
        dest_db2_helper = DB2(commcell=self.commcell,
                              ...
                              instance=self.instance,
                              backupset=backupset)
        dest_db2_helper.restore_table_validation(tables_info=
                                                 {f"SCHEMAFULL.{self.table_name}_FULL": self.table_data[0]})
        self.log.info("Verified Restore.")
        dest_db2_helper.close_db2_connection()

    def tear_down(self):
        """tear down function to delete automation generated data"""
        self.log.info("### Teardown for the test case 62266 ###")
        try:
