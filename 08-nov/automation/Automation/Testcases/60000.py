...
class TestCase(CVTestCase):
    """Class for verifying ZFS block backup and restore using SnapShot to Backup option"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Oracle ZFS Storage - NDMP iDA - ZFS block backup and restore using SnapShot to Backup option"
        self.tcinputs = {
            "HostName": None,
            "Agent": None,
            "FullSnapName": None,
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Run full ZFS backup job for NDMP iDA using SnapShot to Backup option")
            job = self.subclient.backup("FULL", block_backup=True, snap_name=self.tcinputs['FullSnapName'])
            self.log.info("Started full ZFS backup using SnapShot to Backup option with Job ID: %s", job.job_id)
            ...
            self.log.info("Successfully finished running full ZFS backup job using SnapShot to Backup option")
            ...
            if mamachine.get_logs_for_job_from_file(
                    job.job_id,
                    "NasBackup.log",
                    "Backup Snapshot\=YES|Block Backup|"):

                self.log.info(
                    'NDMP full backup job is run with block backup and snapshot to backup option')
            else:
                self.log.info(
                    'NDMP full backup job is NOT run with block backup and snapshot to backup option')
                raise Exception(
                    'NDMP full backup job is NOT run with block backup and snapshot to backup option')

            # Run Inc backup job for NDMP iDA
            self.log.info("Run Inc ZFS backup job for NDMP iDA using SnapShot to Backup option")
            job = self.subclient.backup("INCREMENTAL", block_backup=True, snap_name=self.tcinputs['IncSnapName'])
            self.log.info("Started inc ZFS backup using SnapShot to Backup option with Job ID: %s", job.job_id)
            ...
            self.log.info("Successfully finished running inc ZFS backup job using SnapShot to Backup option")
            ...
            if mamachine.get_logs_for_job_from_file(
                ...
                self.log.info(
                    'NDMP inc backup job is run with block backup and snapshot to backup option')
            else:
                self.log.info(
                    'NDMP inc backup job is NOT run with block backup and snapshot to backup option')
                raise Exception(
                    'NDMP inc backup job is NOT run with block backup and snapshot to backup option')

            # Run Diff backup job for NDMP iDA
            self.log.info("Run Diff ZFS backup job for NDMP iDA using SnapShot to Backup option")
            job = self.subclient.backup("DIFFERENTIAL", block_backup=True, snap_name=self.tcinputs['DiffSnapName'])
            self.log.info("Started Diff ZFS backup using SnapShot to Backup option with Job ID: %s", job.job_id)
            ...
            self.log.info("Successfully finished running Diff ZFS backup job using SnapShot to Backup option")
            ...
            if mamachine.get_logs_for_job_from_file(
                ...
                self.log.info(
                    'NDMP diff backup job is run with block backup and snapshot to backup option')
            else:
                self.log.info(
                    'NDMP diff backup job is NOT run with block backup and snapshot to backup option')
                raise Exception(
                    'NDMP diff backup job is NOT run with block backup and snapshot to backup option')
