...
6: Initiate DV2 for each of DDB Engines
...
11: Check whether they Bad Chunks Marking and DV Status Update for the jobs has worked as expected
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.name = 'Marking Referenced Chunks Bad by DV2 for DiskLibraries'
        self.tcinputs = {
            "MediaAgentName": None,
        }
        self.utility = None
    ...
    def produce_result(self):
        ...
        expected_chunk_mark_flags = [1, 1, 1, 1, 1, 1, -1, -1]
        ...
        expected_primary_records = [1, 1, 1, 1, 1, 1, 0, 0]
        self.result_string = ''
        for sub_case in range(1, 9):
            ...
            # except in referring jobs in CMD cases, in other cases, dv status should be marked failed
            if (sub_case < 6) \
                ...
                # changed in sp34 to not mark dv status as failed if sfile idx is missing
                case_string_2 += ': DV STATUS: VALIDATION: PASSED'
                self.log.info(case_string_2)
            elif (sub_case >= 7) \
                    and (expected_statuses[sub_case - 1] in self.verification_statuses[sub_case - 1]) \
