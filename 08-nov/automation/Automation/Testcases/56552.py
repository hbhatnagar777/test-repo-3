...
class TestCase(CVTestCase):
    """Class for executing Basic acceptance test of O365 Cleanup with source pruning
    combinations"""
    ...
    def __init__(self):
        ...
        self.name = "Office 365 Cleanup basic -- source pruning based on received time"
    ...
    def setup(self):
        ...
        self.log.info("Created the client")
        self._subclient = self.exmbclient_object.cvoperations.subclient
        self.log.info("Associated/ Initialized the Subclient")

        archive_policy_default = ARCHIVE_POLICY_DEFAULT % self.id
        cleanup_source_pruning_messages = CLEANUP_POLICY_PRUNING_MESSAGES % self.id
        cleanup_source_pruning_stubs = CLEANUP_POLICY_PRUNING_STUBS % self.id
        ...
        cleanup_policy_object = self.exmbclient_object.cvoperations.get_policy_object(
            cleanup_source_pruning_messages, "Cleanup")
        cleanup_policy_object.enable_message_rules = False
        cleanup_policy_object.prune_messages = True
        cleanup_policy_object.number_of_days_for_source_pruning = 0
        self.cleanup_source_pruning_messages = (
            self.exmbclient_object.cvoperations.add_exchange_policy(cleanup_policy_object))
        cleanup_policy_object = self.exmbclient_object.cvoperations.get_policy_object(
            cleanup_source_pruning_stubs, "Cleanup")
        cleanup_policy_object.enable_message_rules = False
        cleanup_policy_object.prune_stubs = True
        cleanup_policy_object.number_of_days_for_source_pruning = 0
        self.cleanup_source_pruning_stubs = (
            self.exmbclient_object.cvoperations.add_exchange_policy(cleanup_policy_object))
    ...
    def run(self):
        ...
        try:
            ...
            subclient_content_1 = {
                ...
                'cleanup_policy': self.cleanup_source_pruning_messages,
            }

            subclient_content_2 = {
                'mailboxNames': [self.mailboxes_list[1]],
                'archive_policy': self.archive_policy_default,
                'cleanup_policy': self.cleanup_source_pruning_stubs,
            }
