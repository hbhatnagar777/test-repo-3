...
TestCase:
 ...
 Steps:
    ...
    2. Run backups on subclient
    3. Save the primary id for DDB partition after running backup
    4. Run DR Backup
    5. Run more backups on subclient
    6. Save the primary id for DDB partition after running more backups
    7. Run DR Restore
    8. Verify that DR Resync Restore has taken place by verifying
        a. Maintenance Mode bit is not set on DDB
        b. IdxSIDBResyncHistory row has got added for the DDB with correct values
    9. Verify that Resync Range has got added
    10. Run Full backups
    11. Perform Restore and verify the content checksum
...
class TestCase(CVTestCase):
    ...
    def verify_resync_range(self):
        ...
        for partition in range(0, 2):
            if self.dedup_helper_obj.wait_till_sidb_down(str(self.sidb_store_obj.store_id), self.ma_client_obj):
    ...
    def perform_dr_restore(self):
        ...
        self.log.info("Sleeping for 3 mins")
        time.sleep(180)
        return returnval
    ...
    def perform_resync_validations(self):
        ...
        for itr in range(1, 16):
            ...
            if resync_history[0] != '' and int(resync_history[0]) == 1 :
