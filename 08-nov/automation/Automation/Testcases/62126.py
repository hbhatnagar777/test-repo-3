...
"""Test case to verify configuring Snap based VMWare replication group with backup copy enabled
...
"62126": {
            ...
            "source_vms": ["vmpath1", "vmpath2"]
            "recovery_target": "test-3",
            "snap_engine": "Virtual Server Agent Snap"
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Vmware - Snap based Zeal Replication group CRUD test with backup copy enabled"
    ...
    def setup(self):
        ...
        self.source_vms = [vmpath.split('/')[-1] for vmpath in self.source_vms_input]
        self.recovery_target = self.tcinputs["recovery_target"]
        self.snap_engine = self.tcinputs["snap_engine"]
        ...
        self._view_mode = VendorViewModes.VMWARE.VMs.value
        ...
        self._transportMode = VendorTransportModes.VMWARE.Automatic.value
        self._disk_provisioning = 'Original'
    ...
    def delete_replication_group(self):
        ...
        self.replication_helper.delete_replication_group(
            self.group_name, self.source_vms)
    ...
    def configure_replication_group(self):
        ...
        # Content
        vmware_configure.content.select_vm_from_browse_tree(self.source_vms_input, self._view_mode)
        vmware_configure.next()

        # Storage
        _storagecopy = vmware_configure.storage_cache.Storage_Copy
        vmware_configure.storage_cache.replicate_using_snapshots(self.snap_engine)
        vmware_configure.storage_cache.select_storage(self.primary_storage_name,
                                                      storage_copy=_storagecopy.Primary.value)
        vmware_configure.storage_cache.select_storage(self.secondary_storage_name,
                                                      storage_copy=_storagecopy.Secondary.value)
        vmware_configure.storage_cache.select_copy_for_replication(self.secondary_storage_name)
        vmware_configure.next()
    ...
    def verify_configuration(self):
        ...
        expected_content = {
            ...
            labels["label.snapshotEngineOnSource"]: self.snap_engine,
            labels["header.transportMode"]: self._transportMode,
            labels["label.diskProvisioning"]: self._disk_provisioning
        }
