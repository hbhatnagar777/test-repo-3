...
class TestCase(CVTestCase):
    """Class for executing Point-In-Time restore for informix from command center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Informix Point-In-Time restore from command center"
        self.browser = None
    ...
    def wait_for_job_completion(self, jobid):
        ...
        Args:
            jobid (int)         -- Job id
        Returns:
            job_end_time (str)  -- Job end time in unix time format
        """
        ...
        job_end_time = job_obj.summary['lastUpdateTime']
        return job_end_time
    ...
    def run_backup(self, backup_type=RBackup.BackupType.INCR):
        ...
        job_end_time = self.wait_for_job_completion(job_id)
    ...
    @test_step
    def restore_and_validate(self, metadata_backup, job_end_time):
        ...
        self.informix_helper_object.delete_test_data()
        self.log.info("Stop informix server to perform Point-In-Time "
                      "entire instance restore up to time %s", job_end_time)
        ...
        self.db_instance.select_instance(
            ...
            self.tcinputs["InstanceName"],
            self.tcinputs["ClientName"])
        self.log.info("Add 1 minute to time as seconds value is not included in date_picker()")
        job_end_time += 60
        browse_time = time.strftime('%d-%B-%Y-%H:%M:%S', time.localtime(job_end_time))
        restore_time = time.strftime("%m/%d/%Y %H:%M %p", time.localtime(job_end_time))
        date_time = dict()
        date_time['day'], date_time['month'], date_time['year'], date_time['time'] = browse_time.split("-")
        calender_obj = RCalendarView(self.admin_console)
        calender_obj.set_date_and_time(date_time)
        ...
        job_id = restore_panel.informix_restore('EntireInstance', logical=False,
                                                config_files=True, restore_time=restore_time)
        self.wait_for_job_completion(job_id)
        time.sleep(30)
        self.log.info("Perform logical only restore up to time %s", restore_time)
        restore_panel = self.subclient_page.restore_folders(DBInstances.Types.INFORMIX,
                                                            all_files=True)
    ...
    def run(self):
        ...
        try:
            ...
            job_end_time = self.run_backup()
            self.restore_and_validate(metadata_backup, job_end_time)
