...
TestCase:
    ...
    add_server() --  Function to create a GPFS server.
...
class TestCase(CVTestCase):
    """ Command center: Validate Archive and restore functionality for GPFS client from command center """
    test_step = TestStep()

    def __init__(self):
        """ Initializing the reference variables """
        super(TestCase, self).__init__()
        self.name = "Testcase to Validate Archive and restore functionality for GPFS client from command center"
        self.browser = None
    ...
    @test_step
    def add_server(self):
        """ Function to create a GPFS server. """
        self.navigator.navigate_to_archiving()

        self.archiving.add_distributed_app(pkg=DistributedClusterPkgName.GPFS, server_name=self.hostname,
    ...
    def setup(self):
        ...
        self.test_path = self.tcinputs.get("TestPath", f"/gpfs1/{self.id}")
    ...
    def run(self):
        ...
        2) Add GPFS server
        3) Run Archive job.
        4) Verify eligible files got stubbed.
        5) Run another Archive Job to backup the stubs.
        6) Recall stubs and verify. 	
        7) Restore the data with option restore data instead of stub selected and verify data files are restored.
        8) Verify restored data files.
        9) Restore the stubs with option restore data instead of stub deselected and verify stubs are restored.
        10) Recall and verify restored stub files.
        11) Delete Server.
        12) Delete Archive plan.
        """
        try:
