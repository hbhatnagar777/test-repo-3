...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Validate auto python shutdown in content analyzer machine using SDG FS"
        self.tcinputs = {
            "CAClient": None,
            "SDGClient": None,
            "IndexServer": None
    ...
    def create_plan_inventory(self):
        ...
        self.plans_obj.add_data_classification_plan(
            ...
            content_analyzer=[self.tcinputs['CAClient']],
    ...
    def setup(self):
        ...
        self.sdg_project_name = "AutoShutdownCA_Project_%s" % self.id
        self.plan_name = "AutoShutdownCA_TestPlan_%s" % self.id
        self.ds_name = "AutoShutdownCA_DataSource_%s" % self.id
        self.inv_name = "AutoShutdownCA_Inventory_%s" % self.id
        ...
        self.sdk_helper = ActivateSDKHelper(commcell_object=self.commcell)
        self.client_obj = self.commcell.clients.get(self.tcinputs['CAClient'])
        self.ca_helper.setup_autoshutdown(
            ca_client_name=self.tcinputs['CAClient'],
            shutdown_time=self._shutdown_time_in_mins)
        if not self.ca_helper.validate_autoshutdown(
                ca_client_name=self.tcinputs['CAClient'],
                shutdown_time=(dynamic_constants.AUTO_SHUTDOWN_DEFAULT_TIMER_TASK_DELAY +
                               self._shutdown_time_in_mins)):
            raise Exception(dynamic_constants.AUTO_SHUTDOWN_NOT_WORKING)
    ...
    def run(self):
        ...
        try:
            ...
            self.crawl_helper.monitor_job(commcell_object=self.commcell, job=job_id)

            # make sure python process are up and going down after sometime
            if not self.ca_helper.validate_autoshutdown(
                    ca_client_name=self.tcinputs['CAClient'],
                    shutdown_time=self._shutdown_time_in_mins):
                raise Exception(dynamic_constants.AUTO_SHUTDOWN_NOT_WORKING)
    ...
    def tear_down(self):
        ...
        if self.status == constants.PASSED:
            self.cleanup()
            machine_obj = Machine(machine_name=self.tcinputs['CAClient'], commcell_object=self.commcell)
