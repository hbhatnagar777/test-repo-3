...
class TestCase(CVTestCase):
    """Delete cs cache and validate Download Software job via Roles Manager"""

    def __init__(self):
        """Delete cs cache and validate Download Software job via Roles Manager"""

        super(TestCase, self).__init__()
        self.name = "Delete cs cache and validate Download Software job via Roles Manager"
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                raise Exception("Failed to delete software cache")
            ...
            # Checking whether Download Software Job is triggered
            download_flag = False
            time.sleep(15)
            job_id = None
            self.log.info("Checking active download jobs in CS")
            for jid, data in self.job_controller.active_jobs().items():
                if data['operation'] == 'Download Software':
                    job_id = self.job_controller.get(jid)
                    download_flag = True
                    self.log.info(f"Download software Job is successfully triggered with job id {job_id}")

            if not download_flag:
                raise Exception("Download job is not triggered from Roles Manager")
            ...
            # Checking for Install Job on CS
            time.sleep(15)
            for jid, data in self.job_controller.active_jobs().items():
