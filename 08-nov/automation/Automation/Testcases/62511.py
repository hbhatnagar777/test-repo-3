...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "CCM Merging VSA+FS Clients v2 to v2+FS"
        self.tcinputs = {
            "cs_hostname": None,
            "cs_username": None,
            "cs_password": None,
    ...
    def setup(self):
        ...
        self.dest_helper = CCMVSAHelper(dest_cs, False, 2, self.tcinputs)
        ...
        self.src_helper.setup_entities(vm=False, fs=True)
        self.src_helper.setup_job(vm=False, fs=True)

        self.dest_helper.setup_entities(vm=False, fs=True)
        self.dest_helper.setup_job(vm=False, fs=True)
    ...
    def run(self):
        ...
        errors = self.dest_helper.verify_jobs_merge(fs=True)
        if errors:
            for err in errors:
                self.log.error(err)
