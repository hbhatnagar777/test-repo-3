...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "V2 Hypervisor - Delete API validation for the VM which is in de-configure state with packages"
        ...
        try:
            ...
            try:
                ...
                for eachvm in auto_subclient.vm_list:
                    ...
                    res.retire()
                    client.delete(eachvm+'_1', forceDelete=False)
            except Exception as exp:
                VirtualServerUtils.decorative_log('Failed to delete which is expected: '+str(exp))
            #Validating if VM got deleted
            for eachvm in auto_subclient.vm_list:
                auto_commcell.statuscheck(1, 2,status = 'notdeleted', clientname = [eachvm+'_1'])
        except Exception as exp:
