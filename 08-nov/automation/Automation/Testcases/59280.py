...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Cloud Laptop]: OneDrive moniker backup"
    ...
    def run(self):
        ...
        try:
            ...
            if self.tcinputs['os_type']=='Mac':
                self.slash_format='/'
                subclient_content = ['/%Documents%', '/%Desktop%', '/%Pictures%', '/%OneDrive%']
                filter_content = ["/Library", "<WKF,Library>", "/%Temporary Files (Mac)%"]
                test_folder = laptopconstants.MAC_ONEDRIVE_PATH
                ...
                self.slash_format='\\'
                test_folder = laptopconstants.ONEDRIVE_PATH
                data_folder = test_folder + self.slash_format + "Inc1"    
                subclient_content = [r'\%Documents%', r'\%Desktop%', r'\%Pictures%', r'\%OneDrive%']
            ...
            SCENARIO-1:
                - Add new content under Onedrive path with txt files, doc files, xml files, docx files
            ...
            self._log.info("Generating test data for Onedrive under path: %s", str(test_folder))
            ...
            self._log.info("New content under Onedrive moniker verification completed")
            ...
            self._log.info("***** Validation of Onedrive moniker backup completed successfully *****")
