...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Index Server - Health Indicators - Validate Disk Indicators"
        self.tcinputs = {
            "IndexServer": None
        }
            ...
            for key in thresholds.keys():
                if key == indicator_type:
                    ...
                    for value in current_values.keys():
                        if not self.index_server_helper.validate_default_indicators(indicator_type, value,
                                                                                    current_values[value]):
                            self.log.info("Default Indicator values are not matched for indicator [{}]".format(value))
                            raise CVTestStepFailure("Default Indicator values are not matched")
    ...
    def validate_indicator_override(self):
        ...
        self.index_server_helper.set_validate_disk_indicator()
        self.index_server_helper.wait_for_summary_doc_sync()
        thresholds, action_codes = self.index_server_helper.get_index_server_health_summary()
        if len(action_codes) > 0:
            current_action_code = int(list(action_codes.keys())[0])
            if current_action_code == dynamic_constants.INDEX_SERVER_HEALTH_ACTION_CODE_LOW_DISK_SPACE:
                ...
                raise CVTestStepFailure(
                    "Expected action code [{}] is not set".format(
                        dynamic_constants.INDEX_SERVER_HEALTH_ACTION_CODE_LOW_DISK_SPACE))
    ...
    def cleanup(self, post_cleanup=False):
        ...
        try:
            ...
            self.index_server_helper.check_and_remove_indicators(
                ...
                dynamic_constants.INDEX_SERVER_HEALTH_DISK_SPACE_PERCENTAGE)
            self.index_server_helper.check_and_remove_indicators(dynamic_constants.ANALYTICS_REG_KEY,
                                                                 dynamic_constants.INDEX_SERVER_HEALTH_DISK_SPACE_BYTES)
            self.index_server_helper.wait_for_summary_doc_sync()
            if post_cleanup:
                self.index_server_helper.check_and_remove_indicators(
