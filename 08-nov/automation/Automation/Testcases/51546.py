...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = ("Basic acceptance test of Exchange Content"
                     " Indexing with preview generation")
        ...
        self.tcinputs = {
            ...
            "PreviewDetails": {"Path": None, "Username": None, "Password": None}
        }
    ...
    def setup(self):
        ...
        archive_policy_object.enable_content_index = True
        archive_policy_object.enable_preview_generation = True
        archive_policy_object.path = self.tcinputs['PreviewDetails']['Path']
        archive_policy_object.username = self.tcinputs['PreviewDetails']['Username']
        archive_policy_object.password = b64encode((self.tcinputs[
            'PreviewDetails']['Password']).encode()).decode()
    ...
    def run(self):
        ...
        try:
            ...
            if not solr.is_content_indexed(ci_item_cnt, True,
                                           self.tcinputs['PreviewDetails']['Path']):
                raise Exception("Content indexing failed")
