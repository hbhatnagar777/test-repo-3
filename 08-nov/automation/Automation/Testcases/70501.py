...
class TestCase(CVTestCase):
    """Class for executing OCI Full Backup and In-Place Restore case"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "OCI In Place Restore" 
    ...
    def run(self):
        ...
        try:
            ...
            self.vsa_obj.backup()
            try:
                decorative_log('Case 1: Performing FUll VM In-Place Restore with Unconditional Overwrite')
    ...
    def tear_down(self):
        try:
            if self.vsa_obj:
                self.vsa_obj.cleanup_testdata(update_vm_info=True)
        except Exception as exp:
