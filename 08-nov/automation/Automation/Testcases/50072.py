...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Windows Blocklevel  Cluster Data Protection"\
            ...
            for scan_type in ScanType:
                ...
                for index_type in IndexEnabled:
                    if scan_type.name == 'CHANGEJOURNAL':
                        ...
                        subclient_name = ("subclient_{}_{}_{}".format(self.id, scan_type.name.lower(), index_type.name.lower()))
                        subclient_content = []
                        ...
                        vlr_source = self.client_machine.join_path(str(test_path), str(self.runid))
                        vlr_destination = self.client_machine.join_path(str(self.restore_path), str(self.runid))
                        ...
                        self.helper.update_subclient(content=subclient_content,
                        ...
                        self.log.info(
                            "Step3.4, Run a Volume level Restore"
                            "and verify correct data is restored.")
                        self.helper.volume_level_restore(vlr_source, vlr_destination, \
                        ...
                        self.log.info("Step3.8, Run a Volume level Restore"
                                 "and verify correct data is restored.")
                        self.helper.volume_level_restore(vlr_source, vlr_destination, \
                                                    client_name=self.client_name)
                        self.log.info(
                        ...
                        job_incr1 = self.helper.run_backup_verify(
                            scan_type, "Incremental")[0]
                        self.log.info("Step3.12, Run a Volume level Restore"
                                 "and verify correct data is restored")
                        self.helper.volume_level_restore(vlr_source, vlr_destination, \
                                                    client_name=self.client_name)
                        self.log.info(
                        ...
                        job_incr1 = self.helper.run_backup_verify(
                            scan_type, "Incremental")[0]
                        self.log.info("Step3.16, Run a Volume level Restore"
                                 "and verify correct data is restored.")
                        self.helper.volume_level_restore(vlr_source, vlr_destination, \
                                                    client_name=self.client_name)
                        ...
                        job_incr1 = self.helper.run_backup_verify(
                            scan_type, "Incremental")[0]
                        self.log.info("Step3.20, Run a Volume level Restore"
                                 "and verify correct data is restored.")
                        self.helper.volume_level_restore(vlr_source, vlr_destination, \
                                                    client_name=self.client_name)
                        # Compare Source and Destination and check files restored.
                        ...
                        job_incr1 = self.helper.run_backup_verify(
                            scan_type, "Incremental")[0]
                        self.log.info("Step3.24, Run a Volume level Restore"
                                 "and verify correct data is restored.")
                        self.helper.volume_level_restore(vlr_source, vlr_destination, \
                                                    client_name=self.client_name)
                        # Compare Source and Destination and check files restored.
                        ...
                        self.log.info("Step3.27, Run a Volume level Restore"
                                 "and verify correct data is restored.")
                        self.helper.volume_level_restore(vlr_source, vlr_destination, \
                                                    client_name=self.client_name)
                        # Compare Source and Destination and check files restored.
