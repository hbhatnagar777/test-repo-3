...
class TestCase(CVTestCase):
    """Class for executing VMWare Tags Automation Test Case"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "VMWare - Tags - Automation"
    ...
    def run(self):
        ...
        try:
            ...
            try:
                decorative_log('Case 1: Performing Full VM Restore without adding/removing Tags')
                ...
                for vm_name in source_vm_list:
                    ...
                    if not self.vsa_obj.hvobj.VMs[restore_vm_name].validate_tags(self.vsa_obj.hvobj.VMs[vm_name], \
                                                                                 self.vsa_obj.tags_to_add, self.vsa_obj.tags_to_remove):
            ...
            try:
                self.vsa_obj.tags_to_add = self.tcinputs['tags_to_add']
                self.vsa_obj.tags_to_remove = self.tcinputs['tags_to_remove']
                decorative_log('Case 2: Adding Tags {} and Removing Tags {} to/from the VMs and performing validation.'\
                               .format(self.vsa_obj.tags_to_add, self.vsa_obj.tags_to_remove))
                self.vsa_obj.full_vm_restore()
                for vm_name in source_vm_list:
                    restore_vm_name = self.vsa_obj.vm_restore_prefix + vm_name
                    self.vsa_obj.hvobj.VMs = restore_vm_name
