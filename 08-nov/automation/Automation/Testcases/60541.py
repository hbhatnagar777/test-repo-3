...
class TestCase(CVTestCase):
    """Test for setting automatic owner assignment to the local user groups by installing using the always activate
    flag, such that the users in the local groups specified should become the laptop owners"""

    test_step = TestStep()

    def __init__(self):
        super(TestCase, self).__init__()
        self.name = "[Laptop] [Admin Console]: Test for setting automatic owner assignment to the local user groups by\
        installing using the always activate flag, such that the users in the local groups specified should become the\
        laptop owners"
        self.browser = None
    ...
    def verify_owner_assignment_config(self, want_ownership_type=0):
        ...
        try:
            ...
            ownership_type = self.commcell.verify_owner_assigment_config()["commCellInfo"]["generalInfo"][
    ...
    @test_step
    def install_and_activate_with_activate_flag(self):
        """
            install and activate laptop with always activate flag set
        """
    ...
    def run(self):
        ...
        try:
            ...
            self.install_and_activate_with_activate_flag()
    ...
    def refresh(self, install_with_authcode=False, authcode=None):
        ...
        self.install_kwargs = {
            ...
            'expected_owners': self.tcinputs["User_Group_Members_As_Owners"]
        }
