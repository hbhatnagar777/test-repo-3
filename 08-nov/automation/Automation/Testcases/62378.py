...
"""Main file for executing this test case
     ...
     This Testcase verifies the negative scenarios where RFC operations are involved for recovery
...
TestCase:
    ...
    keep_same_is_for_all_vms()                  -- To assign the same index server for all the VMs that are to
                                                   be backed up
...
class TestCase(CVTestCase):
    ...
    Steps:
        ...
        2.  Have two datapaths for the storage policy and run a full backup.
        3. Scenario 1: During the backup, stop the services on  RFC/Index Sever.
        4. Verify that the other datapath acts as the RFC server.
        5. Validate if RFC files are uploaded to the new RFC cache location on the new
           RFC server.
        6. Verify if the afile for this backup is valid.
        7. Perform browse and verify download of RFC.
        8. Scenario 2: Use one datapath MA for snap job and the other one for backup copy job and verify all RFC
           operations by repeating steps 5,6,7.
        10. Scenario 3: Run a FULL snap and backup job with one datapath MA, Run an INC snap and backup copy job
            with another datapath MA. Repeat steps 5,6,7
...
"""
    ...
    def __init__(self):
        ...
        self.name = 'Indexing - RFC - Negative Scenario Validation'
        self.tcinputs = {
            'IndexServerUserName': None,
            'IndexServerPassword': None,
        }
        self.idx_help = None
        ...
        self._cv_vms = {}
        self.rfc_servers = {}
        self.index_server_username = None
        self.index_server_password = None
    ...
    def setup(self):
        ...
        self.index_server_username = self.tcinputs['IndexServerUserName']
        self.index_server_password = self.tcinputs['IndexServerPassword']
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info('******** Scenario 1: Index Server is down *********')
            is_ma_s1, snapcopy_ma_s1 = self.switch_datapaths_change_is(keep_same=False)
            ma_index_server_name = is_ma_s1.machine_name
            cl_is = self.auto_commcell.commcell.clients.get(ma_index_server_name)
            is_username = self.tcinputs.get(f'{ma_index_server_name}UserName', self.index_server_username)
            is_password = self.tcinputs.get(f'{ma_index_server_name}Password', self.index_server_password)
            index_server_ma = Machine(
                    machine_name=cl_is.client_hostname,
                    username=is_username,
                    password=is_password
                )
            self.log.info('Stopping services on Index server: %s', ma_index_server_name)
            cl_is.stop_service(service_name=f'GxCVD({cl_is.instance})')
            time.sleep(60)
            ...
            self.validate_rfc_operations(
                ...
                is_machine=snapcopy_ma_s1,
                verify_download=False
            )

            self.log.info('Starting services on Index server: %s', ma_index_server_name)
            index_server_ma.start_all_cv_services()
            if not cl_is.is_ready:
                raise Exception(f'Failed to start services on {ma_index_server_name} to continue with next scenario')

            self.log.info('******** Scenario 2: Snap job with MA1 and backup copy with MA2(index server) ********')
            is_ma_s2, snapcopy_ma_s2 = self.switch_datapaths_change_is(keep_same=False)
            self.log.info('********** Running FULL backup **********')
            ...
            self.log.info('******** Scenario 3: FULL with MA1 and INC with MA2 ********')
            is_ma_s3, snapcopy_ma_s3 = self.switch_datapaths_change_is(keep_same=True)
            self.log.info('********** Running FULL backup **********')
            full_job_s3 = self.run_backup(backup_level='FULL')
            self.validate_rfc_operations(
                parent_job=full_job_s3,
