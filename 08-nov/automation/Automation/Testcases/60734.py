...
TestCase:
    ...
    vaildate_data()                 --  Validates data after restore
...
class TestCase(CVTestCase):
    """ Basic acceptance Test for Amazon S3 Cloud Object Storage from command center
            ...
            "60734": {
                ...
                "role_arn": "ROLE_ARN"
                "key_file_path": "JSON KEYFILE PATH OF GCP",
                "bucket_name": "auto-bucket",
                "url": "s3.amazonaws.com",
                "content": ["/auto-bucket/full"],
                "s_content" : "Content for the new subclient"
                "edit_content": ["/auto-bucket/incr"],
                "dest_cloud": "CROSS CLOUD DESTINATION CLIENT ON CS",
                "dest_cloud_path": "/cv-auto-bucket",
                "dest_fs_client": "DESTINATION CLIENT ON CS TO restore to disk"
            }
    """
    ...
    def __init__(self):
        ...
        self.name = "ACC1 for Amazon S3 Object Storage STS Assume Role from command center"
        self.browser = None
    ...
    def setup(self):
        ...
        self.s3_session = self.s3_storage_helper.create_session_s3(self.tcinputs["access_key_s3"],
    ...
    def create_s3_objstorage_client(self):
        ...
        self.credential_name = self.object_storage.add_object_storage_client(
            ...
            role_arn=self.tcinputs.get('role_arn'),
    ...
    def out_of_place_restore(self):
        ...
        self.db_helper.wait_for_job_completion(jobid)
        self.log.info("Restore job ran Successfully")
        self.restored_data_path = self.controller_object.join_path(self.oci_oss_helper.common_dir_path, 'out_of_place')
        self.log.info("Downloading bucket started........")
        self.oci_oss_helper.download_bucket(self.destination_bucket, "out_of_place")
    ...
    def cleanup(self):
        ...
        if self.is_oci_bucket_created:
            self.oci_oss_helper.delete_bucket(self.destination_bucket)
            self.oci_oss_helper.oci_cleanup()

        if self.is_automation_account:
            self.navigator.navigate_to_object_storage()
            self.admin_console.wait_for_completion()
            if self.object_storage.is_account_exists(ObjectStorage.Types.Amazon_S3, self.object_storage_name):
                self.object_storage.delete_object_storage_client_and_credentials(client_name=self.object_storage_name,
                                                                                 credential_name=self.credential_name)
