...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Broken permission report :Orphan user permission case"
        self.tcinputs = {
            "HostToAnalyze": None,
    ...
    def create_ad_user(self):
        """Creates a basic AD user"""
        pyad.set_defaults(ldap_server=self.tcinputs["Ldap"], username=self.tcinputs["ServiceUser"],
                          password=self.tcinputs["ServicePassword"])
        ldap_split = self.tcinputs["Ldap"].split(".")
        dc_path = ",".join(["DC="+i for i in ldap_split])
        ou = adcontainer.ADContainer("CN=Users,"+dc_path)
        self.new_user = aduser.ADUser.create("BrokenUser", ou, "DIVault!12")
    ...
    def create_broken_permission_folder(self):
        ...
        random_user = self.new_user.prefixed_cn[3:]
        random_sid = win32security.ConvertSidToStringSid(self.new_user.sid)
        ...
        self.modified_permissions[random_sid] = new_basic_permissions
    ...
    @test_step
    def remove_ad_user(self):
        """Removes AD user from the active directory"""
    ...
    def cleanup(self):
        """Performs cleanup"""
        if self.new_user:
            self.log.info("Removing the New user created")
            self.remove_ad_user()
            self.log.info("User removed")
        ...
        if self.machine_obj.check_directory_exists(self.root_path):
            ...
            try:
                ...
                self.log.info("Crawl data deletion failed, will delete on next run")
    ...
    def create_fso_client(self):
        ...
        self.fso_helper.file_server_lookup.add_file_server(
            ...
            self.fso_datasource_name, INDIA_COUNTRY_NAME,
    ...
    def validate_permissions_on_filer(self):
        ...
        fixed_permissions: dict = self.activate_utils.get_access_control_list(
            path=self.broken_folder_unc,
            target_machine_name=self.machine_name)
        for permission in BASIC_WINDOWS_PERMISSIONS:
            if len(fixed_permissions) != len(self.original_permissions):
                self.log.info("Broken folders still found on the file system")
                raise Exception("Fix permissions job failed to fix permissions")
            for user in fixed_permissions:
                if permission in fixed_permissions[user] and permission not in self.original_permissions[user]:
                    self.log.info("Broken folders still found on the file system")
                    raise Exception("Fix permissions job failed to fix permissions")
