...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Index Server - Health Indicators - Validate CPU Indicators"
        self.tcinputs = {
            "IndexServer": None
        }
    ...
    def validate_indicator_override(self):
        ...
        self.index_server_helper.set_validate_cpu_indicator()
        self.index_server_helper.wait_for_summary_doc_sync(extended_wait=True)
        thresholds, action_codes = self.index_server_helper.get_index_server_health_summary()
        if len(action_codes) > 0:
            current_action_code = int(list(action_codes.keys())[0])
            if current_action_code == dynamic_constants.INDEX_SERVER_HEALTH_ACTION_CODE_CPU_HIGH_INCREASE_VCPUS:
                ...
                raise CVTestStepFailure(
                    "Expected action code [{}] is not set".format(
                        dynamic_constants.INDEX_SERVER_HEALTH_ACTION_CODE_CPU_HIGH_INCREASE_VCPUS))
    ...
    def cleanup(self, post_cleanup=False):
        ...
        try:
            ...
            self.index_server_helper.check_and_remove_indicators(
                ...
                dynamic_constants.INDEX_SERVER_HEALTH_CPU_USAGE_MEDIAN_THRESHOLD)
            self.index_server_helper.check_and_remove_indicators(
                dynamic_constants.ANALYTICS_REG_KEY,
                dynamic_constants.INDEX_SERVER_HEALTH_MAX_CPU_CORES_THRESHOLD)
            self.index_server_helper.check_and_remove_indicators(
                dynamic_constants.ANALYTICS_REG_KEY,
                dynamic_constants.INDEX_SERVER_HEALTH_MAX_CPU_USAGE_THRESHOLD)
