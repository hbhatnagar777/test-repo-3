...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Metallic GuardRail - AWS cloud vm interactive install & backup using existing aws gateway and S3 storage"
        self.browser = None
        self.fs_obj = None
    ...
    def configure_permission(self):
        ...
        self.gatewayname = self.gateway.display_name + " (" + self.tcinputs["GatewayFQDN"] + ")"
        self.metallic_fs.configure_AWS_permission()

    @test_step
    def select_region(self):
        """configure AWS region"""
        self.metallic_fs.configure_region([self.tcinputs["Region"]])

    @test_step
    def select_backup_gateway(self):
        """ configure backup gateway"""
        self.metallic_fs.select_backup_gatway(gatewayname=self.gatewayname)
    ...
    def configure_server(self):
        ...
        #self.metallic_fs.add_new_server(self.installinputs, cloudmachine=True)
        self.metallic_fs.add_new_server_silent_install(self.installinputs)
        self.commcell.refresh()
        self.client = self.commcell.clients.get(self.tcinputs["EC2FQDN"])
        self.client_name = self.client.display_name

        self.client_machine = Machine(
            self.client_name,
            self.commcell,
            username=self.tcinputs.get("EC2UserName"),
            password=self.tcinputs.get("EC2Password"))
    ...
    def set_installinputs(self, isClient=True):
        ...
        try:
            if "windows (32-bit)" in self.os_name.lower():
                ...
                inputs["registering_user_password"] = self._inputJSONnode['commcell']['commcellPassword']
            else:
                inputs["full_package_path"] = (
            ...
            if len(self.tcinputs.get("EC2PublicIP")) != 0: 
                inputs["remote_clientname"] = self.tcinputs.get("EC2PublicIP")
            else:
                inputs["remote_clientname"] = self.tcinputs.get("EC2PrivateIP")
            #inputs["cloudvm_privateip"] = self.tcinputs.get("EC2FQDN")
            inputs["commcell_client_name"] = self.tcinputs.get("EC2FQDN")
            inputs["remote_username"] = self.tcinputs.get("EC2UserName")
            inputs["remote_userpassword"] = self.tcinputs.get("EC2Password")
