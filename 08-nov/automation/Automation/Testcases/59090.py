...
Sample Input:
        ...
        PartitionPath is an optional parameter, use this as input for Unix MA.
"""
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "StorageAccelerator(SA)_Cloud_TestCase (Unix Client)"
        self.tcinputs = {
            "MediaAgentName": None,
            "CloudLibrary": None,
            "ClientName": None
    ...
    def _validate_sapackage(self, clientid):
        """
        Validate if a unix client had storage accelerator package installed or not.
        ...
        """
        query = f""" select COUNT(1) from simInstalledPackages where simPackageID = 1305
    ...
    def _validate_mapackage(self, clientid):
        """
        Validate if unix client also has mediaagent package installed or not.
        Args:
            Clientid -- This is the client id for which we are verifying packages installed.
        Return:
            (Bool) True if it exists
            (Bool) False if doesn't exists
        """
        query = f""" select count(1) from simInstalledPackages where simPackageID = 1301
                        and ClientId = {clientid}"""
        self.log.info(f"QUERY: {query}")
        self.csdb.execute(query)
        cur = self.csdb.fetch_one_row()
    ...
    def _sa_global_param(self):
        """
        Validate if media management service configuration parameter "MMCONFIG_CONFIG_STORAGE_ACCELERATOR_ENABLED" is
        enabled or disabled.
        Args:
            ...
            for sequence_index in range(0, 4):
                ...
                if job_types_sequence_list[sequence_index] != 'synthetic_full':
                    ...
                    if not self.client_machine.generate_test_data(self.content_path1, dirs=1, file_size=(20 * 1024),
                                                                  files=2, hlinks=False, slinks=False,
                                                                  sparse=False, zero_size_file=False):
