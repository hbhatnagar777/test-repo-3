...
class TestCase(CVTestCase):
    """Class for executing VM Groups CRUD case"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Oracle VM Groups CRUD test"
    ...
    def run(self):
        try:
            decorative_log("Adding VM group content: ".format(self.tcinputs['BackupContent']))
            ...
            self.vmgroup_obj.set_vm_group_content(vendor=HypervisorDisplayName.Nutanix.value,
                                                  vm_content=self.tcinputs['BackupContent1'],
            ...
            self.admin_console.navigator.navigate_to_vm_groups()
            self.vsa_obj.validate_vmgroup(vmgroup_name=self.vm_group_name,
                                          validate_input={"vm_group_content": self.tcinputs['BackupContent1']})

            decorative_log("Deleting VM group")
            self.admin_console.navigator.navigate_to_vm_groups()
            self.vmgroup_obj.action_delete_vm_groups(self.vm_group_name)

            decorative_log("checking for deleted VM group")
            self.admin_console.navigator.navigate_to_vm_groups()
