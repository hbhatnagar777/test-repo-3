...
class TestCase(CVTestCase):
    """ Testcase to perform Cross cloud Database level restores from GUI for PAAS PostgreSQL
        Example:
        "62520" : {
          "google_clientName": "",
          "ali_clientName": "",
    ...
    def __init__(self):
        ...
        self.name = "Cross cloud Database level restores for PAAS PostgreSQL from GUI"
        self.db_list = []
        self.subclients_object = None
        self.db_helper = None
        self.db_prefix = 'cloud_automation' + str(int(time.time()))
    ...
    def generate_test_data(self, pg_helper_object, pg_db_object):
        ...
        """
        postgres_data = self.tcinputs.get('testdata', [1, 1, 10])
        ...
        self.db_list = [db for db in pg_db_object.get_db_list() if db.startswith(self.db_prefix)]
    ...
    def restore(self, subclient, dest_client, dest_instance, pg_helper_object):
        ...
        Args:
            ...
            pg_helper_object    (obj)   --  postgres helper object
        """
        db_list = ["/" + db for db in self.db_list]
        self.log.info("Databases to be restored are {0}".format(db_list))
        pg_helper_object.run_restore(paths=db_list,
                                     subclient=subclient,
                                     is_dump_based=True,
    ...
    def validate_azure_metadata(self, before_backup, after_restore, is_source=False):
        ...
        Args:
            before_backup   (dict)  --  metadata collected before backup of db on src
            after_restore   (dict)  --  metadata collecetd after restore of db to dest
            is_source       (bool)  --  True if source/destination is Azure
        """
        for db1 in before_backup.keys():
            if db1 not in after_restore.keys():
                raise Exception("Database Validation Failed!")
            else:
                tables_db1 = before_backup[db1]
                tables_db2 = after_restore[db1]
                for index in range(0, max(len(tables_db1), len(tables_db2))):
                    src = set(tuple(tables_db1[index]))
                    dest = set(tuple(tables_db2[index]))
                    if is_source:
    ...
    def run(self):
        ...
        try:
            ...
            # Use same version of postgres instances in all clouds(Pg v11 IDEALLY - Azure limits this)
