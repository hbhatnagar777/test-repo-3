...
class TestCase(CVTestCase):
    """ Basic acceptance Test for Aws Aurora postgres metallic
    Example :
    "65540": {
				"Region": "us-east-1",
				"InstanceName": "aurora/pgdb3[us-east-1]",
				"DatabaseUser": "postgres",
				"Password": "pgdbrs22",
				"MaintenanceDb": "postgre",
				"testdata": [2,2,10],
				"key-pair":"Shawn_test",
				"vpc":"vpc-0bc0c93426db0f576",
				"subnet":"subnet-039c50d54c2155537",
				"OS_type":  "Linux",
				"BucketName" : "shawn321-test",
				"Hostname" : "pgdb3.cluster-cfm3msdcsraf.us-east-1.rds.amazonaws.com",
				"Clustername" : "pgdb3[us-east-1]"
    ...
    def __init__(self):
        ...
        self.name = "Metallic - ACC1 for AWS RDS(Export) Aurora PostgreSQL using IAM Role"
        self.browser = None
    ...
    def setup(self):
        ...
        self.comcell_admin = Commcell(webconsole_hostname=ring_hostname,
                                      commcell_username='cvautoexec',
                                      commcell_password=_CONFIG_DATA.Metallic.workflow_password)
        ...
        self.awsdatabases = AWSRDSExport(self.admin_console, s3_helper=self.s3_metallic_helper,
                                         ...
                                         database_type=DBInstances.Types.AURORA_POSTGRES.value)

    @test_step
    def start_rds(self):
        """Method to start RDS instance if it's present and in stopped state"""
        self.region = self.tcinputs['Region']
        self._instance_identifier = self.tcinputs["Clustername"][:self.tcinputs["Clustername"].index('[')]
    ...
    def create_and_update_bgw(self):
        ...
        self.stack_name = "awsmetallicRDSpostgres" + datetime.now().strftime("SJ%d%B%H%M")
        ...
        self.log.info(self.bgw_name, " created and updated successfully.")

    @test_step
    def create_instance(self):
        """ Method to create an instance"""
        self.cloud_account = f"automation-AWSpgsql-{self.timestamp}"
        self.plan = f"AWS_pgsql_plan" + datetime.now().strftime("SJ%d%B%H%M").lower()
        ...
        self.awsdatabases.configure(region=self.tcinputs.get("Region"),
                                    ...
                                    maintenance_db=self.tcinputs["MaintenanceDb"],
                                    is_rds_export=True)
    ...
    def set_postgres_helper_object(self):
        ...
        self.log.info("Creating Aurora PostgreSQL Helper Object")
        self.commcell.refresh()
        _client = self.commcell.clients.get(self.cloud_account)
        _agent = _client.agents.get('PostgreSQL')
        _instance = _agent.instances.get(self.tcinputs['InstanceName'])

        self.postgres_helper_object = pgsqlhelper.PostgresHelper(
            commcell=self.comcell_admin,
            client=_client,
            instance=_instance)
    ...
    def get_metadata(self):
        ...
        return self.postgres_helper_object.generate_db_info(
            db_list=self.generated_database_list,
    ...
    def add_db_group(self, plan_name):
        ...
        self.db_instance_details.click_add_subclient(DBInstances.Types.POSTGRES)
    ...
    def run(self):
        ...
        try:
            ...
            if result:
                self.log.info("Amazon RDS Aurora PostgreSQL Backup and Restore Successful!")
            else:
                self.log.info("Amazon RDS Aurora PostgreSQL Backup and Restore Failed!")
