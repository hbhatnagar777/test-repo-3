...
class TestCase(CVTestCase):
    """Class for executing Basic acceptance Test of Amazon VPC streaming: File level backup and restore"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "AMAZON : Verify config file level streaming backup and restore to windows and linux access nodes."
        self.browser = None
        self.vsa_obj = None
    ...
    def run(self):
        """AWS instance config file level streaming backup and restore to windows and linux access nodes"""
        try:
            decorative_log("Running a streaming backup")
            self.vsa_obj.backup_type = "FULL"
            self.vsa_obj.backup()
            try:
                self.instance_file_restore()
            except Exception as _exception:
                decorative_log("Instance configuration file restore failed")
