...
The tape library should be as following
    ...
    Drive Controller : MA1, MA2

Library and drive controller should be 2 same MAs
...
class TestCase(CVTestCase):
    ...
    def setup(self):
        ...
        if not len(self.MediaAgents) > 1:
            raise Exception("Multiple MediaAgents are expected.")
    ...
    def delete_default_tape_pool(self, lib_name):
        """ Cleanup default tape pool"""
        self.log.info("Checking if automatically created storage pool exists with the library name")
        if self.commcell.storage_policies.has_policy(lib_name):
            self.log.info(f"Storage Pool [{lib_name}] exists, deleting that")
            self.commcell.storage_pools.delete(lib_name)
    ...
    def cleanup(self,):
        ...
        # This block is for this run 
        if self.tape_lib_obj:
            self.log.info("Deleteing the tape librray created")
            self.log.info("Unloading medias if loaded")
            self.unload_all_drives(self.tape_lib_obj)
            self.log.info(f"Deleting the library {self.tape_lib_obj.library_name}")
            self.commcell.tape_libraries.delete(self.tape_lib_obj.library_name)
            return
            
        # This block is to delete the library of previous run
        # This block is not required if tape already created for this run
        self.log.info("Checking if tape library exists from previous run")
        for lib_name, lib_id in self.commcell.tape_libraries._libraries.items():
            if lib_name.startswith(self.library_name.lower() + " "):
                self.log.info(f"Configured tape library found. Library [{lib_name}]")
                self.delete_default_tape_pool(lib_name)
                self.log.info("Unloading medias if loaded")
                old_tape_library_obj = self.commcell.tape_libraries.get(lib_name)
                self.unload_all_drives(old_tape_library_obj)
                self.log.info(f"Deleting the library {old_tape_library_obj.library_name}")
                self.commcell.tape_libraries.delete(old_tape_library_obj.library_name)
                
    def unload_all_drives(self, tape_lib_obj_local):
        """
        Unload the drives of the tape
        """
        self.log.info(f"Unloading drives for tape library [{tape_lib_obj_local._name}]")
        drive_list = tape_lib_obj_local.get_drive_list()
        self.log.info("Drive List"+ str(drive_list))
        
        if drive_list != None and len(drive_list)>0:
            for drive in drive_list:
                self.ma_helper.unload_drive(tape_lib_obj_local._name, drive)
                self.log.info("Waiting for 30 seconds for the unload request to complete")
                time.sleep(30)
    ...
    def wait_for_job_complete(self, timeout=60*60):
        ...
        job_details = self.job_obj._get_job_details()
    ...
    def verify_mmlibrary_table(self, after_delete=True):
        ...
        query = "select count(*) from mmlibrary where aliasname='" \
    ...
    def verify_MMDrivePool_table(self, after_delete=True):
        ...
        self.log.info("Verfying MMDrivePool table")

        i=0
        while (i < len(self.MediaAgents)):
            query ="select count(*) from MMDrivePool where DrivePoolName='"+self.drive_pools[self.MediaAgents_obj[self.MediaAgents[i]].media_agent_id]+"'"
    ...
    def verify_MMMasterPool_table(self, after_delete=True):
        ...
        for mp in self.master_pools.values():
            query ="select count(*) from MMMasterPool where MasterPoolName='{0}'".format(mp)
            self.log.info("Executing the following query")
            self.log.info(query)
            self.csdb.execute(query)
            count = self.csdb.fetch_one_row()[0]
    ...
    def verify_MMDrives_table(self, after_delete = True):
        ...
        for master_pool_id, master_pool_name in self.master_pools.items():
            ...
            query = "select count(*) from mmdrive where masterpoolid={0}".format(master_pool_id)
            self.log.info("Executing the following query")
            self.log.info(query)
            count = self.csdb.fetch_one_row()[0]
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Sleeping for 30 sec so that all reservation released")
            time.sleep(30)
