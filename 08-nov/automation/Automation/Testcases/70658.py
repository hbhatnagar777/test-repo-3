...
class TestCase(CVTestCase):
    """Class for executing Basic acceptance test of SQL Server restore with STANDBY test case"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "SQL - Data Recovery - Restore with STANDBY"
    ...
    def run(self):
        ...
        try:
            ...
            # drop databases
            if not self.sqlhelper.dbinit.drop_databases(self.sqlhelper.dbname + str(1)):
                self.log.error("Unable to drop the dataBase")            

            # run restore in place job - based on TL time in STANDBY
            self.log.info("*" * 10 + " Run Restore in place with STANDBY" + "*" * 10)
            if not self.sqlhelper.sql_restore(
                    self.subclient.content,
                    to_time=tlend,
                    sql_recover_type=sqlconstants.STATE_STANDBY,
                    undo_path=self.sqlhelper.tcdir
            ):
                raise Exception("Restore was not successful!")

            # Checking if databases are online
            if not self.sqlhelper.dbvalidate.is_db_standby(self.sqlhelper.dbname + str(1)):
                raise Exception("Databases is not in standby state.")
