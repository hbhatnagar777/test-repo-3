...
Inputs for smart topology (Optional)
    ...
    MediaAgent : A media agent which will be used for
...
class TestCase(CVTestCase):
    """Command Center- Network topology- Network Gateway(Laptops)"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Command Center- Network topology- Network Gateway(Laptops)"
        self.browser = None
        self.topology_name = "CG_60339_NETWORK_GATEWAY"
        self.admin_console = None
        self.navigator = None
        self._client_group_name1 = "CG_60339_LAPTOP"
        self._client_group_name2 = "CG_60339_INFRASTRUCTURE"
        self._client_group_name3 = "CG_60339_PROXY"
        self.tcinputs = {
            "LaptopClient": "",
    ...
    def init_tc(self):
        ...
        try:
            ...
            self.server_client = self.tcinputs['LaptopClient']
            ...
            self.Server_obj = self.commcell.clients.get(self.tcinputs['LaptopClient'])
    ...
    def pre_check_summary(self):
        """Validate if there are no routes present before the proxy topology"""
        self.get_summaries()
    ...
    def validate_no_routes(self, client_list, summary_list):
        """A function to validate that there are no routes between the client groups"""
        ...
        # Verify the persistent route and a proxy route in the laptop
        for i in cl_summary:
            if client_list[0] + " " + client_list[2] in i and "type=persistent" in i:
                self.log.info(f'Persistent route in {client_list[0]}')
                server_check += 1
            if client_list[0] + " " + '*' in i and "proxy=" + client_list[2] in i:
                self.log.info(f'Proxy route in {client_list[0]} ')
                server_check += 1

        # Verify the two passive routes in proxy summary
        ...
        # Verify the persistent route and proxy route in infrastructure summary
    ...
    def create_network_gateway(self, client_group_list, client_list):
        ...
        try:
            ...
            self.networkpage.add_topology(self.topology_name, "Network gateway", client_group_list, "clienttype", wildcard_proxy=True)
            ...
            self.validate_summary(self.summary_list[1], client_list[1], client_list[0], 2,
                                  ...
                                  Proxy=client_list[2], Protocol='httpsa')
            # Validate the network summary of the Proxy
    ...
    def configure_multi_streams(self, client_group_list, client_list):
        ...
        try:
            # Configure the multi streams - number of tunnels to 4
            ...
            self.validate_summary(self.summary_list[1], client_list[1], client_list[0], 2,
                                  ...
                                  Proxy=client_list[2], Protocol='httpsa')
            # Validate the network summary of proxy
            self.validate_summary(self.summary_list[2], client_list[1], client_list[0], 3,
    ...
    def validate_encrypt_network(self, client_group_list, client_list):
        ...
        try:
            ...
            # Enable the encrypt network traffic toggle
            self.networkpage.edit_topology(self.topology_name, None, EncryptTraffic=True)

            # Update the network summary after editing topology
            self.get_summaries()
    ...
    def delete_topology_validate(self):
        ...
        # Delete topology in the browser
    ...
    def validate_smart_topology(self):
        ...
        try:
            ...
            self.log.info("Creating the smart topology with My CommServe Computer")
            self.networkpage.add_topology(self.topology_name, "Network gateway", self.client_group_list, "clienttype")
            ...
            self.log.info("Edit the topology to smart group My CommServe and Media agents")
            ...
            self.log.info("Edit the topology to smart group My MediaAgents")
            self.networkpage.edit_topology(self.topology_name, [cg1], CgChanges=True)
            validate_summaries()
            ...
            self.log.info("Edit the topology to regular group")
            self.networkpage.edit_topology(self.topology_name, [cg1], CgChanges=True)
            validate_summaries()
        except Exception as exception:
            raise CVTestStepFailure(exception)
