...
class TestCase(CVTestCase):
    """ [Laptop] [AdminConsole]: Restore for Laptop """
    ...
    def __init__(self):
        ...
        self.name = "[Laptop] [AdminConsole]: Restore for Laptop"
    ...
    def run(self):
        ...
        try:
            ...
            self.laptop_helper.tc.log_step("""
                ...
                5. Verify Restore from actions and details page with auto trigger backup job
            ...
            self.laptop_helper.tc.log_step("Laptop Restore Validation started")
            if self.tcinputs['os_type'] == 'Windows':
                osc_options=None
            else:
                osc_options='-testuser root -testgroup admin'

            _jobobj , backup_job_id = self.laptop_obj.auto_trigger_backupjob(options=osc_options)
            self.laptop_helper.tc.log_step("Step1: Restore Verification from Laptop actions")
            ...
            self.laptop_helper.tc.log_step("Step2: Restore verification from details page ")
            self.laptop_obj.navigate_to_laptops_page()
            self.laptop_obj.subclient_restore(backup_job_id, restore_type='restore_from_detailspage')
