...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Restartable backups - suspend/resume and restores from Flexgroup backups"
    ...
    def _kill_nasbackup_exe(self):
        ...
        ma_machine = Machine(self.subclient.storage_ma, self.commcell)
        cmd = 'taskkill /s {0} /F /FI "IMAGENAME eq NasBackup.exe"'.format(ma_machine.get_hostname())
        res = ma_machine.execute_command(cmd)
        self._log.info(f"killing nasbackup result {res}")
        time.sleep(10)
        self._log.info(f"Successfully killed the NasBackup.exe process on {self.subclient.storage_ma}")
    ...
    def _run_restartable_backup(self, job):
        ...
        if job.is_finished:
            self._log.info("Job finished skip the case")
            return
        self._log.info("killing Nasbackup process")
        self._kill_nasbackup_exe()
        self._log.info("killed Nasbackup process")
    ...
    def run(self):
        ...
        try:
            ...
            self.copy = storage_policy.get_copy('Primary')
            self.copy.delete_job(str(full_job_id.job_id))
