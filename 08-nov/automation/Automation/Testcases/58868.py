...
TestCase:
    ...
    selective copy hourly , last full wait until for vsa ida : sub-client re-association
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Selective Copy Cases for Hourly, Last Full Wait Until Configuration For VSA IDA"
        self.tcinputs = {
            "MediaAgentName": None
        }
    ...
    def run_bkp_jobs(self):
        ...
        if timediff.seconds < 180 * 60:
            self.log.info("Will sleep for %d seconds to let scheduled jobs run for 180 min",
    ...
    def check_job_status_on_copy(
        ...
        """
                    ...
                    Args:
                        ...
                        jobs_until_time - end time until which we want to check the job status
        """
        updateQuery = ""
    ...
    def hourly_last_full_wait_until_sc_disassoc(self):
        """
            Steps:
        ...
        """
        copy1_name = str(self.id) + '_selcopyLastFullWait1'
        sp_obj = self.commcell.storage_policies.get(self.sp_name)
        ...
        # we might be well into the hour. since this is a wait until copy we can run a few jobs
        # to account for this hour and wait an hour to check for job selection.

        today = datetime.datetime.now()
        seconds_to_next_hour = (59 - today.minute) * 60 + (60 - today.second)
        self.log.info("Time in seconds until the next hour : %d", seconds_to_next_hour)

        # if we have enough time say 10 min at least let us run the jobs without waiting.
        if seconds_to_next_hour >= 10 * 60:
            self.run_jobs('full', 2, -2)
            # if left with any more time, sleep it off.
            td = datetime.datetime.now() - today
            if seconds_to_next_hour - td.seconds > 0:
                self.log.info("Time left until next hour :%d. Will sleep until then.",
                              seconds_to_next_hour - td.seconds)
                time.sleep(seconds_to_next_hour - td.seconds)
        else:
        ...
        # we are done running jobs. let us wait for this period to get over and check for jobs.
        self.log.info("Will wait another 60 min for job selection to happen")
        time.sleep(60*60)
    ...
    def hourly_last_full_wait_until_sc_reassoc(self):
        """  Steps:
        ...
        # we are into a new period start, let us see if we have enough time to run jobs for
        # this hour.
        ...
        last_hour = ttc - datetime.timedelta(minutes=ttc.minute, seconds=ttc.second)
        self.log.info("Check jobs until time: %s", str(last_hour))
    ...
    def run(self):
        ...
        try:
            ...
            self.hourly_last_full_wait_until_sc_disassoc()
            self.hourly_last_full_wait_until_sc_reassoc()
