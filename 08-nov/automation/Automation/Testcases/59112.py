...
TestCase:
    ...
    review_delete_action_filesystem()            -- Deletes a file for a FS datasource and verifies the operation
...
class TestCase(CVTestCase):
    """Class for executing Delete action in Review page for a single file"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Delete action for a single file with UNC path in Review page in SDG"
        self.tcinputs = {
    ...
    def review_delete_action_filesystem(self):
        """
        Performs delete action on a file for a filesystem datasource and verifies that the file is deleted
        """
        data_dir = self.tcinputs['FileServerDirectoryPath']

        try:
            self.log.info(f"Sensitive data is available at: {data_dir}")
            self.log.info(f"File to be deleted: {self.sensitive_file}")
            self.file_count_before_review = sum(
                [len(files) for r, d, files in os.walk(data_dir)])
            ...
            self.file_count_after_review = sum(
                [len(files) for r, d, files in os.walk(data_dir)])
            self.log.info(
            ...
            if self.file_count_after_review == self.file_count_before_review - 1:
                self.log.info(
                    f"Successfully deleted {self.sensitive_file} from {data_dir}")
            else:
