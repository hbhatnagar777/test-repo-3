...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Testcase to validate actions are disabled for a threat infected file."
        self.tcinputs = {
    ...
    def run_data_curation(self):
        ...
        isMarkSafeDisabled = threat_scan._click_file_action(self.eicar_file_path,cs.MARK_SAFE)
        if isMarkSafeDisabled: 
            self.log.info(f"Mark Safe Action: {isMarkSafeDisabled}")  
        else:
            raise CVTestStepFailure(
                "Threat infected file has the Mark Safe action enabled incorrectly.")    
        
        isMarkCorruptDisabled = threat_scan._click_file_action(self.eicar_file_path,cs.MARK_CORRUPT)
        if isMarkCorruptDisabled: 
            self.log.info(f"Mark Corrupt Action: {isMarkCorruptDisabled}")  
        else:
            raise CVTestStepFailure(
                "Threat infected file has the Mark Corrupt action enabled incorrectly.")    
