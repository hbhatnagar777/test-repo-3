...
class TestCase(CVTestCase):
    """Class for executing Basic acceptance Test of Attach disk restore as a new instance from a snap copy in tenant account"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Attach disk restore as a new instance from a snap copy in tenant account"
    ...
    def setup(self):
        ...
        vsa_obj_inputs = {
            ...
            'ami': self.tcinputs['ami_select'],
            'disks_to_be_attached': self.tcinputs.get('disks', []),
            'overwrite': self.tcinputs['power_on'],
            'source': self.tcinputs['source'],
            '_ec2_instance_type': self.tcinputs['ins_type'],
            'availability_zone': self.tcinputs['AvailabilityZone'],
            'network': self.tcinputs['network'],
            'vpc': self.tcinputs['vpc'],
            'subnet': self.tcinputs['subnet'],
            'volumetype': self.tcinputs['volumetype'],
        }
    ...
    def run(self):
        ...
        try:
            ...
            self.vsa_obj.backup()
            decorative_log("Attach disk restore to a new instance from Snap copy in tenant account")
            self.vsa_obj.attach_disk_restore('New instance',False)
