...
class TestCase(CVTestCase):
    """TestCase to validate backups and restores on docker deployments from admin console - Cassandra"""

    def __init__(self):
        """Initialize TestCase class"""
        super(TestCase, self).__init__()
        self.name = "backup and restores on docker setups - Cassandra"
        self.browser = None
    ...
    def run(self):
        ...
        try:
            ...
            self.cassandra.verify_restore(stagefree=False, sstableloader=False)
            self.cassandra.validate_restoredata(
                self.keyspace, self.tablename, self.rows)

            # drop keyspace, restore, then validate restored data
            self.cassandra.drop_keyspace(self.keyspace)
            self.cassandra.verify_restore(stagefree=False)
            self.cassandra.validate_restoredata(
                self.keyspace, self.tablename, self.rows)

            # drop keyspace, restore data in cluster view, then validate restored data
            self.cassandra.drop_keyspace(self.keyspace)
            self.cassandra.verify_restore(stagefree=False, sstableloader=False, clusterview=True)
            self.cassandra.validate_restoredata(
                self.keyspace, self.tablename, self.rows)

            # drop keyspace, restore data in cluster view, then validate restored data
            self.cassandra.drop_keyspace(self.keyspace)
            self.cassandra.verify_restore(stagefree=False, clusterview=True)
            self.cassandra.validate_restoredata(
                self.keyspace, self.tablename, self.rows)
