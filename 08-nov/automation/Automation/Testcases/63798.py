...
class TestCase(CVTestCase):
    """Class for executing Basic Acceptance for NDMP Advanced NAS Restore option DAR on/off - NetApp Cluster client"""

    def __init__(self):
        """"Initializes TestCase object"""
        super(TestCase, self).__init__()
        self.name = "NDMP Advanced NAS Restore option DAR on/off - NetApp Cluster client"
        self.tcinputs = {
    ...
    def run(self):
        ...
        try:
            ...
            # Run restores with DAR ON
            # Run restore out-of-place with DAR ON
            ...
            if mamachine.get_logs_for_job_from_file(job.job_id, "nasRestore.log", r"Direct Access Recovery: \[ON\]"):
                self.log.info('NDMP restore job is run with DAR ON')
            else:
                self.log.info('NDMP restore job is NOT run with DAR ON')
                raise Exception('NDMP restore job is NOT run with DAR ON. Failing the testcase')
            ...
            # Run restore in-place with DAR ON
            self.log.info("Run Restore in place")
            job = self.subclient.restore_in_place([f'{self.subclient.content[0]}'], DAR=True)
            ...
            # Run restores with DAR OFF
            # Run restore out-of-place with DAR OFF
            job = self.subclient.restore_out_of_place(
                self.client.client_name,
                str(self.tcinputs['FilerRestoreLocation']),
                self.subclient.content,
            ...
            if mamachine.get_logs_for_job_from_file(job.job_id, "nasRestore.log", r"Direct Access Recovery: \[OFF\]"):
                self.log.info('NDMP restore job is run with DAR OFF')
            else:
                self.log.info('NDMP restore job is NOT run with DAR OFF')
                raise Exception('NDMP restore job is NOT run with DAR OFF. Failing the testcase')
            ...
            # Run restore in-place with DAR OFF
            self.log.info("Run Restore in place with DAR OFF")
            job = self.subclient.restore_in_place([f'{self.subclient.content[0]}'], DAR=False)
            self.log.info("Started restore in place job with Job ID: %s", job.job_id)
