...
TestCase:
    ...
    validate_review_action()                      -- Validates whether the risk got ignored
    ...
    review_ignorerisks_action_filesystem()       -- Verifies that a file's risk gets ignored
...
class TestCase(CVTestCase):
    """Class for executing ignore risks action with review request for a file with local path in SDG"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Ignore risks action with review request for a file with local path in SDG"
        self.tcinputs = {
    ...
    def navigate_to_datasource_review(self):
        ...
        self.admin_console.access_tab(self.admin_console.props['label.review'])
    ...
    def review_ignorerisks_action_filesystem(self):
        """
        Applies ignore risks with review request action
        Raises:
            Exception:
                If the review action fails
        """
        ...
        try:
            ...
            status = self.gdpr_base.data_source_review_obj.review_ignore_risks_actions(self.sensitive_file, [cs.RETENTION_NOT_SET], review_request=True,
                                                                                       request_name=self.request_name, reviewer=reviewer, approver=approver)
            ...
            if not status or job_details and running_job_details[cs.ID] == job_details[cs.ID] or \
                    job_status != cs.COMPLETED:
    ...
    def validate_review_action(self):
        """Validates whether the risk got ignored
        Raises:
            Exception:
        ...
        self.navigate_to_datasource_review()
        status = self.gdpr_base.data_source_review_obj.risks_present(
            self.sensitive_file, [cs.RETENTION_NOT_SET]
        )
        if status:
            raise CVWebAutomationException(
                "File's risk failed to get ignored")
