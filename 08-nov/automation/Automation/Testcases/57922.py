    ...
    "testCases":
            {
                "57922":
                        {
                                "ArrayVendor": Dell EMC Storage center (DSM),
                                "ArrayName": 61787,
                                "ArrayUser": administrator,
                                "ArrayPassword": "",
                                "ControlHost": None,
                                "Controllers": None,
                                "Storage Device Group": This is snap config for engines like: Hitachi,
                                 Dell Equallogic, HPEVA , LSI , IBMSVC, HP3PAR, FUJITSU Eternus.
                                "Mount Retry Count":  This is snap config for engines like: Netapp,
                                 Dell Storage Center (Deprecated),Pure Storage, Solidfire, Extreme io,
                                 Unity
                                "Remote SnapMA": This is snap config for engines like: EMC Clariion, Symmetrix,
                                 EMC VPLEX.
                                "Mount retry interval(in seconds)": This is snap config for engines like:
                                 Huawei, Islion, IBM XIV
                                "Snap Operation Retry Interval (in seconds)": This is snap config for engines like:
                                 Nimble, Nutanix, Tegile, Kaminario, Scale IO, DSM , Google cloud platform.
                                "Server Port": Infinidat,
                                "Mount Datastore": Tintri
...
class TestCase(CVTestCase):
    """Class for executing Acceptance test case for Add/Edit/Delete Array in the Array Management for
        Dell EMC Storage center (DSM) Intellisnap from Command Center"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Add/Edit/Delete Array in the array management for Intellisnap test case from Command center"
        self.snapengine_list = ['Dell EMC Storage Center (DSM)']
    ...
    def run(self):
        ...
        try:
            if self.tcinputs['ArrayVendor'] not in self.snapengine_list:
                exp = "Array Vendor name not valid for this TC"
                raise Exception(exp)
