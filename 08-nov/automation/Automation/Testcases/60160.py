...
class TestCase(CVTestCase):
    """Class for checking if user can request access to multiple Commcells and can be able to
       download database dumps from all the Commcells he has access to. User should be able to
       download database dump files from all the Commcells he has access to"""
    ...
    def __init__(self):
        ...
        self.name = "Multiple Commcell Request Access with download"
        self.tcinputs = {
    ...
    def run(self):
        ...
        try:
            ...
            for company_name, company_id, commcell_guid in companyname_companyid_commcellguid:
                self.helper_cloud_user.company_name = company_name
                self.helper_cloud_user.company_id = company_id
                self.helper_cloud_user.commcell_guid = commcell_guid
                ...
                try:
                    ...
                    self.log.info('Download was successful. Test case is successful')
                    _ = self.helper_cloud_admin.approve_deny_revoke_request(int(request_id),
                                                                            4,
                                                                            comments=self.CommentsForApproveDenyRevoke)
                except Exception as exception:
                    self.log.info('Download was not successful. Test case has failed')
                    _ = self.helper_cloud_admin.approve_deny_revoke_request(int(request_id),
                                                                            4,
                                                                            comments=self.CommentsForApproveDenyRevoke)
                    raise exception
