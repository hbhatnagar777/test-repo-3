...
class TestCase(CVTestCase):
    test_step = TestStep()
    """Class for executing
        IBMi - Backup and restore validation of pre-defined subclient *SECDTA from command center.
    """

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "IBMi - Backup and restore validation of pre-defined subclient *SECDTA from command center."
    ...
    def setup(self):
        ...
        try:
            ...
            self.subclient_name = "*SECDTA"
            self.plan_name = self.tcinputs['PlanName']
            self.secdta_obj = ["AUTO{0}".format(self.id), "AUTO{0}1".format(self.id)]
    ...
    def generate_client_data(self, run_count=0):
        ...
        if run_count == 0:
            for each in self.secdta_obj:
                self.client_machine.manage_usrprf(operation="delete", object_name=each)
        self.client_machine.manage_usrprf(operation="create", object_name=self.secdta_obj[run_count])
    ...
    def restore_and_compare(self):
        ...
        for each in self.secdta_obj:
            self.client_machine.manage_usrprf(operation="delete", object_name=each)
            secdta_obj_path.append("/<System Security>/{0}.USRPRF".format(each))
        ...
        self.job_id = self.RfsSubclient.ibmi_restore_subclient(backupset_name=self.backupset_name,
                                                               ...
                                                               selected_files=secdta_obj_path
                                                               )
        self.wait_for_job_completion()
        ...
        self.verify_client_logs(run_count=2)
    ...
    def verify_client_logs(self, run_count=0):
        ...
        if run_count == 2:
            self.helper.verify_from_log('cvrest*.log',
                                        'CMDEXEC',
                                        jobid=self.job_id,
                                        expectedvalue='{0} restored'.format(self.secdta_obj[0]))
            self.helper.verify_from_log('cvrest*.log',
                                        'CMDEXEC',
                                        jobid=self.job_id,
                                        expectedvalue='Private authorities restored for user profile {0}'.format(
                                            self.secdta_obj[0]))
            self.helper.verify_from_log('cvrest*.log',
                                        'CMDEXEC',
                                        jobid=self.job_id,
                                        expectedvalue='{0} restored'.format(self.secdta_obj[1]))
            self.helper.verify_from_log('cvrest*.log',
                                        'CMDEXEC',
                                        jobid=self.job_id,
                                        expectedvalue='Private authorities restored for user profile {0}'.format(
            ...
            self.helper.verify_from_log('cvrest*.log',
                                        'CMDEXEC',
                                        jobid=self.job_id,
                                        expectedvalue='CPC3713')
            self.helper.verify_from_log('cvrest*.log',
                                        'CMDEXEC',
                                        jobid=self.job_id,
                                        expectedvalue='CPC3706')
    ...
    def run(self):
        ...
        try:
            ...
            self.cleanup()
            self.log.info("*** *SECDTA subclient backup & Restore validation is completed successfully **")
