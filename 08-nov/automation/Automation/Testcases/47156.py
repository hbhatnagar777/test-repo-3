...
class TestCase(CVTestCase):
    """Class for executing basic network case to validate acceptance
        case for one way firewall CS-->CC
    ...
    def __init__(self):
        ...
        self.name = ("[Firewall] : One Way Firewall CS->CC - Acceptance "
        ...
        self._client_group_name = networkconstants.CLIENT_GROUP_NAME[1]
        self._cs_client_group = networkconstants.CS_CLIENT_GROUP_NAME[1]
    ...
    def run(self):
        ...
        try:
            ...
            self._network.set_one_way({'clientName': self.tcinputs['FirewallClient2']},
            ...
            # Setting one-way firewall between Commserv and client group
            self._network.set_one_way({'clientGroupName': self._client_group_name},
                                      {'clientName': self.commserv})
            ...
            self._network.enable_firewall([self.tcinputs['FirewallClient4'],
                                           self.tcinputs['FirewallClient4']],
                                          [8403, 8408])
            ...
            self._network.disable_firewall([self.tcinputs['FirewallClient4'],
                                            self.tcinputs['FirewallClient4']],
                                           [8403, 8408])
            ...
            self._network.enable_firewall([self.tcinputs['FirewallClient2'],
                                           self.tcinputs['FirewallClient3'],
                                           self.tcinputs['FirewallClient2'],
                                           self.tcinputs['FirewallClient3'],
                                           self.commserv],
            ...
            self._network.validate([self.tcinputs['FirewallClient2'],
                                    self.tcinputs['FirewallClient3']], self.commserv, max_job_time=300)
            ...
            self._network.serverbase.check_client_readiness([self.tcinputs['FirewallClient3'],
                                                             self.commserv])

            self._network.validate([self.tcinputs['FirewallClient3']], self.commserv, max_job_time=300)
