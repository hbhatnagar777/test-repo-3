...
TestCase:
    ...
    validate_schedule()                 --  Creates plan schedule & Validates schedule invoked job or not
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "CvPySDK Integration Test cases for FSO - Validate FSO Full scan & Plan based schedule"
        self.tcinputs = {
            "FSOClient": None,
            "IndexServer": None
        }
    ...
    def validate_schedule(self):
        """Create plan schedule & validates schedule invoked job or not on FSO Server"""
        schedule_time_in_min = 4
        custom_time = datetime.datetime.now()
        custom_time += datetime.timedelta(minutes=schedule_time_in_min)
        self.log.info(f"Going to do Plan scheduling with start time as - {custom_time}")
        pattern = SchedulePattern().create_schedule_pattern(
            {'freq_type': 'daily', 'active_start_time': custom_time.strftime('%H:%M')})
        self.log.info(f"Pattern JSON formed - {pattern}")
        self.plan_obj.schedule(pattern_json=pattern,
                               schedule_name=f"IntegrationTest_Schedule_{int(time.time())}")
        self.log.info(f"Plan Schedule created successfully")
    ...
    def run(self):
        ...
        try:
            ...
            self.validate_fso_server(raise_error=False)
            self.setup_data()
