...
class TestCase(CVTestCase):
    """
    Class for executing Test Case for Basic gcc cloud client creation and verification of backup, restore
    ...
    def __init__(self):
        ...
        self.name = "Basic gcc cloud client creation and verification of backup, restore"
        self.hub_utils = None
    ...
    def run(self):
        try:
            ...
            self.client = self.teams_client_helper.create_client(client_name=self.tcinputs['clientname'],
                                                                 ...
                                                                 cloud_region=TeamsConstants.CloudRegion.Gcc)
            token = Token.get_token_instance(Token)

            self.teams_helper = TeamsHelper(self.client)
            Token.cloud_region = TeamsConstants.CloudRegion.Gcc
            token.refresh()
