...
from Web.AdminConsole.GovernanceAppsPages.ComplianceSearch import CustomFilter


class TestCase(CVTestCase):
    """Class for executing
    Basic acceptance Test of Custodian based search from Compliance Search of adminconsole"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Basic acceptance Test of " \
                    "Custodian based search from Compliance Search of adminconsole"
    ...
    def init_tc(self):
        ...
        try:
            ...
            self.mssql = MSSQL(
                ...
                password,
                'CommServ',
                as_dict=False, use_pyodbc=False)
    ...
    @test_step
    def apply_search_filter_multiple_custodian(self):
        """
           Apply multiple custodians ,
           fire compliancesearch and Indexserver query
        """
        custom_filter = CustomFilter(self.admin_console)
        custom_filter.apply_custom_filters_with_search(
            {"Custodian": self.custodian_users_list})
        self.count_compliance_search = self.app.get_total_rows_count()
        if self.count_compliance_search == -1:
            raise CVTestStepFailure(f"Error getting Compliance Search results "
                                    f"after applying single custodian filter")
        userslisttostr = ' '.join([str(elem)
                                   for elem in self.custodian_users_list])

        self.log.info("Getting search results count from Index Server"
                      " for all the users: %s", userslisttostr)
        users_count = len(self.custodian_users_list) - 1
        fq_search_string = ""
        while users_count >= 0:
            fq_search_string = fq_search_string + "OwnerName_idx:" + "\"" +\
                self.custodian_users_list[users_count] + "\" OR "
            users_count = users_count - 1
        fq_search_string = fq_search_string[:-4]
        params = {'start': 0, 'rows': 50, 'fq': fq_search_string}
        solr_results = self.solr_helper_obj.create_url_and_get_response(
            {'keyword': self.search_keyword}, op_params=params)
        self.count_solr = self.solr_helper_obj.get_count_from_json(
            solr_results.content)
        self.log.info("Solr Query returns %s items", self.count_solr)

    @test_step
    def apply_search_filter_single_custodian(self):
        """
           Apply single custodian, fire compliancesearch and Indexserver query
        """
        user = self.custodian_users_list[0]
        user_list = []
        ...
        if self.count_compliance_search == -1:
            raise CVTestStepFailure(
                "Error getting Compliance Search results after applying single custodian filter")
        self.log.info(
            "Getting search results count from Index Server for user: %s",
            user)
        fq_search_string = "OwnerName_idx:" + "\"" + user + "\""
        params = {'start': 0, 'rows': 50, 'fq': fq_search_string}
        solr_results = self.solr_helper_obj.create_url_and_get_response(
            {'keyword': self.search_keyword}, op_params=params)
        self.count_solr = self.solr_helper_obj.get_count_from_json(
    ...
    def run(self):
        try:
            ...
            self.apply_search_filter_single_custodian()
            self.validate_search_results()
            self.apply_search_filter_multiple_custodian()
            self.validate_search_results()
