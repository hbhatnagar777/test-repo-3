...
class TestCase(CVTestCase):
    """Command Center- Network topology- OneWay CS->CC(server)"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Command Center- Network topology- OneWay CS->CC(server)"
        self.browser = None
        self.topology_name = "CG_60333_OW_CS_CC"
        self.admin_console = None
        self.navigator = None
        self._client_group_name1 = "CG_60333_CS"
        self._client_group_name2 = "CG_60333_Client"
        self.tcinputs = {
            "DestinationClient": ""
        }
        self.smart_groups = ["My CommServe Computer", "My CommServe Computer and MediaAgents", "My MediaAgents"]
        self.client_tunnel_port = None
    ...
    def init_tc(self):
        ...
        try:
            ...
            self.client_group_obj1.add_clients([self.commserve])
            self.client_name = self.tcinputs['DestinationClient']
            self.client_obj = self.commcell.clients.get(self.tcinputs['DestinationClient'])
    ...
    def validate_no_routes(self, client, server):
        ...
        for i in cl_summary:
            if client + " " + server in i and "type=passive" in i:
                self.log.info(f"Deletion of network routes failed for {client}")
                client_check = True

        for i in server_summary:
            if server + " " + client in i and "type=persistent" in i:
    ...
    def validate_summary(self, summary, source_client, destination_client, route_type, protocol, streams,
        ...
        for i in self.summary:
            ...
            if "keepalive_interval" in i and not check_keep_alive:
                if "keepalive_interval=" + keep_alive in i:
                    ...
                    pass # comment until validation is fixed
        ...
        if (check_persistent or check_passive) and check_tunnel_port and check_keep_alive:
            ...
            pass # raise CVTestStepFailure("Validation of network summary failed")
    ...
    def create_one_way(self, client_group_name1, client_group_name2):
        ...
        try:
            ...
            self.validate_summary(self.client_summary, self.client_name,
                                  self.commserve,
                                  "passive", "httpsa", '1', '180', self.client_obj.network.tunnel_connection_port,
                                  client_group_name2)

            # Validate the network summary of commserve
            self.validate_summary(self.server_summary, self.commserve,
                                  self.client_name,
                                  "persistent", "httpsa", '1', '180', self.server_object.network.tunnel_connection_port,
                                  client_group_name1)
    ...
    def validate_additional_settings(self, client_group_name1, client_group_name2):
        ...
        try:
            ...
            self.validate_summary(self.client_summary, self.client_name,
                                  self.commserve,
                                  "passive", "httpsa", "1", "170", "2500", client_group_name1)
            self.validate_summary(self.server_summary, self.commserve,
                                  self.client_name,
                                  "persistent", "httpsa", "1", "170", "2500", client_group_name2)
    ...
    def validate_encrypt_network(self):
        ...
        try:
            ...
            self.server_summary = self.server_object.get_network_summary()
            self.validate_summary(self.server_summary, self.commserve,
                                  self.client_name, "persistent",
                                  "https", '4', "170", '2500', self._client_group_name1)
            ...
            self.validate_summary(self.server_summary, self.commserve,
                                  self.client_name, "persistent",
                                  "httpsa", '4', "170", '2500', self._client_group_name1)
    ...
    def download_network_summary(self):
        ...
        try:
            if self._client_group_name1 in self.smart_groups or self._client_group_name2 in self.smart_groups:
                ...
                self.validate_summary(self.client_summary, self.client_name,
                                      self.commserve,
                                      "passive", "httpsa", "1", "170", "2500", self._client_group_name2)
    ...
    def run(self):
        try:
            ...
            self.create_one_way("My CommServe Computer", self._client_group_name2)
            self.delete_topology_validate()
