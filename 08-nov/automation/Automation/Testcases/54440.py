...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Cloud Laptop]: Monikers [Pictures] Acceptance test"
    ...
    def setup(self):
        ...
        self.subclient_object.content = ['\%Pictures%']
    ...
    def run(self):
        ...
        # -------------------------------------------------------------------------------------
        #   # Adding text files to validate pictures moniker
        #    SCENARIO-1:
        #       - Pictures moniker validation with new data
        #    SCENARIO-2:
        #       - Pictures moniker validation with Modified data
        #    SCENARIO-3:
        #       - Pictures moniker validation with renamed data
        #    SCENARIO-4:
        #       - Remove %Pictures moniker and validate backup
        # -------------------------------------------------------------------------------------
            pictures_path = "C:\\Users\\admin\\Pictures\\IncData"
            local_time = int(time.time())
            pic_file = 'testpic_'+str(local_time)+".jpg"
            ...
            while each_scenario < len(scenarios_list):
                ...
                # -------------------------------------------------------------------------------------
                    self.server_obj.log_step("""
                        ...
                        SCENARIO-1:
                            - Add new content under Pictures path
                ...
                # -------------------------------------------------------------------------------------
                    while True:
                        ...
                        # -- Status: 0 - means currently no backup job is running --#
                            ...
                            self._log.info("Adding data under Pictures path: %s", str(pictures_path))
                            pic_file = pictures_path + '\\' + pic_file
                            self.laptop_utils.create_file(self.machine_object, pictures_path, pic_file)
                            self.cloud_object.wait_for_incremenatl_backup(self.machine_object)
                            self.cloud_object.source_dir = pictures_path
                            self.cloud_object.subclient_content_dir = pictures_path
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        SCENARIO-2:
                            - Modify the files under Pictures path
                    ...
                    while True:
                        ...
                        # -- Status: 0 - means currently no backup job is running --#
                            ...
                            self._log.info("Modifying test data under Pictures path: %s", str(pictures_path))
                            self.machine_object.modify_test_data(pictures_path, modify=True)
                            self.cloud_object.wait_for_incremenatl_backup(self.machine_object)
                            self.cloud_object.source_dir = pictures_path
                            self.cloud_object.subclient_content_dir = pictures_path
                            self.cloud_object.out_of_place_restore(self.machine_object, self.subclient_object, cleanup=False)
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        SCENARIO-3:
                            - Rename all files under Pictures path
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        while True:
                            ...
                            # -- Status: 0 - means currently no backup job is running --#
                                ...
                                self._log.info("Rename the test data under Pictures path: %s", str(pictures_path))
                                self.laptop_utils.create_file(self.machine_object, pictures_path, files=5)
                                self.machine_object.modify_test_data(pictures_path, rename=True)
                                self.cloud_object.wait_for_incremenatl_backup(self.machine_object)
                                self.cloud_object.source_dir = pictures_path
                                self.cloud_object.subclient_content_dir = pictures_path
                    ...
                    # -------------------------------------------------------------------------------------
                        ...
                        SCENARIO-4:
                            - Remove %Pictures  moniker from subclient content and add some test under Pictures
                            - Wait for the backup job finished as per interval
                            - Perform browse and verify above content backed up or not
                                [added file under Pictures path should not be backed up]
                        """, 100)
                    # -------------------------------------------------------------------------------------
                        ...
                        while True:
                            ...
                            # -- Status: 0 - means currently no backup job is running --#
                                ...
                                self.subclient_object.content = ['\%Music%'] # overwriting Pictures moniker with music
                                pictures_path = "C:\\Users\\admin\\Pictures"
                                dont_backup_path = pictures_path + '\\' + 'Dont_backup'
                                pic_file = dont_backup_path + '\\' + pic_file
                                self.laptop_utils.create_file(self.machine_object, pictures_path, pic_file)
                                self._log.info("Adding test data under pictures path: %s", str(dont_backup_path))
                                ...
                                if browse_result[0] or browse_result[1]: # brwose returns tuple of list and dictioanry values
                                    raise Exception("Data backed up after [Pictures Moniker] removed [{0}]" .format(browse_result))
                                self._log.info("*** Validation of backup after [Pictures Moniker] removal completed successfully ****")
            ...
            try:
                ...
                self.machine_object.remove_directory(pictures_path)
