...
TestCase:
    ...
    verify_job_phases_and_streams()             --  Verifies backup job phases and number of streams on DB2 machine
...
class TestCase(CVTestCase):
    """ Command center: Adding subclient in Backupset Details Page. """
    ...
    def __init__(self):
        ...
        self.name = "DB2 Add subclient Test"
        self.browser = None
    ...
    def run(self):
        ...
        self.run_backup(backup_level="backupset")
        self.run_backup(backup_level="subclient")
    ...
    @test_step
    def run_backup(self, backup_level="subclient"):
        """Runs backup
            Args:
                backup_level               (str)               -- Level to backup from

            Raises:
                Exception:
                    If backup job fails
        """
        if backup_level.lower() == "subclient":
            self.db_backupset.access_subclient(subclient_name=self.id)
        ...
        self.verify_job_phases_and_streams(job, phases)

    @test_step
    def verify_job_phases_and_streams(self, job, phases):
        """Verifies backup job phases and number of streams
            Args:
            ...
            Raises:
                Exception:
                    If Log backup phase exists
                    If backup job has different number of streams
        """

        if "logs" in phases:
            raise CVTestStepFailure("Logs Backup Job also exists!")

        if int(self.db2_helper.get_backup_time_stamp_and_streams(job.job_id)[1])-1 != self.tcinputs["number_of_streams"]:
            raise CVTestStepFailure("Backup Job does not have defined number of streams!")
