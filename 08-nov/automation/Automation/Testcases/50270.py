...
class TestCase(CVTestCase):
    """Class for executing Basic acceptance test of Azure blob backup and Restore test case"""

    def __init__(self):
        """TestCase constructor"""
        super(TestCase, self).__init__()
        self.name = "cloud storage Test Case-Acceptance test for all azure cloud storage functions"
    ...
    def setup(self):
        ...
        self.log.info('CS set to "%s" in test case: "%s"',
                      self.commserver_name, self.id)
    ...
    def run(self):
        ...
        try:
            ...
            account_name = self.instance.account_name
            account_key = self.instance.access_key
            self.session = self.azure_helper.create_session_azure(
            ...
            # downloading the contents to local
            self.azure_helper.download_contents_azure(self.session,
                                                      self.subclient.content,
                                                      'originalcontents',
                                                      False)
            self.log.info(
                "successfully downloaded subclient contents to local file system")
            self.azure_helper.delete_contents_azure(self.session, self.subclient.content)
            ...
            self.log.info(
                "downloading restored contents for in place restore validation")
            self.azure_helper.download_contents_azure(self.session,
                                                      self.subclient.content,
                                                      'inplacecontents',
                                                      False)
            ...
            self.cloud_storage_helper.restore_validation(self.controller_object,
                                                         'originalcontents',
                                                         'inplacecontents',
                                                         'in_place')
            ...
            self.cloud_storage_helper.restore_validation(
                ...
                self.fs_restore_path,
                "fs_restore")
            # add an object for running incremental backups
            self.azure_helper.create_container_azure(self.session, 'testcontainer')
            self.azure_helper.upload_file_azure(self.session, 'testcontainer',
                                                'CVAutomation.py',
                                                'CVAutomation.py')
            new_content = self.subclient.content
            new_content.append(
                f"/{'testcontainer'}/{'CVAutomation.py'}")
            ...
            self.result_string = "Run of test case 50270 has completed successfully"
    ...
    def tear_down(self):
        ...
        new_content.remove(
            f"/{'testcontainer'}/{'CVAutomation.py'}")
        self.subclient.content = new_content
        self.azure_helper.delete_container_azure(
            self.session,
            'testcontainer')
        self.log.info("container deleted successfully")
        self.log.info("Cleanup done successfully")
