...
TestCase:
    ...
    edit_and_monitor_full() -- modifies and monitors schedules
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Server plan] Data full backup schedule execution from server plans"
        ...
        try:
            ...
            self.tc_plans.append(
                self.plans_api_helper['MSPAdmin'].create_base_plan(
                    "{0}-{1}".format(plansconstants.FULL_SERVER_PLAN, time.time()),
                    plan_subtype,
            ...
            entity_inputs = {
                ...
                'backupset':
                {
                    'name': "backupset_{0}_full".format(self.id),
                    'force': True
                },
                'subclient':
                {
                    'name': "subclient_{0}_full".format(self.id),
                    'force': True
                }
            }
            ...
            self.log.info("\t4. First full backup job triggered.")

            # Set plan incremental RPO in years on a particular day of a year to avoid conflicts
            yearly = {
                'year': {
                    'freq_recurrence_factor': date.today().month,
                    'freq_interval': date.today().day - 5,
            ...
            # Set plan full RPO to every day
            daily = {
                'days': {
                    'runEvery': 1,
                    'startTime': time.strftime('%H:%M')
            ...
            self.edit_and_monitor_full(
                self.tc_plans[0].plan_name,
                daily,
                self.cv_entities.entity_object_map['subclient'][0]['object'],
                self.tc_plans[0].schedule_policies['data'].all_schedules[2]['schedule_id'],
                'Asia/Calcutta',
                job_count=1
            )
            ...
            # Set plan full RPO in months on a particular day
            monthly_relative = {
                'months_relative': {
                    'freq_interval': 1,
            ...
            self.plans_api_helper['MSPAdmin'].dissociate_entity(
                ...
                "backupset_{0}_full".format(self.id),
                "subclient_{0}_full".format(self.id)
            )
            ...
            for plan in self.tc_plans:
                ...
                self._log.info("\t14. Successfully deleted plan")

        except Exception as exp:
            self.server.fail(exp)

    def edit_and_monitor_full(self, plan_name, freq_type_dict, subclient_obj, schedule_id, tz, job_count=1):
        """Modifies the schedule of the plan, waits and monitors the trigger of the schedule job"""
        ...
        self.log.info("\t- Set full schedule to a {0} type.".format(list(dict.keys(freq_type_dict))[0]))
        ...
        self.log.info("\t- Montitoring the trigger of {0} backup.".format(list(dict.keys(freq_type_dict))[0]))
