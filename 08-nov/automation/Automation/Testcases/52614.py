...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Restartability Test of PostgreSQL iDA - 1"
    ...
    def _run_backup(
        ...
        if suspend:
            ...
            # Stop client Commvault services to suspend and resume
            ...
            while (not response) and ("completed" not in job.status.lower()):
                ...
                if not response:
                    time.sleep(120)
            self.log.info(
                "Proceeding to create Operation window as atleast one chunk is commited")

            ##################### create a Operation window to suspend job ####
            self.log.info(
                "Sleeping 30 seconds before introducing next interrupt")
            time.sleep(30)
            interrupt_object.operation_window()

            # restart MA CVD processs from MA commandline if
            # CS and MA are different machines
            self.log.info(
                "Sleeping 30 seconds before introducing next interrupt")
            time.sleep(30)
            interrupt_object.ma_cvd_kill()
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Sleeping for 240 seconds before restore")
            time.sleep(240)
