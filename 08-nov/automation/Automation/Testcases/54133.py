...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = ("VSA Amazon Tenant Account: Incremental backup and Full VM restore from snap "
                     "and backup copy(multi proxy, multi guest, Unix MA)")
    ...
    def run(self):
        ...
        try:
            ...
            auto_subclient.validate_inputs("", "unix", "", self.update_qa)
            ...
            try:
                ...
                self.log.info("Sleeping for 12 minutes, waiting for snap to unmount")
                time.sleep(720)
            ...
            try:
                self.log.info("%(boundary)s %(message)s %(boundary)s",
                              {'boundary': "-" * 25,
                               'message': "Full VM out of place restore from snap"})
            ...
            try:
                self.log.info("%(boundary)s %(message)s %(boundary)s",
                              {'boundary': "-" * 25,
                               'message': "Full VM in-place restore from snap"})
            ...
            try:
                self.log.info("%(boundary)s %(message)s %(boundary)s",
                              {'boundary': "-" * 25,
                               'message': "Full VM out of place restore from Backup copy"})
                vm_restore_options.in_place_overwrite = False
            ...
            try:
                self.log.info("%(boundary)s %(message)s %(boundary)s",
                              {'boundary': "-" * 25,
                               'message': "Full VM in-place restore from Backup copy"})
                vm_restore_options.in_place_overwrite = True
                auto_subclient.virtual_machine_restore(vm_restore_options)
            except Exception as exp:
                self.test_individual_status = False
