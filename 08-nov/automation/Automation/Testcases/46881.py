...
with cloud library support, two splits in the dedup store and
SDT mode enabled.
...
input json file arguments required:
    ...
    "46881":{
            ...
            "CloudLib" : "yes or no whether you want to create a cloud library or disk library",
            "S3Region": None,
            "S3CloudBucket": None,
            "S3AccessKey":None,
            "S3SecretKey":None,
    ...
    Enter the cloud library credentials if you want to create a cloud library in the input file for the testcase
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "MA side dedupe with cloud library support"
        self.tcinputs = {
            "MediaAgentName": None,
            "cloudlib": None,
            "S3Region": None,
    ...
    def run(self):
        ...
        try:
            ...
            self.mm_helper.set_opt_lan("enable", self.commcell.commserv_hostname +
                                       "\\commvault", "sqladmin_cv",
                                       self.password, self.media_agent_client_id)
            # sdt mode turned on
            self.log.info("sdt mode turned on")
            ...
            for iterator in range(1, 3):
                ...
                config_strings_clbackup = [
                    ...
                    'Using default pipeline mode [SDT] for [Bkp] pipe']
                error_flag = []
                ...
                self.log.info("CASE 7: -- Checking from logs if "
                              "SDT mode has been activated")
                (matched_line, matched_string) = self.dedup_helper.parse_log(
                    self.client.client_hostname, log_file, config_strings_clbackup[6], job.job_id)

                if matched_line:
                    self.log.info("Result: Pass _ SDT mode has been activated")
                else:
                ...
                network_bytes = self.dedup_helper.get_network_transfer_bytes(
                    job.job_id)
                self.log.info("Network transferred bytes: %s", str(network_bytes))
                ...
                self.log.info(
                    "CASE 9: Did Dedupe Occur correctly ?"
                ...
                self.log.info("Secondary objects: %s", str(secondary_objects_count))
