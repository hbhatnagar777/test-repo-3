...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Dell EMC Celerra / VNX - Wild Cards for Subclient Content and Filter"
    ...
    def validate_filter(self, job, ptrn1, ptrn2):
        ...
        line2 = self.MAMachine.get_logs_for_job_from_file(job.job_id, "NasBackup.log", r"ENV \[EMC_EDIR01\]=")
        line3 = self.MAMachine.get_logs_for_job_from_file(job.job_id, "NasBackup.log", r"ENV \[EMC_EFILE01\]=")
        self.log.info(f"line1 is {line1}")
        self.log.info(f"line2 is {line2}")
        self.log.info(f"line3 is {line3}")
        ...
        for item in ptrn1:
            if item in line1:
                ...
                for item1 in ptrn2:
                    if item1 in line2:
                        self.log.info(f"Wildcard {item1} is a directory filter and is present")
                    elif item1 in line3:
                        self.log.info(f"Wildcard {item1} is a file filter and is present")
                    else:
    ...
    def run(self):
        ...
        try:
            ...
            if not self.subclient.is_intelli_snap_enabled:
                ...
                self.subclient.enable_intelli_snap("Dell EMC VNX / Celerra SnapSure Snap")
