...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Commerve Live Sync]: Production Maintenance cycle"
    ...
    def run(self):
        # run function
        self.perform_failover(self.target_node, CSLiveSync.PRODUCTION_MAINTENANCE)
        self.network_helper = NetworkHelper(self)
        self.network_helper.serverbase.check_client_readiness([self.client, self.media_agent])

        self.perform_failover(self.active_node, CSLiveSync.MAINTENANCE_FAILBACK)
        self.network_helper = NetworkHelper(self)
        self.network_helper.serverbase.check_client_readiness([self.client, self.media_agent])
