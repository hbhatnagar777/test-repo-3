...
This testcase verifies the folder size is correct when FULL job is killed and new FULL job is run
...
class TestCase(CVTestCase):
    """This testcase verifies that the folder size is correct when FULL job is killed and new FULL
    job is run"""
    ...
    def __init__(self):
        ...
        self.name = 'Indexing - Folder size - Full job is killed'
    ...
    def setup(self):
        ...
        try:
            ...
            self.backupset_name = self.tcinputs.get('Backupset', 'FSV_kill_full_run_new')
    ...
    def run(self):
        ...
        try:
            ...
            self.idx_tc.new_testdata(sc_content, dirs=5, files=20, file_size=20480)
            self.idx_tc.run_interrupt_job(self.subclient, 'full', action='kill', wait=7)

            self.idx_tc.run_backup(self.subclient, 'Full')

            self.log.info('********** VERIFICATION 1 - Browse after running new full **********')
