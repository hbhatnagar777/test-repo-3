...
class TestCase(CVTestCase):
    """ Class for executing Basic acceptance Test for PostgreSQL Multi Stream Block-level Backup and Restore """
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Multi Stream backup and restore for Block level subclient"
    ...
    def check_prerequisite(self):
        """Checks if block-level and snapshot is enabled at subclient level"""
        ...
        if not self.subclient_page.is_blocklevel_backup_enabled():
            self.log.info("Enabling block level option as it was disabled")
            self.subclient_page.enable_blocklevel_backup()
    ...
    def validate_log(self, is_backup_job=True, jobid=None, restore_jobid=None):
        """
        Validates the backup and restore in the log files : clBackupParent.log, PostgresRestoreCrdr.log
        ...
        """
        if is_backup_job:
            search_term = rf".*Maximum streams to use.*{self.noOfDataStreamsforBackup}"
            ...
            output = self.machine_object.get_logs_for_job_from_file(restore_jobid, "PostgresRestoreCrdr.log",
                                                                    search_term)
            if output is None:
    ...
    def run_backup_and_restore(self):
        ...
        job_id = self.subclient_page.backup(backup_type=RBackup.BackupType.FULL, immediate_backup_copy=True)

        self.wait_for_job_completion(job_id)
        self.log.info("FSBASED FULL backup is completed")
        log_job = self.dbhelper_object.get_snap_log_backup_job(job_id)
        self.log.info("Log backup job with ID:%s is now completed", log_job)
        backupcopy_job_obj = self.dbhelper_object.get_backup_copy_job(job_id)
