...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - Verify backup cycle Full-Incremental-Incremental for IFS and LFS data
        ...
        Step3, Run backup incremental and verify the data with a restore
        ...
        Step6, Run backup incremental and verify the data with a restore
        Step7, configure BackupSet and Subclient with LFS data with Regular scan and Object level backup.
        Step8, Run backup full + incremental and verify the data with a restore
        Step9, Run backup incremental and verify the data with a restore
    ...
    def __init__(self):
        ...
        self.name = "IBMi - Verify backup cycle Full-Incremental-Incremental for IFS and LFS data"
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("*** STARTING VALIDATION OF BACKUP CYCLE with INC & INC FOR IFS and LFS DATA ***")
            ...
            self.start_backup(backup_level="Incremental")
            self.restore_verify(source=self.srcdir, destination=self.tmp_path)
            self.log.info("Verified IFS backup and restore with inc + inc backup cycle with %s", self.scan_type.name)
            ...
            for each in scans:
                ...
                self.restore_verify(source=self.client_machine.lib_to_path(self.srclib),
                                    destination=self.client_machine.lib_to_path(self.destlib))
                self.log.info("Verified LFS backup and restore with inc + inc backup cycle with %s",
                              self.scan_type.name)

            self.client_machine.manage_library(operation='delete', object_name=self.srclib)
            self.client_machine.manage_library(operation='delete', object_name=self.destlib)

            self.log.info("**LFS, IFS REGULAR SCAN BACKUP CYCLE WITH INC + INC IS SUCCESSFULLY**")
