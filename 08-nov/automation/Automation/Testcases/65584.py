...
TestCase:
    ...
    non_dedupe_worm_validation() -- validate WORM lock configuration on non-dedupe pool
...
Design Steps:
    ...
    Case1:
        ...
        3. Check DB for the worm copy flag, storage is marked with object lock flag, micro pruning is disabled and seal frequency is set as per retention on pool ( in this case it should be 7 days).
    ...
    Case2:
        ...
        4. Check DB for the worm copy flag, storage is marked with object lock flag, micro pruning is disabled and seal frequency is set as per retention on pool (7 days in this case).
    ...
    Case3:
        1. Create a non-deudpe pool
        2. Associate a Copy to it.
        3. Enable hardware WORM and ensure no seal frequency is set and micro pruning is disabled on the mountpath(s).
        4. Reduce retention on dependant copy; it should return an error.
...
class TestCase(CVTestCase):
    ...
    def setup(self):
        ...
        self.dedupe_pool_case2 = '%s_dedupe_pool2-ma(%s)-client(%s)' % (str(self.id), self.tcinputs['MediaAgentName'],
                                                                        self.tcinputs['ClientName'])
        self.non_dedupe_pool = '%s_non_dedupe_pool-ma(%s)-client(%s)' % (str(self.id), self.tcinputs['MediaAgentName'],
                                                                         self.tcinputs['ClientName'])
        ...
        self.policy_case2 = '%s_policy2-ma(%s)-client(%s)' % (str(self.id), self.tcinputs['MediaAgentName'],
                                                              self.tcinputs['ClientName'])
        self.policy_case3 = '%s_policy3-ma(%s)-client(%s)' % (str(self.id), self.tcinputs['MediaAgentName'],
                                                              self.tcinputs['ClientName'])
        ...
        self._cleanup(self.policy_case3, self.non_dedupe_pool)
    ...
    def dedupe_worm_validation(self, cloud_pool, cloud_lib, sp_obj):
        ...
        # Check if object level WORM lock is enabled on cloud storage pool
        if cloud_pool.is_object_level_worm_lock_enabled:
            self.log.info("Object level WORM lock is enabled on cloud storage pool - [%s]", cloud_pool_name)
        else:
            raise Exception("Object level WORM lock is not enabled on cloud storage pool - [%s]", cloud_pool_name)
    ...
    def non_dedupe_worm_validation(self, cloud_pool, cloud_lib, sp_obj):
        """ validate WORM lock configuration on non-dedupe pool"""
        ...
        # check no seal frequency is set on non-dedupe pool
        if self.mmhelper.get_copy_store_seal_frequency(cloud_pool.get_copy())['days'] == 0:
            self.log.info("No seal frequency is set on non-dedupe pool - [%s]", cloud_pool_name)
        else:
            raise Exception("Seal frequency is set on non-dedupe pool - [%s]", cloud_pool_name)
        ...
        # check override retention of pool is enabled on dependent storage policy copy
        if primary_copy_obj.override_pool_retention:
            self.log.info("Override retention of pool is enabled on dependent [Primary] copy of storage policy [%s]", sp_name)
        else:
            raise Exception("Override retention of pool is not enabled on dependent [Primary] copy of storage policy [%s]", sp_name)

        # check if retention on dependent copy can be increased
        retention = (30, 2, -1)
        primary_copy_obj.copy_retention = retention
        self.log.info("Retention on dependent [Primary] copy of storage policy [%s] can be increased", sp_name)

        # check if retention on dependent copy can't be reduced
        retention = (15, 1, -1)
        try:
            primary_copy_obj.copy_retention = retention
            self.log.error("Retention on dependent [Primary] copy of storage policy [%s] can be reduced", sp_name)
            raise Exception("Retention on dependent [Primary] copy of storage policy [%s] can be reduced", sp_name)
        except Exception as exp:
            if "Reducing the basic or extended retention of a worm storage policy copy is not allowed" in str(exp):
                self.log.info("As expected retention on dependent [Primary] copy of storage policy [%s] cannot be reduced with error: %s", sp_name, exp)
            else:
                raise exp
    ...
    def run(self):
        ...
        try:
            ...
            # case3 - Non-dedupe pool and its dependent copy worm lock validation
            self.log.info("Case3 - Non-dedupe pool and its dependent copy worm lock validation")
            # Creating non-dedupe storage pool
            non_dedupe_pool_obj, lib_obj_case3 = self.mmhelper.configure_storage_pool(self.non_dedupe_pool,
                                                                                      self.tcinputs["CloudMountPath"],
                                                                                      self.tcinputs["MediaAgentName"],
                                                                                      username=self.tcinputs[
                                                                                          "CloudUserName"],
                                                                                      ...
                                                                                      cloud_vendor_name=self.tcinputs[
                                                                                          "CloudVendorName"])
            # Create a dependent storage policy associated to the pool
            sp_obj_case3 = self.mmhelper.configure_storage_policy(self.policy_case3,
                                                                  storage_pool_name=self.non_dedupe_pool)

            # Enable WORM on cloud storage pool
            self.mmhelper.enable_worm_storage_lock(non_dedupe_pool_obj, 1)

            # Validate WORM lock configuration
            self.non_dedupe_worm_validation(non_dedupe_pool_obj, lib_obj_case3, sp_obj_case3)
