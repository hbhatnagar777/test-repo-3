...
TestCase:
    ...
    cleanup()   --  cleans existing entities
    
    tear_down() --  tear down method of this test case
    
    run_backups()   --  runs backups on sub client
    
    get_key_id_of_first_job()   --  Fetch the KeyID of first backup job
...
class TestCase(CVTestCase):
    """Verify registry key KeyMapMaxSize for encryption key cleanup"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Verify registry key KeyMapMaxSize for encryption key cleanup"
        ...
        self.commcell_base_directory = None
    ...
    def setup(self):
        ...
        self.commcell_base_directory = self.commcell_machine.join_path(self.commcell.commserv_client.install_directory,"Base")
    ...
    def create_pool(self, pool_name, library_path, ma_obj, ddb_path):
        ...
        Return:
                pool_obj (object) - object of StoragePool class
                pool_copy_o (object) - object of StoragePolicyCopy class of storage pool copy 
        """
    ...
    def create_secondary_copy(self, pool_name, copy_name):
        ...
        Return:
                copy_o (object) - object of StoragePolicyCopy class
        """
    ...
    def tear_down(self):
        ...
        self.log.info("Setting client side deduplication configuratio to \"User storage policy settings\"")
        ...
        if self.file_name is not None:
            ...
            self.commcell_machine.delete_file(self.commcell_machine.join_path(self.commcell_base_directory, self.file_name))
        ...
        self.commcell.clients.get(self.source_ma).restart_services()
    ...
    def get_key_id_of_first_job(self):
        """
        Fetch the KeyID of first backup job
        return: keyID (str)
        """
    ...
    def validate_from_log(self):
        ...
        """
        matched_line = None
        key_id = self.get_key_id_of_first_job()
        self.log.info(F"Received KeyID: {key_id}")
        ptrn = f"Removing archive file encryption key for keyId \[{key_id}\] from the map.*lastAccessTime \[[0-9]*\]"
    ...
    def configure_sub_client(self, bs_name, sc_name, client_content, agent):
        ...
        Args:
            bs_name (str) - BackupSet name 
            sc_name (str) - SubClient name
            client_content (str) - client content path
    ...
    def wait_for_ma_online(self):
        ...
        Exception:
                 - If MedaiAgent is not online within 600 sec
        """
    ...
    def run(self):
        try:
            ...
            self.primary_pool_copy_o.set_encryption_properties(re_encryption=True,
                                                               encryption_type=random.choice(enc_list),
                                                               encryption_length=random.choice(enc_key))

            self.log.info(f"Enabling re-encryption on storage pool[{self.pool_secondary_name}]")
            self.secondary_pool_copy_o.set_encryption_properties(re_encryption=True,
                                                                 encryption_type=random.choice(enc_list),
                                                                 encryption_length=random.choice(enc_key))
            ...
            self.commcell_machine.create_file(self.commcell_machine.join_path(self.commcell_base_directory, self.file_name),file_content)
            ...
            self.log.info(f"Setting reg key KeyMapMaxTimeout on CVD of MA [{self.source_ma}]")
            self.commcell.clients.get(self.source_ma).add_additional_setting("Cvd", "KeyMapMaxTimeout", "INTEGER", "60")
            ...
            self.log.info("Restarting services of source MA after setting the reg key")
