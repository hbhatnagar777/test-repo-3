...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Microsoft Office 365 Teams Custom Category Verification"
        self.browser = None
    ...
    def setup(self):
        self.custom_dict = self.tcinputs['custom_dict']
        self.create_tenant()
    ...
    def run(self):
        ...
        try:
            ...
            time.sleep(20)
            emails = self.office365_obj.get_teams_tab_table_data["Email address"]
            if len(emails) == 3:
                for email in emails:
                    if self.tcinputs['custom_dict']['name'] not in email:
                        raise Exception("Custom category verification failed due to name mismatch")
            else:
                raise Exception("Custom category verification failed due to count mismatch")
            self.log.info("Custom category is verified")
