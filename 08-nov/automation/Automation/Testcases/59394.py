...
class TestCase(CVTestCase):
    """Class for Cross Platform stubs restore Dmnas to Local, Celerra, Netapp."""
    ...
    def setup(self):
        ...
        self.netapp_folder_path = self.tcinputs.get('NetappPath', None) + '{0}{1}_data'.format(
            self.OPHelper.slash_format, str(self.OPHelper.testcase.id))
        self.UNC_dmnas_folder_path = "\\UNC-NT_" + self.dmnas_folder_path[2:]
    ...
    def run(self):
        ...
        2. Run archive jobs to backup, stub and backup stubs on Dmnas.
        3. Verify file size, file size on disk, attributes on Dmnas.
        ...
        try:
            self.log.info(_desc)
            self.prepare_test_data(self.dmnas_folder_path)
            self.prepare_subclient(self.dmnas_folder_path)
            self.OPHelper.run_archive(repeats=3)

            self.log.info("For Dmnas Client")
            time.sleep(240)
            self.verify_file_attributes(path=self.dmnas_folder_path,
                                        valid_attributes=self.tcinputs.get('DmnasStubAttributes',
                                                                           ['OFFLINE']))
            self.verify_file_size(path=self.dmnas_folder_path,
                                  valid_size=self.tcinputs.get("DmansStubSize", 1234))
            ...
            self.OPHelper.restore_out_of_place(client=self.tcinputs.get("ProxyClient"),
                                               ...
                                               paths=[self.OPHelper.client_machine.join_path(
                                                   self.UNC_dmnas_folder_path, file[0])
            ...
            self.verify_file_attributes(path=self.local_folder_path,
                                        valid_attributes=self.tcinputs.get('LocalRestoredStubAttributes',
                                                                           ['ARCHIVE', 'OFFLINE', 'SPARSEFILE',
                                                                            'REPARSEPOINT']))
            self.verify_file_size_on_disk(path=self.local_folder_path, valid_size=self.tcinputs.get("LocalStubSize", 0))
            ...
            self.OPHelper.restore_out_of_place(client=self.tcinputs.get("ProxyClient"),
                                               destination_path=self.celerra_folder_path,
                                               paths=[self.OPHelper.client_machine.join_path(
                                                   self.UNC_dmnas_folder_path, file[0])
                                                   for file in self.OPHelper.test_file_list],
                                               impersonate_user=self.tcinputs.get("ImpersonateUser"),
            ...
            self.OPHelper.restore_out_of_place(client=self.tcinputs.get("ProxyClient"),
                                               destination_path=self.netapp_folder_path,
                                               paths=[self.OPHelper.client_machine.join_path(
                                                   self.UNC_dmnas_folder_path, file[0])
                                                   for file in self.OPHelper.test_file_list],
                                               impersonate_user=self.tcinputs.get("ImpersonateUser"),
            ...
            self.log.info('Cross Platform stubs restore Dmnas to Local, Celerra, Netapp passed')

        except Exception as exp:
            self.log.error('Cross Platform stubs restore Dmnas to Local, Celerra, Netapp failed with error: %s', exp)
