...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Basic acceptance of SDG for backed up Exchange online with passkey disabled"
        self.tcinputs = {
            "IndexServer": None,
            "ProxyServers": None,
            "mailboxes": None,
    ...
    def setup(self):
        ...
        try:
            ...
            if self.commcell.is_passkey_enabled:
                self.commcell.passkey(_EXCHANGE_CONFIG_DATA.Password, "disable")
            ...
            self.proxies = self.tcinputs.get('ProxyServers')
            ...
            self.exch_sdg_helper = ExchangeSDGHelper(self.commcell, self.ex_mb_object)
            self.browser = BrowserFactory().create_browser_object()
            self.browser.open()
            self.admin_console = AdminConsole(self.browser, self.commcell.webconsole_hostname,
                                              username=_EXCHANGE_CONFIG_DATA.UserName,
                                              password=_EXCHANGE_CONFIG_DATA.Password)
            self.admin_console.login(username=_EXCHANGE_CONFIG_DATA.UserName,
                                     password=_EXCHANGE_CONFIG_DATA.Password)
    ...
    def run(self):
        ...
        try:
            ...
            if not self.is_test_step_complete(sc.CREATE_CLIENT):
                self._client = self.exch_sdg_helper.\
                    create_exchange_online_client(client_name=self.client_name,
                                                  ...
                                                  access_nodes_list=self.proxies)
            ...
            if not self.is_test_step_complete(sc.PERFORM_BACKUP):
                self.exch_sdg_helper.exchange_run_backup()
            ...
            if not self.is_test_step_complete(sc.PERFORM_SDG_DS_CRAWL):
                if not self.gdpr_obj.file_server_lookup_obj.wait_for_data_source_status_completion(
                        self.exchange_server_display_name, timeout=self.wait_time):
    ...
    def create_plan(self):
        ...
        if not self.is_test_step_complete(sc.CREATE_SDG_PLAN):
            ...
            self.gdpr_obj.plans_obj.create_data_classification_plan(
                ...
                _EXCHANGE_CONFIG_DATA.ContentAnalyzer, entities_list)
    ...
    def add_exchange_datasource(self):
        ...
        self.gdpr_obj.file_server_lookup_obj.add_exchange_server(
            ...
            cs.USA_COUNTRY_NAME,
            list_of_mailboxes=self.mailboxes,
