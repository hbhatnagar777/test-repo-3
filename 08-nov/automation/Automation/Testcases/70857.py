...
class TestCase(CVTestCase):
    ...
    for SAP Oracle using util_file device"""
    test_step = TestStep()
    ...
    def __init__(self):
        ...
        self.tblspaceg = "TSP70857"
        self.tblnameg = "T70857"
    ...
    def run_third_party_brrestore_data(self):
        ...
        if self.machine.os_info == 'UNIX':
            brrestore_cmd = r"su - " + self.saporacle_osapp_user + ' -c "brrestore -d \
                      util_file -b last -m all -c force -u /'
            self.log.info(" Command we are running is " + brrestore_cmd)
            if self.saporacle_sapsecurestore == 1:
                brrestore_cmd = f"{brrestore_cmd}/"
                self.log.info(" Command we are running is " + brrestore_cmd)
        else:
            brrestore_cmd = '"brrestore -d util_file -b last -m all -c force -u /'
            self.log.info(" Command we are running is " + brrestore_cmd)
            if self.saporacle_sapsecurestore == 1:
                brrestore_cmd = f"{brrestore_cmd}/"
                self.log.info(" Command we are running is " + brrestore_cmd)
    ...
    def run_third_party_brrestore_log(self):
        """ method to run brrestore of log from cmd
        ...
        self.log.info("##Running Third party commandline brrestore of log job##")
        if self.machine.os_info == 'UNIX':
            brrestore_logcmd = r"su - " + self.saporacle_osapp_user + ' -c "brrestore -d \
                      util_file -a ' +self.end_lsn+' -c force -u /'
            self.log.info(" Command we are running is " + brrestore_logcmd)
            if self.saporacle_sapsecurestore == 1:
                brrestore_logcmd = f"{brrestore_logcmd}/"
                self.log.info(" Command we are running is " + brrestore_logcmd)
        else:
            brrestore_logcmd = '"brrestore -d  util_file -a ' +self.end_lsn + ' -c force -u /'
            self.log.info(" Command we are running is " + brrestore_logcmd)
            if self.saporacle_sapsecurestore == 1:
                brrestore_logcmd = f"{brrestore_logcmd}/"
                self.log.info(" Command we are running is " + brrestore_logcmd)
        ...
        detail_file = self.saporacle_helper.thirdparty_cmd_backup_job(brrestore_logcmd, "initauto.utl",
                                                           " BRRESTORE completed successfully")
        if detail_file == None:
            raise cvexceptions.CVTestStepFailure("Failed to run thirdparty commandline brrestore of log job ")
        else:
            self.log.info("Successfully ran thirdparty commandline brrestore of log job")


    def run(self):
        """executes basic Third party commandline backup and inlace restore
         for SAP for Oracle using util_file device"""
