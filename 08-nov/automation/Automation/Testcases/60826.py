...
"""Test Case for global encryption disabled case
...
class TestCase(CVTestCase):
    ...
    3. Set encryption on the storage pool primary copy

    4. Access the disk storage that was created from Command Center

    5. In the Configuration tab, verify the encryption details
    ...
    def __init__(self):
        ...
        self.name = "Test Case for global encryption disable"
        self.result_string = ""
        self.ma_name = ""
        self.tcinputs = {
            "MediaAgentName": None,
    ...
    def create_machine_dirs(self, machine_name, machine_dirs, machine_min_storage):
        ...
        self.log.info(f'Selecting drive in the machine {machine_name} based on space available')
    ...
    def enable_encryption_on_copy(self):
        ...
        primary_copy.copy_reencryption = (True, self.encryption_cipher, self.encryption_key_length, False)
    ...
    def run(self):
        try:
            ...
            # 3. Enable encryption on the primary copy of storage pool
            self.enable_encryption_on_copy()

            # 4. Get the encryption details related to this storage
            info = self.storage_helper.disk_encryption_info(self.storage_pool_name)

            # 5. Verify if the info match what was set in the first step
            ...
            # 7. Create subclient / backupset
            subclient = self.create_subclient()

            # 8. Create data on client to backup
