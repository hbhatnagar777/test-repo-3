...
class TestCase(CVTestCase):
    ...
    """
    Testcase to validate operations as Tenant Operator.
    This testcase does the following --
    1. Connect to Kubernetes API Server using Kubeconfig File
    2. Create testbed
    3. Login to Command Center using Teanant Admin, Configure new Cluster and Application Group
    4. Switch to Tenant Operator account
    5. Verify Full NS level Backup Job
    6. Verify if operator is unable to browse App file restore
    7. Verify if operator is unable to browse Namespace restore
    8. Verify if operator is unable to browse Application restore
    9. Verify if operator is unable to browse Manifest restore
    10. Cleanup testbed
    """
    def __init__(self):
        super(TestCase, self).__init__()

        self.name = "Kubernetes Tenant Operator Operations"
    ...
    def create_testbed(self):
        ...
        self.kubehelper.create_cv_pod(
            self.pod_name, self.namespace, secret=secret_name, configmap=config_name
        )
        time.sleep(30)
    ...
    @test_step
    def switch_accounts(self, username, password, saml=False):
        """
        Switch account from Tenant Admin to tenant Operator
        """
        self.admin_console.logout()
        self.admin_console.login(username, password, saml=saml)
        self.log.info("Switched to Operator account..")
    ...
    @test_step
    def attempt_namespace_restore(self, inplace=False):
        """Step -- Run Namespace level restore"""

        """
        Attempt App file restore
        """
        self.log.info("Attempting App file restore")
        self.k8s_helper.attempt_restore_job(
            cluster_name=self.server_name,
            app_group_name=self.app_grp_name,
            restore_type='Namespace'
        )
        self.log.info("Proceeding with next steps..")

    @test_step
    def attempt_application_restore(self):
        """
        Attempt App file restore
        """
        self.log.info("Attempting App file restore")
        self.k8s_helper.attempt_restore_job(
            cluster_name=self.server_name,
            app_group_name=self.app_grp_name,
            restore_type='Application'
        )
        self.log.info("Proceeding with next steps..")

    @test_step
    def attempt_manifest_restore(self):
        """Step 6 -- Run Manifest level restore"""
        self.log.info("Attempting App file restore")
        self.k8s_helper.attempt_restore_job(
            cluster_name=self.server_name,
            app_group_name=self.app_grp_name,
            restore_type='Manifest'
        )
        self.log.info("Proceeding with next steps..")

    @test_step
    def attempt_app_file_restore(self):
        """
        Attempt App file restore
        """
        self.log.info("Attempting App file restore")
    ...
    def run(self):
        ...
        try:
            ...
            self.switch_accounts(self.tcinputs['ACUsername'], self.tcinputs['ACPassword'], saml=True)

            # Step 3 - Run FULL Backup job
            self.run_full_backup()

            # Step 4 - Attempt App file restore
            self.attempt_app_file_restore()

            # Step 5 - Attempt Manifest restore
            self.attempt_manifest_restore()

            # Step 7 - Attempt Namespace restore
            self.attempt_namespace_restore()
