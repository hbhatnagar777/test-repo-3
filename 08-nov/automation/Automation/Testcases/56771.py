...
class TestCase(CVTestCase):
    """Class for executing basic testcase for Plugin Upload, Configuration, Backup and restore
    for Vcloud through Plugin as Admin User"""

    def __init__(self):
        """" Initializes test case class objects"""""
        super(TestCase, self).__init__()
        self.name = "Basic testcase for Plugin Upload, Configuration, Backup and restore for Vcloud through Plugin as Admin User"
    ...
    def run(self):
        ...
        try:
            self.vcloud_helper.upload_plugin()
            self.vcloud_helper.setup()
            self.vcloud_helper.backup_and_restore(oop=True, standalone=False)
            self.vcloud_helper.enable_advanced_view()
