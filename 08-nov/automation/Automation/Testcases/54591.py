...
TestCase:
    ...
    set_v2_indexing()           -- sets v2 indexing
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "VSA V2.1 : CCM validation for VSA client"
        self.time = None
        self.tcinputs = {
    ...
    def create_pseudo_client_source(self):
        """Creates a VMware pseudo client on source commcell."""
    ...
    def verify_import(self):
        ...
        subclient_dest, backupset_dest = self.CCM_helper.get_latest_subclient(self.tcinputs["vm_name"],
                                                                              destination_commcell=True,
                                                                              agent=constants.Agents.VIRTUAL_SERVER)
        # restore from individual vm
        job4 = subclient_dest.full_vm_restore_out_of_place(vcenter_client=self.pseudo_client_dest_name,
                                                           proxy_client=self.destination_cs.commserv_hostname)
        if not job4.wait_for_completion():
            raise Exception(
                "Failed to run FULL backup with error: {0}".format(job4.delay_reason)
            )
        else:
            self.log.info("Backup job: %s completed successfully", job4.job_id)
