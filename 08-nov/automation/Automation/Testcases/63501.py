...
class TestCase(CVTestCase):
    """ [Laptop] [AdminConsole][EdgeMode]- Erase / Delete Folder Data"""
    ...
    def __init__(self):
        ...
        self.name = "[Laptop] [AdminConsole][EdgeMode]- Erase / Delete Folder Data"
    ...
    def verify_browse_result(self):
        """
        verify able to browse the folder or not after deletion
        """
        ...
        if self.folder_name in browse_res['Name']:
            raise CVTestStepFailure("Folder: {0} found in browse result even after erased".format(self.folder_name))
        self.log.info("*** As Expected ! folder {0} is not found in browse result after erased ***".format(self.folder_name))

    @test_step
    def validate_folder_erase(self):
        """
        Erase / Delete Folder in edgemode 
        """
        self.edgemain_obj.navigate_to_client_restore_page(self.tcinputs["Client_name"])
        self.rbrowse.navigate_path(self.tcinputs["Test_data_path"], use_tree=False)
    ...
    def create_testpath(self):
        ...
        folder = 'TC_'+str(self.id)
        self.folder_name = 'Erase_' + folder
        if self.tcinputs['os_type']=="Windows":
            ...
            self.folder_path = self.tcinputs["Test_data_path"] + '/' + self.folder_name
            self.file_name = 'Erasemacfile_'+str(local_time)+".txt"
            self.folder_file_name = self.folder_path + '/' + self.file_name
        # create test data
        self.laptop_utils.create_file(self.machine_object, self.folder_path, self.folder_file_name)
