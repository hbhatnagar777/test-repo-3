...
class TestCase(CVTestCase):
    """TestCase to validate In place restore and cluster view restore from admin console"""

    def __init__(self):
        """Initialize TestCase class"""
        super(TestCase, self).__init__()
        self.name = "Cassandra: DB/cluster view in place restore(non-stage free restore)"
        self.browser = None
    ...
    def setup(self):
        ...
        self.cassandra_server_name = "automated_cassandra_server_51467"
        ...
        self.keyspace = 'automationks51467'
    ...
    def run(self):
        ...
        try:
            ...
            # db view in place restore scenarios
            ...
            self.cassandra.drop_table_inplacerestore_sstableloader(
                destinstance=self.destinationinstance,
                paths=[
                    self.keyspace],
                keyspace=self.keyspace,
            ...
            # test cluster view in place restore scenarios
            self.cassandra.drop_keyspace_clusterview_inplacerestore(
                destinstance=self.destinationinstance,
                paths=[
                    self.keyspace],
                keyspace=self.keyspace,
            ...
            self.cassandra.drop_table_clusterview_inplacerestore(
                destinstance=self.destinationinstance,
                paths=[
                    self.keyspace],
                keyspace=self.keyspace,
            ...
            self.cassandra.drop_table_clusterview_inplacerestore_sstableloader(
                destinstance=self.destinationinstance, paths=[
                    self.keyspace], keyspace=self.keyspace, tablename=self.tablename, rows=self.rows)
