...
_TENANT_CONFIG_DATA = get_config().DynamicIndex.ActivateTenant.Prod_Tenant01
_SHAREPOINT_CONFIG_DATA = get_config().DynamicIndex.ActivateTenant.Prod_Tenant01.SharePoint


class TestCase(CVTestCase):
    """Class For executing Activate tenant cases for Sensitive data governance of sharepoint datasource
    for creating inventory, plan, project, datasource and review the sensitive content"""
    test_step = TestStep()

    def __init__(self):
        super(TestCase, self).__init__()
        self.name = "Activate Automation to support SDG sharepoint DS in tenant environment for creating inventory, " \
                    "plan, project, datasource and review the sensitive content"
    ...
    def setup(self):
        ...
        try:
            ...
            self.company_name = _TENANT_CONFIG_DATA.Name
            self.browser = BrowserFactory().create_browser_object()
            self.browser.open()
            username = f"{_TENANT_CONFIG_DATA.alias}\\{_SHAREPOINT_CONFIG_DATA.UserName}"
    ...
    def create_plan(self):
        ...
        """
        entity_json = _SHAREPOINT_CONFIG_DATA.EntitiesListMap._asdict().__str__()
        entities_list_map = json.loads(entity_json.replace("_", " ").replace("'", '"'))
    ...
    def add_sharepoint_datasource(self):
        ...
        """
        list_of_sites = f"{_SHAREPOINT_CONFIG_DATA.V2.ClientName.Advanced.lower()}," \
        ...
        self.gdpr_obj.file_server_lookup_obj.add_share_point_server(
            _SHAREPOINT_CONFIG_DATA.V2.ClientName.Advanced,
            cs.CLIENT_NAME,
