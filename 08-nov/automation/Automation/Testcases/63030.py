...
class TestCase(CVTestCase):
    """Testcase for VME to Vmware in a Two Way Topology"""

    def __init__(self):
        super(TestCase, self).__init__()
        self.name = "Virtualize me to VMWare in Two Way Topology"
    ...
    def run(self):
        ...
        try:
            ...
            backupset_name = "Test_63030"

            self.log.info("Step 1: Checking if Client group and CS group already exist on the CS.")
            ...
            self.log.info("Step 2: Creating Client group and CS group")
            ...
            self.log.info("Step 3: Setting Two Way Topology")
            self.topology_name = "63030_twoway_" + self.time_stamp

            self.network.two_way_topology(self.tcinputs['FirewallClientGroup'],
                                        self.tcinputs['CSClientGroup'],
                                        self.topology_name)

            self.log.info("Step 4: Validating Network Gateway Topology")
            self.network.validate_two_way_topology(self.topology_name)
