...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Metallic Automation: Verify incremental backup case for Office 365 exchange app"
        self.browser = None
    ...
    def run(self):
        ...
        try:
            ...
            self.navigator.navigate_to_office365()
            self.office365_obj.access_office365_app(self.app_name)
            incr_job_details = self.office365_obj.run_backup()
            self.log.info(f"Number of items in full job = {bkp_job_details['Successful messages']}, \n"
                          f"Number of items in incr job = {incr_job_details['Successful messages']}")
            if not(bkp_job_details['Successful messages'] > 0 and incr_job_details['Successful messages'] == 0):
                raise Exception(f'Incremental job is not verified')
            self.log.info("Incremental job is verified")
