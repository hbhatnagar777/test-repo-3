...
class TestCase(CVTestCase):
    """Class for executing and verifying Exchange Cleanup with multiple Cleanup Policies"""
    ...
    def __init__(self):
        ...
        self._client = None
        self.name = "Exchange Mailbox Agent : Cleanup Operation and Validation"
    ...
    def setup(self):
        ...
        cleanup_policy_default_obj = self.exmbclient_object.cvoperations.add_exchange_plan(
            plan_name=cleanup_policy_default)
        self.cleanup_policies_list.append(cleanup_policy_default_obj)
        cleanup_policy_older_than_obj = self.exmbclient_object.cvoperations.add_exchange_plan(
            plan_name=cleanup_policy_older_than, cleanup_msg_older_than=30)
        self.cleanup_policies_list.append(cleanup_policy_older_than_obj)
        cleanup_policy_larger_than_obj = self.exmbclient_object.cvoperations.add_exchange_plan(
            plan_name=cleanup_policy_larger_than, cleanup_msg_larger_than=10)
        self.cleanup_policies_list.append(cleanup_policy_larger_than_obj)
        cleanup_policy_add_recall_link_obj = self.exmbclient_object.cvoperations.add_exchange_plan(
            plan_name=cleanup_policy_add_recall_link)
        self.cleanup_policies_list.append(cleanup_policy_add_recall_link_obj)
        cleanup_policy_leave_message_body_obj = self.exmbclient_object.cvoperations.add_exchange_plan(
            plan_name=cleanup_policy_leave_message_body)
        self.cleanup_policies_list.append(cleanup_policy_leave_message_body_obj)
        cleanup_policy_skip_unread_message_obj = self.exmbclient_object.cvoperations.add_exchange_plan(
            plan_name=cleanup_policy_skip_unread_message, skip_unread_msgs=True)
        self.cleanup_policies_list.append(cleanup_policy_skip_unread_message_obj)
        cleanup_policy_has_attachments_obj = self.exmbclient_object.cvoperations.add_exchange_plan(
            plan_name=cleanup_policy_has_attachments, collect_msg_with_attach=True)
        self.cleanup_policies_list.append(cleanup_policy_has_attachments_obj)
