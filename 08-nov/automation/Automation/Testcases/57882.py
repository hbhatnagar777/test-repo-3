...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        using One Drive as Data Source")
    ...
    @test_step
    def review_actions_onedrive(self):
        """
        ...
        try:
            set_retention_action_status = \
                self.gdpr_obj.data_source_review_obj.review_set_retention_action(
                    file2, 1, data_source_type=cs.ONE_DRIVE
                )
        except CVWebAutomationException as error_status:
        ...
        if not set_retention_action_status and not exp_flag:
            self.test_case_error = f'Set Retention Action Failed:- Retention Not Set'
            self.error_dict[f'Review Retention Actions : {file2}'] = self.test_case_error
        ...
        try:
            ingore_files_action_status = \
                self.gdpr_obj.data_source_review_obj.review_ignore_files_action(
                    file1, data_source_type=cs.ONE_DRIVE
                )
        except CVWebAutomationException as error_status:
        ...
        exp_flag = False
        try:
            delete_files_action_status = \
                self.gdpr_obj.data_source_review_obj.review_delete_action(
                    file2, data_source_type=cs.ONE_DRIVE
                )
        except CVWebAutomationException as error_status:
            self.test_case_error = f"Delete Files Action Failed:- {str(error_status)}"
            self.error_dict[f'Review Delete Files Actions : {file2}'] = self.test_case_error
            self.gdpr_obj.data_source_review_obj.close_action_modal()
            exp_flag = True
        if not delete_files_action_status and not exp_flag:
            self.test_case_error = f'Delete Files Action Failed:-File Not Deleted'
            self.error_dict[f'Review Delete Files Actions : {file2}'] = self.test_case_error
