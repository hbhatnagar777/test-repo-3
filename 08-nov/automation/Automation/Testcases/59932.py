...
from FileSystem.FSUtils.fshelper import FSHelper, ScanType, CommitCondition
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Subclient Directory Cleanup - Commit Job Scenario"
        self.tcinputs = {
            "TestPath": None,
    ...
    def setup(self):
        """Setup for automation -
            ...
            Note : Entities created will be deleted in teardown phase
        """
        self.log.info("*" * 20)
        ...
        self.log.info("""Setup for automation - 
            1. Poplulate testcase inputs
            2. Create backupset with name - auto_backupset_<tc_id>
            3. Creating regkey CopySubclientJobResultsFolderAfterScan
        """)
        ...
        self.log.info("Setting up test data, backupsets and subclients.")

        if self.test_path.endswith(self.slash_format):
            self.test_path = str(self.test_path).rstrip(self.slash_format)

        self.log.info("Creating backupset, subclient and initializing the subclient content")
    ...
    def run(self):
        ...
        self.log.info("""Do the following operations for each scan type (Recursive, Optimized, ChangeLog)-
                            Step 1.
                                1.1. Run full job --> Commit
                                1.2. Run Incremental job till completion
                                1.3. Validate SC Directory cleanup
                            Step 2.
                                2.1. Run full job --> Commit
                                2.2. Run inc job --> Commit
                                2.3. Run inc job
                                2.4. Validate SC Dir cleanup
                            Step 3.
                                3.1. Run full job --> Commit
                                3.2. Run Inc job --> Commit
                                3.3. Run Synthfull
                                3.4. Run Inc
                                3.5. Validate SC Dir cleanup
                            Step 4.
                                4.1. Run full job --> Commit
                                4.2. Run Inc
                                4.3. Run full
                                4.4. Validate SC Dir cleanup
        """)
        ...
        try:
            for scan_type in ScanType:
                ...
                self.log.info("1.1. Run full job --> Commit")
                self.log.info("1.2. Run Incremental job.")
                self.log.info("1.3. Validate SC Directory cleanup")
                self.log.info("*" * 20)

                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_FULL_1",
                                         self.slash_format, scan_type, kwargs={"file_size": 4000, "files": 20}
                                         )
                ...
                job_full = helper.run_backup(
                    backup_level="Full",
                    wait_to_complete=False
                )[0]

                helper.commit_job(job_full, self.threshold, CommitCondition.FILES)
                ...
                if not job_incr.wait_for_completion():
                    raise Exception(
                        "Failed to run INCREMENTAL Job with reason: {}".format(job_incr.delay_reason)
                    )
                else:
                    self.log.info("Successfully ran INCREMENTAL Job with ID : {}".format(job_incr.job_id))
                    self.log.info("Vaidating Subclient Directory Cleanup...")
                    helper.validate_subclient_dir_cleanup(job_incr)
                ...
                self.log.info("2.1. Run Full job --> Commit")
                self.log.info("2.2. Run Incremental job --> Commit")
                self.log.info("2.3. Run Incremental job")
                self.log.info("2.4. Validate SC Dir cleanup")
                self.log.info("*" * 20)

                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_FULL_2",
                                         self.slash_format, scan_type, kwargs={"file_size": 4000, "files": 20}
                                         )
                helper.mod_data_incr(scan_type)
                self.folders_created.append(self.test_path + self.slash_format + scan_type.name + "_FULL_2")
                ...
                helper.commit_job(job_incr, self.threshold, CommitCondition.FILES)
                ...
                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_INC_3",
                                         self.slash_format, scan_type)
                helper.mod_data_incr(scan_type)
                self.folders_created.append(self.test_path + self.slash_format + scan_type.name + "_INC_3")
                ...
                self.log.info("3.1. Run Full job --> Commit")
                self.log.info("3.2. Run Incremental job --> Commit")
                self.log.info("3.3. Run Synthetic Full job")
                self.log.info("3.4. Run Incremental job")
                self.log.info("3.5. Validate SC Dir cleanup")
                self.log.info("*" * 20)

                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_FULL_3",
                                         self.slash_format, scan_type, kwargs={"file_size": 4000, "files": 20})
                self.folders_created.append(self.test_path + self.slash_format + scan_type.name + "_FULL_3")
                ...
                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_INC_4",
                                         self.slash_format, scan_type, kwargs={"file_size": 4000, "files": 20}
                                         )
                helper.mod_data_incr(scan_type)
                self.folders_created.append(self.test_path + self.slash_format + scan_type.name + "_INC_4")
                ...
                if not job_synth.wait_for_completion():
                    raise Exception(
                        "Failed to run SYNTHFULL Job with reason: {}".format(job_synth.delay_reason)
                    )
                else:
                    self.log.info("Successfully ran SYNTHFULL Job with ID : {}".format(job_synth.job_id))
                    self.log.info("Vaidating Subclient Directory Cleanup...")

                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_INC_5",
                                         self.slash_format, scan_type, kwargs={"file_size": 4000, "files": 20}
                                         )
                helper.mod_data_incr(scan_type)
                self.folders_created.append(self.test_path + self.slash_format + scan_type.name + "_INC_5")
                ...
                self.log.info("4.1. Run Full job --> Commit")
                self.log.info("4.2. Run Incremental job")
                self.log.info("4.3. Run Full job")
                self.log.info("4.4. Validate SC Dir cleanup")
                self.log.info("*" * 20)

                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_FULL_4",
                                         self.slash_format, scan_type, kwargs={"file_size": 4000, "files": 20})
                self.folders_created.append(self.test_path + self.slash_format + scan_type.name + "_FULL_4")
                ...
                helper.add_new_data_incr(self.test_path + self.slash_format + scan_type.name + "_INC_6",
                                         self.slash_format, scan_type)
                helper.mod_data_incr(scan_type)
                self.folders_created.append(self.test_path + self.slash_format + scan_type.name + "_INC_6")
                ...
                self.log.info("*" * 20)
                self.log.info("All steps completed for scan type {}.".format(scan_type.name))
                self.log.info("*" * 20)
