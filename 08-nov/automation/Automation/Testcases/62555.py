...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Collection Pruning integration test case for Exchange backed up client - validate create, run " \
                    "backup, delete client, delete collections from index directory. Validate pruning of exchange " \
                    "datasources and collections from DB and validate audit trail logs"
        self.tcinputs = {
            "AgentName": None,
            "BackupsetName": None,
            "SubclientName": None,
    ...
    def setup(self):
        ...
        try:
            if isinstance(self.tcinputs["ExchangeServerName"], str):
                self.tcinputs["ExchangeServerName"] = self.tcinputs["ExchangeServerName"].split(",")
            if isinstance(self.tcinputs["ProxyServers"], str):
                self.tcinputs["ProxyServers"] = self.tcinputs["ProxyServers"].split(",")
            if isinstance(self.tcinputs["ServiceAccountDetails"], str):
                self.tcinputs["ServiceAccountDetails"] = [literal_eval(self.tcinputs["ServiceAccountDetails"])]
    ...
    def run(self):
        ...
        try:
            ...
            self.exchange_pruning_helper.delete_collection_folder(datasource_actual_name)
