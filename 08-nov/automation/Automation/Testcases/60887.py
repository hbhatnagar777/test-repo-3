...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Commvault Complete for Kubernetes - Add cluster and application group from Global Search"
    ...
    def init_inputs(self):
        ...
        self.is_metallic = automation_config.IS_METALLIC
    ...
    def __complete_wizard(self, step):
        """Complete wizard steps"""
        self.k8s_helper.wizard_step_caller(
            step=step,
            ...
            plan_name=self.plan,
            deployment_method=self.tcinputs.get('deployment_method', "ONPREM")
        )

    @test_step
    def add_cluster_step_from_global_search(self):
        """Add cluster step of guided setup wizard"""
        self.admin_console.navigator.add_entity_from_search_bar('Cluster')
        wizard_steps = ["Access Node", "Plan", "Add Cluster"]
        if self.is_metallic:
            wizard_steps = ["Select Kubernetes service or distribution", "Backup method overview",
                            "Select Backup Gateways", "Local Storage", "Cloud Storage", "Plan", "Add Cluster"]

        for step in wizard_steps:
            self.__complete_wizard(step=step)

        self.__cluster_created = True

    @test_step
    def add_app_grp_step_global_add(self):
        """Add application group step of guided setup wizard /Add command"""

        self.admin_console.navigator.add_entity_from_search_bar('Application group')

        wizard_steps = ["Select Cluster", "Plan", "Add Application Group"]
        for step in wizard_steps:
            self.__complete_wizard(step=step)
    ...
    def run(self):
        ...
        try:
            ...
            self.add_cluster_step_from_global_search()

            # Step 2 - Add application group from app group listing page
            self.add_app_grp_step_global_add()
