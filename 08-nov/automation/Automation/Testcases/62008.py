...
class TestCase(CVTestCase):
    ...
    def tenant_ops(self):
        """
        tenant level operations, like create new tenant and install ad agent
        return:
        """
        if self.tcinputs["NewTenant"]:
            self.log.info("will create a new tenant")
            self.log.info("will add create tenant opertion later")
        else:
            self.log.info("use input file user name and password to login")
    ...
    @test_step
    def ad_full_backup(self):
        """
        Test ad full backup
        """
        return self.cvad.ad_simple_backup(backuptype="Full", tcinputs=self.tcinputs, prestring=self.id)

    @test_step
    def ad_inc_backup(self):
        """
        Test ad inc backup
        """
        return self.cvad.ad_simple_backup(backuptype="Inc", tcinputs=self.tcinputs, prestring=self.id)

    @test_step
    def ad_restore(self, restoreobjs):
        """
        Test ad restore object only
        Args:
            restoreobjs    (list):    ad objects to restore
        """
        return self.cvad.ad_simple_restore(restoreobjs)

    def aad_create_client(self):
        """
        create azure ad client if necessary
        """
        aad_clientname = f"AAD_{self.id}"
    ...
    def run(self):
        ...
        try:
            if not self.tcinputs["SkipAD"]:
                self.adminconsole.navigator.navigate_to_activedirectory()
                self.adclientspage = ADClientsPage(self.adminconsole)
                self.log.info("select client")
                self.adclientspage.select_client(self.tcinputs['ExistingClientName'])
                self.log.debug(f"client {self.tcinputs['ExistingClientName']} page is opened")
                self.adclientpage = ADPage(self.adminconsole, self.commcell)
                self.cvad = CvAd(self.adclientpage, self.tcinputs)

                self.log.info("start ad agent testing")
                self.ad_full_backup()
                inc_objs = self.ad_inc_backup()
                self.ad_restore(inc_objs)
                self.clean_up_objs['ad_inc'] = inc_objs
            else:
                self.log.info("skip ad agent, just run azure ad client")

            self.adminconsole.navigator.navigate_to_activedirectory()

            if not self.tcinputs["SkipAzureAD"]:
            ...
            if self.tcinputs['KeepAADclient']:
                ...
                if not self.tcinputs["SkipAzureAD"] and not self.tcinputs["NewAADclient"]:
