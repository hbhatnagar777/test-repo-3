...
class TestCase(CVTestCase):
    """Admin Console: Adjusting write capacity during dynamodb restore"""
    ...
    def __init__(self):
        ...
        self.name = "DynamoDB: Adjusting write capacity during restore"
        self.browser = None
    ...
    @test_step
    def validate_capacity_change(self, table_name):
        """Submits in place restore and validates if table's write capacity was
        increased and reverted after restore job is complete"""
        ...
        restore_panel_obj = self.dynamodb_subclient.restore_files_from_multiple_pages(
            self.database_type, mapping_dict, 'Sub_56522')
        jobid = restore_panel_obj.same_account_same_region(adjust_write_capacity=5)
        job_status = self.wait_for_job_completion(jobid)
        if not job_status:
            raise CVTestStepFailure(f"In place restore job: {jobid} failed")
        if (self.dynamodb_helper.get_table_id(table_name) == org_table_id or
                self.dynamodb_helper.get_number_of_decreases(table_name) != 1):
            raise CVTestStepFailure("Table's capacity was not changed or table "
                                    "was not created with expected attributes")
    ...
    def run(self):
        ...
        try:
            ...
            add_subclient_obj.add_dynamodb_subclient('Sub_56522', self.tcinputs['plan'],
                                                     subclient_content, streams=1)
            self.submit_backup()
            self.admin_console.refresh_page()
            self.validate_capacity_change(table_name)
            self.dynamodb_helper.validate_dynamodb_table(table_name, 'id', 50)
            self.cleanup()
    ...
    def tear_down(self):
        ...
        self.dynamodb_helper.delete_dynamodb_table('TC_56522')
