...
class TestCase(CVTestCase):
    ...
    def run(self):
        ...
        try:
            self.log.info("[+] Performing a Production Maintenance failover [+]")
            output = self.active_node.production_maintenance(self.passive_node_name, wait_for_completion=True)
            if output[0] != 0:
                raise Exception(f"Production Maintenance Failover failed with error: \n {output[1]}")
            ...
            if active_node_role == "Stand By" and passive_node_role == "Read Only":
            ...
            time.sleep(120)
            self.log.info("[+] Performing a Maintenance failback [+]")
            output = self.active_node.maintenance_failback(self.active_node_name, wait_for_completion=True)
            if output[0] != 0:
                raise Exception(f"Maintenance Failback failed with error: \n {output[1]}")
            details = self.active_node.latest_failover_details()
            if "SUCCEEDED" not in details["endDetail"]:
                raise Exception(f"Failed to perform Maintenance failback from {self.passive_node_name}.\n"
                                f"{details['endDetails']} ")
            active_node_role = self.active_node.get_node_info(self.active_node_name)
            passive_node_role = self.active_node.get_node_info(self.passive_node_name)
            if active_node_role == "Production Node" and passive_node_role == "Stand By":
                self.log.info(f"Maintenance Failback from node {self.passive_node_name} succeeded")
            else:
                raise Exception(f"Failed to perform Maintenance failback from {self.passive_node_name}.\n"
                                f"{self.active_node_name} -- {active_node_role} \n"
                                f"{self.passive_node_name} -- {passive_node_role}")
