...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - VTL backup and restore of pre-defined subclient *IBM and validate subclient defaults.
    ...
    def __init__(self):
        ...
        self.name = " 	IBMi - VTL backup and restore of pre-defined subclient *IBM and validate subclient defaults."
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Step3: Run a full backup for the subclient *IBM "
                          "and verify if it completes without failures and proper backup command is used.")
            ...
            self.helper.verify_from_log('cvbkpvtl*.log',
                                        ...
                                        expectedvalue="SAVLIB LIB(".format(ibm_lib[0]))
            self.helper.verify_from_log('cvbkpvtl*.log',
                                        'runEachCommand',
                                        jobid=self.job.job_id,
                                        expectedvalue="DEV(")
            ...
            self.helper.verify_from_log('cvbkpvtl*.log',
                                        'runEachCommand',
                                        jobid=self.job.job_id,
                                        expectedvalue="{0}".format(ibm_lib[0]))
            self.helper.verify_from_log('cvbkpvtl*.log',
                                        'runEachCommand',
                                        jobid=self.job.job_id,
                                        expectedvalue="{0}".format(ibm_lib[1]))
            self.log.info("step9: Check scan logs to confirm subclient default values are used.")
            self.verify_sc_defaults(self.job.job_id)

            self.log.info("Step10: run OOP restore of both libraries and verify the data.")
            for each in ibm_lib:
            ...
            self.log.info("**VTL BACKUP & RESTORE OF SC:*IBM COMPLETED SUCCESSFULLY**")
