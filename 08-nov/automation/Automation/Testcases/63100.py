...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Metallic Automation: Verify incremental backup case for Office 365 teams app"
        self.browser = None
    ...
    def run(self):
        ...
        try:
            ...
            full_bkp_job_details = self.office365_obj.run_backup()
            self.navigator.navigate_to_office365()
            self.office365_obj.access_office365_app(self.app_name)
            incr_job_details = self.office365_obj.run_backup()
            self.log.info(f"Number of items in full job = {full_bkp_job_details['No of objects']}, \n"
                          f"Number of items in incr job = {incr_job_details['No of objects']}")
            if not (full_bkp_job_details['No of objects'] > 0 and incr_job_details['No of objects'] == 0):
                raise Exception(f'Incremental is not verified')
