...
"""Test Case for horizontal expansion"""
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Test Case for sds entries population"
    ...
    def run(self):
        try:
            ...
            before_expansion = self.hyperscale_helper.get_associated_mas(self.storage_pool_name)

            self.log.info("Number of nodes in the current storage pool = %s", str(len(rows)))
            old_count = len(before_expansion)
            self.log.info("Current storage pool contains the following nodes: ")
            for row in before_expansion:
                self.log.info(row[0])
            ...
            self.log.info("Checking sds entries population after expansion")
            sds_status = True
            for host in before_expansion:
                hostname = host[0]
                for entry_host in rows:
                    entry_hostname = entry_host[0]
                    if entry_host not in before_expansion:
                        sds_status = sds_status and self.hyperscale_helper.check_sds_entry(entry_hostname, hostname)
            if sds_status:
                self.log.info("All sds entries populated")
            else:
                self.log.error("sds entries not populated upon expansion")
                raise Exception("sds entries not populated upon expansion")
            gluster_brick_status = self.hyperscale_helper.gluster_disk_health(all_nodes, disk_uuids)
