...
TestCase:
    ...
    change_port_number()                --  Gets the current port number and sets
                                            new port number as previous port number + 1
    run_crawl_job()                     --  Runs the crawl job
    verify_crawl_job()                  --  Verifies the crawl job by matching the number of files.
    delete_index_server()               --  Perform Cleanup Operation for the test case and deletes the Index Server.
...
from cvpysdk.index_server import IndexNode
...
class TestCase(CVTestCase):
    """Class for Verifying modification(changing Port Number) of a Linux Index Server"""
    test_step = TestStep()
    ...
    def __init__(self):
        ...
        self.name = "Verifying modification(changing Port Number) of a Linux Index Server"
    ...
    def change_port_number(self):
        """Changes the port number of the index server node"""

        self.log.info("Getting the current Port Number and increasing it by 1.")
        index_node = IndexNode(self.commcell, self.index_server_name, self.tcinputs["Index_Server_Node"])
        port_no = index_node.solr_port
        self.log.info(f"Current Port Number {port_no}")
        index_node.solr_port = port_no+1
        self.log.info(f"New Port Number {index_node.solr_port}")
