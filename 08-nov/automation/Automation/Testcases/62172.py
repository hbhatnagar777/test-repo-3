...
DATABASE = 'tc_62172'
APP_NAME = f"TC_62172_{datetime.now().strftime('%d_%B_%H_%M')}"
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = "Salesforce: Record Level Restore (PostgreSQL)"
        self.browser = None
    ...
    def create_app(self):
        ...
        try:
            infra_options = self.sf_helper.updated_infrastructure_options(
                ...
                **self.sf_helper.postgresql_options.__dict__
            )
        ...
        try:
            for saved_data, version in zip(
                ...
                restore_data = self.sf_restore.get_rows_from_record_level_restore(
                    ...
                    record_ids=[record['Id'] for record in self.data],
                    version=version,
                    fields=[field.lower() for field in self.data[0].keys()]
                )
                ...
                for saved_record in saved_data:
                    if not(any(
                            all(value == restore_record[key.lower()] for key, value in saved_record.items())
