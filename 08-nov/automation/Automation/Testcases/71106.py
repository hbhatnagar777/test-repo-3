...
class TestCase(CVTestCase):
    """ Class for executing Archivelog second copy TestCase using \
    ...
    def __init__(self):
        ...
        self.name = "SAP Oracle Archivelog second copy test from Command Center"
        self.browser = None
        self.admin_console = None
        self.navigator = None
        self.tablespace_name = 'CV_71106_TSP'
        self.table_name = "CV_71106_TABLE"
    ...
    def data_validation(self):
        ...
        status = self.sap_oracle_helper_object.fetch_brtools_log_validation(self.backup_job_id, \
                                                                            "backup", \
                                                                            "brarchive -d util_file  -OSC -cds ")
        if status == 0:
            self.log.info("Brtools validation is correct for archivelog second copy")
        else:
            raise CVTestStepFailure(
                "Brtools validation is not correct for archivelog second copy")
