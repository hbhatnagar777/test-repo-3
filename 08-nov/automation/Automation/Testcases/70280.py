...
class TestCase(CVTestCase):
    """ Class for executing Basic acceptance Test for Sybase HADR on Command Center"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "ACCT1-Acceptance Test for Sybase IDA Command Center"
        self.browser = None
    ...
    def tear_down(self):
        ...
        self.cv_helper[self.primary_node.client_name].sybase_cleanup_test_data(self.database_name)
        self.helper[self.primary_node.client_name].sybase_helper_cleanup()

    @test_step
    def create_sybase_helper_object(self):
        """Creates object of sybasehelper class"""
        # Set Instance and Nodes
        self.pseudoClient = self.commcell.clients.get(self.tcinputs["PseudoClientName"])
        self.instance = self.pseudoClient.agents.get("sybase").instances.get(self.tcinputs["InstanceName"])
        self.primary_node = self.commcell.clients.get(self.instance.hadr_primarynode_id)
        for node in self.tcinputs["Nodes"]:
            client = self.commcell.clients.get(node.get('name'))
            self.Nodes.append(client)

        # Create helper objects Dict
        for node in self.Nodes:
            self.helper[node.client_name] = SybaseHelper(self.commcell, self.instance, node, hadr=True)
            self.cv_helper[node.client_name] = SybaseCVHelper(self.helper[node.client_name])
            self.helper[node.client_name].csdb = self.csdb
            self.helper[node.client_name].sybase_sa_userpassword = self.helper[
                node.client_name].get_sybase_user_password(hadr=True)
    ...
    def navigate_to_instance(self):
        ...
        self.database_instances.select_instance(DBInstances.Types.SYBASE,
                                                ...
                                                self.tcinputs["PseudoClientName"])

    @test_step
    def add_instance(self):
        """Adds new instance"""
        self.database_instances.add_sybasehadr_instance(self.tcinputs['Plan'],self.tcinputs['InstanceName'],
                                                        self.tcinputs['Nodes'])
    ...
    def backup_and_validate(self, backup_type):
        ...
        status = self.cv_helper[self.primary_node.client_name].backup_syntax_check(actual_backup_type, job_id, [self.database_name])
        if status:
    ...
    def restore_validate(self, user_database_name, user_table_list, db_list=None, all_db=False):
        """Executes restore according to restore type input and validates restore
            ...
            lall_db (Boolean):  Executes full server restore
                default: False
        """
        self.admin_console.select_breadcrumb_link_using_text(self.tcinputs["InstanceName"])

        for node in self.Nodes:
            # Shutdown Sybase RMA server
            self.helper[node.client_name].shutdown_sybasehadr_rma()

            # Shutdown Sybase REP server
            self.helper[node.client_name].shutdown_sybasehadr_rep()

        status, all_database_list_before_restore = self.helper[self.primary_node.client_name].get_database_list()
        all_table_list_before_restore = self.cv_helper[self.primary_node.client_name].get_all_database_tables()
        table_content_before = dict.fromkeys(user_table_list, None)
        for table in user_table_list:
            status, table_content_before[table] = self.helper[self.primary_node.client_name].get_table_content(
                user_database_name,
                table)

        # Drop all user databases before full server restore
        self.cv_helper[self.primary_node.client_name].drop_user_databases(all_database_list_before_restore)

        primary_server_shutdown = False
        # Disable sybase Hadr admin on Primary Node
        if not self.helper[self.primary_node.client_name].deactivate_sybase_hadr_admin():
            primary_server_shutdown = self.helper[self.primary_node.client_name].shutdown_with_no_wait()

        for node in self.Nodes:
            # Shutdown sybase server before full server restore
            if node.client_name == self.primary_node.client_name and primary_server_shutdown:
                continue
            self.helper[node.client_name].shutdown_sybase_server()
        ...
        self.restore_panel = self.db_instance_details.restore_folders(DBInstances.Types.SYBASE,
        ...
        status, all_database_list_after_restore = self.helper[self.primary_node.client_name].get_database_list()
        all_table_list_after_restore = self.cv_helper[self.primary_node.client_name].get_all_database_tables()
        table_content_after = dict.fromkeys(user_table_list, None)

        for table in user_table_list:
            status, table_content_after[table] = self.helper[self.primary_node.client_name].get_table_content(
                user_database_name,
                table)

        if all_database_list_before_restore == all_database_list_after_restore:
            database_status = True
        if self.cv_helper[self.primary_node.client_name].comparing_dict_of_list(
                all_table_list_before_restore,
                all_table_list_after_restore):
            table_status = True
        table_content_status = self.cv_helper[self.primary_node.client_name].comparing_dict_of_list(
    ...
    def run(self):
        ...
        try:
            ...
            self.cv_helper[self.primary_node.client_name].sybase_populate_data(self.database_name, user_tables[0])
            self.backup_and_validate(RBackup.BackupType.FULL)
            self.cv_helper[self.primary_node.client_name].single_table_populate(self.database_name, user_tables[1])
