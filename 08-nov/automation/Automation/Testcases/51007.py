...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - Validation of backup and restore with object level LFS content with special characters
        Step1, configure BackupSet and Subclients for TC with object level special char content
    ...
    def __init__(self):
        ...
        self.name = "IBMi - Validation of backup and restore with object level LFS content with special characters"
    ...
    def generate_lfs_data(self):
        ...
        for each in self.src_lib:
            ...
            self.client_machine.populate_lib_with_data(library_name=each, tc_id=self.id, count=0, prefix="F")
    ...
    def run(self):
        ...
        try:
            ...
            self.src_lib = ['L_$@#X', '"l_$&#x"']
            self.content = ['/QSYS.LIB/L_$@#X.LIB/Z_$@#X.FILE',
                            '/QSYS.LIB/L_$@#X.LIB/"z_$@#x".FILE',
                            '/QSYS.LIB/"l_$&#x".LIB/Z_$@#X.FILE',
                            '/QSYS.LIB/"l_$&#x".LIB/"z_$@#x".FILE',
                            '/QSYS.LIB/L_$@#X.LIB/Z_$@#I.FILE',
                            '/QSYS.LIB/L_$@#X.LIB/"z_$@#i".FILE',
                            '/QSYS.LIB/"l_$&#x".LIB/Z_$@#I.FILE',
                            '/QSYS.LIB/"l_$&#x".LIB/"z_$@#i".FILE',
                            '/QSYS.LIB/L_$@#X.LIB/Z_$@#D.FILE',
                            '/QSYS.LIB/L_$@#X.LIB/"z_$@#d".FILE',
                            '/QSYS.LIB/"l_$&#x".LIB/Z_$@#D.FILE',
                            '/QSYS.LIB/"l_$&#x".LIB/"z_$@#d".FILE'
                            ]
            self.dest_lib = "TR{0}".format(self.id)
            self.log.info("Create subclient for LFS with object level paths")
            ...
            self.restore_verify()
            self.log.info("Verified LFS backup and restore for object level content with special characters ")
            self.cleanup()
            self.log.info("**LFS CONTENT WITH OBJECT LEVEL PATHS AND SPECIAL CHARS BACKUP AND RESTORE "
