...
class TestCase(CVTestCase):
    ...
    option2, such that owners should be set honoring the chosen options"""
    ...
    def __init__(self):
        ...
        Commcell-disabled, Company1-option1, Company2-option2, such that owners should be set honoring the chosen\
        options"
        self.browser = None
    ...
    def select_option_for_commcell(self, enable=False, option=""):
        """
            Unserts automatic ownership assignment at the commcell level
        """
    ...
    def verify_owner_assignment_config_for_company(self, company_id, want_ownership_type):
        ...
        try:
            ...
            Organization(self.commcell, organization_id=company_id, organization_name=self.tcinputs['Tenant_company'])\
                .verify_owner_assignment_config(want_ownership_type)
        except:
            self.log.error("Failed to verify ownership assignment configuration")
        else:
    ...
    def refresh(self, install_with_authcode=False, with_org=False, company=1):
        ...
        Args:
            ...
            company (int): Differentiate between company 1 and 2
        ...
        if company == 2:
            self.install_kwargs["expected_owners"] = self.tcinputs["Expected_LoggedInFirstUsers_Owners"]
