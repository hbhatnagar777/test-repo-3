...
TestCase:
    ...
    review_delete_from_source_action_fs()        --  Deletes from CI source a file and verifies the operation
...
class TestCase(CVTestCase):
    """Class for executing delete from source a single file from CIed DS in SDG"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "Delete from source a single file from CIed DS in SDG"
        self.tcinputs = {
    ...
    def run_backup_ci(self):
        ...
        if not CrawlJobHelper.monitor_job(self.commcell, ci_job):
            raise Exception("Content Indexing job failed.")
    ...
    def review_delete_from_source_action_fs(self):
        """
        Performs delete from source action on a file and verifies that it is deleted
        """
        try:
