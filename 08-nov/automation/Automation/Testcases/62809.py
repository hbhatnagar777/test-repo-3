...
TestCase:
    ...
    verify_delete_review_action()   --  Verifies that deleted items are not present in the directory
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Basic acceptance test for backed up data (Quick Scan) with request manager (delete from source)"
        self.tcinputs = {
    ...
    def verify_delete_review_action(self, deleted_file):
        """
        Verifies that deleted items are not present in the directory
        Args:
            deleted_file (str): Deleted file name
        """
        try:
            machine_obj = Machine(self.tcinputs['FSOClient'], self.commcell)
            if machine_obj.check_file_exists(deleted_file):
                raise Exception('Delete File Operation failed. File exists in directory')
        except Exception as err_status:
            self.error_dict["Validate Delete Operation"] = str(err_status)
