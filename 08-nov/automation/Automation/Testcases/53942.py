...
class TestCase(CVTestCase):
    """Class for executing VMConversion from AzureRM To Azure Stack"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "VSA Conversion from AzureRM to Azure Stack from BackupCopy"
    ...
    def run(self):
        ...
        try:
            ...
            backup_option.advance_options = _adv
            backup_option.backup_method = "SNAP"
            common_utils = CommonUtils(auto_commcell.commcell)
            _backup_job = auto_subclient.subclient.backup(backup_option.backup_type,
            ...
            backup_job_id = _backup_job.job_id

            if backup_option.backup_method == "SNAP" \
                    and backup_option.backup_type != 'SYNTHETIC_FULL':
                backupcopy_job_id = common_utils.get_backup_copy_job_id(backup_job_id)
                backupcopy_job = Job(auto_commcell.commcell, backupcopy_job_id)
                ...
                self.log.info("Backup Job %s completed successfully" %backupcopy_job.job_id)

            # """

            self.log.info("---------------------VMConversion to Azure Stack---------------------")
