...
class TestCase(CVTestCase):
    """Class for verifying creation/deletion of a Linux Index Server"""
    test_step = TestStep()
    ...
    def __init__(self):
        ...
        self.name = "Verifying creation/deletion of a Linux Index Server"
    ...
    def delete_index_server(self):
        """Cleans up the testcase created entities"""
        self.pre_cleanup()
        if self.machine_obj.is_process_running(Binary.DATA_CUBE['Unix']):
            raise Exception(f"{Binary.DATA_CUBE['Unix']} process is still running")
