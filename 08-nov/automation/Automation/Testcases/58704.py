...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Openstack volume snapshot size backup quota validation"
    ...
    def run(self):
        ...
        try:
            ...
            try:
                ...
                self.log.info('Got volumesnapsize quota limits for the project'
                              +str(project_name))
            except Exception as err:
                self.log.error('Failed to get volumesnapsize quota limits for the project'
                               +str(project_name))
                raise Exception
            self.log.info('-----setting volumesnapsize quota limits for a project-----')
            try:
            ...
            failurereason = 'Insufficient cinder quota to create VM Snapshot: Failed to meet minimum 10 percent avaliable quota limit'
            if output != ():
            ...
            try:
                ...
                self.log.error(
                    'Failed to reset volumesnapsize quota limits for the project'
                    +str(project_name))
                raise Exception
            ...
            try:
                ...
                raise Exception
            self.log.info(
                '---Getting config values after backup job----')
            try:
                vm_details2 = []
                for each_vm in self.subclient._vmContent['children']:
                    print("true")
