...
TestCase:
    ...
    perform_defrag_tuning()         -- Perform tuning specific to Defrag
    ...
    2. Perform Space Reclamation related tuning - Adding DedupeDrillHoles=0 on MA and setting attribute 128 to off
        on MOuntpath Properties.
    3. Generate Unique data and run backup
    4. Copy half the data from previous backup and run a new incremental backup
    5. Get a list of volumes for these backups
    6. Change Volume Update interval, MM Thread interval and Pruner interval to 15 and 5 and 2 mins respectively
    7. Delete 1st backup job and run data aging
    8. Wait for pruning to catch up
    9. Verify that RMSpareStatusUPdateTime has changed to future value
    10. Change RMSpareStatusUpdateTime to its current value - 86400 seconds
    11. Allow 20 minutes for volume update to happen - keep checking every 5 minutes
    12. Validate that RMSpareStatusUpdateTime is set to -1
    13. Fetch Physical Size for volumes
    14. Fetch Size from PhysicalBytesMB column for all the volumes
    15. Compare the two set of values for each volume and verify that they are same
    16. Run Space Reclamation Job
    17. Repeat steps 9 to 15 again
    18. Make sure that sum of sizes of volumes as fetched from CSDB before space reclamation > sum of sizes of
        volumes after space reclamation
"""
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "volume size update: space reclamation scenario"
        self.tcinputs = {
            "MediaAgentName": None,
      }
    ...
    def setup(self):
        ...
        if self.is_user_defined_mp:
            ...
            self.mountpath = self.ma_machine_obj.join_path(self.tcinputs.get("mount_path"), self.id, "mount")
        else:
            self.mountpath = self.ma_machine_obj.join_path(self.ma_library_drive, self.id, "mount")
    ...
    def get_volumes_for_jobs(self):
        ...
        query = f"select volumeid from mmvolume where volumeid in (select volumeid from archchunk where id in " \
                f"(select archchunkid from archchunkmapping where jobid in ({job_ids})))"
        self.log.info("Query => %s", query)
    ...
    def set_volume_update_time(self, volume_list, time_to_set):
        ...
        query = f"update mmvolume set RMSpareStatusUpdateTime = RMSPareStatusUpdateTime {operation} " \
                f"{abs(time_to_set)} where volumeid in ({','.join(volume_list)}) and RMSpareStatusUpdateTime <> -1"
        self.log.info("Query => %s", query)
        self.optionobj.update_commserve_db(query)
    ...
    def wait_for_mm_thread_invocation(self, mins, check_pruning=False):
        ...
        Args:
            mins            (int)       --      Timeout
            check_pruning   (int)       --      Check for Phase 3 pruning
        ...
        if check_pruning:
            ...
            for iteration in range(1, 6):
                self.log.info("Sleeping for %s mins before running Data Aging job", int((mins / 5)))
                ...
                matched_lines = self.dedup_helper_obj.validate_pruning_phase(self.store_obj.store_id,
    ...
    def validate_volumes_update_time(self,future_time=False):
        ...
        query = f"select volumeid, RMSpareStatusUpdateTime from mmvolume where " \
                f"RMSpareStatusUpdateTime {equality_string} and volumeid in ({','.join( self.volumes_list)})"
        self.log.info("Query => %s", query)
        self.csdb.execute(query)
    ...
    def run_space_reclamation_job(self):
        """
        Runs space reclamation job on DDB Store
        """
        self.store_obj.refresh()
        self.log.info("Starting Space Reclamation Job")
        job_obj = self.store_obj.run_space_reclaimation(level=4, clean_orphan_data=True, use_scalable_resource= True)
        self.log.info("Successfully started Space Reclamation Job - [%s]", job_obj.job_id)
        job_obj.wait_for_completion()

    def validate_size_reduction(self, csdb_size_dict_before, csdb_size_dict_after):
        """
        Validate whether CSDB shows reduction in volume size after space reclamation

        Args:
            csdb_size_dict_before (dictionary obj) : CSDB size dictionary before space reclamation
            csdb_size_dict_after (dictionary obj)  : CSDB size dictionary after space reclamation

        Return:
            True if sum of volume size after space reclamation is less than earlier
        """
        before_size = 0
        after_size = 0
        ...
        self.log.info("Total Size of volumes as per CSDB *before* space reclamation => %s", before_size)
        self.log.info("Total Size of volumes as per CSDB *after* space reclamation => %s", after_size)

        if before_size > after_size:
            self.log.info("Sum of volume sizes after space reclamation has reduced as expected")
            return True

        self.log.error("Sum of volume sizes after space reclamation has not reduced as expected")
        return False
    ...
    def clean_test_environment(self):
        ...
        """
        try:
            self.log.info("Disabling Space Reclmation related settings")
            self.perform_defrag_tuning(False)
    ...
    def initiate_volume_size_update_process(self, after_which_action, size_on_disk = False):
        """
        Initiated sequence of actions to verify volume size update after actions like pruning / space reclamation

        Args:
            after_which_action  (str)   -- Action that initiated volume size update eg. pruning/space reclamation
        ...
        self.log.info("Allowing volume update after %s to complete", after_which_action)
        ...
        self.log.info("Fetching Volume size IN CSDB")
        ...
        self.log.info("Validate that before space reclamation, sizes are up to date and same at both places")
    ...
    def run(self):
        ...
        try:
            ...
            self.backup_job_list = []
            backup_content_dirs = []
            self.log.info("+++++++++++++++++VOLUME SIZE UPDATE : DEDUP SPACE RECLAMATION+++++++++++++++++")
            self.perform_defrag_tuning(True)
            ...
            self.log.info("Setting various MM config parameters")
            ...
            self.log.info("Pruning the jobs")
            self.prune_jobs([self.backup_job_list[x] for x in range(0,len(self.backup_job_list),2)])

            self.log.info("Waiting for 40 minutes so that pruning gets completed in the background")
            ...
            self.log.info("Initiating volume size update process after Pruning.")
            csdb_size_before = self.initiate_volume_size_update_process(after_which_action="pruning")

            self.log.info("Launching Space Reclamation Job")
            self.run_space_reclamation_job()

            self.log.info("Initiating volume size update process after Space Reclamation")
            csdb_size_after = self.initiate_volume_size_update_process(after_which_action="space reclamation",
                                                                       size_on_disk=True)
    ...
    def tear_down(self):
        ...
        self.mahelper_obj.update_mmconfig_param('MMS2_CONFIG_MM_MAINTAINENCE_INTERVAL_MINUTES', 5,
                                                self.mm_admin_thread)
