...
TestCase:
    ...
    add_reg_key_on_access_node()--  Method to add registry key on access node

    remove_reg_key_from_access_node()--  Method to delete registry key from access node
...
DATABASE = 'tc_70625'
APP_NAME = f"TC_70625_{datetime.now().strftime('%d_%B_%H_%M')}"
FOREVER_FULL_KEY = 'sf_backupCycleTransparency'
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = "Salesforce: Compare across full backups"
        self.browser = None
    ...
    def tear_down(self):
        ...
        if self.status == PASSED:
            ...
            self.remove_reg_key_from_access_node()
    ...
    def add_reg_key_on_access_node(self):
        """
        Method to add registry key on access node
        """
        try:
            self.sf_helper.add_additional_key_on_access_node(FOREVER_FULL_KEY + "_" + self.org_name, "true")
        except Exception as exp:
            raise CVTestStepFailure from exp

    def remove_reg_key_from_access_node(self):
        """
        Method to delete registry key from access node
        """
        try:
            self.sf_helper.remove_additional_key_from_access_node(FOREVER_FULL_KEY + "_" + self.org_name, 1)
        except Exception as exp:
            raise CVTestStepFailure from exp
    ...
    def run(self):
        ...
        try:
            ...
            if "ClientName" in self.tcinputs:
                ...
                self.create_app()
            # Add registry key on access node
            self.add_reg_key_on_access_node()
            ...
            self.data3 = self.sf_helper.create_incremental_data(self.sf_object.fullName, data=self.data2.copy(),
                                                                add=3, modify=4, delete=2)
            # Run full backup
            job3 = self.backup_full()[0]
