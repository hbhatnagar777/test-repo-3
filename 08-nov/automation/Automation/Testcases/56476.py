...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        '''Initialises the testcase object'''
        super(TestCase, self).__init__()
        self.name = "Apply license with combination of Permanent+Evaluation"
    ...
    def run(self):
        ...
        try:
            ...
            license_expiration_days = self.tcinputs.get('license_expiration_days', licenseconstants.license_expiration_days)
            license_evaluation_days = self.tcinputs.get('license_evaluation_days', licenseconstants.license_evaluation_days)
            ...
            self.license.adminconsole.logout()
            '''then apply permanent with expiry'''
            license_type = license_types[3]
            self.license.applyfrom_ac_licensepage(
                self.license.modify_license(
                    license_type,
                    license_sku_perm,
                    license_sku_perm_quantity,
                    license_sku_eval,
            ...
            self.license.validate_license_details
            self.license.adminconsole.logout()
            '''Apply permanent license with eval skus'''
            license_type = license_types[2]
            license_sku_perm = "CV-BR-FT"
            license_sku_eval = "CV-BR-FT"
            license_sku_eval_quantity = 1
            license_sku_perm_quantity = 1
            self.license.applyfrom_ac_licensepage(
                self.license.modify_license(
                    license_type,
                    license_sku_perm,
            ...
            self.license.adminconsole.logout()
            '''Change skus'''
            license_sku_perm = "CV-BR-VM10-VOI,CV-BR-OI"
            license_sku_eval = "CV-BR-VM10-VOI,CV-BR-OI"
            license_sku_eval_quantity = 1
            license_sku_perm_quantity = 1
            self.license.applyfrom_ac_licensepage(
                self.license.modify_license(
