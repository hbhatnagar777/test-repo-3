...
Non-MemDB Reconstruction GDSP with Skip DDB Validation
...
2. config GDSP, dependent SP SC, DB
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Non-MemDB Reconstruction GDSP with Skip DDB Validation" \
                    " - Regular and Full recon checked"
    ...
    def create_resources(self):
        ...
        if self.is_user_defined_mp:
            ...
            if (not self.is_user_defined_lib) or (self.is_user_defined_lib and
                                                  not self.commcell.disk_libraries.has_library(
                                                      self.tcinputs.get("library_name"))):
                # mount path not pre-defined and library not supplied
                # or mount path not pre-defined and user defined library does
                # not exist
                self.mount_path = self.media_agent_machine.join_path(
        ...
        # if user defined library exists don't create new else create new library - already handled in
        # configure_disk_library
    ...
    def prune_records_after_ddb_backup(self, job):
        ...
        for i in range(10):
            ...
            if matched_lines:
                ...
                self.log.info(f"No phase 2 pruning activity on sidb - {self.global_sidb_id} yet. Checking after 240 seconds")
                time.sleep(240)

        if not pruning_done:
            self.log.error("Pruning is not over even after 40 minutes")
            raise Exception("Pruning is not over even after 40 minutes")
    ...
    def run(self):
        ...
        try:
            ...
            if sidb_up_job:
                # time.sleep(5)
                self.log.info('check whether sidb2 process started for the engine due to job or not?')
                ...
                # chances are backup job will cause it to be running
                # but if other factors do affect it
                ...
                while not running_sidb_list:
                    ...
                    if count == 180:
                        self.log.error("did not find running SIDB2 process for DDB %d", self.global_sidb_id)
