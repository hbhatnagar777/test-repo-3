...
TestCase:
    ...
    run_auxcopy_jobs()  --  initiates AuxCopy Jobs for each of the storage policies
...
1: Configure the environment: a library, 8 Storage Policies(with a Secondary Copy), a BackupSet,8 SubClients

2: Run Backups for all the SubClients

3: Identify the Chunks Created for the respective Jobs

4: Make Alterations to the chunks based on the 8 SubCases we need to test

5: Run Scalable AuxCopy Job on all the 8 Storage Policies
...
8: Run Second Full Backups with same content

9: Check whether the Bad Chunks Marking, DV Status Update, new Primary Records for the jobs has worked as expected
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = 'Marking Bad Chunks by AuxCopy for DiskLibraries'
        self.tcinputs = {
        ...
        self.copy_name = None
        self.library_1_name = None
        self.library_2_name = None
    ...
    def setup(self):
        ...
        if not self.is_user_defined_mp or not self.is_user_defined_dedup:
            ma_1_drive = self.utility.get_drive(self.ma_machine_1, 10240)
        ...
        if not self.is_user_defined_copy_mp or not self.is_user_defined_copy_dedup:
            ma_2_drive = self.utility.get_drive(self.ma_machine_2, 10240)
        ...
        if self.is_user_defined_lib:
            ...
            self.library_1_name = self.tcinputs.get("library_name")
        else:
            self.library_1_name = f"{self.id}_Lib_{self.tcinputs.get('PrimaryCopyMediaAgent')[1:]}"
        ...
        if self.is_user_defined_copy_lib:
            self.log.info("Existing library name supplied")
            self.library_2_name = self.tcinputs.get("copy_library_name")
        else:
            self.library_2_name = f"{self.id}_Lib2_{self.tcinputs.get('SecondaryCopyMediaAgent')[1:]}"
            if not self.is_user_defined_mp:
        ...
        if self.is_user_defined_dedup:
            ...
            self.ddb_1_path = self.ma_machine_1.join_path(self.tcinputs.get("dedup_path"),
        ...
        if self.is_user_defined_copy_dedup:
            ...
            self.ddb_2_path = self.ma_machine_2.join_path(self.tcinputs["copy_dedup_path"],
                                                          'test_' + self.id, "CopyDDBs")
        else:
    ...
    def cleanup(self):
        ...
        try:
            ...
            if not self.is_user_defined_lib:
                ...
                if self.commcell.disk_libraries.has_library(self.library_1_name):
                    self.commcell.disk_libraries.delete(self.library_1_name)
                    self.mm_helper.remove_content(self.mount_path_1, self.ma_machine_1, suppress_exception=True)

            if not self.is_user_defined_copy_lib:
                self.log.info('Deleting Library if exists')
                if self.commcell.disk_libraries.has_library(self.library_2_name):
                    self.commcell.disk_libraries.delete(self.library_2_name)
                    self.mm_helper.remove_content(self.mount_path_2, self.ma_machine_2, suppress_exception=True)
            self.mm_helper.remove_content(self.ddb_1_path, self.ma_machine_1, suppress_exception=True)
            self.mm_helper.remove_content(self.ddb_2_path, self.ma_machine_2, suppress_exception=True)
    ...
    def run(self):
        ...
        try:
            ...
            self.run_auxcopy_jobs()
            self.wait_for_jobs(self.aux_copy_jobs)
            self.run_backups()
    ...
    def initial_setup(self):
        ...
        if not self.is_user_defined_lib:
            self.mm_helper.configure_disk_library(self.library_1_name,
                                                  self.tcinputs.get('PrimaryCopyMediaAgent'),
                                                  self.mount_path_1)
        if not self.is_user_defined_copy_lib:
            self.mm_helper.configure_disk_library(self.library_2_name,
                                                  self.tcinputs.get('SecondaryCopyMediaAgent'),
                                                  self.mount_path_2)

        for index in range(1, 9):
            self.storage_policies.append(self.dedupe_helper.configure_dedupe_storage_policy(
                f'{self.storage_policy_name}{index}', self.library_1_name,
                self.tcinputs.get('PrimaryCopyMediaAgent'),
                self.ma_machine_1.join_path(self.ddb_1_path, f'Dir{index}'),
                self.tcinputs.get('PrimaryCopyMediaAgent')))
            # get sidb store id's
            self.sidb_stores.append(self.dedupe_helper.get_sidb_ids(
                self.storage_policies[-1].storage_policy_id, 'Primary')[0])

            secondary_copy = self.dedupe_helper.configure_dedupe_secondary_copy(
                self.storage_policies[-1], self.copy_name, self.library_2_name,
                self.tcinputs.get('SecondaryCopyMediaAgent'),
                self.ma_machine_2.join_path(self.ddb_2_path, f'Dir{index}_{self.utility.get_custom_str()}'),
                self.tcinputs.get('SecondaryCopyMediaAgent'))
            self.mm_helper.remove_autocopy_schedule(f'{self.storage_policy_name}{index}', self.copy_name)
            # limit max chunk size on the copy to produce more chunks
            copy_id = self.storage_policies[-1].get_copy('Primary').copy_id
            # adding partition 2
            partition_2_path = self.ma_machine_1.join_path(self.ddb_1_path, f'Part2Dir{index}')
            if not self.ma_machine_1.check_directory_exists(partition_2_path):
                self.ma_machine_1.create_directory(partition_2_path)
            self.storage_policies[-1].add_ddb_partition(
                copy_id, self.sidb_stores[-1], partition_2_path,
                self.tcinputs.get('PrimaryCopyMediaAgent'))
            # limit max chunk size on the copy to produce more chunks
            self.log.info('Setting the max Chunk Size on Copies to 100 MB')
            query = f'update MMDataPath set ChunkSizeMB = 100 where CopyId in ({copy_id},{secondary_copy.copy_id})'
        ...
        if self.ma_machine_1.os_info.lower() == 'windows':
            ...
            self.commcell.media_agents.get(self.tcinputs.get('PrimaryCopyMediaAgent')).set_ransomware_protection(False)
        ...
        for query_result in self.query_results:
            ...
            if self.ma_machine_1.os_info.lower() == 'windows':
                for volume in volume_set:
                    self.ma_machine_1.modify_ace('Everyone', volume[1], 'DeleteSubdirectoriesAndFiles',
                                                 'Deny', remove=True, folder=True)
                    self.ma_machine_1.modify_ace('Everyone', volume[1], 'Delete',
                                                 'Deny', remove=True, folder=True)
        ...
        for sub_case in range(1, 9):
            ...
            if sub_case == 2:
                ...
                for volume in volume_set:
                    ...
                    for index in range(length):
                        if volume[0] in chunk_list[index][1]:
                            file = self.ma_machine_1.join_path(chunk_list[index][1], 'SFILE_CONTAINER.idx')
                            if self.ma_machine_1.check_file_exists(file):
                ...
                for volume in effected_volumes:
                    self.ma_machine_1.remove_directory(volume)
            if sub_case == 5:
