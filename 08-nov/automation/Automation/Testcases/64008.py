...
class TestCase(CVTestCase):
    """Testcase : Push Service Pack Upgrade of a 32bit Client on 64-bit OS"""

    def __init__(self):
        """Initializes test case class object"""
        super().__init__()
        self.name = "Push Service Pack Upgrade of a 32bit Client on 64 bit OS"
    ...
    def run(self):
        ...
        try:
            ...
            if self.windows_machine.check_registry_exists(reg_key + "\\Installer", "nCVDPORT"):
            ...
            reset = False
            self.windows_machine.create_registry(installer_constants.COMMVAULT_REGISTRY_GALAXY_INSTALLER_FLAGS,
                                                 "bAllow32BitInstallOn64Bit", int(not reset), "DWord")
            self.log.info(f"bAllow32BitInstallOn64Bit flag set to {int(not reset)} in registry")
            ...
            # Adding the client to a client to a group
            ...
            if not self.commcell.client_groups.has_clientgroup(self.clientgrp):
                ...
                _client_group_obj.add_clients(
                    [self.commcell.clients.get(self.windows_machine.machine_name).client_name], overwrite=True)
