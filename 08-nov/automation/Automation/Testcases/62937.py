...
class TestCase(CVTestCase):
    """Class for Basic Acceptance Test for Multi-Threaded Index Query using onepass subclient"""
    ...
    def __init__(self):
        ...
        self.name = "Basic Acceptance Test for Multi-Threaded Index Query using onepass subclient"
    ...
    def add_registry_key(self):
        ...
        if self.tcinputs.get("Coordinator"):
            ...
            coord_client.create_registry("WinFSDataMigrator", "NumFSTurboStubCollectSplitAt",
                                                         self.reg_key_val,
                                                         reg_type='DWord')
        else:
            self.OPHelper.client_machine.create_registry("WinFSDataMigrator", "NumFSTurboStubCollectSplitAt",
                                                         self.reg_key_val,
                                                         reg_type='DWord')
        time.sleep(30)
    ...
    def remove_registry_key(self):
        ...
        if self.tcinputs.get("Coordinator"):
            coord_client = Machine(machine_name=self.tcinputs.get("Coordinator"), commcell_object=self.commcell)
            coord_client.remove_registry("WinFSDataMigrator", "NumFSTurboStubCollectSplitAt")
        else:
            self.OPHelper.client_machine.remove_registry("WinFSDataMigrator", "NumFSTurboStubCollectSplitAt")
        time.sleep(30)
    ...
    def setup(self):
        ...
        #create backupset and onepass subclient
        self.OPHelper.create_backupset(delete=True, is_nas_turbo_backupset=self.is_nas_turbo_type)
        if "linux" in self.client.os_info.lower():
