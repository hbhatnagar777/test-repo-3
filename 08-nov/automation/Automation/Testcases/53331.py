...
class TestCase(CVTestCase):
    ...
    """Class for executing Basic acceptance Test of Vmware To Azure restores"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "VMWare to Azure automation"
        self.test_individual_status = True
        self.test_individual_failure_message = ""
        self.browser = None
        self.vsa_obj = None
        self.azure_vsa_obj = None
    ...
    def run(self):
        ...
        try:
            ...
            try:
                self.azure_vsa_obj = VirtualServerUtils.create_adminconsole_object(self, is_destination_client=True)
                self.vsa_obj_inputs = {
                    ...
                    'vm_size':self.tcinputs.get('VMSize',"--Auto Select--"),
                    'network_interface':self.tcinputs.get('Vnet',"--Auto Select--")
                }

                set_inputs(self.vsa_obj_inputs,self.azure_vsa_obj)

                self.vsa_obj.unconditional_overwrite = True
                self.azure_vsa_obj.unconditional_overwrite = True

                self.vsa_obj.full_vm_conversion_restore(self.azure_vsa_obj)
    ...
    def tear_down(self):
        try:
            ...
            if self.azure_vsa_obj:
                self.azure_vsa_obj.post_conversion_clean_up(status=status)
        except Exception as exp:
            self.utils.handle_testcase_exception(exp)
