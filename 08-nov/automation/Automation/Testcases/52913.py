...
TestCase:
    ...
    run()           --  Creates a tablespace, user, table with records, Validates table records,
                        Performs Backup, Drops Table,  Perforems table level restore with
                        a user defined auxiliary instance and Validates table records.
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "FS Table Level Restore with user defined auxiliary instance"
        self.oracle_helper = None
        self.status = constants.FAILED
        self.result_string = "Run of test case 52913 is incomplete"
        self.tcinputs = {
            "SnapEngine": None,
            "OracleOptions": None,
            "commonOptions": None
    ...
    def setup(self):
        ...
        self.subclient = self.oracle_helper.create_subclient(
            "52913", storage_policy, self.tcinputs['SnapEngine'], 2, True, True, False, 0, False)
        ...
        try:
            ...
            if not job.wait_for_completion():
                raise Exception("Failed to run restore job with error: {0}".format(
                    job.delay_reason))
            self.log.info(
                " STEP 2: FS Table Level restore - user defined auxiliary instance JOB ID: %s",
                job.job_id)
            ...
            self.result_string = "Run of test case 52913 has completed successfully"
