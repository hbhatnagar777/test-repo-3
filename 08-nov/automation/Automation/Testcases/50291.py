...
class TestCase(CVTestCase):
    """Class for executing
        IBMi - Verify backup cycle Full-Incremental-Differential for IFS and LFS data
    ...
    def __init__(self):
        ...
        self.name = "IBMi - Verify backup cycle Full-Incremental-Differential for IFS and LFS data"
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("*** STARTING VALIDATION OF BACKUP CYCLE with INC & DIFF FOR IFS and LFS DATA ***")
            ...
            self.restore_verify(source=self.srcdir, destination=self.tmp_path)
            self.log.info("Verified IFS backup and restore with inc + diff backup cycle with %s", self.scan_type.name)
            ...
            for each in scans:
                ...
                self.restore_verify(source=self.client_machine.lib_to_path(self.srclib),
                                    destination=self.client_machine.lib_to_path(self.destlib))
                self.log.info("Verified LFS backup and restore with inc + diff backup cycle with %s",
                              self.scan_type.name)

            self.client_machine.manage_library(operation='delete', object_name=self.srclib)
            self.client_machine.manage_library(operation='delete', object_name=self.destlib)

            self.log.info("**LFS, IFS REGULAR SCAN BACKUP CYCLE WITH INC + DIFF IS SUCCESSFULLY**")
