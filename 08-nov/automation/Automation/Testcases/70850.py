...
"""ACC test case for postgresql cluster reg mgr
...
TestCase:
    ...
    cluster_mgr_validate() -- Validates whether the rep mgr has started by checking the log backup on the master node
    ...
    "testCases":
        {
            ...
            note -->
                nodes: The nodes list where each element is a dictionary representing a node,
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        """
        super().__init__()
        self.name = "ACC test case for postgresql cluster rep mgr"
        self.browser = None
    ...
    def add_instance(self):
        ...
        self.db_instance.add_postgresql_cluster_instance(
            ...
            cluster_type="Repmgr for Postgres",
    ...
    def cluster_mgr_validate(self):
        """
        Validates whether rep mgr has started by checking the restore logs the master node.

        Raises:
            Exception: If there is an error executing the command on the client machine or other failures.
        """
        try:
            job_id = self.jobs['fs_restore']
            self.log.info("Checking rep mgr startup for job id %s", job_id)

            node_priority = self.pg_cluster_object.get_node_priority()
            for node in node_priority.values():
                ...
                command = f"cat {log_path} | grep {job_id} | grep 'Successfully started repmgrd on the Node'"
                self.log.debug(f"Executing command on {node} machine: %s", command)

                output = lin_machine_object.execute_command(command)

                if output.exception_message:
                    self.log.error(f"Error executing command on {node}: %s", output.exception_message)
                    raise RuntimeError("Unable to run the command on client machine")

                if str(job_id) in output.formatted_output:
                    self.log.info("rep mgr startup confirmed for job id %s", job_id)
                else:
                    self.log.info("rep mgr startup not found for job id %s", job_id)

        except Exception as e:
            self.log.error("Error checking rep mgr startup: %s", str(e))
