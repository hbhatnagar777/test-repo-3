...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "[Global CC]: Validate Events Functionality"
    ...
    def verify_date_for_critical(self):
        ...
        if all(self.current_timestamp - parse(date).timestamp() <= 24 * 60 * 60 for date in event_date_list):
    ...
    def setup(self):
        ...
        self.table = Rtable(self.global_console)
        self.event_obj = Events(self.global_console)

    def run(self):
        """Main function for test case execution"""
        try:
            self.global_console.navigator.switch_service_commcell('Global')
            self.global_console.navigator.navigate_to_events()
            self.current_timestamp = datetime.now().timestamp()
            self.verify_event_table_displayed()
            self.verify_filters()
            # self.verify_events_list TO BE ADDED
