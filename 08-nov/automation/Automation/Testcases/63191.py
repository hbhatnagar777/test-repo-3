...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = 'VSA CVC streaming backup, Restore and Browse operation'
    ...
    def testcase_validations(self, operation):
        """
        ...
        """
        try:
            decorative_log(f'{operation} operation and validation')
            ...
            self.validation(self.full_data_path, 0, self.dir_level, operation,
                            tmp_path, user_name, self.vm_name, backupset_name)
        except Exception as exp:
    ...
    def run_backup(self):
        ...
        """
        try:
            decorative_log('Running backup job')
            _backup_jobs = self.tc_utils.sub_client_obj.subclient.backup()
            ...
            self.log.exception('Backup job failed due to: {}'.format(exp))
