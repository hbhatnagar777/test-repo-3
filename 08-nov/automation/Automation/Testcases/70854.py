...
TestCase:
    ...
    run_outofplace_restore()    --  method to run db copy restore
...
class TestCase(CVTestCase):
    ...
    util_file device SAP for Oracle """
    test_step = TestStep()
    ...
    def __init__(self):
        ...
        self.name = "SAP Oracle db copy restore test from Command Center"
        self.browser = None
        self.admin_console = None
        self.navigator = None
        self.tablespace_name = 'CV_70854_TSP'
        self.table_name = "CV_70854_TABLE"
    ...
    def tear_down(self):
        ...
        if self.dest_sap_oracle_helper_object:
            status = self.dest_sap_oracle_helper_object.drop_tablespace(self.tablespace_name)
            if status == 0:
                self.log.info("tablespace spaces are cleaned up sucessgully from \
                destination client")
            else:
                self.log.info("there is some issue with tablespace cleanup created by \
                automation.please cleanup manually on destination client")
