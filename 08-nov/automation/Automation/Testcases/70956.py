...
class TestCase(CVTestCase):
    """Class for executing Basic hird party commandline jobs
    for SAP Oracle using rman_util device"""
    test_step = TestStep()

    def __init__(self):
        """Initializes test case class object"""
        super().__init__()
        self.name = "Basic Acceptance Test of SAPOracle Backup and Restore from cmd using rman_util device"
    ...
    def run_third_party_brbackup_job(self):
        ...
        detail_file = self.saporacle_helper.thirdparty_cmd_backup_job(brbackup_cmd, "initauto.utl",
                                                                      " BRBACKUP completed successfully")
        if detail_file == None:
    ...
    def run_third_party_brrestore_data(self):
        """ method to run brrestore of data from cmd
        ...
        self.log.info("##Running Third party commandline brrestore of data ##")
        if self.machine.os_info == 'UNIX':
            brrestore_cmd = r"su - " + self.saporacle_osapp_user + ' -c "brrestore -d \
                          rman_util -b last -m all -c force -u /'
            self.log.info(" Command we are running is " + brrestore_cmd)
            if self.saporacle_sapsecurestore == 1:
                brrestore_cmd = f"{brrestore_cmd}/"
                self.log.info(" Command we are running is " + brrestore_cmd)
        else:
            brrestore_cmd = '"brrestore -d rman_util -b last -m all -c force -u /'
            self.log.info(" Command we are running is " + brrestore_cmd)
            if self.saporacle_sapsecurestore == 1:
                brrestore_cmd = f"{brrestore_cmd}/"
                self.log.info(" Command we are running is " + brrestore_cmd)
        ...
        if detail_file == None:
            raise cvexceptions.CVTestStepFailure("Failed to run Thirdparty commandline brrestore of data  ")
        else:
            self.log.info("Successfully ran Thirdparty commandline brrestore of data job")

    @test_step
    def run_third_party_brrestore_log(self):
        """ method to run brrestore of logs from cmd
        ...
        self.log.info("##Running Third party commandline brrestore of logs##")
        if self.machine.os_info == 'UNIX':
            brrestore_logcmd = r"su - " + self.saporacle_osapp_user + ' -c "brrestore -d \
                          rman_util -a ' + self.end_lsn + ' -c force -u /'
            self.log.info(" Command we are running is " + brrestore_logcmd)
            if self.saporacle_sapsecurestore == 1:
                brrestore_logcmd = f"{brrestore_logcmd}/"
                self.log.info(" Command we are running is " + brrestore_logcmd)
        else:
            brrestore_logcmd = '"brrestore -d  rman_util -a ' + self.end_lsn + ' -c force -u /'
            self.log.info(" Command we are running is " + brrestore_logcmd)
            if self.saporacle_sapsecurestore == 1:
                brrestore_logcmd = f"{brrestore_logcmd}/"
                self.log.info(" Command we are running is " + brrestore_logcmd)
        ...
        detail_file = self.saporacle_helper.thirdparty_cmd_backup_job(brrestore_logcmd, "initauto.utl",
                                                           " BRRESTORE completed successfully")
        if detail_file == None:
            raise cvexceptions.CVTestStepFailure("Failed to run Thirdparty commandline brrestore of logs job ")
        else:
            self.log.info("Successfully ran Thirdparty commandline brrestore of logs job")

    def run(self):
        """executes basic Third party commandline backup and inlace restore
         for SAP for Oracle using rman_util device"""
