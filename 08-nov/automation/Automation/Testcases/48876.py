...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Virtual Server - VMW - Live sync - VM Synthetic Full backup," \
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Sleeping for 120 secs")
            time.sleep(120)
            # Run only synthetic backup
            bkp_job = self.subclient.backup("SYNTHETIC_FULL", False)

            self.log.info(f"Backup Job with job id: {bkp_job.job_id} started")
            if not bkp_job.wait_for_completion():
                self.log.info(f"Backup job with job id: {bkp_job.job_id} failed")
                raise Exception
            ...
            auto_subclient.backup_job = bkp_job

            if not job.job_id > bkp_job.job_id:
                self.log.info("No replication job started for Synthetic Full backup")
                self.log.info("No replication job validation successful")
