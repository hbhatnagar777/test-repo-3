...
TestCase:
    ...
    Input Example:
        "testCases":{
            ...
            "rename_destination": "<true>" (optional, renames data and run restore)
            }
        }
"""
import copy
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "MongoDB IDA Command Center - ACCT1 Out of Place Granular Restore With " \
                    "Overwrite"
        self.browser = None
    ...
    def _restore(self):
        """starts a restore"""
        self.log.info("start restore out of place")
        if self.tcinputs.get('rename_destination'):
            self.restore_obj.restore_out_of_place(self.tcinputs['pseudo_client_name'],
                                                  des_rename=True)
        else:
            self.restore_obj.restore_out_of_place(self.tcinputs['pseudo_client_name'],
                                                  overwrite=True)
    ...
    def discover_and_backup(self):
        ...
        self.backup_hash = self.mongo.get_data_hash(self.generated_data)
    ...
    def restore_and_validate(self, onlycolls=False):
        """Restores databases/collections and validates"""
        conf = self.overview.access_configuration()
        conf.access_restore()
        browse = Browse(self.admin_console)
        browse.switch_to_collections_view()
        datalist = copy.deepcopy(self.generated_data)
        if self.tcinputs.get('rename_destination'):
            self.restore_data = {f'{database}_restore': coll_list for database, coll_list in
                                 datalist.items()}
        else:
            self.restore_data = datalist
        if onlycolls:
            ...
            if self.tcinputs.get('rename_destination'):
                coll_list = copy.deepcopy(self.generated_data[database])
                self.restore_data = {database: [f'{coll}_restore' if isinstance(coll, str) else
                                                coll for coll in coll_list]}
            else:
                self.restore_data = {database: copy.deepcopy(self.generated_data[database])}
        self.overview.restore_all()
        self.log.info("Data to be restored: %s", self.restore_data)
        self._restore()
        if self.mongo.validate_restore(self.restore_data, self.tcinputs['clientName'],
                                       self.port, self.tcinputs.get('dbUser') or '',
                                       self.tcinputs.get('dbPassword') or '',
