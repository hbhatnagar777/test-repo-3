...
TestCase:
    ...
    run()               --  run function of this test case calls SnapHelper Class to execute
                            ...
                            Snap Backup, backup Copy using Proxy client and restores from Tape copy
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = """Automation : IFind test cases to be covered for each SP testing using Proxy client
                    """
    ...
    def run(self):
        ...
        try:
            ...
            if self.tcinputs['MediaAgent'] == self.tcinputs['ProxyMA']:
                raise Exception("ClientMA and ProxyMA Shouldn't be same. Provide ProxyMA with different MA")
            self.log.info("*" * 20 + "Setup of Intellisnap Entities" + "*" * 20)
            snap_helper.setup()

            #Step 1 Backups and restore from Backup copy
