...
Input Example:
    "testCases":{
        ...
        "test_data" : "<dict of test_data> (optional, required is using bare validation)",
        "app_display_name" : <Name of type of DevOps Application>,
        "app_type" : <Integer Identifier of DevOps App>
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Azure DevOps IDA Command Center - ACCT1 Create and Delete Azure DevOps client"
        self.browser = None
    ...
    def setup(self):
        ...
        try:
            ...
            self.azure_app_helper = DevOpsAppHelper(self.commcell, self.admin_console, self.tcinputs, self.log,
                                                    is_azure=True)
            self.app_name = self.tcinputs['azure_app_name']
    ...
    def create_instance(self):
        ...
        self.azure_app_helper.create_azure_instance_from_instances()
        self.log.info("successfully created instance")
        self.tcinputs["access_nodes"] = self.azure_app_helper.access_nodes
        self.verification.app_properties_validation(self.azure_app_helper.tcinputs)
    ...
    def edit_instance(self):
        ...
        self.azure_app_helper.edit_access_nodes_from_configuration(self.tcinputs['access_nodes'][0:2])
    ...
    def create_and_delete_repository_group(self):
        ...
        self.azure_app_helper.create_repository_group_from_repository_groups("cvproblematic")
        repo_properties = {"plan_name": self.tcinputs['plan'],
                           "repository_group": 'automated_repo_group',
                           "organization_name": "cvproblematic"}
        self.verification.app_properties_validation(repo_properties)
        self.azure_app_helper.delete_repository_group_from_toolbar()
    ...
    def backup(self):
        ...
        self.admin_console.access_tab("Repository groups")
        self.azure_app_helper.backup_from_repository_groups(self.tcinputs.get('repository_group', 'default'),
                                                            for_validation=True)
        self.log.info('Successfully backed up data')

    @test_step
    def restore_and_validate_data(self):
        """Restores to azure and validates the data"""
        self.azure_app_helper.restore_from_repository_groups(self.tcinputs.get('repository_group', 'default'),
                                                                      validate_data=True,
                                                                      in_place=True)
        self.log.info('All projects and repositories validated successfully.')
    ...
    def delete_instance(self):
        ...
        if self.azure_app_helper.check_if_instance_exists_from_instances():
    ...
    def run(self):
        ...
        1) Creates azure devops instance, verifies if same is reflected in db.
        ...
        4) Run a backup and restore of the default sub-client or the provided repository_group in testcase inputs
        5) Deletes azure devops instance.
        """
        try:
            ...
            self.azure_app_helper.cleanup_files()
