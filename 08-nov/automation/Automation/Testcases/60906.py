...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Performance testcase (Multiple CA) - SDG FS live crawl job"
        self.tcinputs = {
    ...
    def create_plan(self):
        ...
        self.gdpr_obj.plans_obj.create_data_classification_plan(
            ...
            self.tcinputs['ContentAnalyserCloudName'].split(','), entities_list=['Date'])
    ...
    def run(self):
        ...
        try:
            self.ca_client_name = [self.commcell.clients.get(self.commcell.content_analyzers.get(i).client_id).name
                                   for i in self.tcinputs['ContentAnalyserCloudName'].split(',')]
            self.perf_config = self.perf_helper.form_sdg_fs_monitor_param(
                job_type=JobTypes.FILE_SYSTEM_ONLINE_CRAWL_JOB_WITH_CONTENT,
                index_server=self.index_server_name,
                access_node=self.tcinputs['HostNameToAnalyze'],
                content_analyzer=self.ca_client_name
            )
            self.perf_config.pop('Live Scan')
