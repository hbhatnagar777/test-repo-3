...
TestCase:
    ...
    run_synthetic_full_jobs()   --  initiates Synthetic Fulls for each of the subclients
...
5: Initiate Synthetic Full Backups on all the SubClients
...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.name = 'Marking Bad Chunks by Synthetic Full Job for DiskLibraries'
        self.tcinputs = {
            "MediaAgentName": None
        }
        self.utility = None
    ...
    def setup(self):
        ...
        if self.is_user_defined_lib:
            ...
            self.library_name = f"{self.id}_Lib_{self.tcinputs.get('MediaAgentName')[1:]}"
        ...
        try:
            ...
            for index in range(1, 9):
                self.mm_helper.create_uncompressable_data(
                    self.client_machine,
                    self.client_machine.join_path(self.content_path, f'Data{index}'), 0.2, 1)
            self.run_backups('Incremental')
            self.wait_for_jobs(self.backup_jobs[8:])
            self.identify_chunks()
            self.identify_paths_of_chunks()
            self.make_alterations()
            self.run_synthetic_full_jobs()
            self.wait_for_jobs(self.synthfull_jobs)
            self.run_backups('Full')
    ...
    def initial_setup(self):
        ...
        for index in range(1, 9):
            ...
            query = f'update MMDataPath set ChunkSizeMB = 100 where CopyId = {copy_id}'
    ...
    def run_backups(self, backup_type):
        """Runs Backup Jobs on the subclients"""
        self.log.info('Running %s Backups', backup_type)
        for index in range(8):
            self.backup_jobs.append(
                self.subclients[index].backup(backup_type, advanced_options={'mediaOpt': {'startNewMedia': True}}))
        ...
        self.log.info('Completed Initiating all %s Backups', backup_type)
    ...
    def run_synthetic_full_jobs(self):
        """Runs the Verification Jobs"""
        self.log.info('Running the Verification Jobs: Synthetic fulls')
        log_line = ''
        for subclient in self.subclients:
            job = subclient.backup('synthetic_full', advanced_options={'mediaOpt': {'startNewMedia': True}})
            self.synthfull_jobs.append(job)
            log_line += f'{subclient.subclient_name}: {job.job_id}, '
            ...
            while True:
                ...
                if job.status.lower() == 'pending':
                    if retry_count == 3:
                        ...
                        self.log.warning('Killing Job(Id: %s)', job.job_id)
                        break
    ...
    def produce_result(self):
        ...
        expected_chunk_mark_flags = [0, 0, 1, 1, 0, 0, 0, 0]
        ...
        expected_primary_records = [0, 0, 1, 1, 0, 0, 0, 0]
        self.result_string = ''
        for sub_case in range(1, 9):
            ...
            # job dv
            if ((sub_case not in [3, 4, 8]) and
                expected_statuses[sub_case - 1] not in self.verification_statuses[sub_case - 1]) \
                    or ((sub_case in [3, 4, 8])
