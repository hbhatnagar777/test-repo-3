...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Test case for No new brick present for replacement "
        self.result_string = ""
        self.policy = ""
        self.policy_name = ""
        self.subclient_obj = ""
    ...
    def run(self):
        ...
        try:
            self.log.info("****************Replace Fail, No new disk available or disk of lower capacity**********"
                          "***************")
            status_fstab = False
            ...
            if not flag & 4:
                ...
                # Un mounting disk to make it offline and bad
                ...
                if not status:
                    ...
                    self.log.error(response_log)
            else:
                status_fstab = True
                self.log.info("Replace request cannot be submitted as previous request in progress")

            # New disk, replace failed
            new_brick_info = self.hyperscale_helper.get_disk_by_disk_id(self.disk_id)
            self.new_device_os_path = str(new_brick_info[3])
            if self.device_os_path != self.new_device_os_path:
                self.log.error("old disk %s and new disk %s are not same", self.device_os_path,
                               self.new_device_os_path)
            else:
                self.log.info("old disk %s and new disk %s are same", self.device_os_path,
                              self.new_device_os_path)

            # Check entries for heal
            if not self.hyperscale_helper.gluster_heal_entries(self.node, self.storage_pool_name):
                self.log.info("for replace fail, Gluster healing got triggered")
            else:
                self.log.info("For replace fail, Gluster healing did not trigger ")
            ...
            # checking fstab entries and verifying ids for the disk
            if status_fstab is False:
                status_fstab, res = self.hyperscale_helper.replace_fstab_entry(self.node, entry_old,
                                                                               entry_new)
                if status_fstab is False:
                    self.log.error(res)
            # Getting flag status for the brick
            flag = self.hyperscale_helper.brick_flag_status(self.node_id, self.new_device_os_path)

            # Brick status
            health_status = self.hyperscale_helper.gluster_healthy_brick_status(self.node_id,
                                                                                self.new_device_os_path)
            if health_status:
                self.log.info("Brick %s healthy", self.new_device_os_path)
            else:
                self.log.error("Brick %s not healthy, \tbrick details %s", self.new_device_os_path,
                               new_brick_info)
            # Checking brick still part of gluster or not also checking online status
            ...
            if not glus_size_staus:
                self.log.info("Gluster size not same after replacement failed")
