...
class TestCase(CVTestCase):
    """ Basic acceptance Test for AWS MariaDB metallic
    Example :
    "65543":{
            "Region": "ap-south-1",
    ...
    def __init__(self):
        ...
        self.name = "Metallic - ACC1 for AWS RDS(Export) MariaDB using IAM Role"
        self.browser = None
    ...
    def setup(self):
        ...
        self.awsdatabases = AWSRDSExport(self.admin_console, s3_helper=self.s3_metallic_helper,
                                         ...
                                         database_type=DBInstances.Types.MARIA_DB.value)
    ...
    def start_rds(self):
        ...
        if self.amazon_rds_helper.is_instance_present(self.region, self._instance_identifier, availability=False):
            if not self.amazon_rds_helper.is_instance_present(self.region, self._instance_identifier):
    ...
    def stop_rds(self):
        ...
        if self.amazon_rds_helper.is_instance_present(self.region, self._instance_identifier):
    ...
    def create_and_update_bgw(self):
        ...
        self.stack_name = "awsmetallicRDSmariadb" + datetime.now().strftime("SJ%d%B%H%M")
    ...
    def create_instance(self):
        ...
        self.cloud_account = f"automation-AWS-MariaDB-{self.timestamp}"
        self.plan = f"AWS_MariaDB_plan" + datetime.now().strftime("SJ%d%B%H%M").lower()
        ...
        self.awsdatabases.configure(region=self.tcinputs.get("Region"),
                                    ...
                                    plan_name=self.plan,
                                    instance_name="mariadb/"+ self.tcinputs["InstanceName"],
    ...
    def set_mysql_helper_object(self):
        ...
        _instance = _agent.instances.get("mariadb/"+ self.tcinputs["InstanceName"])
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("Amazon RDS MariaDB Backup and Restore Successful!")
