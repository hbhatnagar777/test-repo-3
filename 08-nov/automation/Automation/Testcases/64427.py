...
TestCase:
    ...
    get_timestamp()                     -- Returns current timestamp in YY-MM-DD HH:MM:SS format
...
class TestCase(CVTestCase):
    """ GUI all types of log only restores from both Dump and MA post sweep job """
    ...
    def __init__(self):
        ...
        self.name = "Test for DB2 1 Min RPO from GUI to verify full log restore, log range restore and log time based restore from both Dump and MA"
        ...
        self.fromTimeValue = None
        self.toTimeValue = None
        self.tcinputs = {
            'Instance': None,
            'Backupset': None,
            'Subclient': None,
    ...
    def setup(self):
        ...
        self.table_name = "T64427"
        self.tablespace_name = "TS64427"
    ...
    def get_timestamp(self):
        """ Method to get current time stamp in GMT - 5:30 YY-MM-DD HH:MM:SS format

        Returns:
            result_timestamp (Str) -- Current time stamp in YY-MM-DD HH:MM:SS

        """
        gmt_timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        self.log.info(f"GMT Timestamp is {gmt_timestamp}")
        gmt_datetime = datetime.strptime(gmt_timestamp, '%Y-%m-%d %H:%M:%S')
        gmt_offset = timedelta(hours=-5, minutes=-30)
        gmt_minus_530_datetime = gmt_datetime + gmt_offset
        result_timestamp = gmt_minus_530_datetime.strftime('%Y-%m-%d %H:%M:%S')
        self.log.info(f"GMT delta 5:30 time stamp is {result_timestamp}")
    ...
    def run(self):
        ...
        try:
            ...
            self.log.info("#### CASE 1 : Restoring logs from DUMP location ####")
            self.db2_helper.reconnect()
            self.log.info("Fetching active log number from Client")
            active_log_number1 = self.get_active_log()
            self.log.info(f"Current active log number of the given database is {active_log_number1}")
            self.fromTimeValue = self.get_timestamp()
            self.log.info(f"Storing current time stamp for log time range restore in case4 {self.fromTimeValue}")
            ...
            self.db2_helper.verify_logs_on_ma(active_log_number1, self.ma_obj)
            self.log.info("Triggering LOG Only restore job from DUMP location")
            self.prepare_db_for_restore()
            self.db2_helper.gui_inplace_log_only_restore()
            self.db2_helper.verify_retrieve_logs_on_client(active_log_number=active_log_number1)
            self.db2_helper.delete_logs_from_retrieve_location()

            self.log.info("#### CASE 2 : Restoring logs from MA post sweep job ####")

            self.log.info("Fetching active log number from Client")
            active_log_number2 = self.get_active_log()
            self.log.info(f"Current active log number of the given database is {active_log_number2}")
            self.log.info("Generating 10 log files")
            self.generate_logs(10)
            self.log.info("sleeping for 60 sec before verifying logs inside MA")
            time.sleep(60)
            self.log.info("Verifying logs on MA")
            self.db2_helper.verify_logs_on_ma(active_log_number2, self.ma_obj)
            self.run_and_validate_sweep_job()
            self.prepare_db_for_restore()
            self.db2_helper.gui_inplace_log_only_restore()
            self.db2_helper.verify_retrieve_logs_on_client(active_log_number=active_log_number2)
            self.db2_helper.delete_logs_from_retrieve_location()

            self.log.info("#### CASE 3 : Restoring logs from both DUMP and MA using log range restore####")
            self.log.info("Fetching active log number from Client")
            active_log_number3 = self.get_active_log()
            self.log.info(f"Current active log number of the given database is {active_log_number3}")
            self.log.info("Generating 50 log files")
            self.generate_logs(50)
            self.log.info("sleeping for 60 sec before verifying logs inside MA")
            time.sleep(60)
            self.log.info("Verifying logs on MA")
            self.db2_helper.verify_logs_on_ma(active_log_number3, self.ma_obj)
            self.prepare_db_for_restore()
            self.db2_helper.gui_inplace_log_only_restore(roll_forward=False, archiveLogLSN=True, startLSN=True,
                                                         endLSN=True, startLSNNum=active_log_number1,
                                                         endLSNNum=active_log_number3 + 30)

            self.db2_helper.verify_retrieve_logs_on_client(active_log_number=active_log_number1)
            self.db2_helper.verify_retrieve_logs_on_client(active_log_number=active_log_number3)
            self.db2_helper.delete_logs_from_retrieve_location()

            self.log.info("#### CASE 4 : Restoring logs from both DUMP and MA using log time range restore####")
            self.log.info("Fetching active log number from Client")
            active_log_number4 = self.get_active_log()
            self.log.info(f"Current active log number of the given database is {active_log_number3}")
            self.log.info("Generating 50 log files")
            self.generate_logs(50)
            self.log.info("sleeping for 60 sec before verifying logs inside MA")
            time.sleep(60)
            self.log.info("Verifying logs on MA")
            self.db2_helper.verify_logs_on_ma(active_log_number4, self.ma_obj)
            self.prepare_db_for_restore()
            self.toTimeValue = self.get_timestamp()
            self.log.info(f"Storing to time value for log time range restore {self.toTimeValue}")
            self.db2_helper.gui_inplace_log_only_restore(roll_forward=False, archiveLogTime=True, logTimeStart=True,
                                                         logTimeEnd=True, fromTimeValue=self.fromTimeValue,
                                                         toTimeValue=self.toTimeValue)

            self.db2_helper.verify_retrieve_logs_on_client(active_log_number=active_log_number1)
            self.db2_helper.verify_retrieve_logs_on_client(active_log_number=active_log_number2)
            self.db2_helper.verify_retrieve_logs_on_client(active_log_number=active_log_number3)
            self.db2_helper.verify_retrieve_logs_on_client(active_log_number=active_log_number4)
