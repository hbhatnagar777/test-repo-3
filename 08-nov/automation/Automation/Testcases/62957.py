...
class TestCase(CVTestCase):
    ...
    def __init__(self):
        ...
        self.name = "Vmware extent validation basic case"
    ...
    def run(self):
        ...
        try:
            ...
            VirtualServerUtils.decorative_log(
                'Synthfull job failed as expected and removing registry key')
            if vmobj.os_info == 'WINDOWS':
            ...
            VirtualServerUtils.decorative_log(
                'registry key removed on proxy machine successfully')
            #Run incremental to backup missed extents
            VirtualServerUtils.decorative_log(' Run incremental to backup missed extents')
            backup_options = OptionsHelper.BackupOptions(auto_subclient)
            backup_options.backup_type = "INCREMENTAL"
            auto_subclient.backup(backup_options)
            try:
                VirtualServerUtils.decorative_log(
                    "-" * 25 + " FULL VM out of Place restores from Incremental job " + "-" * 25)
            ...
            try:
                VirtualServerUtils.decorative_log(
                    "-" * 25 + " FULL VM out of Place restores from synthfull " + "-" * 25)
                vm_restore_options = OptionsHelper.FullVMRestoreOptions(auto_subclient, self)
                vm_restore_options.power_on_after_restore = True
                vm_restore_options.unconditional_overwrite = True
                auto_subclient.virtual_machine_restore(vm_restore_options)
