...
Test cases to validate Basic operation window Data Management features at backupset level
...
class TestCase(CVTestCase):
    """Class for executing Basic operation window Data Management features validation at backupset level"""

    def __init__(self):
        """Initializes test case class object"""
        super(TestCase, self).__init__()
        self.name = "[Functional] : Basic operation window Data Management features validation at backupset level " \
                    "for monthly rule with do not submit job enabled"
    ...
    def run(self):
        ...
        Follows the following steps
                ...
                2) Creates an operation window at backupset level for a list of operations:
        ...
        try:
            ...
            self.log.info("Validating Backupset level operation window for the features:%s",
                          operations_list)
            ...
            self.op_window = OpHelper(self, self.backupset)
            self.log.info("Initialised the operation window successfully")

            machine = Machine(self.client)
            op_rule = self.op_window.monthly_rule(operations_list, machine, do_not_submit_job=True)
