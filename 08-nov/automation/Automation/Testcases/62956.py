...
class TestCase(CVTestCase):
    """ Command center: Testcase to check all types backup/restore and
                        verify Verify BLocklevel File indexing cases command center.
    """
    test_step = TestStep()
    ...
    def __init__(self):
        ...
        self.name = "Verify BLocklevel File indexing cases"
        self.browser = None
    ...
    @test_step
    def check_out_of_place_restore(self, backuptype='INCR', fileindexing= False):
    ...
    def run(self):
        ...
        try:
            ...
            time.sleep(30)
            self.enable_blocklevel_file_indexing()
            self.navigate_to_client_page()
            self.check_out_of_place_restore(backuptype='FULL', fileindexing=True)
            self.navigate_to_client_page()
            self.check_out_of_place_restore(backuptype='INCR', fileindexing=True)
            self.navigate_to_client_page()
            self.check_out_of_place_restore(backuptype='SYNTH', fileindexing=True)
