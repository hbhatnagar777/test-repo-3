...
"""Helper class for LN Agents
    ...
    CVRestHelper:
        ...
        parse_find()                --  Parses find() result into a dictionary with list of note ids
        with their corresponding databases as keys

        clean_properties_fetched()  --  Parses collect properties result into a list of tuples note ids
        and embedded object details

        remove_extension()          --  Parses collect properties result into a list of tuples note ids
        and embedded object details
...
class CVRestHelper:
    ...
    def check_for_api(self):
        ...
        response = requests.get(constant.API_URL % self.tc.tcinputs.get("DominoServerHostName"))
    ...
    def get_all_apis(self):
        ...
        url = constant.CVREST_URL % self.tc.tcinputs.get("DominoServerHostName")
    ...
    def run_restore(
        ...
        """"Runs restore operation on an LNDB Subclient
            ...
            Args:
                ...
                interrupt       (bool)      --  boolean specifying whether to interrupt and return the job
        ...
        try:
            ...
            self.log.info(
                f"lndb_restore_options: {self.tc.lndb_restore_options}")
            if 'OUT' in type_of_restore:
                ...
                dest_path = self.tc.machine.join_path(
                    self.tc.domino_data_path, constant.OUT_PLACE_FOLDER_PATH)
    ...
    def remove_restored_dbs(
        ...
        if 'OUT' in type_of_restore_performed:
            for database in dbs_in_subclient:
                temp = f'{constant.OUT_PLACE_FOLDER_PATH}-{str(database)}'
            ...
            if 'OUT' in type_of_restore_performed:
                for database in dbs_in_subclient:
                    for browse_path in browse_paths_dictionary[database]:
                        temp = f'{constant.OUT_PLACE_FOLDER_PATH}-{str(browse_path)}'
                        response = self.return_result_json(
                            'get document properties',
                            param=[temp.replace('.nsf', '')]
                        )
        ...
        for _ in range(4):
            ...
            try:
                ...
                response = requests.get(
                    constant.BASE_URL % self.tc.tcinputs.get("DominoServerHostName")
                )
    ...
    def remove_extension(self, database_list, machine_os):
        ...
        for lndbcontent in database_list:
            ...
            if 'ntf' in database:
                self.log.info(f'{content} is a template.')
            if 'WINDOWS' in machine_os:
                self.tc.dbs_in_default_before.append(database.replace('.nsf', ''))
            else:
                self.tc.dbs_in_default_before.append(
