...
class LNDBHelper:
    ...
    def backup_template_and_encrypted_db(self):
        ...
        self.tc.subclient.content = self.cvhelp.remove_extension(
    ...
    def deleted_subclient(self):
        ...
        count = 0
        if self.tc.backupset.subclients.has_subclient('new_sub_50995'):
            self.tc.backupset.subclients.delete('new_sub_50995')
        ...
        self.tc.default_subclient.refresh()
        for lndbcontent in self.tc.newsubclient.content:
            self.tc.dbs_in_def_subclient.append(
                lndbcontent['lotusNotesDBContent']['databaseTitle'])
        if dbname in self.tc.dbs_in_def_subclient:
            count += 1
    ...
    def mark_job_bad(self):
        ...
        qscript = ("-sn 'MarkJobsOnCopy' -si 'SP1' -si 'Primary' "
    ...
    def unconditional_overwrite(self):
        ...
        if self.tc.backupset.subclients.has_subclient('auto_50993'):
            self.tc.backupset.subclients.delete('auto_50993')
        ...
        for step in test_steps:
            ...
            for database in dbs_in_subclient:
                if 'IN' in step:
                    ...
                    out_path_database = f'{constant.OUT_PLACE_FOLDER_PATH}-{str(database)}'
                    response = self.cvhelp.return_result_json(
    ...
    def full_data_protection(self):
        ...
        """
        if self.tc.backupset.subclients.has_subclient('new_sub_50994'):
            self.log.info(self.tc.backupset.subclients.delete('new_sub_50994'))
        self.tc.subclient = self.tc.backupset.subclients.add(
            'new_sub_50994', 'SP1')
        self.cvhelp.add_database_to_subclient(self.tc.subclient, "1", "10")
        for lndbcontent in self.tc.subclient.content:
    ...
    def defsubclient_autodiscovery(self):
        ...
        self.tc.default_subclient.refresh()
        self.tc.default_subclient.content = self.cvhelp.remove_extension(
    ...
    def tr_log_backup_with_indexing(self):
        ...
        job.resume(True)
        self.log.info(job.status)
        self.cvhelp.wait_for_job_to_complete(job)
        try:
    ...
    def backup_many_tr_logs(self):
        ...
        self.cvhelp.run_backup()
        dbname = self.cvhelp.add_database_to_subclient(
            self.tc.subclient, "1", "1", "null", "50919")
        dbname = dbname[0]
        self.cvhelp.run_backup()
        response = self.cvhelp.return_result_json(
            'add documents', param=[dbname, "10"])
        response = self.cvhelp.return_result_json('get document properties', param=[dbname])
        self.log.info(f'Total docs for {dbname} is {len(response.get("documents"))}')
        doc_count_before = len(response.get('documents'))
    ...
    def osc_schedule_backup(self):
        ...
        schedule_pattern = {
            ...
            "freq_type": "automatic",
            "min_interval_hours": constant.OSC_MIN_HOURS,
            "min_interval_minutes": constant.OSC_MIN_MINS,
            "max_interval_hours": constant.OSC_MAX_HOURS,
            "max_interval_minutes": constant.OSC_MAX_MINS
        }
        self.log.info(
            '***********************BACKUP*TR*LOG*SUBCLIENT************************')
        self.cvhelp.run_backup(
        ...
        self.tc.client.restart_services()
        time.sleep(constant.OSC_MAX_MINS * 60)
        job_dict = self.tc.commcell.job_controller.all_jobs(
            lookup_time=constant.OSC_MAX_MINS / 60)
        if job_dict:
