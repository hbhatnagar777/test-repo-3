...
GMail:
        ...
        _get_messages_prop()    --  Gets the message property of all the messages
        in the mailbox of a user

        get_labels()    --  Fetches the list of labels for a GMail user

        get_messages_prop() --  Gets message properties for user mailbox
        and stores it in TinyDB file

        delete_messages()   --  Deletes specific messages inside user's mailbox

        append_message()    --  Appends a message into the user's mailbox
        based on mail_insertion_type setting
...
class GMail:
    ...
    def _get_messages_prop(
                ...
                Args:
                    ...
                    google_resp_format (str) -- The format to return the message in
                                                  ...
                                                  Allowed values
                                                    ...
                                                    - raw - With this parameter entire
                                                    message payload is returned as
        ...
        for content in sc_content:
            ...
            try:
                ...
                self.dbo.dbo.purge_table(f'{user_id}{table_type}')
                ...
                for ids in messages:
                    ...
                    if google_resp_format == 'full':
                        ...
                        msg_id = re.search('<.*@*\.*>', raw_msg).group()
                        self.log.info('MSG ID extracted from RAW string: %s', msg_id)
                ...
                if not after_restore:
                    ...
                    archived_mails_dict = {
                        'name': '~ArchivedMail',
                        'messagesTotal': archived_mails_count}
                    ...
                    self.log.info('Purging the db before saving current properties')
                    self.dbo.dbo.purge_table(f'{user_id}{constants.LABELS_TABLE}')
    ...
    def delete_messages(self):
        ...
        if sc_content:
            for content in sc_content:
                ...
                gmail_service = self.auth_object.build_service(
                    self.api_name, delegated_user=user_id)
                msg_idlist = self.dbo.get_message_id_list(user_id)
                msg_list = self.dbo.get_content(
                    f'{user_id} {constants.MESSAGES_AFTER_OOP_DESTINATION}')
                if msg_list:
                    for msg in msg_list:
                        msg_idlist.append(msg['msg_id'])
                idlist = []
                for msg_id in msg_idlist:

                    response = gmail_service.users().messages().list(
                        userId=user_id, includeSpamTrash=True, q=f'rfc822msgid:{msg_id}').execute()

                    if 'messages' in response and len(response['messages']) == 1:
                        message = response.get('messages')
                        idlist.append(message[0].get('id'))
                        self.log.info('Google id got by sending message id')
                        self.log.info('msg id: %s', msg_id)
                        self.log.info('Google ID: %s', message[0].get('id'))
                ...
                Args:
                    ...
                    mail_insertion_type (str): Whether to import or append message.
                        ...
                        Allowed values:
                            ...
                            APPEND: Directly inserts a message into only this user's mailbox
                            similar to IMAP APPEND, bypassing most scanning and classification.
                            Does not send a message.

                            IMPORT: Imports a message into only this user's mailbox,
    ...
    def _create_message(self, sender, to, unicode_data, mail_insertion_type):
        """Creates a MIME message by fetching a random page from wikipedia
                ...
                Returns:
                    ...
                    dict of following form: {
                    'raw': base64 encoded MIME message,
                    'labelIds': [labels to apply]
                    }
    ...
    def _select_random_sender(self):
        """Selects the random sender from available Google users
        to be added in From field while creating a MIME mail
    ...
    def compare_msg_props(
            ...
            for content in sc_content:
                ...
                if table1_size != table2_size:
                    ...
                    for msg1 in msg_list:
                        ...
                        for msg2 in msg_list_after_res:
                            ...
                            if msg1.get('raw') == msg2.get('raw'):
                                if msg1.get('sizeEstimate') == msg2.get(
                                    ...
                                    self.log.warning(
                                        'Size is not matching but this can be becasue of '
                                        'removing illegal characters from the mail.')
                                if oop:
                                    self.log.info(
                        ...
                        if not matched:
                            self.log.exception(
                                'Message property comparison failed for raw format '
                                'for restored message: %s', msg2)
        ...
        if not key_match:
            ...
            self.log.exception(
                ...
                'Before backup number of messages: %s and '
                'after restore number of messages: %s for user mailbox: %s',
...
class GDrive:
    ...
    def create_files(
        ...
        """This method creates and upload files on user's GDrive
                ...
                Args:
                    ...
                    new_folder  (boolean)  --  If True creates a new folder on GDrive
                ...
                try:
                    ...
                    if not new_folder:
                        ...
                        request = gdrive_service.files().list(
                            q=f"name = '{constants.GDRIVE_FOLDER}' and "
                              f"mimeType = 'application/vnd.google-apps.folder'").execute()
                        folder = {'id': request.get('files')[0]['id']}
                    else:
                        self.log.info('Creating a new folder on GDrive')
                        constants.FOLDER_METADATA['name'] = constants.GDRIVE_FOLDER
                        folder = gdrive_service.files().create(
                            body=constants.FOLDER_METADATA).execute()
                            ...
                            while time_out <= 3:
                                try:
                                    request = gdrive_service.files().create(
                                        ...
                                        fields='name,parents,id,properties,md5Checksum,mimeType'
                                    ).execute()
                                    self.log.info('File uploaded successfully')
                                    count += 1
                                    break
                                except HttpError as err:
                                    # If the error is a rate limit,
                                    # connection error or backend error,
                                    # wait and try again.
                                    time_out += 1
                                    ...
                                    if err.resp.status in [400, 403, 429, 500, 502]:
                                        self.log.info(
                                            ...
                                            'Waiting for few seconds before checking the status',
                                            err.resp.status)
                                        time.sleep(time_out * 10)
                                        # This is backend error from Google
                                        # so we need the check whether the entity exists
                                        # If the entity exists,
                                        # get the details and save into local db.
                                        # Retry if the entity doesn't exist
                                        request_file = gdrive_service.files().list(
                                            q=f"name = '{body['name']}' and '{folder['id']}' "
                                        ...
                                        if not request_file['files']:
                                            ...
                                            if time_out == 3:
                                                self.log.info(
                                                    'Time out attempt exceeded (3 times) '
                                                    'for this file. '
                                                    'Uploading next file.')
                                            continue
                                        else:
                                        ...
                                        raise
                            del media_body
                            self.dbo.save_into_table(user_id, request, constants.LATEST_FILES)
    ...
    def compare_file_properties(self, oop=False, to_disk=False, incremental=False):
        """
    ...
    def get_file_properties(self, user=None):
        ...
        try:
            for content in sc_content:
                ...
                request = gdrive_service.files().list(
                    q=f"name = '{constants.GDRIVE_FOLDER}' and mimeType = "
    ...
    def _compare_files(self, user_id, incremental=False):
        ...
        """
        if incremental:
            db_content = self.dbo.get_content(f'{user_id}{constants.LATEST_FILES}')
        else:
            db_content = self.dbo.get_content(f'{user_id}{constants.GDRIVE_TABLE}')
        if db_content:
            folder_id = db_content[0]['parents'][0]
