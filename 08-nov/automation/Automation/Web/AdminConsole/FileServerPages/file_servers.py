...
AddWizard:
    ...
    get_all_file_servers()          :       Returns all the file servers names present on file servers page
...
class FileServers:
    ...
    def __init__(self, admin_console):
        ...
        self.__panel = RBackup(self._admin_console)
    ...
    def enable_snap(self, client_name):
        ...
        self.__panelinfo.enable_toggle(self._admin_console.props['label.snapEnabled'])
    ...
    def backup_subclient(
        ...
        job_id = self.__panel.submit_backup(backup_level, notify)
    ...
    def restore_subclient(
        ...
        if not kwargs.get('impersonate_user', False) and \
                not kwargs.get('ndmp') and kwargs.get('nas', None) and kwargs.get('agent').upper() == 'NFS':
    ...
    def __select_ibmi_plan(self, plan):
        ...
        self._driver.find_element(By.XPATH, f"//span[contains(text(),'{plan}')]").click()
    ...
    def action_add_software(
        ...
        add_server_modal = RModalDialog(admin_console=self._admin_console, title='Add software')
        if select_all_packages:
            add_server_modal.select_dropdown_values(drop_down_id='packageSelectionListDropdown', values=None, select_all=True)
        elif packages:
            add_server_modal.select_dropdown_values(drop_down_id='packageSelectionListDropdown', values=packages)
        else:
            raise CVWebAutomationException('Packages list is not provided')
        if reboot:
            add_server_modal.checkbox.check(id='rebootClient')
        add_server_modal.click_submit()

        if reboot:
            confirm_reboot_modal = RModalDialog(self._admin_console, title='Confirm reboot')
            confirm_reboot_modal.fill_text_in_field('confirmText', 'Change control for reboot confirmed')
            confirm_reboot_modal.click_submit()
        return self._admin_console.get_jobid_from_popup()
    ...
    def __get_protocols_text(self):
        ...
        return self._admin_console.driver.find_element(By.TAG_NAME, "div").text
    ...
    def __release_licence_agent(self, agent):
        """ Release CIFS and NFS Licenses
            Args:
                Agent - str - CIFS/NFS/NDMP
        """
        xp = f'//span[contains(text(), "{agent}")]/../../input[@type="checkbox"]/following-sibling::label'
    ...
    def release_license(self):
        ...
        self._admin_console.select_hyperlink("Release license")
        if "Server File System - NFS" in self.__get_protocols_text():
            self.__release_licence_agent("Server File System - NFS")

        if "Server File System - CIFS" in self.__get_protocols_text():
            self.__release_licence_agent("Server File System - CIFS")
    ...
    @WebAction()
    def __select_file_server_tab(self):
        """Selects the File Server tab"""
        self.__page_container.select_tab("File servers")
        self._admin_console.wait_for_completion()
    ...
    def get_all_file_servers(self, company=None) -> list:
        """ Returns all the file servers names present on file servers page

        Args:
            company (string): Company to filter

        Returns:
            List: List of all the servers
        """

        if company:
            self.__table.select_company(company)

        filer_server_names = self.__table.get_column_data("Name", fetch_all=True)
        return filer_server_names
...
class RestorePanel(ModalPanel):
    ...
    def _click_backupset_and_subclient(self, backupset_name, subclient_name):
        ...
        """
        old_table = Table(self._admin_console)
        old_table.view_by_title(value=backupset_name, label="Backup sets")
        self._admin_console.wait_for_completion()
    ...
    def __click_subclient(self, subclient_name):
        ...
        subclient = self._driver.find_element(By.XPATH,
                                              f"//div[normalize-space()='{subclient_name}']/ancestor::tr//input")
...
class AddWizard:
    ...
    def __click_download(self, os_name=None):
        ...
        if self._admin_console.check_if_entity_exists('xpath', xpath_gateway_windows):
            ...
            elif os_name and os_name.lower() == "unix":
    ...
    @PageService()
    def select_file_server_env(self, physical=False, backuptocloud=True, on_premise=True):
        """
    ...
    def add_new_server(self, installinputs):
        ...
        self.__wizard.fill_text_in_field(id="serverName", text=servername)
        serverxpath = "//*[contains(text(),'" + servername + "')]"
        if self._admin_console.check_if_entity_exists('xpath', serverxpath):
            self._admin_console.driver.find_element(By.XPATH, 
                serverxpath).click()
        else:
            self._admin_console.log.info(
                "failed to show newly installed server")
        self.__wizard.click_next()
    ...
    def check_if_localstorage_exist(self, localstoragename):
        ...
        if len(existingstorages) and (localstoragename in existingstorages):
            return True
        else:
            return False
    ...
    def select_local_storage(
        ...
        if not self.check_if_localstorage_exist(localstoragename):
            ...
            if networkpath:
                self.__dialog.select_radio_by_id(radio_id='networkRadioDisk')
                self.__dialog.fill_text_in_field(
                    element_id='credential.userName-custom-input', text=smbusername)
                self.__dialog.fill_text_in_field(
                    element_id='credential.password-custom-input', text=smbpassword)
            self.__dialog.fill_text_in_field(element_id='path', text=diskpath)
    ...
    def add_new_plan(self, plan_name, retention=None):
        ...
        if retention:
            self.__dialog.select_radio_by_id(radio_id='custom')
